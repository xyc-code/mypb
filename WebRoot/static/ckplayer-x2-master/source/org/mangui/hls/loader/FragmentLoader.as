...
package org.mangui.hls.loader {
    ...
    public class FragmentLoader {
        ...
        private function _keyLoadCompleteHandler(event : Event) : void {
			...
			else if(_keystreamloader.bytesAvailable == 32){
				 _keyRetryCount = 0;
                _keyRetryTimeout = 1000;
				 var keyData2 : ByteArray = new ByteArray();
				_keystreamloader.readBytes(keyData2, 0, 0);
				keyData2.position=0;
				var esc:String=des.getVString(Base64.encodeByteArray(keyData2));
				keyData=Base64.decodeToByteArray(esc);
                _keymap[_fragCurrent.decrypt_url] = keyData;
                // now load fragment
                try {
                    CONFIG::LOGGING {
        ...
        */
        ...
        private function _fragLoadCompleteHandler(event : Event) : void {
            ...
            CONFIG::LOGGING {
				...
				_hls.dispatchEvent(new HLSEvent(HLSEvent.LOADED_BYTES,((8000 * _metrics.size / _loading_duration) / 1024)));//niandeng 2018.12.19
			}
