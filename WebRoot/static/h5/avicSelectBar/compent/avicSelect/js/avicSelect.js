(function($) {
    $.fn.avicselect = function(opt) {
        ...
        options = {
            ...
            showkey: 'VALUE', // 如果用数据渲染时，选择展示的字段
        ...
        function inputEvent() {
            ...
            // 本页表单执行
            $(".avicselect-list").undelegate(setting.listen , 'mousedown').delegate(setting.listen, 'mousedown', function(e) {
                //解决’mousedown‘事件默认的点击后失去焦点导致下拉框关闭问题
                e.preventDefault();
                //ie8 e.preventDefault()方法不能阻止mousedown事件默认行为，导致input失去焦点关闭弹层，使用以下方式处理
                if (navigator.appName == "Microsoft Internet Explorer" && navigator.appVersion.match(/8./i) == "8.") {
                    $(".avicselect-list").find(setting.listen).focus();
                }
                if($(this).hasClass('on')){
                    $(this).removeClass('on');
                }else{
                ...
                $.each(data, function(i, v) {
                    ...
                    $.each(v, function(j, k) {
                        ...
                        if (findName) {
                            ...
                            if(setting.findDom.find('input[name=' + findName + ']').attr("type") == "hidden"){
                                setting.findDom.find('input[name=' + findName + ']').trigger("change");
                            }
				 ...
				 success : function(r){
					 if (r.flag == "success"){
                         ...
                         if (r.error == null || r.error == "null") {
                             errorMsg = "编码不存在";
                         } else if (r.error.indexOf("Error querying database.  Cause: java.lang.RuntimeException: java.lang.NullPointerException") > -1) {
                             errorMsg = "数据源配置有误，请检查!";
                         }
        ...
        function openselect()
        {
    		...
    		"<div style='padding:5px 10px;font-size:12px;float:left;"+(setting.listStyle||'')+"'><span>共<span id='avicit_page_tital'></span>条数据，</span>" +
    		"第<span id='avicit_page_pagecount'></span>/<span id='avicit_page_sumpage'></span>页</span></div>" +
    		"<div style='float:right;padding:5px 10px;font-size:16px;position: absolute;right: 0;'>" +
    		"<i class='icon iconfont icon-jiantou-zuo avicselect-page'></i>" +
    		"<i class='icon iconfont icon-jiantou-you avicselect-page'></i></div></div>";
		    setting.content.append(content);
			$(".icon-jiantou-zuo").on('mousedown', function(e){
		    	selectIndex = -1; 
		    	pagecut(-1);
		    	e.preventDefault();
                //ie8 e.preventDefault()方法不能阻止mousedown事件默认行为，导致input失去焦点关闭弹层，使用以下方式处理
		    ...
		    $(".icon-jiantou-you").on('mousedown', function(e){
		    	selectIndex = -1; 
		    	pagecut(1);
		    	e.preventDefault();
		    	//ie8 e.preventDefault()方法不能阻止mousedown事件默认行为，导致input失去焦点关闭弹层，使用以下方式处理
       ...
       var selectIndex = -1;
       if(setting.remote){
           that.find(setting.ipt).on('focus', function(e){openselect();load(true);});
           ...
           that.find(setting.action).on('click', function(){openselect();load(true);});
       }else{
           ...
           that.find(setting.ipt).on('click', function() {
               layer.open({
                   type: setting.type,
                   area: [layerWidth + 'px', setting.height],
                   skin: 'avicselectui',
