 ...
 * @returns {string}
 */
function get6yhDate(startDate,n){
	if(startDate&&n){
...
function selectElem(value, options) {
	...
	elem.attr('oldName', value);
	elem.attr('oldValue', rowData[forId]);
...
/**
 * jqGrid单表单元格编辑日期控件扩展(只选年)
 * @param value
 * @param options
 */
function dateYearElem(value, options) {
	//console.log(value);
    //feng start
    $jqGrid = $(this);
    //feng end
	var elem = $('<div class="input-group input-group-sm">'
			//feng start
			//+ '<input class="form-control date-picker" type="text" data-status=""/>'
			+ '<div tabIndex="-1"><input class="form-control year-picker" type="text" data-status="" onfocus="WdatePicker({skin:\'whyGreen\',dateFmt:\'yyyy\'}) " placeholder="请选择年份"></div>'
			//feng end
			+ '<span class="input-group-btn ui-datepicker-trigger">'
			+ '<button class="btn btn-default cell-edit-button" type="button">'
			+ '<span class="glyphicon glyphicon-calendar"></span>'
			+ '</button>' + '</span>' + '</div>');
	$('input.year-picker', elem).val(value);
	//$('input.year-picker', elem).val(value);
	setTimeout(function() {
	/*setTimeout(function() {
		$('input.date-picker', elem).datepicker({
		...
		$('input.year-picker').on('keydown',nullInput);
...
function dateYearValue(elem, operation, value) {

	if (operation === 'get') {
		return $('input.year-picker', elem).val();
	} else if (operation === 'set') {
		$('input.year-picker', elem).val(value);
	}
}
/**
 * jqGrid单表单元格编辑时间控件扩展(只选年)
 * @param value
 * @param options
 */
 ...
 * @param options
 */
function timeElemWithSecond(value, options) {
	//feng start
	$jqGrid = $(this);
	//feng end
	var elem = $('<div class="input-group input-group-sm">'
...
function userElem(value, options) {
	...
	var secretCol = options.secretCol || '';
	...
	//end ws
	var rowData = $(this).jqGrid('getRowData', rowId);
	var secretLevelValue = rowData[secretCol];
    ...
    //start ws
    var h5SelectCommOption = {secretLevel:secretLevelValue,type : 'userSelect',idFiled : 'cellUserid',textFiled : 'cellUseridAlias',selectModel:selectModel};
...
function orgElem(value, options) {
	...
	var inputFocusFunctionForCallBack = function(org){
	   if(v_callback != null && typeof(v_callback) != "undefined" && v_callback != ""){
	     var func = eval(v_callback);
	     new func(org);
	   }
          ...
          yes: function(index, layero){
	          ...
	          if(selectIds != null && selectIds.length > 0){
                 ...
                 $elem.parent().parent().parent().find('td[aria-describedby="'+jqObject[0].id+'_'+options.detail+'"]').attr("joindetail",JSON.stringify(selectRows));
				  ...
				  dataRow[options.name] = null;
				  $elem.parent().parent().parent().find('td[aria-describedby="'+jqObject[0].id+'_'+options.name+'"]').text(null);
				  jqObject.jqGrid('setRowData', rowId, dataRow);
				  $("#"+rowId).addClass("edited");
			  }
...
//地址控件
function urlElem(value, options){
	var rowId = options.rowId;
	var _this = this;
	var rowData = $(this).jqGrid('getRowData', rowId);
	var jqObject = $(this);
	...
	if(ynMax == "setting"){
		...
		if(options.height!= null && options.height != undefined){
			if(options.height.lastIndexOf("%") != -1){
				layerHeigth = options.height;
			}else{
				layerHeigth = options.height+"px";
			}
		}else{
			layerHeigth = "auto";
		}
	}
	var dicContent = options.urlAddress;

	var inputFocusFunctionForCallBack = function(obj,setJqGridColData){
		if(v_callback != null && typeof(v_callback) != "undefined" && v_callback != ""){
			var func = eval(v_callback);
			new func(obj,setJqGridColData,rowId,$elem);
		}
		$('#cellGroupidAlias').focus();
	};
	...
	$elem.find('#'+options.name+', .input-group-addon').on('click',function() {
		layer.open({
			...
			yes: function(index, layero){
				...
				var iframeWin = layero.find('iframe')[0].contentWindow;//子页面的窗口对象
				var obj = null;
				if(iframeWin.commitData != null && typeof(iframeWin.commitData) != "undefined" && iframeWin.commitData != ""){
					obj = iframeWin.commitData();
				}

				this.setJqGridColData = function(colName,colValue){
					var colId = $elem.find('input.form-control').attr("id");
					if(colId == colName){
						$elem.find('input.form-control').val(colValue);
					}else{
						$elem.parent().parent().parent().find('td[aria-describedby="'+jqObject[0].id+'_'+colName+'"]').text(colValue);
					}
				};

				inputFocusFunctionForCallBack(obj,this.setJqGridColData);
				$("#"+rowId).addClass("edited");
				layer.close(index);
			},
			btn2: function(index, layero){
...
//地址控件
function urlValue(elem, operation, value) {
	var rowId = $(elem).find('#cellRowId').val();
	if (operation === 'get') {
		if($(elem).find('input.form-control').val()===undefined){
			return "";
 ...
 * @param options
 */
function secretElem(value, options) {
	var rowId = options.rowId;
	var datas = options.value;
	if (typeof options.value === 'function'){
		datas = options.value.call();
...
function secretValue(elem, operation, value) {
	if (operation === 'get') {
		var rowId = $(elem).attr('data-rowid');
		var forId = $(elem).attr('data-forid');
		var selectText = $(elem).find('option:selected').text();
...
editoptions:{
	...
	title:'标题',
	urlParam:{'参数名':'元素ID'},//获取表单中其他元素的值，如input框等
	urlDataParam:{'参数名':'列名称'},//获取grid当前行对应的某一列的值
	urlFunParam:函数名,//用函数返回参数，传递的参数是行ID，返回的参数格式为{'参数名':'值'}
	urlFun:functionName,//来源于函数，传递的参数是行ID,可用于实现不同行打开不同页面的功能
	url:'页面地址',
	beforeOpenDialog: 在调用打开页面之前，校验function，必须返回true或者false
	fields:{'textId':'field','textId':'field',...},//textId:父页面需要回填数据的控件ID，field:选择页面对应的数据字段

	//如果需要使用enter键功能的，请填写下面2项
	useEnterKey: true,//不必填
    checkUrl:'ims/voucher/vouitemproduct/vouItemProductController/checkSubName'//不必填

    callback:选择完成后的回调函数 可为空
    parentObj:子页面在父页面弹出选择页面
}
说明：
checkURl的返回的数据格式：{"flag": OpResult.success,"rows":{"field":"xxxxx","field":"xxxxx"}}或者{"flag": OpResult.failure}
*/
function customDialogElem(value, options) {
	var width = options.width,
		height = options.height,
		url = options.url,
		urlParam = options.urlParam,
		urlDataParam = options.urlDataParam,
		urlFunParam = options.urlFunParam,
		urlFun = options.urlFun,
		title = options.title,
		beforeOpenDialog = options.beforeOpenDialog,
		fields = options.fields,
		useEnterKey = options.useEnterKey ? options.useEnterKey : false,
		checkUrl = options.checkUrl,
		callback = options.callback,	// 回调函数
		parentObj = options.parentObj,//子页面弹出父页面窗口
		jqgrid = this;
	...
	var vl = '<div class="input-group input-group-smm" style="margin: 0px">'+
					'<input type="hidden" id="rowid" value="'+options.rowId+'" />'+
					'<input type="hidden" id="name" value="'+options.name+'" />'+
					'<input type="hidden" id="data" />'+
					'<input type="hidden" id="fds" value=\''+JSON.stringify(fields)+'\'/>'+
					'<input class="form-control" id="value" placeholder="请选择" value="'+value+'" />'+
					'<span class="input-group-addon">'+
						'<i class="fa fa-search-plus"></i>'+
					'</span>'+
				'</div>';
	...
	if(useEnterKey){
		// 绑定enter键
		elem.find('#value').on('keydown',function(e) {
			if (e.keyCode === 13 && !e.shiftKey) {
				...
				if (value) {
                    var data = {};
                    if(urlParam){
                        for(var pa in urlParam){
                            data[pa] = $("#"+urlParam[pa]).val();
                        }
                    }
                    if(urlDataParam){
                    	var dataObj=$(jqgrid).jqGrid("getRowData",options.rowId);
                    	for(var pa in urlDataParam){
                            data[pa] = dataObj[urlDataParam[pa]];
                        }
                    }
                    if(typeof(urlFunParam)=="function"){
                    	var dataObj=urlFunParam(options.rowId);
                    	for(var pa in dataObj){
                            data[pa] = dataObj[pa];
                        }
                    }
					$.ajax({
						url: checkUrl,
						data: {param: JSON.stringify(data), inputValue:value},
						type: 'post',
						dataType: 'json',
						async: false
					}).success(function (r) {
						if (r.flag == "success") {
                            elem.find('#data').val(JSON.stringify(r.rows));
                            $(jqgrid).jqGrid('endEditCell');
						} else {
							elem.find('.input-group-addon').trigger('click');
						}
					});
				}else{
                    elem.find('.input-group-addon').trigger('click');
        ...
        clickElem = elem.find('.input-group-addon');
	}else{
        elem.find('#value').prop("readonly",true);
        clickElem = elem.find('#value, .input-group-addon');
	}
    ...
    clickElem.on('click',function(){
        ...
        if(urlParam){
            for(var en in urlParam){
            	if(pm == "?"){
            		pm += en +"="+$("#"+urlParam[en]).val();
            	}else{
            		pm += "&"+en +"="+$("#"+urlParam[en]).val();
            	}
            }
        ...
        if(urlDataParam){
        	var dataObj=$(jqgrid).jqGrid("getRowData",options.rowId);
        	for(var en in urlDataParam){
        		if(pm == "?"){
            		pm += en +"="+dataObj[urlDataParam[en]];
            	}else{
            		pm += "&"+en +"="+dataObj[urlDataParam[en]];
            	}
            }
		...
		var beforeOpenDialogResult = true;
        if(typeof(beforeOpenDialog)=="function"){
            beforeOpenDialogResult = beforeOpenDialog();
        }
        if(beforeOpenDialogResult) {
        	if(parentObj){
				parentObj.showCustomDialog(title, url + pm, width, height, null, ';',function (d) {
					elem.find('#data').val(JSON.stringify(d));
					$(jqgrid).jqGrid('endEditCell');
					// 回调函数
				...
				showCustomDialog(title, url + pm, width, height, null,';', function (d) {
					elem.find('#data').val(JSON.stringify(d));
					$(jqgrid).jqGrid('endEditCell');
					// 回调函数
					if(typeof(callback)=="function"){
...
//列表自定义选择窗回填
function customDialogVlaue(elem, operation, value) {
	if(operation == 'get'){
		var rowid = elem.find('#rowid').val(),
		name = elem.find('#name').val(),
		data = elem.find('#data').val(),
		fds = elem.find('#fds').val(),
		jqgrid = this;
			...
			if(fds){
				...
				if(Object.prototype.toString.call(data) == '[object Array]'){
					...
					for(var i=0;i<data.length;i++){
						for(var en in data[i]){
							if(!_tempRowData[en]){
								_tempRowData[en] = data[i][en];
							}else{
								_tempRowData[en] += ';'+ data[i][en];
							}
						}
				...
				// 设置值
				for(var en in fds){
					setdata[en] = data[fds[en]];
				}
			}else{
				setdata = data;
			}

			$(jqgrid).jqGrid('setRowData', rowid, setdata);
			return setdata[name] == null ? '' : setdata[name];
		}
		return elem.find('#value').val();
	}else{
		elem.find('#value').val(value)
	}
}
