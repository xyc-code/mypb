(function ($) {
    ...
    $.fn.uploaderExt = function (options, param) {
        ...
        return this.each(function () {
            ...
            if (state.uploader) {
                ...
                $.extend(state, {uploader: webUploader});
            }
        });
    };
    ...
    $.fn.uploaderExt.fileSizeLimit = 100*1024*1024; // 默认100M，与后台保持一致，参照spring-mvc.xml的multipartResolver配置
    ...
    $.fn.uploaderExt.fileTypeIco = {
        text: 'glyphicon-file', //文本样式
        code: 'glyphicon-file', //xml,html样式
        word: 'glyphicon-file', //word样式
        excel: 'glyphicon-file', //excel样式
        ppt: 'glyphicon-file', //ppt样式
        pdf: 'glyphicon-file', //pdf样式
        zip: 'glyphicon-inbox', //压缩文件样式
        image: 'glyphicon-picture', //图片样式
        video: 'glyphicon-film', //视频样式
        audio: 'glyphicon-headphones', //音频样式
        file: 'glyphicon-file' //其他文件样式
    };
    ...
    $.fn.uploaderExt.fileTypeExts = {
        ...
        ppt: ['ppt', 'pptx', 'dps'],
        pdf: ['pdf'],
        zip: ['zip', 'rar', 'tar', 'gz', 'cab', 'uue', 'arj', 'bz2',
            'lzh', 'jar', 'ace', 'iso', '7z', 'z'],
    ...
    function init(target) {
        ...
        if ($.fn.uploaderExt.needFlash == true) {
            ...
            $(	
            	'<div class="uploader-panel-tit">附件:</div>'+
            	'<div class="uploader-panel-heading" style="margin:4px 14px 0 136px ;border-radius: 4px;">' +
            	
                '<div class="webuploader-container uploader-file-picker" style="border:1px solid #cccccc;font-size:14px!important;border-radius:4px;line-height:20px;">' +
                '<span class="icon-add">&nbsp;</span>选择文件</div>' + 
               
                '<button class="uploader-btn-upload"><i class="glyphicon glyphicon-open fa-fw"></i>开始上传</button>' +

                '<div class="webuploader-container uploader-file-all-download" style="float:right;padding-bottom:3px; ">' +
                /*'<div class="webuploader-pick "><span class="iconfont icon-download">&nbsp;</span>下载全部</div></div>' +*/

                '<div class="uploader-heading-infos">' +
                '<span class="uploader-panel-title" style="display:none"> ' +
                '附件(已添加<span class="uploade-file-number">0</span>个附件)' +
                '</span>' +
                '<i class="glyphicon glyphicon-triangle-bottom fa-lg uploader-btn-expend"></i>' +
                '</div>' +
                '</div>' +
                '<div class="uploader-panel-body">' +
                '<div class="uploader-file-list"></div>' +
    ...
    function initWebUploader(target) {
        ...
        if ($.fn.uploaderExt.needFlash == true) {
            ...
            //top.layer.alert('您当前的浏览器没有安装flash插件或flash插件的版本较低；请安装或者升级flash插件！点击【<a href="https://www.flash.cn/">flash</a>】下载！',  {icon: 7, title:"提示", area: ['400px', '']});
            //return false;
        }
        ...
        uploader.on('uploadSuccess', function (file, response) {
            if (response.result && response.result == 'success') {
                ...
                if (opts.secretLevel && opts.secretLevel !== '') {
                    var secret = $('#' + file.id).find('select[name="secretLevel"]');
                    ...
                    if (!opts.allowEditsecretLevel) {
                        ...
                        secret.bind('change', function(e){fileItem.getSecretData(response.fileId,secret,opts,fileItem);});
                    }
                }
            } else {
                //修改状态
    ...
    function FileItem(target, data) {
        ...
        this.create = function () {
            ...
            if (allowEditsecretLevel && !isAdd) {
                this.$element.find("select[name='secretLevel']").bind('change', function(e){thisObj.getSecretData(fileData.fileId,thisObj.$element,opts,thisObj);});
        ...
        this.getSecret = function () {
            return this.$element.find('select[name="secretLevel"]').val();
        };
        ...
        this.getHtml = function () {
            ...
            if (eformUI === "easyui") {
                ...
                html.push('<i class="glyphicon ' + getFileTypeIco(data.ext) + ' uploader-file-ico"></i>');
            }
            ...
            if (isAdd) {
                ...
                if (eformUI === "easyui") {
                    ...
                    //html.push('<i class="glyphicon glyphicon-remove uploader-btn-remove" title="删除"></i>');
                    html.push('<span class="icon-remove uploader-btn-remove" title="删除">&nbsp;</span>');
                }
        ...
        this.bindFileItemInfos = function () {
            ...
            this.$element.bind("mouseenter", function (e) {
                ...
                html.push('<span class="uploader-file-infos-tip"><i class="glyphicon glyphicon-triangle-top"></i></span>');
                ...
                if (opts.allowDownload) {
                    ...
                    $(fileInfos).find('.uploader-file-infos-download').unbind().bind('click', function (e) {
                        downloadFile(data.fileId,_allowEncry);
                    });
                }
                ...
                if (opts.allowDelete) {
                    $(fileInfos).find('.uploader-file-infos-delete').unbind().bind('click', function () {
                    	 $.messager.confirm('请确认','确定要删除此附件吗？', function (index) {
                    	     if(index){
                    	         $.ajax({
                                cache: false,
                                type: "post",
                ...
                fileInfos.off("mouseleave mouseenter").on("mouseleave", function (e) {
                    ...
                    if ($(toElement).closest('.uploader-file-item').length == 0
                        || $(toElement).closest('.uploader-file-item')[0] != thisObj.$element[0]) {
                ...
                thisObj.$element.off("mouseenter.leave mouseleave").on("mouseleave", function (e) {
                    var toElement = e.relatedTarget || e.toElement;
                    if ($(toElement).closest('.uploader-file-infos').length == 0
                        || $(toElement).closest('.uploader-file-infos')[0] != fileInfos[0]) {
                        window['uploader-ext-timer'] = setTimeout(function () {
                            fileInfos.hide(100, function () {
                                fileInfos.remove()
                            });
    ...
    function getSecretLevelHtml(target, serectvalue) {
        ...
        if (opts.secretLevel != "" && list && list.length > 0) {
            html.push('<select name="secretLevel">');
            for (var i = 0; i < list.length; i++) {
     ...
     * @param fileId
     */
    function downloadFile(fileId,_allowEncry) {
    ...
    function multiDownload(fileIds, options) {
        ...
        if(typeof _fileupload_entryId != "undefined" && _fileupload_entryId != "" && typeof _fileupload_taskName != "undefined" && _fileupload_taskName != ""){
            $downloadFrame.attr('src', 'platform/webuploader/multidownload?fileIds=' + fileId + '&allowEncry=' + _allowEncry + "&_taskName=" + _fileupload_taskName + "&_entryId=" + _fileupload_entryId);
        }else {
    ...
    function validateSecret(target){
        ...
        if (opts.secretLevel!=""&&opts.formSecret!=""){
            ...
            if ((value==""||value==undefined||value==null) && files.length>0){
            	$.messager.show({
   				 title : '提示',
   				 msg : "表单密级["+opts.formSecret+"]没有值，请检查！"
   			    });
