(function($) {
		...
		return this.each(function() {
			...
			if (state) {
				...
				state = $.data(this, 'uploaderExt', {
					...
					tableGrid: r.tableGrid,//展示类型为table时的表格对象
			...
			if (state.uploader) {
			    ...
			    //此处修复列表展示附件时默认展开表格宽度过小的问题
			    if(state.options.showType=='table'){
			        var expendBtn = $(this).find('.uploader-btn-expend');
	...
	//如果是列表展示，此处为表头设置
	$.fn.uploaderExt.dataGridColModel =  [
			{ label: 'id', name: 'id', key: true, width: 75, hidden:true }
			,{ label: 'target', name: 'target', width: 75, hidden:true }
			,{ label: 'fileId', name: 'fileId', width: 75, hidden:true }
			,{ label: '附件名称', name: 'attachmentname',align:'left', width: 20}
			,{ label: '附件密级id', name: 'attachmentsecret', align:'center',width: 20,hidden:true}
			,{	label : '附件密级', name : 'attachmentsecretName', align:'center',width : 20, editable : true, edittype : "custom",editoptions : {
								custom_element : selectElem,
								custom_value : selectValue,
								forId : 'attachmentsecret',
								value : '',
								require:true
							}}
			,{ label: '附件大小', name: 'attachmentsize', align:'center',width: 20}
			,{ label: '上传人', name: 'creatBy', align:'center',width: 20}
			,{ label: '上传时间', name: 'creatTime', align:'center',width: 20,formatter:fmtDate}
			,{ label: '状态', name: 'status', align:'center',width: 20,formatter:formatStatus}
			,{ label: '排序', name: 'orderBy', align:'center',width: 20,editable : true,
							edittype : 'custom',
							editoptions : {
			...
			,{ label: '操作', name: 'operate', align:'center',width: 20,formatter:formatOpera}
	...
	function init(target,options) {
		...
		if(typeof(options.showType)==='undefined'){
			options.showType="span";
		}

		var downloadAll = "";
		if(options.allowDownload){
			downloadAll = '<div class="webuploader-container uploader-file-all-download" style="*float:left;padding-bottom:3px;float: left;">'
				+ '<div class="webuploader-pick"><div><a class="btn btn-primary form-tool-btn btn-sm" ><i class="iconfont icon-download">&nbsp;</i>批量下载</a></div></div></div>';
		}

		if ($.fn.uploaderExt.needFlash == true) {
			$('<div class="uploader-panel-body">' + '<div class="uploader-file-list"></div>' + '</div>').appendTo(target);
		//如果展示类型是列表类型
		} else if(options.showType=="table"){
			$(
					'<div class="uploader-panel-heading">' + '<div class="webuploader-container uploader-file-picker" style="*float:left;padding-bottom:3px;">'
							...
							+ '<table id="'+tid+'_attachmentTable"></table>' + '</div>').appendTo(target);
		...
		//展示类型为table时的jqgrid对象
		var tableGrid = t.find('#'+tid+'_attachmentTable');
	...
	function bindEvents(target) {
		...
		t.find('.uploader-heading-infos').unbind().bind("click", function() {
			...
			var  isshow=t.find('.uploader-panel-body').css("display");
			...
			if (expendBtn.hasClass('glyphicon-triangle-bottom')) {
			 ...
			 /*//附件区域宽度与父级容器一致:20210107
            	if(isshow=="block"&&state.options.showType=='table'){
            		initTable(state.tableGrid,target)
				}*/
		});
	}
	...
	function initWebUploader(target) {
		...
		if ($.fn.uploaderExt.needFlash == true) {
			loadInfos(target);
			top.layer.alert('您当前的浏览器没有安装flash插件或flash插件的版本较低；请安装或者升级flash插件！点击【<a href="http://download.da.net/install_flash_player_ax25.exe">flash</a>】下载！', {
				icon : 7,
				title : "提示",
				area : [ '400px', '' ]
			});
		...
		var uploader = WebUploader.create({
			...
			formData : {
				...
				fileCategoryList:opts.fileCategoryList
			},
			chunked : false, //分片在上传
		...
		//如果展示类型为table开始初始化表格
		if(opts.showType=="table"){
			initTable(tableGrid,target);
		}
		loadInfos(target);
		//文件上传之前，传入自定义参数
		uploader.on('uploadBeforeSend', function(block, data) {
			...
			var file = block.file;
			if(opts.showType=="table"){
				state.tableGrid.jqGrid("endEditCell");
				var rowData=state.tableGrid.jqGrid("getRowData",data.id);//这是获得 某一行的数据
				data.secretLevel = rowData.attachmentsecret;
				data.orderBy=rowData.orderBy;
			}else{
				var fileDIV = $('#' + file.id);
			...
			//流程指针ID
			data.executionId = typeof _bpm_executionId != 'undefined'?_bpm_executionId:"";
			//流程任务ID
			data.taskId = typeof _bpm_taskId != 'undefined'?_bpm_taskId:"";
		});
		...
		uploader.on('uploadProgress', function(file, percentage) {
			/*附件列表展示时进度条样式:20210309*/
			var $li = $('#' + file.id);
			var $progress;
			if(state.options.showType=='table'){
				$progress = $li.find('.uploader-file-progress-td');
			}else {
				$progress = $li.find('.uploader-file-progress');
			}
			...
			if (response.result && response.result == 'success') {
				if(opts.showType=="table"){
					...
					if (isUploaded(target)) {
						state.tableGrid.jqGrid('clearGridData');
						loadInfos(target);
				...
				if (opts.secretLevel && opts.secretLevel !== '') {
					...
					secret.attr("oldValue",secret.find("option:selected").val());
	 ...
	 * @returns {Number}
	 */
    function initTable(tableObj,target){
    	var _self=$.fn.uploaderExt;
    	var state = $.data(target, 'uploaderExt');
		var opts = state.options;
		/*var parentwidth=$(state.tableGrid).parent().width();
		if(parentwidth<200){
			var divwidth=$("#attachmentTable").width();
			if(divwidth>1000){
				$(state.tableGrid).parent().css("width",divwidth);
			}
        ...
        height: 120, //初始化表格高度
		//表格重叠问题，设置该属性,自动根据width属性值计算宽度:20210107 $(window).width()
		//width: parentwidth , //初始化表格宽度*/
		...
		beforeSaveCell : function(rowid,celname,value,iRow,iCol) {
	 		...
	 		var fileItem=getFileItemById(target,rowid);
			var fileData = fileItem.getData();
			var secret=rowdata.attachmentsecret;
			var fileId=rowdata.fileId;
	 		if(celname=="attachmentsecretName"){
	 			if(rowdata.fileId!=""&&fileData.secret!=secret){
					if (opts.secretLevel != "" && opts.formSecret != "") {
						var formvalue = $("[id='" + opts.formSecret + "']").val() || $("[name='" + opts.formSecret + "']").val();
						if ((formvalue == "" || formvalue == undefined || formvalue == null) && files.length > 0) {
							layer.msg("表单密级[" + opts.formSecret + "]没有值，请检查！");
							return false;
						}

						if (secret > formvalue) {
							layer.msg("该附件密级高于文档密级，请重新选择！");
							var  oldValue=$("*[ data-rowid ='"+rowid+"']").attr('oldValue');
							var  oldName=$("*[ data-rowid ='"+rowid+"']").attr('oldName');
							$("*[ data-rowid ='"+rowid+"']").val(oldValue);
							$(this).jqGrid("setCell",rowid,"attachmentsecret",oldValue);
							return oldName;
						}
					}
		...
		afterSaveCell : function(rowid,celname,value,iRow,iCol) {
			...
			var secret=rowdata.attachmentsecret;
			var fileId=rowdata.fileId;
			var orderBy=rowdata.orderBy;
	 		if(celname=="orderBy"){
	 			if(rowdata.fileId!=""&&fileData.orderBy!=orderBy){
					$.ajax({
						cache : false,
						type : "post",
						url : "platform/webuploader/EditInfo.json",
        ...
        loadComplete:function(){
	 		if(!opts.allowAdd){
				state.tableGrid.setColProp('attachmentsecretName',{editable:false});
			}
		},
        styleUI : 'Bootstrap',//Bootstrap风格
		viewrecords: true, //是否要显示总记录数
		multiselect: true, //可多选
		//表格重叠问题，取消该属性,自动根据width属性值计算宽度:20210107
    	...
    	setTimeout(function() {
				if(!opts.expand){
					$(target).find('.uploader-panel-body').hide();
				}
				$(target).find("#gview_"+tid+"_attachmentTable").css("width",$(target).find(".ui-jqgrid-hdiv").width());
						}, 300);
    	state.tableGrid.hideCol("cb");

    	/*var width=$(state.tableGrid).parent().find("ui-jqgrid-view table-responsive").width();
			$(state.tableGrid).setGridWidth(width);*/
    //解决附件表格宽度太小问题
	...
	function loadInfos(target) {
		...
		$.ajax({
			...
			data : {
				...
				attIds : opts.attIds,
                //流程指针ID
				executionId:typeof _bpm_executionId != 'undefined'?_bpm_executionId:"",
                //流程任务ID
				taskId:typeof _bpm_taskId != 'undefined'?_bpm_taskId:""
			},
			async : false,
			error : function(request) {
			},
			success : function(data) {
				...
				var map = {};
				if(opts.showType=="table"){
					for(var i=0;i<data.secretLevelList.length;i++){
					secretLevel=data.secretLevelList[i];
					map[secretLevel.lookupCode]=secretLevel.lookupName
					}
					//此处控制是否显示密级列  显示密级列的前提前提条件是 secretlevel有值
				if (opts.secretLevel && opts.secretLevel !== '') {
					state.tableGrid.setGridParam().showCol("attachmentsecretName");
					state.tableGrid.setColProp('attachmentsecretName',{editoptions:{value:map}});
				}else{
					state.tableGrid.setGridParam().hideCol("attachmentsecretName");
				}
				}
				//写入已存附件
	...
	function FileItem(target, data) {
		...
		this.tableCreate = function() {
			...
			var thisObj = this;
			var orderBy="";
			if(thisObj.data.fileId!=""){
				rowKey=thisObj.data.fileId;
				...
				rowKey=thisObj.data.id;
				//初次上传自动计算排序号，也可通过行编辑自行修改
				var rowIds=state.tableGrid.jqGrid("getDataIDs");
				orderBy=rowIds.length*10+10;
				if(state.secretLevelList.length>0){
					data.secretName= state.secretLevelList[0].lookupName;
					data.secret=state.secretLevelList[0].lookupCode;
				}
			}
			this.getData().id=rowKey;

			var parameters = {
				rowID : rowKey,
				initdata : {attachmentname:data.name,attachmentsize:getFileSize(data.size),fileId:data.fileId,target:thisObj.target,creatBy:data.creatBy,creatTime:data.creatTime,attachmentsecretName:data.secretName,attachmentsecret:data.secret,orderBy:orderBy},
				group :"first",
				useDefValues : true,
				useFormatter : true,
				addRowParams : {extraparam:{}}
			}
			state.tableGrid.jqGrid('addRow', parameters);
			//添加进表格数据后控制密级列是否可编辑,新增进表格的附件都可以进行密级编辑
			if(thisObj.data.fileId!=""){
				if(state.options.allowEditsecretLevel){
                    state.tableGrid.jqGrid('setCell', data.fileId, "attachmentsecretName", '', 'edit-cell');
				}else{
					state.tableGrid.jqGrid('setCell', data.fileId, "attachmentsecretName", '', 'not-editable-cell');
				}
			}else{
				state.tableGrid.jqGrid('setCell', data.id, "attachmentsecretName", '', 'edit-cell');
			}
			//预览按钮按钮绑定事件
			$('#tablePreview'+index).bind('click', function(e) {
				previewFile(data.fileId,data.ext);
			});
			//删除按钮绑定事件
			$('#tableDel'+index).bind('click', function(e) {
						delFile(data.fileId,target);
			});
			//下载按钮绑定事件
			$('#tableDown'+index).bind('click', function(e) {
						downloadFile(data.fileId,state.options.allowEncry,state);
			});
			//移除按钮绑定事件
			$('#tableRomove'+index).bind('click', function(e) {
						removeFile(data.id,target);
			});
			index++
			this.addToList();
		};
		...
		this.create = function() {
			...
			this.$element.find("select[name=\'fileSecretLevel\']").attr("oldValue",data.secret)
			//增加删除事件
		...
		this.getSecretData = function(fileId, obj, opts, fileItem) {
			...
			if (opts.secretLevel != "" && opts.formSecret != "") {
				...
				if (secret > value) {
					if(obj.find("select[name='fileSecretLevel']").length==0){
							obj.val(obj.attr("oldValue"));
					}else{
						obj.find("select[name='fileSecretLevel']").val(obj.find("select[name='fileSecretLevel']").attr("oldValue"));
					}
			...
			$.ajax({
				...
				success : function(data) {
					...
					fileData.secret = secret;
					if(obj.find("select[name='fileSecretLevel']").length==0){
							obj.attr("oldValue",secret);
					}else{
						obj.find("select[name='fileSecretLevel']").attr("oldValue",secret);
					}
		...
		this.getSecret = function() {
			if(state.options.showType=="table"){
				var rowData=state.tableGrid.jqGrid("getRowData",data.id);//这是获得 某一行的数据
				return rowData.attachmentsecret;
			}else{
...
//				html.push('<i class="icon ' + getFileTypeIco(data.ext) + ' uploader-file-ico"></i>');
		...
		this.bindFileItemInfos = function() {
			...
			this.$element.bind("mouseenter", function(e) {
				...
				//增加预览事件
				if (opts.allowPreview) {
                    $(fileInfos).find('.uploader-file-infos-preview').unbind().bind('click',function(e){
                        previewFile(data.fileId,data.ext);
                    })
				}
	...
	function getUploadFileData(target) {
		...
		$.each(state.fileItems, function(index, fileItem) {
			...
			if (data.status == $.fn.uploaderExt.status.inited) {
				...
				var orderBy=fileItem.getOrderBy();
	...
	function  removeFile(id,target){
		...
		fileItems.splice(index, 1);
		state.tableGrid.jqGrid("delRowData",id);
		//减少附件数量
		$(target).find('span.uploade-file-number').text(--state.fileNumber);
		state.uploader.removeFile(id, true); //移除想要放弃上传的附件
	}
	 ...
	 * @param options 操作属性
	 * @param rowObject 行数据对象
	 * @returns {String}
	 */
	function formatOpera(cellvalue, options, rowObject){
		var state= $.data(rowObject.target, 'uploaderExt');
		var html="";
		//已经保存的附件的操作列
		if(rowObject.fileId!=""){
			if(state.options.allowPreview){
				html=html+'<a id="tablePreview'+index+'" class="iconfont icon-eye3 tableeye'+index+'"'
			+'  style="cursor:pointer" title="预览" onClick=""></a>&nbsp;&nbsp;';
			}
			if(state.options.allowDelete) {
				html = html + '<a  id="tableDel'+index+'" class="iconfont icon-delete attachment_tableDel"'
					+ '  title="删除" ></a>&nbsp;&nbsp;';
			}
			if(state.options.allowDownload){
				html=html+'<a  id="tableDown'+index+'" class="iconfont icon-download "'
			+'  title="下载" ></a>&nbsp;&nbsp;';
			}
		}else{
			//未保存的附件的操作列
			html=html+'<a id="tableRomove'+index+'" class=" iconfont icon-delete"'
					+'  title="删除" ></a>&nbsp;&nbsp;'
		}
		return html;
	}
	...
	function formatStatus(cellvalue, options, rowObject){
		...
		if(rowObject.fileId!=""){
			...
			/*附件列表展示时进度条样式:20210309*/
			return '<span id="unUploadmsg'+index+'">未存附件</span><span class="uploader-file-progress-td" style="display:none"></span>';
		}
		return html;
	}
	/**
	...
	function delFile(id,target){
		...
		layer.confirm('确定要删除此附件吗？', {
							...
							$.ajax({
								cache : false,
								type : "post",
								url : state.options.deleteAttachUrl,
								dataType : "json",
								data : {
									fileId : id
								},
								...
								success : function(data) {
									var fileItem=getFileItemById(target,id);
									fileItem.removeFromList();
									state.tableGrid.jqGrid("delRowData",id);
									// 当删除文件后文件数量 <= 1 时，隐藏下载全部按钮
									var downAllBtn = $(target).find('.uploader-file-all-download');
									if (downAllBtn.length == 1 && downAllBtn.is(":visible") && state.fileNumber <= 1) {
										downAllBtn.hide();
									}
								}
	...
	function previewFile(fileId,type){
		//支持的文件格式
		var fileType = ['doc','docx','xls','xlsx','txt','pdf','jpeg','jpg','png','gif']
		//判断文件是否支持文件在线预览
		if($.inArray(type.toLowerCase(),fileType)>=0){
			//判断插件是否安装
			var extensionInstalled = ntkoBrowser.ExtensionInstalled();
			if(extensionInstalled){
				ntkoBrowser.openWindow(BASE_URL+"static/h5/ntko/editindex.jsp?fileId="+fileId,"","文件预览开发测试专用","6683E1D0FDDF45F343DC3A821A4560C89FC5B7E8","","","欢迎使用NTKO OFFICE文档控件平台版PLUS（2019）");
			}else{
				parent.layer.open({
					type:2,
					title:'下载插件',
					area:['330px','230px'],
					content:BASE_URL+"static/h5/ntko/exeindex.html"
				})
			}
		}else{
			parent.layer.open({
				content: '不支持'+type+'格式的文件预览',
			})
		}
	}
})(jQuery);
