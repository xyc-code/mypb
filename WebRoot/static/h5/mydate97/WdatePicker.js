...
if($("base").length > 0){
   if(typeof($("base")[0].href) != "undefined" && $("base")[0].href != ""){
       var name =$("base")[0].href;
       name=name.replace("//","/");
        var Path = name.split("/")
       if(Path[2]==""){
           return"/static/h5/mydate97/";
       }
       return "/"+Path[2] + "/static/h5/mydate97/";
   }
}
    //若无base标签且tomcat未配置上下文，此处的 pathname将会出现错误。
    var hostname = location.hostname;
    var pathname = location.pathname;
    var contextPath = pathname.split("/")[1];
    return "/"+contextPath + "/static/h5/mydate97/";
}();
