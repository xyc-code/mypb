 ...
 *
 * add:option.responsivePar responsive window's parent Dom.
 * add:$.resize  can listen nodeDom's change
 *
        ...
        styleUI : {
            ...
            Bootstrap : {
                ...
                base : {
                    ...
                    pgButtonBox : "",
                    icon_set: "glyphicon glyphicon-cog",
                    icon_export: "glyphicon glyphicon-floppy-save",
    ...
    $.fn.jqGrid = function( pin ) {
        ...
        return this.each( function() {
            ...
            var p = $.extend(true,{
                ...
                styleUI : "jQueryUI",
                responsive : false,
                responsivePar:$(window),
                restoreCellonFail : true,
                colFilters : {},
                colMenu : false
                //feng start
                ,editCellEvent :{} //编辑单元格时的事件
                //feng end
            }, $.jgrid.defaults , pin );
                ...
                dragEnd: function( events ) {
                    ...
                    if(this.resizing) {
                        ...
                        if(p.forceFit===true){
                            ...
                            p.tblwidth = $(ts).jqGrid("getGridParam","width")-20;
            ...
            var sortkeys = ["shiftKey","altKey","ctrlKey"],
                ...
                updatepager = function(rn, dnd) {
                    ...
                    if(pgboxes) {
                        ...
                        if(ts.p.pginput===true) {
                            $("#input"+tspg).html($.jgrid.template($.jgrid.getRegional(ts, "defaults.pgtext", ts.p.pgtext) || "","<input "+getstyle(stylemodule, 'pgInput', false, 'ui-pg-input') + " type='text' size='2'  value='0' role='textbox'/>","<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>"));
                            if(ts.p.toppager) {
                                $("#input_t"+tspg_t).html($.jgrid.template($.jgrid.getRegional(ts, "defaults.pgtext", ts.p.pgtext) || "","<input "+getstyle(stylemodule, 'pgInput', false, 'ui-pg-input') + " type='text' size='2' value='0' role='textbox'/>","<span id='sp_1_"+$.jgrid.jqID(pgid)+"_toppager'></span>"));
                            }
                        ...
                        if(ts.p.pgbuttons===true) {
                          ...
                          //分页按钮加载完成后执行该方法针对IE8图标库文件修改样式不重绘问题，其他浏览器该方法不做操作
                            redrawPagerPseudoEl(tspg);
                        }
                    }
                ...
                setPager = function (pgid, tp){
                    ...
                    /*
				201.12.12 qinym
				id=cent+_center display:none
			 */
                    $("#"+$.jgrid.jqID(pgid) )
                        .append("<div id='"+pgcnt+"' class='ui-pager-control' role='group'><table " + getstyle(stylemodule, 'pagerTable', false, 'ui-pg-table ui-common-table ui-pager-table') + "><tbody><tr><td id='"+lft+"' align='left'></td><td id='"+cent+"' align='center' style='white-space:pre;display:none;'></td><td id='"+rgt+"' align='right'></td></tr></tbody></table></div>")
                    ...
                    if(dir==="rtl") { pgl += str; }
                    if(ts.p.pginput===true) {
                        pginp= "<td id='input"+tp+"' dir='"+dir+"'>"+$.jgrid.template( $.jgrid.getRegional(ts, "defaults.pgtext", ts.p.pgtext) || "","<input class='ui-pg-input' type='text' size='2'  value='0' role='textbox'/>","<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>")+"</td>";
                    }
                    if(ts.p.pgbuttons===true) {
                        var po=["first"+tp,"prev"+tp, "next"+tp,"last"+tp, "set" + tp, "exportExcel" + tp], btc=getstyle(stylemodule, 'pgButtonBox', true, 'ui-pg-button'),
                            pot = [($.jgrid.getRegional(ts,"defaults.pgfirst",ts.p.pgfirst) || ""),
                                ...
                                ($.jgrid.getRegional(ts,"defaults.pgtset",ts.p.pgtset) || ""),
                                ($.jgrid.getRegional(ts,"defaults.pgtexportExcel",ts.p.pgtexportExcel) || "")];
                        if(dir==="rtl") {
                            po.reverse();
                            pot.reverse();
                        }
                        ...
                        if(isIE11||(/msie/i.test(navigator.userAgent) && !window.opera)){
                            widthStr=";width:240px";
                        }
                        if(ts.p.hasColSet){
                            pgl += "<td id='"+po[4]+"' class='"+btc+"' title='"+ pot[4] +"'" + "><span " + getstyle(stylemodule, 'icon_set', false, iconbase)+"></span>"+
                                "</td><td class='jq-btn-menu'><div class='row submenu' style='z-index:5;overflow:hidden"+widthStr+"'></div></td>";
                        }
                        if(ts.p.hasTabExport){
                            pgl += "<td id='"+po[5]+"' class='"+btc+"'  title='"+ pot[5] +"'" +"><span " + getstyle(stylemodule, 'icon_export', false, iconbase)+"></span></td>";
                        }
                        pgl += pginp !== "" ? sep:"";
                    ...
                    if(ts.p.pgbuttons===true) {
                        ...
                        $("#set"+$.jgrid.jqID(tp)).click( function(event) {
                            var cm =  $(ts).jqGrid("getGridParam","colModel");
                            var jqGridID =  $(ts).jqGrid("getGridParam","id");
                            var jqGridUrl =  $(ts).jqGrid("getGridParam","url");
                            var jqGridHeight =  $(ts).jqGrid("getGridParam","height");
                            var subMenuPar= $(this).parent().find('.jq-btn-menu'),
                                subMenuDom = subMenuPar.find('.submenu'),
                                os = $(this).offset(),
                                that = $(this);
                            subMenuDom.css({
                                bottom:$('body').innerHeight() - os.top + that.height()/2,
                                left:os.left
                            });
                            subMenuPar.toggleClass('on');
                            subMenuDom.empty();
                            subMenuDom.append(
                                /*清空按钮暂时隐藏*/
                                /*<a id='" + jqGridID + "_COL_CLEAR' href='javascript:void(0);' style='text-decoration:none;'><i class='icon icon-delete'></i></a>*/
                                "<div class='col-xs-12 alert-info userviewkads' style='background-color: #F0F0F0;'>" +
                                "<div style='padding: 0 0 0 6px;box-sizing: border-box;' class='col-xs-4'>列名称</div>" +
                                "<div style='padding: 0 0 0 10px;box-sizing: border-box;' class='col-xs-2'>显示</div>" +
                                "<div style='padding: 0 0 0 31px;box-sizing: border-box;text-align: center;' class='col-xs-3'>列宽</div>" +
                                "<div style='padding: 0 0 0 26px;box-sizing: border-box;text-align: center;' class='col-xs-3'>排序</div>" +
                                "</div>"
                            );
                            var template = '<div style="border-bottom: 1px solid #e8e8e8;background: #ffffff;height:33px" class="col-xs-12 userviewkads" id=\"kads_#1#\">'+
                                '<div class="col-xs-4" style="padding: 0 6px;box-sizing: border-box;overflow: hidden; white-space: nowrap; text-overflow: ellipsis;word-break: break-all;">#0#</div>' +
                                '<div style="padding: 3px 9px;box-sizing: border-box;" class="col-xs-2"><input onclick="$(\'#centerDiv .userviewkads\').css(\'background\',\'#FFFFFF\');" id="#1#"   class="' + jqGridID + 'columnSetting#6#" data=\'{\"columnLabel\":\"#0#\", \"columnField\": \"#1#\", \"columnShow\":#2#}\' type="checkbox"  #4#  style="#5#"></div>' +
                                '<div style="padding: 0 17px;box-sizing: border-box;" class="col-xs-3" ><input onclick="$(\'#centerDiv .userviewkads\').css(\'background\',\'#FFFFFF\');" style="height:25px;width:45px;margin-top:3px" id="#1#widthInput" class="form-control input-sm " onkeyup="this.value=this.value.replace(/\\D/g,\'\')" maxlength="3" value="#3#"/></div>'+
                                '<div style="padding: 0 0 0 20px;box-sizing: border-box;" class="col-xs-3"><i style="margin-right: 2px;" class="icon icon-layer-up iconLayerSort"></i><i class="icon icon-layer-down iconLayerSort"></i></div>'+
                                '</div>';
                            var centerStr = "";
                            for(var i = 0; i < cm.length; i++ ){
                                if(!(oldObject['oldModel'][oldObject['oldModel'].map(function (item){return item.name}).indexOf(cm[i].name)].hidden)===true){
                                    var temp = template.concat();
                                    var preamArray = [];
                                    if((typeof(cm[i].hasInSetting) !='undefined' && cm[i].hasInSetting != true) || cm[i].name === 'cb' || cm[i].name==='subgrid' || cm[i].name==='rn'
                                        || cm[i].name=== 'ID' || cm[i].name === 'id' || cm[i].name === 'VERSION' || cm[i].name === 'version'){
                                        continue;
                                    }
                                    //如果label不包含中文或者id，则不显示,如果时表示了必须的字段，则不能设置显隐
                                    if((!/[\u4E00-\u9FA5]+/.test(cm[i].label)) || (cm[i].label.indexOf("id") != -1) || (cm[i].label.indexOf("remind") != -1)){
                                        continue
                                    }
                                    preamArray.push(cm[i].label);
                                    preamArray.push(cm[i].name);
                                    preamArray.push(cm[i].hidden);
                                    preamArray.push(cm[i].width);
                                    if(cm[i].hidden){
                                        preamArray.push(" ");
                                    }else{
                                        preamArray.push("checked");
                                    }
                                    if(cm[i].label.startsWith('<span') || cm[i].label.startsWith('<SPAN')){
                                        preamArray.push("display:none;");
                                        preamArray.push("hidden");
                                    }else{
                                        preamArray.push(" ");
                                        preamArray.push("");
                                    }
                                    for(var j = 0; j < preamArray.length; j++){
                                        temp = temp.replaceAll("#" + j + "#", preamArray[j]);
                                    }
                                    centerStr += temp;
                                }
                            }
                            subMenuDom.append('<div id="centerDiv" style="height: '+(jqGridHeight/2)+'px;width:260px;max-height:220px;min-height:24px;overflow-x:auto">'+centerStr+'</div>');
                            subMenuDom.append(
                                "<div class='col-xs-12 toolbar-right' style='padding: 3px 19px'>" +
                                "<a  id='" + jqGridID + "_COL_SAVE' href='javascript:void(0)' class='btn btn-primary form-tool-btn btn-sm btn-point' role='button' title='保存'><i class='glyphicon glyphicon-ok-sign'></i>  保存</a>"+
                                "<a  id='" + jqGridID + "_COL_RESET' href='javascript:void(0)' class='btn btn-primary form-tool-btn btn-sm' role='button' title='重置'><i class='glyphicon glyphicon-minus-sign'></i>  重置</a>" +
                                "<a  id='" + jqGridID + "_COL_CLOSE' href='javascript:void(0)' class='btn btn-primary form-tool-btn btn-sm' role='button' title='关闭'><i class='glyphicon glyphicon-remove-sign'></i> 关闭</a></div>"
                            );
                            function removeBackground(){
                                $("#centerDiv .userviewkads").css("background","#FFFFFF");
                            }
                            //向上移动
                            $('.icon-layer-up').bind('click',function(){
                                removeBackground();
                                var $this = $(this);
                                var curTr = $this.parents('#centerDiv .col-xs-12');
                                var prevTr = $this.parents('#centerDiv .col-xs-12').prev();
                                if(prevTr.length == 0 || prevTr[0]["className"] == 'col-xs-12 alert-info userviewkads'){
                                    layer.msg('已经是第一行！');
                                }else{
                                    prevTr.before(curTr);
                                    $(prevTr).css("background","#FFFFFF")
                                    $(curTr).css("background","#E5F3FF")
                                }
                            })
                            //向下移动
                            $('.icon-layer-down').bind('click',function(){
                                removeBackground();
                                $("#centerDiv .userviewkads").css("background","#FFFFFF");
                                var $this = $(this);
                                var curTr = $this.parents("#centerDiv .col-xs-12");
                                var nextTr = $this.parents("#centerDiv .col-xs-12").next();
                                if(nextTr.length == 0 || nextTr[0]["className"] == 'col-xs-12 toolbar-right'){
                                    layer.msg('已经是最后一行！');
                                }else {
                                    nextTr.after(curTr);
                                    $(curTr).css("background","#E5F3FF")
                                    $(nextTr).css("background","#FFFFFF")
                                }
                            })

                            var jqGridwidth =  $(ts).jqGrid("getGridParam","width");
                            $("#"+ jqGridID + "_COL_SAVE").click( function(event) {
                                var columnArray = [];
                                $("." + jqGridID + 'columnSetting').each(function(){
                                    var data = $.parseJSON($(this).attr("data"));
                                    data['columnWidth'] = $("#"+data['columnField']+"widthInput").val()
                                    if($(this).is(":checked")){
                                        data.columnShow = true;
                                        data['attribute01'] = columnArray.length;
                                        columnArray.push(data);
                                        $(ts).setGridParam().showCol(data.columnField);
                                    }else{
                                        $(ts).setGridParam().hideCol(data.columnField);
                                        data.columnShow = false;
                                        data['attribute01'] = columnArray.length;
                                        columnArray.push(data);
                                    }
                                });
                                var columnArrayJson = JSON.stringify(columnArray);
                                $(ts).jqGrid("saveOrEditDtoToRedis", {columnArrayJson: columnArrayJson,saveOrEditDtoToRedis: jqGridwidth});
                                subMenuDom.empty();
                                subMenuPar.removeClass('on');
                            });
                            //取消选中
                            $("#"+ jqGridID + "_COL_CLEAR").click( function(event) {
                                $("[type='checkbox']").prop("checked","");
                            });

                            $("#"+ jqGridID + "_COL_CLOSE").click( function(event) {
                                subMenuDom.empty();
                                subMenuPar.removeClass('on');
                            });

                            $("#"+ jqGridID + "_COL_RESET").click( function(event) {
                                $(ts).jqGrid("deleteDtoToRedis");
                                subMenuDom.empty();
                                subMenuPar.removeClass('on');
                            });

                        });
                        $("#exportExcel"+$.jgrid.jqID(tp)).click( function() {
                            //var hrefcolumn = null;//跳转详细页超链接
                            var columnFieldsOptions = [];
                            $(ts).jqGrid("endEditCell");
                            var allColsOption =  $(ts).jqGrid("getGridParam","colModel");
                            for(var i = 0;i < allColsOption.length; i++ ){
                                if(allColsOption[i].colmenu){
                                    columnFieldsOptions.push(allColsOption[i]);
                                    //if(i == 2){
                                    //    hrefcolumn = allColsOption[i];
                                    //}
                                }
                            }
                            var jqGridID =  $(ts).jqGrid("getGridParam","id");
                            var columnArray = [];
                            if (null != columnFieldsOptions && columnFieldsOptions.length > 0) {
                                for (var i = 0; i < columnFieldsOptions.length; i++) {
                                    var columnOptions = columnFieldsOptions[i];
                                    var hidden = false;
                                    var colmenu = false;
                                    var name = columnOptions.name;
                                    if (columnOptions.hidden) {
                                        hidden = columnOptions.hidden;
                                    }
                                    if(columnOptions.colmenu){
                                        colmenu = columnOptions.colmenu;
                                    }
                                    if(hidden || name == 'id' || name == 'ID'){
                                        columnArray.push(name);
                                    }
                                }
                            }
                            var jqGridFields = JSON.stringify(columnFieldsOptions).replaceAll('"title":','"customoption":').replaceAll('"label":','"title":').replaceAll('"name":','"field":').replaceAll('"key":','"checkbox":');
                            //将bootstrap属性与easyui属性特殊字段对应
                            var rows = $('#' + jqGridID).jqGrid('getRowData');
                            for(var i = 0; i < rows.length; i++){
                                for(var j=0;j<allColsOption.length;j++){
                                    if(allColsOption[j].name!="cb" && ((rows[i][allColsOption[j].name].startsWith('<a') && rows[i][allColsOption[j].name].endsWith('</a>')) || (rows[i][allColsOption[j].name].startsWith('<A') && rows[i][allColsOption[j].name].endsWith('</A>')))){
                                        rows[i][allColsOption[j].name] = $(rows[i][allColsOption[j].name])[0].innerText;
                                    }
                                }
                                //if((rows[i][hrefcolumn.name].startsWith('<a') && rows[i][hrefcolumn.name].endsWith('</a>')) || (rows[i][hrefcolumn.name].startsWith('<A') && rows[i][hrefcolumn.name].endsWith('</A>'))){
                                //    rows[i][hrefcolumn.name] = $(rows[i][hrefcolumn.name])[0].innerText;
                                //}
                            }
                            ...
                            var expSearchParams = {
                                dataGridFields : jqGridFields,// 表头信息集合
                                datas : datas,// 数据集
                                unContainFields : unContainFields,//不导出的列
                                sheetName : 'sheet1',// 如果该参数为空，默认为导出的Excel文件名
                                fileName : 'exportExcel'  // 导出的Excel文件名
                            };
                            var actionUrl = "platform/api/excelExportController/exportExcelClientData";
                            var iframeName = "_iframe_" + "xlsExport";
                            var formName = "_form_" + "xlsExport";
                            //先销毁对象，再创建
                            ...
                            if (jQuery("#" + iframeName).length == 0) {
                                ...
                                for (var key in expSearchParams) {
                                    var headInput = document.createElement("input");
                                    headInput.setAttribute("name", key);
                                    headInput.setAttribute("type", "hidden");
                                    if (typeof(expSearchParams[key]) == 'string') {
                                        headInput.setAttribute("value", expSearchParams[key]);
                                    } else {
                                        var jsonStr = JSON.stringify(expSearchParams[key]);
			...
			*/
            ...
            for(i=0;i<this.p.colNames.length;i++){
                ...
                thead += "<div class='ui-th-div";
                if (tmpcm.name === "cb"){
                    thead += " cb";
                }
                thead += "' id='jqgh_"+ts.p.id+"_"+tmpcm.name+"' "+tdc+">"+ts.p.colNames[i];
                if(!tmpcm.width)  {
                    tmpcm.width = 150;
                } else {
            ...
            if(ts.p.responsive) {
                ...
                ts.p.responsivePar.on( orientationEvent, function(){
    ...
    $.jgrid.extend({
        ...
        setGridWidth : function(nwidth, shrink) {
            return this.each(function(){
                ...
                if(shrink===true) {
                    ...
                    $('table:first',$t.grid.hDiv).find("tr:eq(0) th").each(function(index){
                        this.style.width = $t.grid.headers[index].width + "px";
                    });
                }
                if($t.p.tblwidth) {
        ...
        setLabel : function(colname, nData, prop, attrp ){
            return this.each(function(){
                ...
                if(pos>=0) {
                    var thecol = $("tr.ui-jqgrid-labels th[id$='"+colname+"']",$t.grid.hDiv);
                    if (typeof  $t.id != undefined){
                        thecol = $("tr.ui-jqgrid-labels th[id='"+$t.id+"_"+colname+"']",$t.grid.hDiv);
                    }
                    if (nData){
    ...
    $.jgrid.extend({
            ...
            return this.each(function (){
                ...
                if($t.p.selfColumn!=""){
                    ...
                    var ids=$($t).jqGrid("getDataIDs");
                     if(!cc.hasClass("_selfColumn")){
                         if(ids.length==1){
                             if(($t.p.savedRow.length==1)&&($t.p.savedRow[0].ic===undefined)){
                                $t.p.savedRow.length=0;
                            }else if($t.p.savedRow.length==999&&$t.p.savedRow[998].ic!=undefined){
                                  $t.p.savedRow.length=0;
                             }
                         }else{
                ...
                if ($t.p.savedRow.length>0) {
                    ...
                    //start ws
                    var currCol = $t.p.colModel[$t.p.savedRow[0].ic], currColName = $.jgrid.jqID(currCol.name);
                    var inputContent = $("#"+$t.p.iRow+"_"+currColName,$t.rows[$t.p.iRow]).val();
                    var flag = (/<script.*?>.*?<\/script>/ig).test(inputContent);
                    if(flag){
                        $t.p.afterSaveCell.call($t, $t.rows[iRow].id, currCol.name, inputContent, $t.p.iRow, currCol);
                        return;
                    }else{
                        // save the cell
                ...
                if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell") && (!$.isFunction($t.p.isCellEditable) || $t.p.isCellEditable.call($t,nm,iRow,iCol))) {
                    ...
                    $("input, select, textarea",cc).on("keydown",function(e) {
                        ...
                        //end ws
                        if (e.keyCode === 9 || e.keyCode === 13)  { //加了e.keyCode === 13 enter键切换 2018.7.4
                            if(!$t.grid.hDiv.loading ) {
        ...
        saveCell : function (iRow, iCol){
            return this.each(function(){
                ...
                if (!$t.grid || $t.p.cellEdit !== true) {return;}
                //start ws
                /*var currCol = $t.p.colModel[iCol], currColName = $.jgrid.jqID(currCol.name);
			var inputContent = $("#"+iRow+"_"+currColName,$t.rows[iRow]).val();
			var flag = (/<script.*?>.*?<\/script>/ig).test(inputContent);
            if(flag){return;}*/
                //end ws
                ...
                if(fr !== null) {
                    ...
                    if (v2 !== $t.p.savedRow[fr].v){
                        ...
                        // 回车保存后文字跑左上角问题，其实和css有关,但css也自定义了，于是就从这里修复好了
                        $(cc).removeClass('edit-cell');
                        //end
                        $($t).jqGrid("restoreCell",iRow,iCol);
                        //feng start
                        //单元格值没有变化时，编辑下个单元格
                            ...
                            if ( nm !== 'cb' && nm !== 'subgrid') {
                                if (mthd==='dirty') {
                                    if ($(this).hasClass('dirty-cell')) {
                                        try {
                                            if(($t.p.colModel[i].editoptions) && ((typeof $t.p.colModel[i].editoptions.custom_element!="undefined" && $t.p.colModel[i].editoptions.custom_element.name == "spinnerElem" )|| (typeof $t.p.colModel[i].editoptions.hascommafy!="undefined" && $t.p.colModel[i].editoptions.hascommafy))){//判断是否为数字
                                                res[nm] = delcommafy($.unformat.call($t,this,{rowId:$t.rows[j].id,colModel:$t.p.colModel[i]},i),true);
                                            }else{
        ...
        //增加新方法，将编辑中的单元格保存
        endEditCell : function(){
            return this.each(function (){
                this.flag = false; //校验没通过
                var $t = this;
                var highlight = $(this).jqGrid('getStyleUI',$t.p.styleUI+'.common','highlight', true);
                if (!$t.grid || $t.p.cellEdit !== true) {return;}
                if($t.p.selfColumn!=""){
                    if($t.p.savedRow.length>1||$t.p.savedRow.length===999){
                        $t.p.savedRow.length=0;
                    }
                }

                if ($t.p.savedRow.length > 0) {
                    var iRow = $t.p.savedRow[0].id;
                    var iCol = $t.p.savedRow[0].ic;
                    //start ws 2018.7.9
                    var currCol = $t.p.colModel[iCol], currColName = $.jgrid.jqID(currCol.name);
                    var inputContent = $("#"+$t.p.iRow+"_"+currColName,$t.rows[$t.p.iRow]).val();
                    var flag = (/<script.*?>.*?<\/script>/ig).test(inputContent);
                    if(flag){
                        $t.p.afterSaveCell.call($t, $t.rows[iRow].id, currCol.name, inputContent, $t.p.iRow, currCol);
                        this.flag = true; //校验没通过
                        return;
                    }else{
                        $($t).jqGrid("saveCell", iRow, iCol);
                        $("td:eq("+iCol+")",$t.rows[iRow]).removeClass("edit-cell " + highlight);
                    }
                    //end ws 2018.7.9
                }
                $t.p.iCol = undefined; $t.p.iRow = undefined;
            });
        }
/// end  cell editing
    });
            ...
            return this.each(function(){
                ...
                for (i = 0; i < cml; i++) {
                    ...
                    if (iCol >= 0) {
                        ...
                        var l = 0,numberOfColumns1 = 0;
                        for (var j=i;l<numberOfColumns && j<cml;j++){
                            var s = "Id";
                            var d = colModel[j].label.length - s.length;
                            if (!(colModel[j].hidden && (d >= 0 && colModel[j].label.lastIndexOf(s) == d))) {
                        ...
                        $colHeader.attr("style","border-top:1px solid #ddd");
...
//Editing
        editRow : function(rowid,keys,oneditfunc,successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc) {
            ...
            o = $.extend(true, {
                ...
                mtype: "POST",
                focusField : true,
                /*add by suntao 此处是为了默认开启行编辑时，可以指定某些列开启 某些不开启*/
                selfColumn:""
            }, $.jgrid.inlineEdit, o );
                ...
                if (editable === "0" && !$(ind).hasClass("not-editable-row")) {
                    ...
                    $('td[role="gridcell"]',ind).each( function(i) {
                        ...
                        if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
                            if($t.p.autoencode) { tmp = $.jgrid.htmlDecode(tmp); }
                            svr[nm]=tmp;
                            var selfColumn=false;
                            if(cm[i].editable===true) {
                                if (o.selfColumn===""){
                                    selfColumn=true
                                }else if(o.selfColumn.indexOf(cm[i].name) != -1 ){
                                    $t.p.selfColumn=o.selfColumn;
                                    selfColumn=true
                                }
                                if(selfColumn) {
                                    if (focus === null) {
        ...
        if(unformatFunc !== undefined && $.isFunction(unformatFunc) ) {
            ...
            switch(formatType) {
                ...
                case 'actions':
                    return "";
                default:
                    var cellt = $(cellval).text();
                    if(cellt && (cellt==='&nbsp;' || cellt==='&#160;' || (cellt.length===1 && cellt.charCodeAt(0)===160))) {
                        ret="";
                    }else{
                        ret= cellt;
                    }
                ...
                var currentRow, rowNode,
                    addRow = function ( row, header ) {
                        ...
                        for ( var key  in data.header[0]) {
                            ...
                            if(!header) {
                                v = v !== '' ? $.jgrid.formatCell( v, data.map[i], row, cm[data.map[i]], $t, false) : v;
                            }
