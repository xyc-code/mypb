 ...
 * version: 1.7.4
 * number:001
 * 2019.12.20版本  跟随  金航数码科技有限责任公司20200320   产品发送
 */
"use strict";
var varNtkoGUID = Math.random().toString(36);
var ntkoWinOpen=null;
var  timer;
...
ExtensionInstalled:function()
{
   ...
   if(!ntkobr){
	    if(browser=="IE"){
		    var ntkoarray=["NTKO.OfficeControl20160_X64","NTKO.OfficeControl20160","NTKO.OfficeControl20161_X64","NTKO.OfficeControl20161","NTKO.OfficeControlQiYe","NTKO.AttachManage","NTKO.OfficeControlQiYe20162","NTKO.OfficeControlQiYe20162_X64","NTKO.OfficeControl20162","NTKO.OfficeControl20162_X64"];
   ...
   var strURL=ntkoBrowser.NtkoStrURL(strURL);
	...
	if(ntkobr){
		window.postMessage({ type: "FROM_NTKO_PAGE", text: jsonValue }, "*");
		 ...
		 ntkoWinOpen=window.open("",ntkowin,"height="+ntkoheight+", width="+ntkowidth+", top=0,left=0,titlebar=no,toolbar =no, menubar=no, scrollbars=no, resizable=yes, location=no, status=no");
         if(ntkoWinOpen.location.href=="about:blank"){
		 ntkoWinOpen.close();
		 }else{
		 ntkoWinOpen.close();
		 }
...
openWindow:function( strURL,IsShowOnExtendMonitor,ProductCaption, ntkoBrowserProductKey, ntkoBrowserNoExpireKey, ntkoBrowserOptions,varUrlData )
{
	  ...
	  if(varUrlData!=null&&varUrlData!=""){
         ...
         if (window.top!=window.self) {
        	 ...
        	 if(winTopHref.indexOf("#")!=-1){
				 //var winend= winTopHref.lastIndexOf("#");
				 var winend= winTopHref.indexOf("#");
				 winTopHref= winTopHref.substring(0,winend);
			 }
              if(winTopHref.indexOf("?")!=-1){
				 var winend= winTopHref.lastIndexOf("?");
				winTopHref= winTopHref.substring(0,winend);
	...
	jsonValue += '}';
		  ...
		  if(ntkobr){
            ...
            if(ntkoWinOpen.location.href=="about:blank"){
				...
				ntkoWinOpen.resizeTo(screen.availWidth,screen.availHeight);
				ntkoWinOpen.focus();
			}else{
				ntkoWinOpen.focus();
			}
