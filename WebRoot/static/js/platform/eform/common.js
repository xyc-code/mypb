...
function setLinkagePara(value,linkageColumnid,pageParams,linkageResult) {
	linkageResult = decodeURIComponent(linkageResult);
    $.ajax({
	...
	$.ajax({
		...
		success: function (backData, status) {
			...
			if(linkageResultType == "2") {
				$("#" + linkageColumnid.toUpperCase()).val(backData).trigger("change");
			}
		}
	});
}
...
function GenNonDuplicateID(len, radix) {
...
function  customSetValidate(domId,value) {
	if(customValidate != null && customValidate != undefined){
		var regexpTxt = customValidate.get(domId)
		if(regexpTxt != null && regexpTxt != "" && regexpTxt != undefined){
			return regexpTxt.test(value)
		}
	}
	return true;
}
