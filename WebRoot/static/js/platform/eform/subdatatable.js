...
function initHiddenAttrArea(curId){
	...
	if (jsondata.elementType == "select"){
        ...
        $("#hiddem_attr_mult").html('<li class="property-list"><span class="property-title">小数位数：</span><input type="text" name="attribute02" id="attribute02" value="2" class="input-medium"></li>'
            ...
            +'<li class="property-list"><span class="property-title">最大值：</span><input type="text" name="maxValue" id="maxValue" class="input-medium"></li>'
            +'<li class="property-list"><span class="property-title">默认值：</span><input type="text" name="defaultvalue" id="defaultvalue" class="input-medium"></li>'
            +'<li class="property-list"><span class="property-title">币种：</span><select id="selectCurrency" name="selectCurrency"  placeholder="请选择币种" class="SlectBox 4col active">'
            +'<option value="￥">￥-人民币</option>'
            +'<option value="$">$-美元</option>'
            +'<option value="€">€-欧元</option>'
            +'<option value="￡">￡-英镑</option>'
            +'<option value="">无币种</option>'
            +'</select></li>');
        $("#hiddem_attr_mult").find("input").on("keyup", function () {
            update(curId);
        });
		...
		$("#hidden_attr_area").html('' +
            '<span class="property-title">数据字典配置</span><a title="数据字典配置" id="propertyBtn"><i class="fa fa-fw fa-lg fa-pencil-square-o"></i></a>' +
				'<input type="hidden" name="dictionaryList" id="dictionaryList" class="input-medium">' +
    			'	<ul id="dictionaryArea" class="item-list">' +
				'</ul>');
        $("#hiddem_attr_mult").attr("style","");
        $("#hiddem_attr_mult").html('<li class="property-list"><span class="property-title">键值存储：</span>' +
            '<input type="checkbox" name="colIsKeyValue" value="Y" class="item-width"></li>' +
            '<li class="property-list" id="dicAutoTransformLi"><span class="property-title">自动转换：</span>' +
            '<input type="checkbox" name="dicAutoTransform" value="Y" class="item-width"></li>');

        $("#hiddem_attr_mult").find("input").on("change", function () {
            update(curId);
        });

        $("input[name='colIsKeyValue']").on("change",function(){
            var isChecked = $("input[name='colIsKeyValue']").prop("checked")
            if(isChecked){
                $("#dicAutoTransformLi").show();
                $("input[name='dicAutoTransform']").prop('checked',true);
            }else{
                $("#dicAutoTransformLi").hide();
            }
        })

		$("#propertyBtn").click(function () {
            var colIsKeyValue = $("input[name='colIsKeyValue']:checked").val();
            linkagedialog = top.layer.open({
                type: 2,
                title: '数据字典配置维护',
                skin: 'bs-modal',
                area: ['60%', '85%'],
                maxmin: false,
                content: "avicit/platform6/eform/formdesign/js/plugins/dictionary-box/dictionary-property.jsp?isMain=0&colIsKeyValue="+colIsKeyValue,
                btn: ['确定', '取消'],
                success: function (layero, index) {
                    //传入参数，并赋值给iframe的元素
                    ...
                    setTimeout(function () { iframeWin.changeTargetNameFunc(dataJsonList); }, 1500);
                },
                yes: function(index, layero){
                    var frm = layero.find('iframe')[0].contentWindow;
                    var reData = frm.commitForm();
