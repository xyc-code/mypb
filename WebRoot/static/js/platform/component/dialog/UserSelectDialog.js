...
UserSelectDialog = function(id,width,height,content,title,model,buttons,appendIframe){
...
//				iframe.attr("src",content);
				...
				jQuery(document.body).append("<div id='" + id + "' style='overflow:auto;'>" + content + "</div>");
			}
		} else {
	...
	this.show = function(){
		...
		iframe.attr("src",content);
		if (appendIframe){
			iframe.parent().parent().parent().parent().append("<iframe class='platform6BackgroundIframe' frameborder='no'></iframe>");
		}
	};
};
