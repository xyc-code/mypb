...
function AddSecSignFromURL(URL)
{
	try {
		/**
		 ...
		 URL	必选	string	服务器印章图片URL路径
		 left	可选	long	相对光标的位置，一般传递0
		 top	可选	long	相对光标的位置，一般传递0
		 relative	可选	long	定位相对值，一般传递1
		 PrintMode	可选	long	控制打印 0=不打印，1=打印灰色，2=打印原始
		 IsUseCertificate	可选	bool	是否使用证书 true或者false，默认值是false
		 IsLocked	可选	bool	是否锁定印章 true或者false，默认值是false
		 IsCheckDocChange	可选	bool	是否检查文档改变 一般应该传true
		 IsShowUI	可选	bool	是否显示签章设置对话框 默认值是true
		 signpass	可选	string	默认密码 如果指明了此参数，在签章时将不再提示用户输入印章密码。
		 IsAddComment	可选	bool	是否允许用户输入批注 默认值是true。
		 IsBelowText	可选	bool	是否盖在文字下方 默认值是true。

		 */
	   if(TANGER_OCX_OBJ.doctype==1||TANGER_OCX_OBJ.doctype==2){
	   		TANGER_OCX_OBJ.AddSecSignFromURL("匿名用户",URL,0,0,1,2,false,false,true);
	   }else{
...
//清稿下载
function TANGER_OCX_DoDownLoad()
{
...
{
	TANGER_OCX_OBJ.IsStrictNoCopy  = boolvalue;
}
...
//域同步
function JsonToBookMark(wordFieldArray){
	try {
		for(var key in wordFieldArray){
			var value = wordFieldArray[key];
