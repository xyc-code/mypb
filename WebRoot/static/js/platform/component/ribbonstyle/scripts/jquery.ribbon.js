    ...
    $.fn.Ribbon = function(ribbonSettings) {
        ...
        $('.ribbon a').each(function() {
            if ($(this).attr('accesskey')) {
                $(this).append('<div rel="accesskeyhelper" style="display: none; position: absolute; background-image: url(avicit/platform6/modules/system/ribbonstyle/images/accessbg.png); background-repeat: none; width: 16px; padding: 0px; text-align: center; height: 17px; line-height: 17px; top: ' + $(this).offset().top + 'px; left: ' + ($(this).offset().left + $(this).width() - 15) + 'px;">' + $(this).attr('accesskey') + '</div>');
            }
        });

        $('head').append('<link href="avicit/platform6/modules/system/ribbonstyle/ribbon/themes/' + settings.theme + '/ribbon.css" rel="stylesheet" type="text/css" />" />');
        ...
        if (isLoaded) {
            ...
            $('.ribbon-list div img[src*="/avicit/platform6/modules/system/ribbonstyle/images/arrow_down.png"]').remove();
        }

        if (!settings.backStage) {
            $('.orb li:first ul:first').append('<img src="avicit/platform6/modules/system/ribbonstyle/ribbon/themes/' + settings.theme + '/images/menu_bottom.png" style="margin-left: -10px; margin-bottom: -22px;" />');
            $('.orb li:first ul:first').prepend('<img src="avicit/platform6/modules/system/ribbonstyle/ribbon/themes/' + settings.theme + '/images/menu_top.png" style="margin-left: -10px; margin-top: -22px;" />');
        }

        $('.ribbon-list div').each(function() { if ($(this).children('ul').length > 0) { $(this).append('<img src="avicit/platform6/modules/system/ribbonstyle/ribbon/themes/' + settings.theme + '/images/arrow_down.png" style="float: right; margin-top: 5px;" />') } });
