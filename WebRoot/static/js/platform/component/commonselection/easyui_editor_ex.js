(function($) { 
	$.extend($.fn.datagrid.defaults.editors, {   
	    CommonSelector: {   
	        ...
	        var showTabsList = options.showTabsList;
	        ...
	        //	divObj.children("input").validatebox(options);
				...
				var combo = $('<input class="onCellClickCommonSelector" readonly="readonly type="text" name ="commonSelector_datagrid-row-editor">').appendTo(container);
				combo.combobox(options || {textField:targetName,valueField:valueField});	
				container.on("click",function(){ //绑定点击事件
					$("#"+dataGridId).datagrid("options").popWindowCount++; //选人，选部门，选角色，选群组，选岗位弹出框数量控制
	        		openCommonSelector(selectType,dataGridId,Number(rowIndex),dialogShowField,targetId,targetName,callBackFun,extParameter,selectLimit,dialogWidth,dialogHeight,splitChar,afterCloseFun,multipleOrg,displaySubDeptUser, showTabsList);
				});
				return combo;
	        }, 
 ...
 */
function openCommonSelector(selectType,dataGridId,rowIndex,dialogShowField,targetId,targetName,callBackFun,extParameter,selectLimit,dialogWidth,dialogHeight,splitChar,afterCloseCallFun,multipleOrg,displaySubDeptUser,showTabsList){
	var commonSelector = new GridCommonSelector(selectType,dataGridId,rowIndex,dialogShowField,{targetId:targetId,targetName:targetName},callBackFun,extParameter,dialogWidth,dialogHeight,selectLimit,splitChar,afterCloseCallFun,multipleOrg,displaySubDeptUser,showTabsList);//
