...
function addPmTab(tabName,targetUrl,icon,menuCode,iconPosition){
	var initIframeHeight ;
	if($("#divBodyWidth").css("display") == "block"){
		initIframeHeight = getHeight()-bannerHeight;
	}else{
	...
	if ($('#tabs').tabs('exists',tabName)){
				...
				if(targetUrl != '' && targetUrl.indexOf('javascript') == -1){//判断当前的targetUrl是url时添加新的tab
					$('#tabs').tabs('add',{
						title:tabName,
						iconCls : "_" + randomIcon+'tabIcon',
						content:"<iframe id=iframePmBody class='" + tabName  + "' style='overflow:hidden; height:"+initIframeHeight+"px;width:100%' src="+targetUrl+"  frameborder=0></iframe>",
						closable:closeFlag,
						onclick:closeMegaDrop()
					});
					$("._" + randomIcon + "tabIcon").css("background","url('" + icon + "') no-repeat").css("background-position",iconPosition);
...
function refreshTodoOnCloseTab(){
	var obj = $(".panel").contents().find("iframe").contents().find("iframe");
	if(typeof(obj)!="undefined"){
		obj.each(function(){
			if(this.src.indexOf("Todo.jsp")>0){
				this.src=this.src;
			}
		});
	}
...
$(function(){   
    ...
    $('#tabs').tabs({
		onSelect : function(title){
			...
			if(flag.$isSplitPage){
				$('.spliter_up').css("display","block");
				$('.spliter_down').css("display","block");
			}else{
				$('.spliter_up').css("display","none");
				$('.spliter_down').css("display","none");
			}
			return;
		}
    });
...
function refreshMessageCount(){
	$.ajax({
	     ...
	     url: baseUrl+"platform/sysmessage/sysMessageController/getSysMessageCount",//要访问的后台地址
