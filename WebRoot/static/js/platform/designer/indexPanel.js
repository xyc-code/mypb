...
MyAction.prototype.open = function() {
	...
	toolbar.attachEvent("onClick", function(id) {
		...
		if (id == "open") {
			...
			var pdId = $.trim(tree.getUserData(nodeId, "pdId"));
			var deployId = $.trim(tree.getUserData(nodeId, "deployId"));
			...
			dhtmlx.confirm({
				...
				callback : function(result) {
					if (result) {
						var url = _controlPath + "downJpdl?id=" + nodeId + "&type=" + type + "&name=" + nodeName + "&extName=jpdlx"+"&pdId="+pdId+"&deployId="+deployId;
						var t = new exportData('iframe', 'form', {}, url);
						t.excuteExport();
					}else{
						var url = _controlPath + "downJpdl?id=" + nodeId + "&type=" + type + "&name=" + nodeName + "&extName=jpdl"+"&pdId="+pdId+"&deployId="+deployId;
						var t = new exportData('iframe', 'form', {}, url);
						t.excuteExport();
					}
				}
...
MyAction.prototype.addProcess = function(parentXml, node) {
	for (var i = 0; i < node.length; i++) {
		...
		var userdata_pdid = ComUtils.createElement("userdata");
		userdata_pdid.setAttribute("name", "pdId");
		userdata_pdid.appendChild(ComUtils
			.createTextNode(node[i].pdId));
		nodeXml.appendChild(userdata_pdid);
		var userdata_deployId = ComUtils.createElement("userdata");
		userdata_deployId.setAttribute("name", "deployId");
		userdata_deployId.appendChild(ComUtils
			.createTextNode(node[i].deployId));
		nodeXml.appendChild(userdata_deployId);
