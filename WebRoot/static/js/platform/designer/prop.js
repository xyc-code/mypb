...
$(function() {
    ...
    $('table.need_check').find('input[type="checkbox"]').on('change',function (){
        ...
        if(!$(this).parents('tr').next().find('td').eq(0).text()){
			...
			if(!$(this).parents('tr').next().next().find('td').eq(0).text()){
				$(this).parents('tr').next().next().find('input').attr('disabled',!key);
			}
        }
    });
    //处理方式下拉框
...
//            liu_cheng_zhuan_fa.attr('disabled',true);
    ...
    $('select.js_status').on('change',function (){
        if($(this).val()==='allfinish' || $(this).val()=='dept_one'){
    ...
    //手动催办
	$("input[name='handHastenTask']").on("change", function(){
		var value = $(this).val();
		var $table = $(this).parents("table");
		if(value == "yes"){
			$table.next().show();
		}else{
			var autoHastenTask = $table.find("input[name='autoHastenTask']:checked").val();
			if(autoHastenTask == "no"){
				$table.nextAll().hide();
			}
		}
	});
	//自动催办
	$("input[name='autoHastenTask']").on("change", function(){
		var value = $(this).val();
		var $table = $(this).parents("table");
		if(value == "yes"){
			$table.nextAll().show();
		}else{
			var handHastenTask = $table.find("input[name='handHastenTask']:checked").val();
			if(handHastenTask == "no"){
				$table.nextAll().hide();
			}else {
				$table.next().next().hide();
			}
		}
	});
});