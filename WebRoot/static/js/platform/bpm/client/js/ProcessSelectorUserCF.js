			...
			if($("#workflowSelectorSource").length > 0 ){
				...
				if(conditions.isBranch()){//多个分支时
					for(var i = 0 ; i < getDatas.getBranchLength() ; i++){
						...
						drawSecondLayerTabs(dataJson,dataJson.nextTask[branchNo],secondLayerTabs,branchNo,type);//绘制第二层
					...
					//drawSecondLayerTabs(dataJson.nextTask[branchNo],secondLayerTabs,branchNo);//绘制第二层
					drawSecondLayerTabsForTargetDataGrid('selectorUserTabForTargetDataGrid_' + branchNo,branchNo);
					drawSecondLayerTabs(dataJson,dataJson.nextTask[0],tabs,branchNo,type);
					//选中第一标签页
					tabs.tabs('select',0);
				}
 ...
 * 绘制第二层
 */
function drawSecondLayerTabs(taskUserSelect,data,tabs,branchNo,type){
...
/*	if(data.roleList != null && data.roleList.length > 0){//绘制角色标签页
	...
	if(data.roleList != null && data.roleList.length > 0){//绘制自定义角色标签页
		drawer.drawTab(tabs,'tabs_roleTree_' + branchNo , '角色' ,'roleTree_' + branchNo , 'icon-role');
		drawer.drawCustomRoleTabContent(taskUserSelect,data,data.roleList,'roleTree_' + branchNo,tabNo++,type);//绘制角色标签页内容
	}
...
//	    			$('#selectorUserTabForTargetDataGrid_' + branchNo).datagrid('appendRow',record);
	 ...
	 * 绘制自定义角色标签页
	 */
	this.drawCustomRoleTabContent = function(taskUserSelect,nextActivity,data,objectId,tabNo,type){
		var datas = [];
		var roleIds = '';
		var deptImpl = '';
		for(var i = 0 ; i< data.length ; i++){
		...
		//TODO 增加判断是提交还是转办
		if('dosubmit'==type && type!= 'undefined'){
			deptImpl=((nextActivity.currentActivityAttr.deptImpl==undefined)?"":nextActivity.currentActivityAttr.deptImpl);
		}else if('dosupersede'==type && type!='undefined'){
			deptImpl=((nextActivity.currentActivityAttr.zhuanDeptImpl==undefined)?"":nextActivity.currentActivityAttr.zhuanDeptImpl);
		}
		$.ajax({
			type: "POST",
			url: getPath2() + '/platform/user/bpmSelectUserAction/initCustomRole',
			async: true,
			data: 'roleIds='+roleIds+'&formId='+taskUserSelect.formId
				+'&procInstId='+processInstanceId
				+'&executionId='+executionId
			    +'&fromActivityName='+taskUserSelect.currentActivityAttr.activityName
			    +'&toActivityName='+nextActivity.currentActivityAttr.activityName
				+'&secretLevel=' + secretLevel  + '&filterUser=' + filterUser
				+'&deptImpl='+deptImpl
			   // +'&deptImpl='+((nextActivity.currentActivityAttr.deptImpl==undefined)?"":nextActivity.currentActivityAttr.deptImpl)
			...
			success: function(treeList){
				...
				for(var i=0;i<treeList.length;i++){
					if(treeList[i].id != 'commonContacts'){
						array.push(treeList[i]);
					}
				}
				if(treeList && treeList.length){
					var beforeExpandUrl = '/platform/user/bpmSelectUserAction/getCustomUsersByDeptAndRole?';
					createComponent.createTreeComponent(objectId,array,'',beforeExpandUrl,tabNo,'role',nextActivity);
					var paramDeptIds = "";
					var nodeType = treeList[0].attributes.nodeType;
					if(nodeType=='dept'){
						paramDeptIds = treeList[0].attributes.deptIds;
					}
					//绘制搜索input
					createComponent.createSearchInputComponent(objectId,'role',roleIds,branchNo,paramDeptIds);
...
function CreateComponent(branchNo){
	 ...
	 * @param tabNo 第二层页签no
	 */
	this.createTreeComponent = function(objectId,datas,treeUrl,beforeExpandUrl,tabNo,type,nextActivity){
		var object = $('#' + objectId);
		...
		object.tree({
			...
			onBeforeExpand:function(node,param){
				...
				if(type == 'dept'){
					...
					var subNodeType = node.attributes.subNodeType;
					if(subNodeType=='user'){
						var url = "/platform/user/bpmSelectUserAction/getCustomUsersByDeptAndRole?";
						var deptId = node.attributes.deptId;
						var roleId = node.attributes.roleId;
						object.tree('options').url = getPath2() + url + 'nodeType=' + node.attributes.nodeType +'&para=' + para + '&secretLevel='+secretLevel + '&filterUser='+filterUser
						+'&deptId='+deptId+'&roleId='+roleId+'&processKey='+processKey
                        +'&toActivityName='+nextActivity.currentActivityAttr.activityName;
					}else if(subNodeType == 'upRole'){//全厂角色，根据全厂角色ID查询用户列表
						var roleId = node.attributes.roleId;
						var url = "/platform/user/bpmSelectUserAction/getUserListByCompanyRoleId?";
						object.tree('options').url = getPath2() + url + 'nodeType=' + node.attributes.nodeType +'&para=' + para + '&secretLevel='+secretLevel + '&filterUser='+filterUser
							+'&roleId='+roleId+'&processKey='+processKey +'&toActivityName='+nextActivity.currentActivityAttr.activityName;
					}else if(subNodeType == 'dRole'){//不是全厂角色，根据角色加载部门
						var roleId = node.attributes.roleId;
						var url = "/platform/user/bpmSelectUserAction/getDeptListByRoleId?";
						object.tree('options').url = getPath2() + url + 'nodeType=' + node.attributes.nodeType +'&para=' + para + '&secretLevel='+secretLevel + '&filterUser='+filterUser
							+'&roleId='+roleId+'&processKey='+processKey +'&toActivityName='+nextActivity.currentActivityAttr.activityName;
					}else if(subNodeType == 'dDept'){//根据角色和部门获取用户
						var roleId = node.attributes.roleId;
						var deptId = node.attributes.deptId;
						var url = "/platform/user/bpmSelectUserAction/getCustomUsersByDeptAndRole?";
						object.tree('options').url = getPath2() + url + 'nodeType=' + node.attributes.nodeType +'&para=' + para + '&secretLevel='+secretLevel + '&filterUser='+filterUser
							+'&roleId='+roleId+'&deptId='+deptId+'&processKey='+processKey +'&toActivityName='+nextActivity.currentActivityAttr.activityName;
					}else if(subNodeType=='commonContacts'){
						var url = "/platform/user/bpmSelectUserAction/getCommonContacts?";
						var deptIds = node.attributes.deptIds;
						var roleIds = node.attributes.roleIds;
						object.tree('options').url = getPath2() + url + 'nodeType=' + node.attributes.nodeType +'&para=' + para + '&secretLevel='+secretLevel + '&filterUser='+filterUser
							+'&roleIds='+roleIds+'&deptIds='+deptIds+'&processKey='+processKey
							+'&toActivityName='+nextActivity.currentActivityAttr.activityName;
					}
				}else {
					object.tree('options').url = getPath2() + beforeExpandUrl + 'nodeType=' + node.attributes.nodeType +'&para=' + para + '&secretLevel='+secretLevel + '&filterUser='+filterUser;
			...
			onExpand : function(node){
				...
				if(node.attributes.expandFlag == '' || node.attributes.expandFlag == "" || node.attributes.expandFlag == undefined){
...
//		    		processSelectUserComponentEvent.refreshSelectedTree(branchNo);
				} else if(node.attributes.nodeType == 'position' || node.attributes.nodeType == 'group' || node.attributes.nodeType == 'dept' || node.attributes.nodeType == 'role'){
...
//		    	}
			...
			onContextMenu: function(e, node){
				...
				if(node.attributes.nodeType=='user'){
					//$('#'+objectId).tree('select', node.target);
					$(this).tree('select',node.target);
					if(node.attributes.isAddToContacts=='1'){
						var toActivityName = node.attributes.toActivityName;
						commonContacts = {
							"contactId":node.id,
							"contactName":node.text,
							"contactDeptId":node.attributes.deptId,
							"contactDeptName":node.attributes.deptName,
							"processKey":processKey,
							"toActivityName":toActivityName,
							"treeId":objectId
						};
						$('#addContactMenu').menu('show', {
							left: e.pageX,
							top: e.pageY
						});
					}else if(node.attributes.isRemoveContacts=='1'){
						var toActivityName = node.attributes.toActivityName;
						commonContacts = {
							"contactId":node.id,
							"contactName":node.text,
		...
		//if(tabNo==0 && (type == "group" || type == "role" || type == "position")){
		if(tabNo==0 && (type == "group" || type == "position")){
			var roots = object.tree('getRoots');
			for(i=0;i<roots.length;i++){
				var tempNode = roots[i];
				object.tree('expand',tempNode.target);
	 ...
	 * @param objectId
	 */
	this.createSearchInputComponent = function(objectId,searchType,para,branchNo,paramDeptIds){
		...
		$('#searcher #searcherInfo_' + objectId).liveSearch({url: getPath2() + '/platform/user/bpmSelectUserAction/liveSearcher?searchType=' + searchType + '&para=' + para  + '&secretLevel=' + secretLevel  + '&filterUser=' + filterUser + '&branchNo=' + branchNo + '&paramDeptIds='+ paramDeptIds + '&searchInfo='});
	};
}
