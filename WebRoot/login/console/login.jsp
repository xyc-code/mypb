...
<%@ page import="avicit.platform6.commons.utils.AesEncryptUtil" %>
<html>
<head>
    <%
	...
	String input_username = (String)session.getAttribute("INPUT_USERNAME");
	String input_password_encrypted = (String)session.getAttribute("INPUT_PASSWORD");
	String input_password = "";
	if (input_password_encrypted != null) {
		input_password = AesEncryptUtil.aesDecrypt(input_password_encrypted, AesEncryptUtil.KEY);
	}
		/* 判断是否存在登录失败的情况 */
...
<body>
	...
	<div class="header">
		...
		<div class="login_content">
			<div class="logo_box">
					<img class="logo" src="login/console/img/logoDA.png" />
					<h1 class="logo_title">V6测试平台</h1>
			</div>
			<div class="login_box">
			<h4>欢迎使用</h4>
			<p>业务基础平台</p>
			<form action="<%=ViewUtil.getRequestPath(request)%>admin/login" onsubmit="return check();"
				method="post">
				<div class="login_input" id="login_input_username">
						...
						if (input_username != null && input_username != "") {
							...
							out.print(PlatformLocalesJSTL.getBundleValue("login.username.tip.loginname"));
						}
						%>"
						 tabindex="1"
						value=""/>
				</div>
				<div class="login_input" id="login_input_password">
						<i class="icon icon-unlock-fill tb" style="display: none;"></i>
						<input type="" id="showPwd" placeholder="请在此输入密码" focucmsg="<%=
							PlatformLocalesJSTL.getBundleValue("login.password.tip.name")
							%>"
							value="" tabindex="2" style="<%
						...
						<input autocomplete="off" style="<%
							...
							value="<%
							if (input_password != null && input_password != "") {
								out.print(input_password);
							}
							%>"
						/>
				</div>
				<%if(isShow){ %>
				<div class="login_input login_input_img" id="login_input_inputcode">
	  ...
	  function closeSubFrame() {
		...
		var check=function(){
            ...
            if(!userName || userName==='登录名'){
                ...
                $("#login_input_username").attr("class", "login_input_red");
                return false;
            }
            ...
            if(!password || password==='密码'){
				...
				$("#login_input_password").attr("class", "login_input_red");
                return false;
            }
            <%if(isShow){ %>
            ...
            if(!inputCode || inputCode==='验证码'){
				...
				$("#login_input_inputcode").attr("class", "login_input_red login_input_img");
                return false;
            }
            <%} %>
		...
		<!-- 修正ie8下图标偶尔丢失问题 -->
			...
			$("#username_").each(function() {
				...
				$(this).focus(function() {
				    $('#msgContent').remove();
					$("#login_input_username").attr("class", "login_input");
			...
			$("#showPwd").each(function() {
				...
				$(this).focus(function() {
        	        $('#msgContent').remove();
					$("#login_input_password").attr("class", "login_input");
						if ($(this).val() == $(this).attr("focucmsg")) {
							 $("#showPwd").hide();
							 $("#password").show().focus();
						}
	...
	function decrypt(data) {
		var _0  = CryptoJS.enc.Utf8.parse('dufy20170329java');
		var decryptedData = CryptoJS.AES.decrypt(data, _0, {mode:CryptoJS.mode.ECB,padding:CryptoJS.pad.Pkcs7}).toString(CryptoJS.enc.Utf8);
