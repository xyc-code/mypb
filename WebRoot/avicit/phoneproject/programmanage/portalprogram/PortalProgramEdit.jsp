...
<!-- ControllerPath = "pdmt/programmanage/portalprogram/PortalProgramController/operation/Edit/id" -->
<title>修改</title>
...
<body class="easyui-layout" fit="true">
	<div data-options="region:'center',split:true,border:false"
		...
		<form id='form'>
			<input type="hidden" name="id" value='${portalProgramDTO.id}' />
			<table width="100%" style="padding-top: 10px;">
				<tr>
					<th align="right"><span class="remind">*</span>应用名称:</th>
					<td><input title="应用程序名称" class="inputbox easyui-validatebox"
						data-options="validType:'maxLength[200]',required:true" style="width: 170px;"
					...
					<td><input title="应用程序代码（菜单的唯一性标识）" readonly="readonly" 
				...
				<%-- <tr>
					<th align="right"><span class="remind">*</span>应用程序图标:</th>
					<td colspan="3"><input title="应用程序图标" class="inputbox easyui-validatebox"
						data-options="validType:'maxLength[200]'" style="width: 469px;"
						type="text" name="programImg" id="programImg"
						value='${portalProgramDTO.programImg}' /></td>
				</tr> --%>
				<tr>
					<th align="right"><span class="remind">*</span>责任人（多选）:</th>
					<td colspan="3"><input title="责任人（多选）" class="inputbox easyui-validatebox"
						data-options="validType:'maxLength[2000]',required:true" style="width: 470px;"  readonly="readonly"
						type="text" name="programResponsibleShow" id="programResponsibleShow" onclick="insertUser()"
						/>
						<input title="责任人（多选）" class="inputbox easyui-validatebox" type="hidden"
						data-options="validType:'maxLength[2000]',required:true" style="width: 469px;"  readonly="readonly"
						type="text" name="programResponsibles" id="programResponsibles" onclick="insertUser()"
						/>	
					</td>
						<td>
				...
				<tr>
					...
					<!-- <td><input title="应用程序状态(0 启用；1 禁用)"
						...
						type="radio" name="programState" id="programState" value="0"/>启用
					<input title="应用程序状态" class="inputbox easyui-validatebox"
						type="radio" name="programState" id="programState" value="1"/>禁用</td>	</td>
				</tr>
				<tr>
					<th align="right">应用程序描述:</th>
					<td colspan="3"><textarea title="应用程序描述" class="inputbox easyui-validatebox"
						data-options="validType:'maxLength[2000]'" style="width: 466px;height:150px"
						 name="programDesc" id="programDesc">${portalProgramDTO.programDesc}</textarea></td>
				</tr>
	...
	<script type="text/javascript">
		...
		$(function() {
			var doc = document;
			var usersAndId = '${portalProgramDTO.programResponsibles}';
			var strs=usersAndId.split(";");
			
			doc.getElementById('programResponsibleShow').value=strs[0];
			doc.getElementById('programResponsibles').value=strs[1];
		...
		function saveForm() {
			...
			if (document.getElementById("programImg").value != '') {
					...
					$('#ImgForm').form('submit',{
						...
						success : function(data) {
							...
							if (json.flag == "success") {
								file = json.file;
								parent.portalProgram.save(serializeObject($('#form')), "#edit",file);
							}
				...
				$.messager.alert('警告', '请选择要上传的文件!', 'warning');
				return ;
