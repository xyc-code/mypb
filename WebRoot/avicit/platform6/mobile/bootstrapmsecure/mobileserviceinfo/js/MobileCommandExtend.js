 ...
 * 
 */
function MobileCommandExtend(datagrid,url,keyWordId,pid,dataGridColModel){
	...
	this._jqgridToolbar="#t_"+datagrid;
	 this.pid = pid;
	this._doc = document;
/*	this._formId="#"+formSub2;
	this._searchDialogId ="#"+searchDialogSub2;*/
...
MobileCommandExtend.prototype.init=function(pid){
	...
	$(_self._datagridId).jqGrid({
    	url: this.getUrl()+'getMobileCommandExtend',
    	postData : {pid : _self.pid},
    	mtype: 'POST',
        datatype: "json",
        toolbar: [true,'top'],
        colModel: this.dataGridColModel,
        height:($(window).height()-78)/2 - 70,
...
//打开高级查询框
/*MobileCommandExtend.prototype.openSearchForm = function(searchDiv){
	var _self = this;
...
MobileCommandExtend.prototype.formValidate = function(form) {
	form.validate({
		rules : {
			commandId : {
				required:true,
...
//重载数据
MobileCommandExtend.prototype.reLoad=function(pid){
	if(pid != undefined){
		this.pid = pid;
	}
...
//高级查询
/*MobileCommandExtend.prototype.searchData =function(){
	var searchdata = {
			keyWord: null,
			pid:this.pid,
			param:JSON.stringify(serializeObject($(this._formId)))
...
//关键字段查询
/*MobileCommandExtend.prototype.searchByKeyWord =function(){
...
//隐藏查询框
/*MobileCommandExtend.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};*/
/*清空查询条件*/
/*MobileCommandExtend.prototype.clearData =function(){
