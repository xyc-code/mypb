...
<body class="easyui-layout" fit="true">
	<div data-options="region:'center',split:true,border:false">
		<form id='form'>
			<input type="hidden" name="id"
				value="<c:out  value='${mobileServiceHeadersDTO.id}'/>" /> <input
				type="hidden" name="version"
				value="<c:out  value='${mobileServiceHeadersDTO.version}'/>" /> <input
				type="hidden" name="serviceId"
				value="<c:out  value='${mobileServiceHeadersDTO.serviceId}'/>" />
			<table class="form_commonTable">
				<tr>
					<th width="10%"
						style="word-break: break-all; word-warp: break-word;"><label
						for="headerName">header键名称:</label></th>
					<td width="39%"><input class="form-control input-sm"
						type="text" name="headerName" id="headerName"
						value="<c:out  value='${mobileServiceHeadersDTO.headerName}'/>" />
					</td>
					<th width="10%"
						style="word-break: break-all; word-warp: break-word;"><label
						for="headerValue">header值:</label></th>
					<td width="39%"><input class="form-control input-sm"
						type="text" name="headerValue" id="headerValue"
						value="<c:out  value='${mobileServiceHeadersDTO.headerValue}'/>" />
					</td>
				</tr>
				<tr>
				</tr>
			...
			<table class="tableForm"
				...
				<tr>
					<td width="50%" style="padding-right: 4%;" align="right"><a
						...
						title="保存" id="mobileServiceHeaders_saveForm">保存</a> <a
						href="javascript:void(0)"
						class="btn btn-grey form-tool-btn btn-sm" role="button" title="返回"
						id="mobileServiceHeaders_closeForm">返回</a></td>
				</tr>
			</table>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		function saveForm() {
			...
			parent.mobileServiceHeaders.save($('#form'), "eidt");
		}
