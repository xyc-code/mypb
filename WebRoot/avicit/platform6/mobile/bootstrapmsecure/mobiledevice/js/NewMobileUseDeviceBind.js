function MobileUseDeviceBind(datagrid, url, formSub, dataGridColModel,
...
MobileUseDeviceBind.prototype.init = function(pid) {
	...
	$(_self._datagridId).jqGrid({
		url : this.getUrl() + 'getMobileUseDeviceBind',
		postData : {
			pid : _self.pid
		},
		mtype : 'POST',
...
MobileUseDeviceBind.prototype.formValidate = function(form) {
	form.validate({
		rules : {
			userId : {
				required : true,
...
//重载数据
MobileUseDeviceBind.prototype.reLoad = function(pid) {
	if (pid != undefined) {
		this.pid = pid;
	}
	var searchdata = {
...
//打开高级查询框
MobileUseDeviceBind.prototype.openSearchForm = function(searchDiv, par) {
	var _self = this;
	par = null;
	//if(!par) par = $(window);
...
//高级查询
MobileUseDeviceBind.prototype.searchData = function() {
	var searchdata = {
		keyWord : null,
		pid : this.pid,
...
//关键字段查询
MobileUseDeviceBind.prototype.searchByKeyWord = function() {
...
//隐藏查询框
MobileUseDeviceBind.prototype.hideSearchForm = function() {
	layer.close(searchDialogindex);
};
/*清空查询条件*/
MobileUseDeviceBind.prototype.clearData = function() {
