 ...
 * 
 */
function MobileServiceInfo(datagrid,url,searchD,form){
...
//初始化操作
MobileServiceInfo.prototype.init=function(){
	var _self = this;
	...
	this._datagrid=$(this._datagridId).datagrid({
		url:this.getUrl()+"getMobileServiceInfosByPage.json",
		onSelect: function(rowIndex, rowData){
			_self._selectId=rowData.id;
			_self.getOnSelectRow()(rowIndex, rowData,rowData.id);
			$('#dgmobileCommandInfo').datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
			$('#dgmobileCommandInfo').datagrid('reload');
			$('#dgmobileServiceHeaders').datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
			$('#dgmobileServiceHeaders').datagrid('reload');
		},
...
//添加页面
MobileServiceInfo.prototype.insert=function(){
	this.nData = new CommonDialog("insert","790","500",this.getUrl()+'Add/null',"添加",false,true,false);
	this.nData.show();
};
//修改页面
MobileServiceInfo.prototype.modify=function(){
...
//详细页
MobileServiceInfo.prototype.detail=function(id){
	this.nData = new CommonDialog("edit","790","500",this.getUrl()+'Detail/'+id,"详情",false,true,false);
	this.nData.show();
};
//保存功能
MobileServiceInfo.prototype.save=function(form,id){
	var _self = this;
	$.ajax({
...
//删除
MobileServiceInfo.prototype.del=function(){
	var rows = this._datagrid.datagrid('getChecked');
	var _self = this;
	var ids = [];
	var l =rows.length;
...
//重载数据
MobileServiceInfo.prototype.reLoad=function(){
	this._datagrid.datagrid('load',{});
};
//关闭对话框
MobileServiceInfo.prototype.closeDialog=function(id){
	$(id).dialog('close');
};

//打开查询框
MobileServiceInfo.prototype.openSearchForm =function(){
	this.searchDialog.dialog('open',true);
};
//去后台查询
MobileServiceInfo.prototype.searchData =function(){
...
//隐藏查询框
MobileServiceInfo.prototype.hideSearchForm =function(){
	this.searchDialog.dialog('close',true);
};
/*清空查询条件*/
MobileServiceInfo.prototype.clearData =function(){
	this.searchForm.find('input').val('');
	this.searchData();
};
MobileServiceInfo.prototype.formate=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd");
	}
	return '';
};
MobileServiceInfo.prototype.formateDateTime=function(value){
	if(value){
...
//返回主表的id
MobileServiceInfo.prototype.getSelectID=function(){
	return this._selectId;
};
MobileServiceInfo.prototype.searchDataBySfn =function(conditions){
    this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
