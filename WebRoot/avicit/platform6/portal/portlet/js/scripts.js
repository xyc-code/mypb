...
function saveEvent(){
    //删除遮罩层
    $("[name='portletConfigDesignerMatteLayer']").each(function(){$(this).removeClass("portletConfigDesignerMatteLayer");
    })
...
function deletePortletAjaxEvent(layerIndex){
    $.ajax({
        ...
        success : function(msg){
            ...
            try{
                var topifrme=top.$('#tabs-panel').tabs('select', "门户配置").tabs("getTab", "门户配置").find('iframe');
                $(topifrme).contents().find('#portletFrame')[0].contentWindow.reloadGrid();
            }catch(e){
...
function setting(){
    var settinObj = $(".demo").find('.setting').popover({
        ...
        // 兼容ie8
        if (document.all) {
            _self.parent().find('#portletName').on('propertychange',function(e){
