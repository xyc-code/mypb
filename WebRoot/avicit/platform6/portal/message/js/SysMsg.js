...
SysMessage.prototype.reLoad=function(){
	//解决：
	//当高级查询中“我的接收”选择全部,然后点击查询,再标记已读或者未读
	//列表页面会同时展示“我的接收”和“我的发送”
    var searchdataPre = serializeObject($(this._formId));
    if ('0'==searchdataPre.msgTypes) {
        //searchdataPre["isReaded"]="0";
        searchdataPre["recvOrSend"]="1";
    }else if ('1'==searchdataPre.msgTypes) {
        searchdataPre["isReaded"]="0";
        searchdataPre["recvOrSend"]="1";
    }else if ('2'==searchdataPre.msgTypes) {
        searchdataPre["isReaded"]="1";
        searchdataPre["recvOrSend"]="1";
    }else if('3'==searchdataPre.msgTypes){
        searchdataPre["recvOrSend"]="0";
    }
	var searchdata = {param:JSON.stringify(searchdataPre)};
...
SysMessage.prototype.searchData =function(){
	...
	//解决：
	//高级查询选择“我的发送”,点查询后没有隐藏“删除”、“标记已读”、“标记未读”按钮
    if('3'==param.msgTypes){
        $('#sysMessage_del,#sysMessage_read,#sysMessage_unread').css('display','none');
    }else{
        $('#sysMessage_del,#sysMessage_read,#sysMessage_unread').css('display','inline-block');
    }
	if ('0'==param.msgTypes) {
//		param["isReaded"]="0";
		param["recvOrSend"]="1";
