...
function leftMenuEvent() {
    ...
    $('.navList').delegate(".taburl", 'click', function(e) {
       ...
       /* var preTitle = title;
        ...
        var url = $(this).attr("rel"),
            exist = false,//判断是否存在tabs选项卡了，返回false或true
            tabIndex = 0,
            allTabs=$('#tabs-panel').tabs('tabs');
        ...
        if (!exist && url != null && url != '') {
            $('#tabs-panel').tabs({
                onAdd:function(title,index){
                    var target = this;
                    var tab = $('#tabs-panel').tabs('getTab', index);
                    setTimeout(function(){
                        $(target).tabs('select', index).tabs('update',{
                            tab:tab,
                            options:{
                                id: id,
                                content: '<iframe scrolling="auto" frameborder="0"  src="' + url + '"style="width:100%;height:100%;"></iframe>'
                    ...
                    var currentTab = $(target).tabs('select', index).find(".tabs-selected").find("span.tabs-title");
            ...
            $('#tabs-panel').tabs('select', tabIndex); //匹配到已打开标签，就显示这个窗口
        }*/
    });
...
//            perfectScroll($('.subMenuOverflow'));
    ...
    //解决navlist高度计算不正确导致的样式问题, $('.leftAside').height()取到的高度不正确
    $('.navList').height($('.leftAside').height()-$('.leftAside .logoDom').height()-2-48);
