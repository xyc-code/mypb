...
function personGroupJqGridInit($personGroupJqGrid) {
    ...
    $personGroupJqGrid.jqGrid({
        ...
        colModel : [
            ...
            {label : '群组名称', name : 'sysGroupName', width : 250, sortable:false,editable:true},
            {label : '群组成员ID', name : 'sysUserId',  hidden : true},
            {label : '群组成员', name : 'sysUserName', width : 575, sortable:false,editable:true, edittype:"custom",editoptions: {custom_element: userNameElem, custom_value:userNameValue, forId:"sysUserId"}}
        ],
...
function addPersonGroup($personGroupJqGrid) {
	...
	if(maxorderBy==""){
		$.ajax({
			url :  "platform/console/customed/getMaxOrderby",
			type : 'post',
			dataType : 'json',
			//async:false,
			success : function(r) {
				if(r.flag=="success"){
					maxorderBy=r.maxOrderBy+10;
				}
			}
		});
	}
...
function approvalOptionInit($approvalOptionJqGrid) {
    ...
    $approvalOptionJqGrid.jqGrid({
        ...
        colModel : [
            {label : 'id', name : 'id', key : true, hidden : true},
            {label : '审批意见', name : 'value',width : 150, sortable:false,editable:true},
            {label : '类型', name : 'processType', width : 140, sortable:false,align:"center", formatter:processTypeFormatter,unformat:processTypeUnFormatter, editable:true, edittype:"custom",editoptions:{custom_element: processTypeElem, custom_value:processTypeValue}},
            ...
            {label : '流程名称', name : 'processName', width : 400,sortable:false, editable:true, edittype:"custom",editoptions: {custom_element: processNameElem, custom_value:processNameValue, forId:"processIds"}},
            {label : '排序', name : 'orderBy',width : 136, sortable:false,editable:true}
        ],
        height : $(window).height() - 168,
        scrollOffset : 20, // 设置垂直滚动条宽度
        rowNum : 10,
