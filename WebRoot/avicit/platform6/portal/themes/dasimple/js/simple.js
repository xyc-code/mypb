...
function initFirstMenuClickEvent(){
    ...
    if(!$(this).hasClass("more")){
            var firstmenu=$(this).find("li .taburl ");
             //给一级菜单增加点击事件
            e.stopPropagation();
            e.preventDefault();
            var _slef = firstmenu;
            var _id_ = _slef.attr("id");
            var title = _slef.attr("title");
            var url = _slef.attr("rel");
            var openType = _slef.attr("openType");//打开方式
            if(openType == '_blank'){
                openBlank(title,url);
...
function leftMenuEventDrawer() {
    $('.accordion').delegate(".taburl, li", 'click', function(e) {
        e.stopPropagation();
        e.preventDefault();
        var _slef = $(this);
        //解决二级、三级菜单只能点击菜单名称才能打开标签页问题
        if(!$(this).hasClass("taburl")){
            _slef = $(this).find(".taburl");
        }
        var _id_ = _slef.attr("id");
        var title = _slef.attr("title");
        var url = _slef.attr("rel");
...
var Accordion = function(el, multiple) {
    ...
    var linkthrees = this.el.find('.linkthree');
    var linkfours = this.el.find('.linkfour');
    // Evento
    links.on('click', { el: this.el, multiple: this.multiple }, this.dropdown);
    linkthrees.on('click', { el: this.el, multiple: this.multiple }, this.dropdown);
    linkfours.on('click', { el: this.el, multiple: this.multiple }, this.dropdown);
    ...
    if(  $(this).hasClass("secondlevel")){
    	 if($(this).children().next()[0].className.indexOf("fa-chevron-down")!=-1){
    	    ...
    	    if (!e.data.multiple) {
    	        ...
    	        $el.find('.foursubmenu').not($next).slideUp().parent().removeClass('open');
    	    }
    	   setTimeout(function(){
    	        perfectScrollDrawer($('.accordion'),'update');
    	    },500); 
    	 ...
    	 if($(this).children()[0].className.indexOf("fa-chevron-down")!=-1){
 	    	if($(this)[0].nextSibling.className=="menu-label menu-bg-red jump"){
 	    		$next = $this.next().next();
 	    	}else{
 	    	   $next = $this.next() 
 	    	}
 	    ...
 	    $next.slideToggle();
 	    $this.parent().toggleClass('threeopen').siblings().removeClass('threeopen');
 	    if (!e.data.multiple) {
 	        $el.find('.foursubmenu').not($next).slideUp().parent().removeClass('threeopen');
 	    }
 	   setTimeout(function(){
 	        perfectScrollDrawer($('.accordion'),'update');
 	    },500); 
