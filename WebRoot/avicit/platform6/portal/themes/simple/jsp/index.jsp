...
<body>
	<div class="mainBody">
		...
		<div class="bodyCont">
			...
			<div class="header">
				<div class="logoDom">
					<div class="logo" style="font-size:  ">
						<c:choose>
							<%--isTemp判断是否预览--%>
							<c:when test="${isTemp}">
				...
				<div class="toolbar">
					<div class="leftTool">${menus}</div>
					<div class="rightTool">
						<div class="top_search">
							<div class=" d6">
								<div class="top_search_box">
									<i id="headSubmit" class="icon iconfont icon-search" style="top: -2px;display:<%=isOpenSearch%>;"></i> <input style="width: 0px; right: 32px; top: -2px;" id="keywords" type="text" name="keywords" placeholder="搜索从这里开始..."
													autocomplete="off">
								</div>
							</div>
						</div>
...
<%--									</div></li>--%>
	...
	<script type="text/javascript" src="avicit/platform6/portal/themes/${_theme}/js/simple.js"></script>
...
// 	    $('#oacollection').oaCollectionPortal();//个人收藏初始化
	    ...
	    var subChild=$(".all-list li").clone();
	    function changeMenu(keywords){
	    	...
	    	$.each(menuArr,function(index,menu){
	    		...
	    		if(menu.rel != null && menu.rel != "" && menu.rel != undefined){
		    		newHtml = newHtml + "<li> <div class='link taburl' id='"+menu.id+"' opentype='"+menu.opentype+"'  title='"+menu.innerText+"' rel='"+menu.rel+"'>"+
		    					"<i class='"+menu.iconClass+"'></i>"+menu.innerText+"</div>";
	    		}
	    		
	    	});
	    	$(".la-list > ul").html(newHtml);
	    }
	  	//搜索菜单并记录
	    function searchKeywords(list,keywords,menuArr){
	    	$.each(list,function(index,menu){
	    		if($(this).attr("title") != undefined && $(this).attr("title").indexOf(keywords)!=-1){
	    			var iconClass;
		   	    	if($(this).attr('icon')){
		   	    		iconClass = $(this).attr('icon');
		   	    	}
