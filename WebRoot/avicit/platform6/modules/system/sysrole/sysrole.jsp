...
var editIndex = -1;
var validRole={};
var validComboData=[{lookupCode:"3",lookupName:'无效'},{lookupCode:"1",lookupName:'有效'}];
...
function loadSearchRoleInfo(appId, roleName, roleCode, roleGroup, roleType){
    /*20200724：easyui版查询角色时，如果查询条件是角色名称，查询不到结果*/
	$("#dgRole").datagrid("options").url ="platform/sysrole/getSysRoleListByPage.json";
...
function getLength(str) { 
	...
	var realLength = 0;
	var charCode = -1;
	if(str !="" && str != null && str != undefined){//解决str为空时,ie下保存时报错问题
        var len = str.length;
        for (var i = 0; i < len; i++) {
...
function searchRole(){
	...
	loadSearchRoleInfo(appid, roleName, roleCode, null, null);
};
...
function searchUser() {
	...
	function loadSearchUserInfo(roleId, name, loginName, status) {
		...
		$('#dgUser').datagrid("reload", {
			filter_EQ_ur_sysRoleId : roleId,
			filter_EQ_u_status : status,
			filter_LIKE_u_name : name,
			filter_LIKE_u_loginName : loginName
		});
	...
	var expSearchParams = {};
	function importRole() {
		var imp = new CommonDialog("importData", "700", "400",
				'platform/excelImportController/excelimport/importRole/xls?extPara='
						+ JSON.stringify({
							appid : appid
	...
	function exportServerData() {
		$.messager.confirm('确认', '是否确认导出Excel文件?', function(r) {
			if (r) {
				...
				expSearchParams.unContainFields = 'id,roleType';
