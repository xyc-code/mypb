 ...
 * 
 */
function RestFul(formSub,searchSubNames,keyWord,searchDialogSub,datagrid, url, form,parentId) {
...
//初始化操作
RestFul.prototype.init = function() {
	var _self = this;
	var param = {id:_self._parentId};
	$(_self._datagridId).jqGrid({
		url : _self.getUrl() + 'getRestFulsByPage.json',
		mtype : 'POST',
		postData : param,
		datatype : "json",
		toolbar : [ false, 'top' ],
...
//添加页面
RestFul.prototype.insert = function() {
...
//编辑页面
RestFul.prototype.modify = function() {
...
//详细页
RestFul.prototype.detail = function(id) {
...
RestFul.prototype.formValidate = function(form) {
	form.validate({
		rules : {
			name : {
				required: true,
...
//保存功能
RestFul.prototype.save = function(form, id,classId) {
	var _self = this;
...
//保存编辑
RestFul.prototype.saveEdit = function(form, id,classId) {
	var _self = this;
	var formSerializeValue = form.serialize();
	//formSerializeValue = decodeURIComponent(formSerializeValue,true);//解决序列化时乱码
    var formDataJson = convertFormSerializeValueToJson(formSerializeValue);
...
//删除
RestFul.prototype.del = function(classId) {
...
//重载数据
RestFul.prototype.reLoad = function(id) {
	if (id != undefined) {
		this._parentId = id;
	}
	var searchdata = {
...
//关闭对话框
RestFul.prototype.closeDialog = function(id) {
	if (id == "insert") {
...
//打开高级查询框
RestFul.prototype.openSearchForm = function(searchDiv, par) {
	var _self = this;
	par = null;
	//if(!par) par = $(window);
...
//高级查询
RestFul.prototype.searchData = function() {
	var searchdata = {
		id : this._parentId,
		keyWord : "",
...
//关键字段查询
RestFul.prototype.searchByKeyWord = function() {
...
/*清空查询条件*/
RestFul.prototype.clearData = function() {
...
//测试
RestFul.prototype.testRestFul = function(form) {
	var _self = this;
	var formSerializeValue = form.serialize();
    var formDataJson = convertFormSerializeValueToJson(formSerializeValue);
	avicAjax.ajax({
		url : _self.getUrl() + "testRestFul",
		data : {formDataJson:formDataJson},
		type : 'post',
		dataType : 'json',
		success : function(r) {
			if (r.flag == "success") {
				layer.alert('返回结果！' + r.result, {
					icon : 7,
					area : [ '400px', '' ], //宽高
					closeBtn : 0
				});
			} else {
				layer.alert('测试失败！' + r.error, {
					icon : 7,
					area : [ '400px', '' ], //宽高
					closeBtn : 0
				});
