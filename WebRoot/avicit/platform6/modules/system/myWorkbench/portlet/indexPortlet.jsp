...
function saveNewPortlet(portletList,layout,newConX){
	...
	$.ajax({ 
		url: '<%=ViewUtil.getRequestPath(request)%>platform/myWorkbenchController/saveIndexPortlet.json',
		async : false,
		type: "POST",
...
function addPortlet(ids){
	...
	for(var i=0; i<idArray.length; i++){
		...
		$.ajax({
			url : '<%=ViewUtil.getRequestPath(request)%>platform/myWorkbenchController/getPortletInfo.json?portletId='+idArray[i]+'&baseUrl='+baseUrl+'&xy='+xy,
			data : {},
			type : 'post',
			dataType : 'json',
			async:false,
...
function loadPortlet(){
	$.ajax({
		url : baseUrl+'platform/myWorkbenchController/getIndexPortlet.json',
