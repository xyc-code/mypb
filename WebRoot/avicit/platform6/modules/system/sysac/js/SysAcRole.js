 ...
 * 
 */
function SysAcRole(datagrid, url, searchD, form, keyWordId, searchNames,
...
//初始化操作
SysAcRole.prototype.init = function() {
	var _self = this;
...
//添加页面
SysAcRole.prototype.insert = function(parentId) {
	var _self = this;
	var r1 = {
			type : 'roleSelect',
			idFiled : '',
			...
			callBack : function(role) {
                if((role.roleids == "" || role.roleids == null || role.roleids ==undefined ) || (role.roleNames == "" || role.roleNames == null || role.roleNames ==undefined )){
                    layer.msg('保存失败，保存前请选择角色！');
                    return false;
                }
				avicAjax.ajax({
					url : _self.getUrl() + "save",
					data : {
						acOrgId : role.roleids,
						acOrgType : "acRole",   
						acType : "r",
						resourceId : parentId
					},
...
SysAcRole.prototype.formValidate = function(form) {
	form.validate({
		rules : {
			acOrgType : {
				required : true,
...
//保存功能
SysAcRole.prototype.save = function(form, id) {
	var _self = this;
	avicAjax.ajax({
		url : _self.getUrl() + "save",
		data : {
...
//授权
SysAcRole.prototype.allow = function(){
...
//禁止
SysAcRole.prototype.deny = function(){
	var rows = $(this._datagridId).jqGrid('getGridParam', 'selarrrow');
	var _self = this;
	var ids = [];
	var l = rows.length;
...
//删除
SysAcRole.prototype.del = function() {
	var rows = $(this._datagridId).jqGrid('getGridParam', 'selarrrow');
	var _self = this;
	var ids = [];
	var l = rows.length;
...
//重载数据
SysAcRole.prototype.reLoad = function() {
	var searchdata = {
...
//关闭对话框
SysAcRole.prototype.closeDialog = function(id) {
	if (id == "insert") {
