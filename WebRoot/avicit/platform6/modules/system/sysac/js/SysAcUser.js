 ...
 * 
 */
function SysAcUser(datagrid, url, searchD, form, keyWordId, searchNames,
...
//初始化操作
SysAcUser.prototype.init = function() {
	var _self = this;
...
//添加页面
SysAcUser.prototype.insert = function(parentId) {
	var _self = this;
	var u1 = {
			type : 'userSelect',
			idFiled : '',
			...
			callBack : function(user) {
				if((user.userids == "" || user.userids == null || user.userids ==undefined ) || (user.usernames == "" || user.usernames == null || user.usernames ==undefined )){
                    layer.msg('保存失败，保存前请选择用户！');
                    return false;
				}
				avicAjax.ajax({
					url : _self.getUrl() + "save",
					data : {
						acOrgId : user.userids,
						acOrgType : "acUser",   
						acType : "r",
						resourceId : parentId
					},
...
SysAcUser.prototype.formValidate = function(form) {
	form.validate({
		rules : {
			acOrgType : {
				required : true,
...
//保存功能
SysAcUser.prototype.save = function(userId) {
	var _self = this;
	avicAjax.ajax({
		url : _self.getUrl() + "save",
		data : {
...
//授权
SysAcUser.prototype.allow = function(){
...
//禁止
SysAcUser.prototype.deny = function(){
	var rows = $(this._datagridId).jqGrid('getGridParam', 'selarrrow');
	var _self = this;
	var ids = [];
	var l = rows.length;
...
//删除
SysAcUser.prototype.del = function() {
	var rows = $(this._datagridId).jqGrid('getGridParam', 'selarrrow');
	var _self = this;
	var ids = [];
	var l = rows.length;
...
//重载数据
SysAcUser.prototype.reLoad = function() {
	var searchdata = {
...
//关闭对话框
SysAcUser.prototype.closeDialog = function(id) {
	if (id == "insert") {
