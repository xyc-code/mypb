...
function startGroupJob(){
	...
	if(selected == null){
		$.messager.show({title:'提示',msg :'请先选择一个任务分组！'});
		return false;
	}
	var jobRows =  $('#datagridGroupJob').datagrid('getRows');
	if(jobRows.length == 0){
		$.messager.show({title:'提示',msg :'此任务分组下无任务，无法执行！'});
		return false;
	}
...
<div region="north" border="false" style="height: 160px;">
	<table id="datagridGroup" class="easyui-datagrid" 
		...
		<thead>
			<tr>
				<th data-options="field:'description', halign:'center', align:'left'" width="100">任务分组</th>
			</tr>
		</thead>
	</table>
