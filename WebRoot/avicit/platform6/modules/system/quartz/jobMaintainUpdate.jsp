...
<%
	...
	if (request instanceof HttpServletRequest) {
		...
		Object obj = session.getAttribute("CURRENT_LANGUAGE");
		if (obj != null) {
			languageCode = obj.toString();
		}
	}
	List<SysLookupSimpleVo> platform_quartz_job_group =
			(List<SysLookupSimpleVo>) sysLookupAPI.getLookUpListByTypeByAppIdWithLg("PLATFORM_QUARTZ_JOB_GROUP", RestClientConfig.systemid(), languageCode);
	String substring = "[]";
	if (CollectionUtils.isNotEmpty(platform_quartz_job_group)) {
		StringBuilder data = new StringBuilder();
		data.append("[");
		for (SysLookupSimpleVo sysLookupSimpleVo: platform_quartz_job_group) {
			data.append("{lookupCode:'" + sysLookupSimpleVo.getLookupCode() + "', lookupName: '" + sysLookupSimpleVo.getLookupName()
					+ "'},");
		}
		...
		<form id="formJob" fit="true">
			...
			<table class="form_commonTable">
				<tr>
                	...
                	<td width="40%">
                    	<input id="group" name="group" title="任务分组" class="easyui-combobox" type="text" value="${job.group}"
						    data-options="required:true,editable:false,panelHeight:'auto',valueField:'lookupCode',textField:'lookupName',
						    data:<%= substring%>" />
                	</td>
                	<th width="10%"><span class="remind">*</span>任务名称</th>
                	<td>
                    	<input id="name" name="name" title="任务名称" class="easyui-validatebox" type="text" data-options="required:true,validType:'maxByteLength[50]'" value="${job.name}" />
                	</td>
                </tr>
                <tr>
                	...
                	<td>
                    	<input id="cron" name="cron" title="表达式" class="easyui-subwindow" type="text" data-options="required:true,editable:false,onClick:openSelectCron" value="${job.cron}"/>
                	</td>
                </tr>
                <tr>
                	<th>类型说明</th>
                ...
                <tr>
                	...
                	<td colspan="3">
                    	<input id="program" name="program" title="执行程序" class="easyui-validatebox" type="text" data-options="required:true,validType:'maxByteLength[1000]'" value="${job.program}" />
                	</td>
                </tr>
                <tr>
                	<th>描述</th>
                	<td colspan="3">
                    	<input id="description" name="description" title="描述" class="easyui-validatebox" type="text" data-options="required:false,validType:'maxByteLength[200]'" value="${job.description}" />
                	</td>
                </tr>
            </table>
		</form>
