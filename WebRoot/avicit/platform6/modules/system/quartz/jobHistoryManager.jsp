...
function deleteHistroy(){
	$.messager.confirm('确认', '您确认清除数据吗?', function(r){
        if (r){
        	var param = {
        		jobId: $('#jobId').val()
        	};
			...
			$.ajax({
				...
				success : function(result) {
					if (result.flag == 'success') {
						...
						$.messager.show({title:'提示',msg :'清除数据成功！'});
					} else {
						$.messager.show({title:'提示',msg :'清除数据失败！'});
					}
				}
			});
        }
...
<div region="north" border="false">
	<div id="toolbar" class="datagrid-toolbar" style="height:auto;display: block;">
		<div style="padding:0 0 0 2px;">
			...
			<a id="deleteButton" href="javascript:void(0)" onclick="deleteHistroy()" class="easyui-linkbutton" data-options="iconCls:'icon-remove'" plain="true">清除数据</a>
		</div>
	</div>
</div>
