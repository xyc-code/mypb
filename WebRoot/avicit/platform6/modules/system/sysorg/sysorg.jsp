...
function myOnBeforeExpand(row){
	$("#deptTree").tree("options").url = "platform/sysorg/sysOrgController/getChildOrgDeptByIdAndOrgId.json?type="+row.attributes.type;
	return true;
};
...
function initForSearch(){
 $('#searchWord').searchbox({
        ...
        searcher: function (value) {
        	...
        	if(value==null||value==""){
        		path="platform/sysorg/sysOrgController/getChildOrgDeptByIdAndOrgId";
        	}
        	$.ajax({
                cache:false,
                type: "POST",
...
{
	var usd = new CommonDialog("formEditDeptDialog","900","500",path+"avicit/platform6/modules/system/sysorg/editDeptForm.jsp?id="+id,"编辑部门",true,true,false);
	usd.show();
};
function showAddDept()
{
	...
	var usd = new CommonDialog("formAddDeptDialog","900","500",path+"avicit/platform6/modules/system/sysorg/addDeptForm.jsp","添加部门",true,true,false);
	usd.show();
};
function showAddOrg()
{
	...
	var formAddOrgDialog = new CommonDialog("formAddOrgDialog","900","500",path+"avicit/platform6/modules/system/sysorg/addOrgForm.jsp","添加组织",true,true,false);
	formAddOrgDialog.show();
};
...
function showEditOrg(id){
	var usd = new CommonDialog("formEditOrgDialog","900","400",path+"avicit/platform6/modules/system/sysorg/editOrgForm.jsp?id="+id,"编辑组织",true,true,false);
	usd.show();
};
	...
	if (rows.length == 1) {
		$.messager.confirm('请确认','您确定执行该操作？',function(b){
			if(b){
				var rowData = rows[0];
			   $.ajax({
				 url:'platform/sysorg/sysOrgController/doSaveValidFlag',
...
<div data-options="region:'west',split:true,title:''" style="width:250px;padding:0px;">
	...
	<ul id="deptTree" class="easyui-tree" data-options="
			...
			url:'platform/sysorg/sysOrgController/getChildOrgDeptByIdAndOrgId.json',
