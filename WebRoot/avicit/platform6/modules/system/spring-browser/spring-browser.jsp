...
<body>
    <h3>
        Spring 容器中 Bean 定义信息 (共<%=beanMap.size()%>个Bean)
    </h3>
	<div align="left">
		Spring ApplicationContext ID:<%=appContext.getId()%> <br>
		Spring ApplicationContext DisplayName:<%=appContext.getDisplayName()%> <br>
		Spring ApplicationContext Parent:<%=appContext.getParent()%> <br>
		Spring ApplicationContext StartDate:<%=new Date(appContext.getStartupDate())%> <br>
		ClassName:<%=appContext.getClass().getName()%>
	</div>
...
<table width="100%">
    <tr>
        ...
        <td class="tdfont">class</td>
        <td class="tdfont">scope</td>
<%--        <td class="tdfont" nowrap="nowrap">配置是否正确?</td>--%>
        <td class="tdfont">configLocation</td>
    </tr>
    <%
