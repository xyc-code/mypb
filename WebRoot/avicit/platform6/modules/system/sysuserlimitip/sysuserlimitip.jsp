...
<body class="easyui-layout">
		...
		<table id="dgUserLimit" class="easyui-datagrid"
			...
			<thead>
				<tr>
					...
					<th data-options="field:'userLimitUserName',halign:'center',align:'left'" editor="{type:'validatebox', options: {required: true}}" width="220"><span style="color:red;">*</span>受限人</th>
					<th data-options="field:'limitTypeIpType',required:true,halign:'center',align:'left',formatter: formatIpLimit" editor="{type:'combobox',options:{required:true,panelHeight:'auto',editable:false,valueField:'lookupCode',textField:'lookupName'}}" width="220"><span style="color:red;">*</span>IP类型</th>
					<th data-options="field:'limitUserType',required:true,halign:'center',align:'left', formatter: formatUserLimit" editor="{type:'combobox',options:{required:true,panelHeight:'auto',editable:false,valueField:'lookupCode',textField:'lookupName'}}" width="220"><span style="color:red;">*</span>用户访问类型</th>
					<th data-options="field:'userLimitIpFrom',halign:'center',align:'left'" editor="{type:'validatebox', options:{required:true, validType: 'ip',tipPosition:'left'}}"  width="220"><span style="color:red;">*</span>开始IP</th>
	...
	<div id="userLimitSearchDialog" class="easyui-dialog" data-options="iconCls:'icon-search',modal:false,buttons:'#userLimitsearchBtns'" 
			style="width: 600px;height:150px;overflow: hidden;" title="查询条件">
		<form id="userLimitSearchForm">
    		
    		<table>
    			<tr>
    				<input id="filter_EQ_userLimitUserId" type="hidden" name="filter_EQ_userLimitUserId"/>
    				<td>受限人:</td><td><input id="filter_LIKE_userLimitUserName" type='text' name="filter_LIKE_userLimitUserName" style="width: 150px"/></td>
    				<td>IP类型:</td><td><input id="filter_EQ_limitTypeIpType" class='easyui-combobox' data-options="panelHeight:'auto',editable:false, 
    					valueField:'lookupCode',textField:'lookupName'" name ="filter_EQ_limitTypeIpType"/></td>
    			</tr>
    			<tr>
    				<td>访问类型:</td>
    				<td><input id="filter_EQ_limitUserType" class='easyui-combobox' data-options="panelHeight:'auto',editable:false, 
