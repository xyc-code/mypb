...
<body class="easyui-layout">
	<!-- 左侧菜单信息 -->
	<div data-options="region:'west',split:true,title:'菜单信息'"
						...
						<tr>
							...
							<td>
								<div style="position: relative;padding-right:5px;height: 78px">
									<textarea
										style="position: relative; overflow-y: hidden; width:100%; height:90%;"
						...
						<table>
							<tr>
								<td><a class="easyui-linkbutton" iconCls="icon-add"
									...
									href="javascript:void(0);">刷新资源</a></td>
								<td><a class="easyui-linkbutton" iconCls="icon-search"
									plain="true" onclick="saveResource();"
									href="javascript:void(0);">保存</a></td>
								<td><a class="easyui-linkbutton" iconCls="icon-edit"
									plain="true" onclick="deleteResource();"
									href="javascript:void(0);">删除</a></td>
								<td><a class="easyui-linkbutton" iconCls="icon-search"
									plain="true" onclick="openSearchForm();"
						...
						<thead>
							<tr>
								<th data-options="field:'id', halign:'center',checkbox:true"
									width="220">id</th>
								<th
									data-options="field:'menuname', required:true,align:'center'"
									width="120">菜单名称</th>
								<th data-options="field:'datagrid',required:true,align:'center'"
									width="120">权限资源标识</th>
								<th data-options="field:'dataset',align:'center',align:'center'"
									width="220">请求路径</th>
								<th
									data-options="field:'metadata',align:'center',align:'center'"
									width="120">DataGrid标识</th>
								<th
									data-options="field:'status',align:'center',align:'center',formatter:formatStatus"
									width="120">权限状态</th>
								<th
									data-options="field:'sql',align:'center',align:'center',hidden:true"
									width="120">前置sql</th>
							</tr>
						</thead>
					</table>
			...
			<div data-options="region:'south',split:true,border:false"
				...
				<div style="padding:0px;height:50px;">
					<table id="datarole" class="easyui-datagrid"
						...
						<thead>
							<tr>
								<th data-options="field:'type',align:'center',hidden:true"
									"  width="150">权限粒度</th>
								<th data-options="field:'typeName',required:true,align:'center'"
									width="150">权限粒度</th>
								<th
									data-options="field:'accessRoleName',required:true,align:'center',styler:styleCell"
									width="120">角色</th>
								<th
									data-options="field:'accessDeptName',align:'center',styler:styleCell"
									width="140">部门</th>
								<th
									data-options="field:'accessUserName',align:'center',styler:styleCell"
									width="140">用户</th>
								<th
									data-options="field:'accessGroupName',align:'center',styler:styleCell"
									width="140">群组</th>
								<th
									data-options="field:'accessPositionName',align:'center',styler:styleCell"
									width="180">岗位</th>
								<th
									data-options="field:'accessDept', halign:'center',hidden:true"
									width="220">deptId</th>
								<th
									data-options="field:'accessRole', halign:'center',hidden:true"
									width="220">roleId</th>
								<th
									data-options="field:'accessUser', halign:'center',hidden:true"
									width="220">userId</th>
								<th
									data-options="field:'accessGroup', halign:'center',hidden:true"
									width="220">groupId</th>
								<th
									data-options="field:'accessPosition', halign:'center',hidden:true"
									width="220">positionId</th>
								<th data-options="field:'presql', halign:'center',hidden:true"
									width="220">sql</th>
							</tr>
						</thead>
					</table>
	...
	<div id="searchDialog"
		...
		style="width: 600px;height:200px;display: none;">
		<form id="form1" method="post">
			<table>
				<tr>
					<td>权限资源标识:</td>
					<td><input class="easyui-validatebox" type="text"
						name="filter-LIKE-datagrid"></input></td>
					<td>请求路径 :</td>
					<td><input class="easyui-validatebox" type="text"
						name="filter-LIKE-dataset"></td>
				</tr>
				<tr>
					<td>DataGrid标识:</td>
					<td colspan="2"><input class="easyui-validatebox" type="text"
						name="filter-LIKE-metadata"></td>
				</tr>
			</table>
		</form>
		<div id="searchBtns">
			<a class="easyui-linkbutton" plain="false" onclick="searchData();"
				href="javascript:void(0);">查询</a> <a class="easyui-linkbutton"
				plain="false" onclick="clearData();" href="javascript:void(0);">清空</a>
			<a class="easyui-linkbutton" plain="false" onclick="hideSearch();"
	...
	<script type="text/javascript">
		...
		function myOnBeforeExpand(row) {
			$("#memuTree").tree("options").url = "platform/sysPermissionTreeController/listMemuTreeById.json?id="
					+ row.id;
			return true;
		};
		...
		function onClickCell(rowIndex, field, value) {
			...
			var type = $('#datarole').datagrid('getData').rows[tempIndex].type == null ? ""
					: $('#datarole').datagrid('getData').rows[tempIndex].type;
			if (type == "2") {
		...
		function showDept(rowIndex, field, value) {
			...
			var userSelect = new GridCommonSelector("dept", "datarole",
					rowIndex, "dept", {
						targetId : 'accessDept'
					}, "selectDialogCallBack", null, null, null, false, null,
					null, "1");//
			var rows = $('#datarole').datagrid("getRows");
			userSelect.init(rows[rowIndex]);
		};
		function showUser(rowIndex, field, value) {
			var userSelect = new GridCommonSelector("user", "datarole",
					rowIndex, "dept", {
						targetId : 'accessUser'
					}, "selectDialogCallBack", null, null, null, false, null,
					null, "1");//
		...
		function showRole(rowIndex, field, value) {
			var userSelect = new GridCommonSelector("role", "datarole",
					rowIndex, "dept", {
						targetId : 'accessRole'
					}, "selectDialogCallBack", null, null, null, false, null,
					null, "1");//
		...
		function showgroup(rowIndex, field, value) {
			var userSelect = new GridCommonSelector("group", "datarole",
					rowIndex, "dept", {
						targetId : 'accessGroup'
					}, "selectDialogCallBack", null, null, null, false, null,
					null, "1");//
			var rows = $('#datarole').datagrid("getRows");
			userSelect.init(rows[rowIndex]);
		...
		function showPosition(rowIndex, field, value) {
			var userSelect = new GridCommonSelector("position", "datarole",
					rowIndex, "dept", {
						targetId : 'accessPosition'
					}, "selectDialogCallBack", null, null, null, false, null,
					null, "1");//
			var rows = $('#datarole').datagrid("getRows");
			userSelect.init(rows[rowIndex]);
		...
		function onClickRow(rowNum, record) {
			...
			$.ajax({
				cache : true,
				type : "POST",
				url : 'platform/sysPermissionTreeController/listAccess.json?id'
						+ rows[0].id,
				dataType : "json",
				data : {
					id : rows[0].id
		...
		function resetResource() {
			myDatagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid(
					'clearSelections');
		};
		//刷新主表grid 
		...
		function openDeptLeaderDialog() {
			var usd = new CommonDialog(
					...
					"600",
					"300",
					"avicit/platform6/modules/system/sysdatapermission/deptleader.jsp",
					"选择部门领导", false, false, false);
			usd.show();
		};
