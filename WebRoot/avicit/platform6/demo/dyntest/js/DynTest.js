 ...
 */
function DynTest(datagrid,url,searchD,form,keyWordId,searchNames,dataGridColModel){
    ...
    this._formCode = "dynTest"; //表单的code，启动流程时需要
 ...
 * 初始化操作
 */
DynTest.prototype.init=function(){
	var _self = this;
	//初始化流程帮助类
	flowListEditor = new FlowListEditor(this._formCode);
	$(_self._datagridId).jqGrid({
    	url: _self.getUrl()+'getDynTestsByPage.json',
        mtype: 'POST',
        datatype: "json",
 ...
 * 添加页面
 */
DynTest.prototype.insert=function(){
	//添加流程
 ...
 * 编辑页面
 */
DynTest.prototype.modify=function(){
 ...
 * 详情页面
 */
DynTest.prototype.detail=function(id,value){
	flowUtils.detail(id);
};
/**
 * 打开高级查询框
 */
DynTest.prototype.openSearchForm = function(searchBtn, contentWidth, contentHeight){
	var _self = this;
	var _resizeFunc;
 ...
 */
DynTest.prototype.formValidate=function(form){
	form.validate({
		rules: {
						  		     			 		  				  				  				  		     			    				   					text01:{
						maxlength: 4000
 ...
 * 保存方法
 */
DynTest.prototype.save=function(form,windowName){
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
DynTest.prototype.del=function(){
	var _self = this;
 ...
 * 重载数据
 */
DynTest.prototype.reLoad=function(){
	var searchdata = {
		keyWord: null,
 ...
 * 关闭对话框
 */
DynTest.prototype.closeDialog=function(windowName, msg){
	layer.close(layer.getFrameIndex(windowName));
	if(msg && msg !== ''){
		layer.msg(msg);
	}
 ...
 * 后台查询	
 */
DynTest.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
DynTest.prototype.searchByKeyWord =function(){
 ...
 * 根据流程状态查询
 */
DynTest.prototype.initWorkFlow = function(status){
	$('#bpmState').val(status);
	if(status == "start"){
		$("#dynTest_modify").show();
		$("#dynTest_del").show();
 ...
 * 清空查询条件
 */
DynTest.prototype.clearData =function(){
