 ...
 */
function CreateSingle(datagrid,url,searchD,form,keyWordId,searchNames,dataGridColModel){
    ...
    this._formCode = "createSingle"; //表单的code，启动流程时需要
...
CreateSingle.prototype.init=function(){
	...
	$(_self._datagridId).jqGrid({
    	url: _self.getUrl()+'getCreateSinglesByPage.json',
        mtype: 'POST',
        datatype: "json",
 ...
 * 添加页面
 */
CreateSingle.prototype.insert=function(){
	//添加流程
 ...
 * 编辑页面
 */
CreateSingle.prototype.modify=function(){
 ...
 * 详情页面
 */
CreateSingle.prototype.detail=function(id,value){
	flowUtils.detail(id);
};
/**
 * 打开高级查询框
 */
CreateSingle.prototype.openSearchForm = function(searchBtn, contentWidth, contentHeight){
	var _self = this;
	var _resizeFunc;
 ...
 */
CreateSingle.prototype.formValidate=function(form){
	form.validate({
		rules: {
						  		     			 		  				  		     			    				   			       applyUseridAlias:{
						required:true,
 ...
 * 保存方法
 */
CreateSingle.prototype.save=function(form,windowName){
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
CreateSingle.prototype.del=function(){
	var _self = this;
 ...
 * 重载数据
 */
CreateSingle.prototype.reLoad=function(){
	var searchdata = {
		keyWord: null,
 ...
 * 关闭对话框
 */
CreateSingle.prototype.closeDialog=function(windowName, msg){
	layer.close(layer.getFrameIndex(windowName));
	if(msg && msg !== ''){
		layer.msg(msg);
	}
 ...
 * 后台查询	
 */
CreateSingle.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
CreateSingle.prototype.searchByKeyWord =function(){
 ...
 * 根据流程状态查询
 */
CreateSingle.prototype.initWorkFlow = function(status){
	$('#bpmState').val(status);
	if(status == "start"){
		$("#createSingle_modify").show();
		$("#createSingle_del").show();
	}else{
		$("#createSingle_modify").hide();
		$("#createSingle_del").hide();
	}
	this.searchData();
};
/**
 * 清空查询条件
 */
CreateSingle.prototype.clearData =function(){
