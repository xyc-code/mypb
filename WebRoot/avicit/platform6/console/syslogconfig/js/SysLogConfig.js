 ...
 */
function SysLogConfig(datagrid, url, searchForm, dataGridColModel, searchDialog, afterInit, clickRowLoad,searchMainNames,demoMainDept_KeyWord,selectTreeId){
 ...
 * 回车查询
 */
SysLogConfig.prototype.init=function(){
	var _self = this;
	var issubinit = false;
	$(_self._datagridId).jqGrid({
    	url: _self.getUrl()+'getSysLogConfigsByPage.json',
		postData: {param:null,menuId:_self.selectTreeId},
        mtype: 'POST',
        multiselect:true,
        datatype: "json",
        toolbar: [true,'top'],
        colModel: _self.dataGridColModel,
        height:$('#main').height() - $("#toolbar_sysLogConfig").innerHeight()-80,//120:顶部工具栏高+表头高+底部翻页模块高，17：顶部toolbar的内边距高度
        scrollOffset: 10,
        rowNum: 20	,
 ...
 * 添加页面
 */
SysLogConfig.prototype.insert=function(){
 ...
 * 编辑页面
 */
SysLogConfig.prototype.modify=function(){
 ...
 * 详情页面
 */
SysLogConfig.prototype.detail=function(id){
 ...
 */
SysLogConfig.prototype.formValidate=function(form){
	form.validate({
		rules: {
			required:true,
			logConfigClass:{
 ...
 * 保存方法
 */
SysLogConfig.prototype.save=function(form,id){
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
SysLogConfig.prototype.del=function(){
	var _self = this;
	var rows = $(_self._datagridId).jqGrid('getGridParam','selarrrow');
	var ids = [];
	var l =rows.length;
 ...
 * 重载数据
 */
SysLogConfig.prototype.reLoad=function(){
	var param = serializeObject($(this.searchForm));
	var searchdata = {
		param:JSON.stringify(param),
		menuId:this.selectTreeId
	}
 ...
 * 关闭对话框
 */
SysLogConfig.prototype.closeDialog=function(id){
	if(id == "insert"){
 ...
 * 打开高级查询框	
 */
SysLogConfig.prototype.openSearchForm = function(searchDiv,par){
	var _self = this;
	par = null;
 ...
 * 去后台查询
 */
SysLogConfig.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
SysLogConfig.prototype.searchByKeyWord =function(){
 ...
 * 隐藏查询框
 */
SysLogConfig.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
SysLogConfig.prototype.clearData =function(){
 ...
 * 返回主表id
 */
SysLogConfig.prototype.getSelectID=function(){
