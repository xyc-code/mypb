function SysProfileOptionValue(datagrid, url, formSub, dataGridColModel,
		searchDialogSub, pid, demoSubUser_KeyWord) {
...
//初始化操作
SysProfileOptionValue.prototype.init = function(pid) {
	var _self = this;
	$(_self._datagridId).jqGrid({
		url : this.getUrl() + 'getSysProfileOptionValue',
		postData : {
			pid : _self.pid
		},
		mtype : 'POST',
...
//添加页面
SysProfileOptionValue.prototype.insert = function(pid) {
	if (this.pid == null || this.pid == "") {
		layer.alert('请选择配置文件数据！', {
			icon : 7,
			area : [ '400px', '' ], //宽高
			closeBtn : 0,
			title:'提示'
...
//修改页面
SysProfileOptionValue.prototype.modify = function() {
	if (this.pid == null || this.pid == "") {
		layer.alert('请选择配置文件数据！', {
			icon : 7,
			area : [ '400px', '' ], //宽高
...
//详细页
SysProfileOptionValue.prototype.detail = function(id) {
...
SysProfileOptionValue.prototype.formValidate = function(form) {
	form.validate({
		rules : {
			sysProfileOptionsId : {
				maxlength : 50
...
//保存功能
SysProfileOptionValue.prototype.save = function(form, id) {
	var _self = this;
	avicAjax.ajax({
...
//删除
SysProfileOptionValue.prototype.del = function() {
...
//重载数据
SysProfileOptionValue.prototype.reLoad = function(pid) {
	if (pid != undefined) {
		this.pid = pid;
	}
	var searchdata = {
...
//关闭对话框
SysProfileOptionValue.prototype.closeDialog = function(id) {
	if (id == "insert") {
...
//打开高级查询框
SysProfileOptionValue.prototype.openSearchForm = function(searchDiv, par) {
	var _self = this;
	par = null;
	//if(!par) par = $(window);
...
//高级查询
SysProfileOptionValue.prototype.searchData = function() {
	var searchdata = {
		keyWord : null,
		pid : this.pid,
...
//关键字段查询
SysProfileOptionValue.prototype.searchByKeyWord = function() {
...
//隐藏查询框
SysProfileOptionValue.prototype.hideSearchForm = function() {
	layer.close(searchDialogindex);
};
/*清空查询条件*/
SysProfileOptionValue.prototype.clearData = function() {
