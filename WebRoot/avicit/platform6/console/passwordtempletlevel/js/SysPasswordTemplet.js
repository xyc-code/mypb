function SysPasswordTemplet(datagrid, url, formSub, dataGridColModel,
...
//初始化操作
SysPasswordTemplet.prototype.init = function(pid) {
	var _self = this;
	$(_self._datagridId).jqGrid({
		url : this.getUrl() + 'getSysPasswordTemplet',
		postData : {
			pid : _self.pid
		},
		mtype : 'POST',
...
//添加页面
SysPasswordTemplet.prototype.insert = function(pid) {
...
//修改页面
SysPasswordTemplet.prototype.modify = function() {
	if (this.pid == null || this.pid == "") {
		layer.alert('请选择一条主表数据！', {
			icon : 7,
			area : [ '400px', '' ], //宽高
...
//详细页
SysPasswordTemplet.prototype.detail = function(id) {
...
SysPasswordTemplet.prototype.formValidate = function(form) {
	form.validate({
		rules : {
			templetKey : {
				maxlength : 50
...
//保存功能
SysPasswordTemplet.prototype.save = function(form, id) {
	var _self = this;
	$(this._datagridId).jqGrid('endEditCell');
	var data = $(this._datagridId).jqGrid('getChangedCells');
	if (data.length > 0) {
		for ( var i = 0; i < data.length; i++) {
			if (data[i].templetState == "有效") {
				data[i].templetState = '1';
			} else {
				data[i].templetState = '0';
			}
		}
	}
	if(data.length <= 0){
...
//删除
SysPasswordTemplet.prototype.del = function() {
...
//重载数据
SysPasswordTemplet.prototype.reLoad = function(pid) {
	if (pid != undefined) {
		this.pid = pid;
	}
	var searchdata = {
...
//关闭对话框
SysPasswordTemplet.prototype.closeDialog = function(id) {
	if (id == "insert") {
...
//打开高级查询框
SysPasswordTemplet.prototype.openSearchForm = function(searchDiv, par) {
	var _self = this;
	par = null;
	//if(!par) par = $(window);
...
//高级查询
SysPasswordTemplet.prototype.searchData = function() {
	var searchdata = {
		keyWord : null,
		pid : this.pid,
...
//关键字段查询
SysPasswordTemplet.prototype.searchByKeyWord = function() {
...
//隐藏查询框
SysPasswordTemplet.prototype.hideSearchForm = function() {
	layer.close(searchDialogindex);
};
/*清空查询条件*/
SysPasswordTemplet.prototype.clearData = function() {
