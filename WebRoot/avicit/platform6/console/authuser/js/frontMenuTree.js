 ...
 * @returns
 */
function frontMenuTree(ztreeId, url, form, searchD, searchbtn,id,param){
...
//初始化树节点
frontMenuTree.prototype.init=function(){
	var _self = this;
	...
	_self.setting = {
			...
			callback: {
				onClick: function(event, treeId, treeNode){ //绑定左右联动事件
					 ...
					 selectColor(event,treeId,treeNode);
			    },
...
//查询框查询
frontMenuTree.prototype.onseach = function(ecode, value){
	var _self = this;
	if(ecode == 13){
...
function selectColor(event,treeId,treeNode){ 
	var aId = event.target.parentNode.id;
	 $("#"+aId).removeClass("curSelectedNode"); 
	  $("#"+aId).removeClass("current");
	var   lId = event.target.parentNode.parentNode.id;
	  $("#"+lId).removeClass("current");
	return;
}
