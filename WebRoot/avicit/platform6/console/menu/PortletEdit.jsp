...
<body class="easyui-layout" fit="true">
		...
		<form id='form'>
			...
			<table class="form_commonTable">
				<tr>
					...
					<td width="39%">
		                <input class="form-control input-sm" type="text" name="menuName" id="menuName" value='${menu.menuName}'/>
					</td>
					
					<th width="10%" style="word-break: break-all; word-warp: break-word;"><label for="menuCode">小页编码:</label></th>
					<td width="39%">
						<input class="form-control input-sm" type="text" name="menuCode" id="menuCode"value='${menu.menuCode}'/>
					</td>
				</tr>
				<tr>
					<th width="10%" style="word-break: break-all; word-warp: break-word;"><label for="menuUrl">小页URL:</label></th>
					<td width="39%"><input type="text" class="form-control input-sm" rows="3" title="小页URL" name="menuUrl" id="menuUrl" value='${menu.menuUrl}'></input></td>
				...
				<tr>
					...
					<td width="39%"><input type="text" class="form-control input-sm" rows="3" title="跳转地址" name="moreUrl" id="moreUrl" value='${menu.moreUrl}'></input></td>
					<th width="10%" style="word-break: break-all; word-warp: break-word;"><label for="refresh">刷新频率:</label></th>
					<td width="39%" nowrap="nowrap"><input type="number" class="form-control input-sm" style="width: 88%;display:inherit;" rows="3" title="刷新频率" name="refresh" id="refresh" value='${menu.refresh}'></input><span>--单位：分</span></td>
				</tr>
				<tr>
					...
					<td width="39%">
		                <input class="form-control input-sm" type="text" name="css" id="css" 
		                 value='<c:out  value='${menu.css}'/>' /> 
					</td>
					
					<th width="10%" style="word-break: break-all; word-warp: break-word;"><label for="height">小页高度:</label></th>
					<td width="39%">
						<input class="form-control input-sm" type="text" name="height" id="height" value='${menu.height}'/>
					</td>
				</tr>
				<tr>
					...
					<td width="39%">
						<label class="radio-inline"> <input name="showTitle" title="显示" type="radio" value="1" <c:if test="${menu.showTitle eq 1}">checked</c:if>>显示</label>
						<label class="radio-inline"> <input name="showTitle" title="隐藏" type="radio" value="0" <c:if test="${menu.showTitle eq 0}">checked</c:if>>隐藏</label>
					</td>
				</tr>
				...
				<tr style="display: none">
					...
					<td width="39%">
						<label class="radio-inline"> <input name="isClose" title="允许" type="radio" value="1" <c:if test="${menu.isClose eq 1}">checked</c:if>>允许</label>
						<label class="radio-inline"> <input name="isClose" title="禁止" type="radio" value="0" <c:if test="${menu.isClose eq 0}">checked</c:if>>禁止</label>
					</td>

					<th width="10%" style="word-break: break-all; word-warp: break-word;"><label for="isDrag">能否拖拽:</label></th>
					<td width="39%">
						<label class="radio-inline"> <input name="isDrag" title="允许" type="radio" value="1" <c:if test="${menu.isDrag eq 1}">checked</c:if>>允许</label>
						<label class="radio-inline"> <input name="isDrag" title="禁止" type="radio" value="0" <c:if test="${menu.isDrag eq 0}">checked</c:if>>禁止</label>
					</td>
				</tr>
				...
				<tr>
					...
					<td width="39%" colspan="3"><input class="form-control input-sm" type="text" name="menuView" id="menuView" value='${menu.menuView}'/></td>
				</tr>
	...
	<script type="text/javascript">
		function closeForm(){
			parent.menuPortal.closeDialog("update");
		}
		function saveForm(){
			...
			$.ajax({
				url: parent.menuPortal.getUrl() + "/validatePortletUrl/"+serializeObject($('#form'))["id"],
				type: 'post',
				data:{url:serializeObject($('#form'))["menuUrl"]},
				dataType: 'json',
				async:false,
				success: function (r) {
					if(r!==''&&r!==null&&r.length>0){
						...
						layer.confirm('当前菜单URL已存在与菜单'+title+'中，是否继续修改？', {icon: 3, title:'提示'}, function(index){
							parent.menuPortal.save($('#form'),"update");
							layer.close(index);
						});
					}else{
						parent.menuPortal.save($('#form'),"update");
