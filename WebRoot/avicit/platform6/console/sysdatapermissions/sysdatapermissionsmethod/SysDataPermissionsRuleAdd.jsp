...
<body class="easyui-layout" fit="true">
	<div data-options="region:'center',split:true,border:false">
		<h3 style="text-align: center; font-weight: 600;">添加规则</h3>
		<form id='form' style="width: 96%;">
			...
			<table class="form_commonTable">
				...
				<tr>
					...
					<td width="23%">
						<label class="radio-inline">
							<input type="radio" name="matchSymbol" value="0" checked="checked" />或（or）
						</label>
						<label class="radio-inline">
							<input type="radio" name="matchSymbol" value="1" />且（and）
						</label>
					</td>
					<th width="10%"><label for="status">状态:</label></th>
					<td width="23%">
						<label class="radio-inline">
							<input type="radio" name="state" value="0" checked="checked" />有效
						</label>
						<label class="radio-inline">
							<input type="radio" name="state" value="1" />无效
						</label>
					</td>
				...
				<tr>
					...
					<td colspan="3">
						<label class="radio-inline">
							<input type="radio" name="ruleType" value="0" checked="checked" />配置字段
						</label>
						<label class="radio-inline">
							<input type="radio" name="ruleType" value="1" />配置自定义类
						</label>
					</td>
					<th width="10%"></th>
				...
				<tr class="columnTr">
					...
					<td width="23%" colspan="3">
						<textarea class="form-control input-sm" name="filterConditionSql" id="filterConditionSql" rows="5"></textarea>
					</td>
				</tr>
				<tr class="columnTr">
					<th width="10%"><label for="filterCondition">过滤条件:</label></th>
					<td width="23%" colspan="3">
						<textarea class="form-control input-sm" name="filterCondition" id="filterCondition" rows="5"></textarea>
					</td>
				</tr>
				...
				<tr class="classTr" style="display: none;">
					...
					<td width="23%" colspan="3">
						...
						<textarea class="form-control input-sm" name="columnValue" id="columnValue" rows="3"></textarea>
					</td>
				</tr>
				<tr class="classTr" style="display: none;">
			...
			<table class="tableForm"
				...
				<tr>
					<td width="50%" style="padding-right: 4%;" align="right">
						<a href="javascript:void(0)" class="btn btn-primary form-tool-btn btn-sm btn-point" role="button" title="保存" id="sysDataPermissionsRule_saveForm">保存</a>
						<a href="javascript:void(0)" class="btn btn-grey form-tool-btn btn-sm" role="button" title="返回" id="sysDataPermissionsRule_closeForm">返回</a>
					</td>
				</tr>
			</table>
		</div>
	...
	<script type="text/javascript">
		function closeForm() {
			parent.sysDataPermissionsRule.closeDialog("insert");
		}
		function saveForm() {
			...
			var methodId = parent.sysDataPermissionsRule.pid;
			$("#methodId").val(methodId);

			//限制保存按钮多次点击
			$('#sysDataPermissionsRule_saveForm').addClass('disabled').unbind("click");
			parent.sysDataPermissionsRule.save($('#form'), "insert");
		}
		function example(){
			layer.open({
	            type: 2,
