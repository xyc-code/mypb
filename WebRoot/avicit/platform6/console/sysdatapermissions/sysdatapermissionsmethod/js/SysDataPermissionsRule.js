...
SysDataPermissionsRule.prototype.init=function(pid){
	...
	$(_self._datagridId).jqGrid({
        ...
        height : $('#north2').height() - $("#toolbar_sysDataPermissionsRule").innerHeight()-36-32,
        	...
        	$(':input[name="my-checkbox"]').on('switchChange.bootstrapSwitch', function(event, state) {
        		if(tempFlag){
            		...
            		if (state.value) {
            			...
            			layer.confirm('置为无效后会删除已分配的规则且公式会重新生成！是否继续？',{btn:["是","否"],closeBtn:0},function(a,b){
            				updateState(rowid,'1');
            				layer.close(a);
            			},function(){
            				tempFlag = true;
    						$("input[data-rowid='"+rowid+"']").bootstrapSwitch('state',true);
    					});
            		}
        		}
        	});
