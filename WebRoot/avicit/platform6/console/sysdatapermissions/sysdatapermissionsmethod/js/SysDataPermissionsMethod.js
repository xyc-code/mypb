...
SysDataPermissionsMethod.prototype.init=function(){
	...
	$(_self._datagridId).jqGrid({
        ...
        height : $('#north1').height() - $("#toolbar_sysDataPermissionsMethod").innerHeight()-36-38,
        scrollOffset: 10,
        rowNum: 10,
        rowList:[30,20,10],
        altRows:true,
 ...
 * 添加页面
 */
SysDataPermissionsMethod.prototype.insert=function(menuId){
	this.insertIndex = layer.open({
	    ...
	    content: this.getUrl()+'Add/null?menuId='+menuId 
	});
};
/**
 * 编辑页面
 */
SysDataPermissionsMethod.prototype.modify=function(menuId){
	...
	this.editIndex = layer.open({
	    ...
	    skin: 'bs-modal', // bootstrap 风格皮肤 需加载skin
        maxmin: false, //开启最大化最小化按钮
	    content: this.getUrl()+'Edit/'+rowData.id+'?menuId='+menuId 
	});
};
/**
 * 详情页面
 ...
 */
SysDataPermissionsMethod.prototype.formValidate=function(form){
}
/**
 * 复制方法
 */
SysDataPermissionsMethod.prototype.copyMethod=function(formData){
	var _self = this;
	avicAjax.ajax({
		 url:"platform6/sysdatapermissionsmethod/sysDataPermissionsMethodController/copyMethod",
		 data : {data : formData},
		 type : 'post',
		 dataType : 'json',
		 success : function(r){
 ...
 * 复制规则
 */
SysDataPermissionsMethod.prototype.copyRule=function(formData,methodId){
	var _self = this;
	avicAjax.ajax({
		 url:"platform6/sysdatapermissionsmethod/sysDataPermissionsMethodController/saveCopyRuleData",
		 data : {data : formData,methodId:methodId},
		 type : 'post',
		 dataType : 'json',
		 success : function(r){
			 if (r.flag == "success"){
 ...
 * 保存方法
 */
SysDataPermissionsMethod.prototype.save=function(form,id){
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
SysDataPermissionsMethod.prototype.del=function(){
	var _self = this;
	var rows = $(_self._datagridId).jqGrid('getGridParam','selarrrow');
	var ids = [];
	var l =rows.length;
	if(l > 0){
		layer.confirm('删除模块也会级联删除对应的规则及已分配的权限。是否继续？',  {icon: 3, title:"提示", area: ['400px', '']}, function(index){
				for(;l--;){
					 ids.push(rows[l]);
				 }
				 avicAjax.ajax({
...
SysDataPermissionsMethod.prototype.getRowId=function() {
	var _self = this;
	var rows = $(_self._datagridId).jqGrid('getGridParam', 'selarrrow');
	if(rows.length != 1){
		layer.msg("请选择一条记录");
	...
	return $(_self._datagridId).jqGrid('getRowData', rows[0]);
}