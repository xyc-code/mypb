...
function initTips() {
	...
	$("#sysdatapermissionTips1").mouseover(function () {
		var message = "<span style='color:#333333;'>选择表将自动匹配对应的Mapper，如没有匹配出请手动选择！手动选择的Mapper需要和表有关联！</span>";
	...
	$("#sysdatapermissionTips2").mouseover(function () {
		var message = "<span style='color:#333333;'>平台自定义选择页：请填写编码值<br/>电子表单数据字典：请填写电子表单编码-字段id<br/>自定义选择页：请填写能代表该页面的唯一标识</span>";
		tipsIndex = layer.tips(message, $(this), {
			tips: 1,
			area: ['auto','auto'],
			time: 0
	...
	$("#sysdatapermissionTips3").mouseover(function () {
		var message = "<span style='color:#333333;'>平台自定义选择页：系统>>配置管理>>自定义选择页<br/>" +
				"电子表单数据字典：电子表单设计>>高级控件>>数据字典控件<br/>" +
				"自定义选择页：将SQL维护在数据库中，弹出页面只有1个，页面通过传递标识来读取对应的SQL进行数据展示</span>";
		tipsIndex = layer.tips(message, $(this), {
			tips: 1,
			area: ['auto','auto'],
			time: 0
	...
	$("#sysdatapermissionTips4").mouseover(function () {
		var message = "<span style='color:#333333;'>执行方法至少需要包含以下参数：@Param(\"code\") String code @Param(\"sql\") String sql</span>";
		tipsIndex = layer.tips(message, $(this), {
			tips: 1,
			area: ['auto','auto'],
			time: 0
	...
	$("#sysdatapermissionTips5").mouseover(function () {
		var message = "<span style='color:#333333;'>" +
			"SqlServer:<br/>" +
	...
	$("#sysdatapermissionTips6").mouseover(function () {
		var message = "<span style='color:#333333;'>DM DBMS数据库请填写DM</span>";
		tipsIndex = layer.tips(message, $(this), {
			tips: 1,
			area: ['auto','auto'],
			time: 0
	...
	$("#sysdatapermissionTips7").mouseover(function () {
		var message = "<span style='color:#333333;'>选择mapper文件后，会默认自动回填分页查询方法以及导出查询方法。</span>";
		tipsIndex = layer.tips(message, $(this), {
			tips: 1,
			area: ['auto','auto'],
			time: 0
	...
	if(val == "0"){
		layer.open({
			...
			yes : function(index, layero) {
				...
				var rowData = iframeWin.selectRowdata;
					...
					if (r.flag == "success"){
						if("" != r.method){
							...
							$.each(r.method,function(i,d){
								var str = d.substr(d.lastIndexOf(".")+1);
								methodHtml+="<option value='"+str+"'>"+str+"</option>";
								if(str.indexOf("ByPage") != -1||str.indexOf("ForExport") != -1){
									checkedOption += str+",";
								}
							});
							$("#method").html(methodHtml);
