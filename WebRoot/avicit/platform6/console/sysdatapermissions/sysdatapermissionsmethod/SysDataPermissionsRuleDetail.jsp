...
<body class="easyui-layout" fit="true">
	<div data-options="region:'center',split:true,border:false">
		<h3 style="text-align: center; font-weight: 600;">规则详情</h3>
		<form id='form'>
			<table class="form_commonTable">
				<tr>
					...
					<td width="23%"><input class="form-control input-sm"
						type="text" name="ruleName" id="ruleName" disabled="disabled"
					...
					<td width="23%"><input class="form-control input-sm"
						type="text" name="ruleRemarks" id="ruleRemarks" disabled="disabled"
				...
				<tr>
					<th width="10%" style="word-break: break-all; word-warp: break-word;"><label for="matchSymbol">相邻规则匹配符:</label></th>
					<td width="23%">
						<label class="radio-inline">
							<input type="radio" name="matchSymbol" id="matchSymbol0" value="0" disabled="disabled"/>或（or）
						</label>
						<label class="radio-inline">
							<input type="radio" name="matchSymbol" id="matchSymbol1" value="1" disabled="disabled"/>且（and）
						</label>
					</td>
					...
					<td width="23%">
						<label class="radio-inline">
							<input type="radio" name="state" id="state0" value="0" disabled="disabled"/>有效
						</label>
						<label class="radio-inline">
							<input type="radio" name="state" id="state1" value="1" disabled="disabled"/>无效
						</label>
					</td>
				...
				<tr>
					...
					<td colspan="3">
						<label class="radio-inline">
							<input type="radio" name="ruleType" id="ruleType0" value="0" disabled="disabled" />配置字段
						</label>
						<label class="radio-inline">
							<input type="radio" name="ruleType" id="ruleType1" value="1" disabled="disabled" />配置自定义类
						</label>
					</td>
					<th width="10%"></th>
				...
				<tr class="columnTr">
					<th width="10%"><label for="columnName">字段:
						<i class="tips-iconhelp icon iconfont icon-question-circle" id="sysdatapermissionTips"></i></label>
					</th>
					<td width="23%" colspan="3" style="padding-left: 0px;">
						<table style="width: 100%;">
							<tr>
								<td style="width: 40%;">
									<select class="js-example-basic-multiple" id="columnName" name="columnName" disabled="disabled" style="width: 100%;">${selectColumnHtml }</select>
								</td>
								<td style="width: 10%;">
									<select class="js-example-basic-multiple" id="columnSymbol" name="columnSymbol" disabled="disabled" style="width: 100%;">
								...
								<td style="width: 30%;">
									<select class="js-example-basic-multiple" id="columnType" name="columnType" disabled="disabled" style="width: 80%;">
				...
				<tr class="columnTr">
					...
					<td width="23%" colspan="3">
						<textarea class="form-control input-sm" disabled="disabled" name="filterConditionSql" disabled="disabled" id="filterConditionSql" rows="5">${sysDataPermissionsRuleDTO.filterConditionSql}</textarea>
					</td>
				</tr>
				<tr class="columnTr">
					<th width="10%"><label for="filterCondition">过滤条件:</label></th>
					<td width="23%" colspan="3">
						<textarea class="form-control input-sm" disabled="disabled" name="filterCondition" disabled="disabled" id="filterCondition" rows="5">${sysDataPermissionsRuleDTO.filterCondition}</textarea>
					</td>
				</tr>
				...
				<tr class="classTr" style="display: none;">
					...
					<td width="23%" colspan="3">
						...
						<textarea class="form-control input-sm" name="columnValue" id="columnValue" rows="3" disabled="disabled">${sysDataPermissionsRuleDTO.columnValue}</textarea>
					</td>
				</tr>
				<tr class="classTr" style="display: none;">
			...
			<table class="tableForm"
				...
				<tr>
					<td width="50%" style="padding-right: 4%;" align="right">
						<a href="javascript:void(0)" class="btn btn-grey form-tool-btn btn-sm" role="button" title="关闭" id="sysDataPermissionsRule_closeForm">关闭</a>
					</td>
				</tr>
			</table>
		</div>
	...
	<script type="text/javascript">
		...
		function simulationSql(){
			...
			avicAjax.ajax({
				...
				data : {className:columnValue,simulationUser:simulationUser,pid:'${sysDataPermissionsRuleDTO.methodId}'},
				type : 'get',
				dataType : 'json',
				success : function(r){
					if (r.flag == "success"){
		...
		function initTips() {
			...
			$("#sysdatapermissionTips").mouseover(function () {
				var message = "<span style='color:#333333;'>使用变量时大括号左右两侧不能带有空格，且注意大小写。<br/>" +
						"目前可使用的变量包含：<br/>" +
						"{t}：解析时将替换成主表别名/表名<br/>" +
