...
<div data-options="region:'west',split:true" style="width: 250px;border-top-style: hidden;overflow: hidden; height: 100%">
		...
		<div id='mdiv' style="overflow:auto; height: 90%;">
			<ul id="protalMenu" class="ztree"></ul>
		</div>
	</div>
				...
				<div id="toolbar_sysDataPermissionsMethod" class="toolbar">
					<div class="toolbar-left">
						...
						<sec:accesscontrollist hasPermission="3" domainObject="formdialog_sysDataPermissionsMethod_button_log" permissionDes="数据权限日志">
							<input type="checkbox" id="startLog" <c:if test="${isOpenLog eq true }">checked="checked"</c:if>/>
							<i class="tips-iconhelp icon iconfont icon-question-circle" id="sysdatapermissionTips1"></i>
						</sec:accesscontrollist>
					...
					<div class="toolbar-right">
						<div class="input-group form-tool-search" style="width: 210px">
	...
	<form id="form">
		<table class="form_commonTable">
			<tr>
				...
				<th width="16%">表名称:</th>
				<td width="33%"><input title="表名称" class="form-control input-sm" type="text" name="tableName" id="tableName" /></td>
			</tr>
			<tr>
				<th width="16%">mapper名称:</th>
				<td width="33%"><input title="mapper名称" class="form-control input-sm" type="text" name="mapperName" id="mapperName" /></td>
				<th width="16%">执行方法:</th>
				<td width="33%"><input title="执行方法" class="form-control input-sm" type="text" name="method" id="method" /></td>
			</tr>
			<tr>
				<th width="16%">选择页标识:</th>
				<td width="33%"><input title="选择页标识" class="form-control input-sm" type="text" name="selectId" id="selectId" /></td>
			</tr>
		</table>
	</form>
</div>
...
<script type="text/javascript">
	...
	function initTips() {
		...
		$("#sysdatapermissionTips1").mouseover(function () {
			var message = "<span style='color:#333333;'>是否开启数据权限日志</span>";
	...
	$(document).ready(function() {
		...
		//菜单树初始化
		menuTree = new MenuTree('protalMenu','${urlMenu}','','txt','searchbtn');
		...
		//复制方法
		var urlMenu = ('${urlMenu}' == '') ? '1-ORG_ROOT-1-zh_CN' : '${urlMenu}'.split('/')[3];
		$('#sysDataPermissionsMethod_copy_method').bind('click',function() {
			var rowObject = sysDataPermissionsMethod.getRowId();
			if (rowObject == false) {
				layer.msg("请选择一条记录");
				return false;
			}
			...
			copyPageIndex = layer.open({
			    ...
			    skin: 'bs-modal', // bootstrap 风格皮肤 需加载skin
		        maxmin: false, //开启最大化最小化按钮
			    content: "platform6/sysdatapermissionsmethod/sysDataPermissionsMethodController/toCopyMethodPage?urlMenu=" + urlMenu + "&methodId=" + rowId
			});
		});
		//数据权限日志查询
