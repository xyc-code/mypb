...
<body class="easyui-layout" fit="true" style="overflow:hidden;">
	...
	<div data-options="region:'west',title:'角色选择',split:false,border:true,collapsible:false" style="width:460px;overflow-x: hidden;">
	...
	<div id="center" data-options="region:'center',title:'模块选择',split:false,border:true" style="width:240px;overflow: hidden;">
        <ul id="myTab" class="nav nav-tabs apermission-conten">
            <li class="active"><a rel='portal_ifrm' href="#portal" data-toggle="tab" onclick="changeMenu('portal')">前台菜单</a></li>
            <li><a rel='console_ifrm' href="#console" data-toggle="tab" onclick="changeMenu('console')">后台菜单</a></li>
        </ul>
        <div id="myTabContent" class="tab-content myTabContent-conten" style="overflow: hidden;">
            <div class="tab-pane fade in active" id="portal" style="height: 100%">
                <div id="tableToolbarGroup" class="toolbar tableToolbarGroup-conten">
                ...
                <div id="mdiv" class="mdiv-conten">
                    <ul id="menuTree" class="ztree"></ul>
                </div>
            </div>
            <div class="tab-pane fade" id="console" style="height: 100%">
                <div id="tableToolbarGroupConsole" class="toolbar tableToolbarGroup-conten">
                    <div class="toolbar-left">
                        <div class="input-group form-tool-search">
                            <input type="text" name="txt" id="txtConsole" style="width:240px;" class="form-control input-sm" placeholder="请输入菜单名称" />
                            <label id="searchbtnConsole" class="icon icon-search form-tool-searchicon"></label>
                        </div>
                    </div>
                </div>
                <div id="mdivConsole" class="mdiv-conten">
                    <ul id="menuTreeConsole" class="ztree"></ul>
                </div>
            </div>
        </div>
	</div>
	
	<div data-options="region:'east',title:'规则授权',split:false,border:true,collapsible:false" style="width:640px;">
		<div id="methodDiv">
			<form id='methodForm'>
			...
			<div data-options="region:'south',border:false" style="text-align: right;margin-right: 26px;">
				...
				<a href="javascript:void(0)" class="btn btn-grey form-tool-btn btn-sm" role="button" title="重置" id="sysDataPermissionsAuth_closeForm">重置</a>
			</div>
		</div>
	</div>    
</body>
...
<script type="text/javascript">
	var listRole,menuTree,menuTreeConsole,appId='${appId}';
	var roleId="";
	var methodId="";
	var authId="";
	var authMap;
	...
	function changeMenu(menu) {
		if (menu != tabId) {
			if (menu == "portal") {
				tabId = menu;
				clearFormData();
				menuTree.getOnSelect()(menuTree._selectNode,menuTree);
			} else {
				tabId = menu;
				clearFormData();
				menuTreeConsole.getOnSelect()(menuTreeConsole._selectNode,menuTreeConsole);
			}
		}
	}
	...
	$(function(){
		...
		var params = '${params}';
		var paramsConsole = params.split('-')[0] + '-' + params.split('-')[1] + '-' + '0' + '-' + params.split('-')[3]
		//后台菜单
		menuTreeConsole = new MenuTree('menuTreeConsole','${urlMenu}/console/' + paramsConsole,'','txtConsole','searchbtnConsole');
		menuTreeConsole.setOnSelect(function(node){
			var menuId=node.id;
			if(undefined != node.menuType){
				if(node.menuType == "method"){ // 点击了模块
					methodId = menuId;
	...
	function initTips() {
		var tipsIndex;
		$("#sysdatapermissionTips").mouseover(function () {
			var message = "<span style='color:#333333;'>请按如下规则编辑条件组合<br/>" +
					"( 1 and 2 ) or ( 3 and 4 ) √<br/>" +
					"1.有and和or必须加括号<br/>" +
					"&nbsp;&nbsp;&nbsp;&nbsp;1 and 2 or 3 and 4 ×<br/>" +
					"2.一个括号里不能同时出现and和or<br/>" +
					"&nbsp;&nbsp;&nbsp;&nbsp;(1 and 2 or 3) and 4 ×<br/>" +
					"3.不允许出现不存在的编号<br/>" +
					"&nbsp;&nbsp;&nbsp;&nbsp;1 and 2 and 3 and 4 and 5 ×<br/>" +
					"4.括号都是成对出现的<br/>" +
					"&nbsp;&nbsp;&nbsp;&nbsp;(1 and 2 and 3 and 4 ×<br/>" +
					"5.标识符，括号，运算符之间应带有空格<br/>" +
					"&nbsp;&nbsp;&nbsp;&nbsp;(1or2or3) ×</span>";
			tipsIndex = layer.tips(message, $(this), {
				tips: 1,
				area: ['auto', 'auto'],
