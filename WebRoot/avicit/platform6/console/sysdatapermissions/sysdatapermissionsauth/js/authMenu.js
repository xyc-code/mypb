...
MenuTree.prototype.onseach = function(ecode, value){
	var _self = this;
    var falg = this.getUrl().indexOf("0");
    if(ecode == 13){
		...
		$.ajax({
			cache: true,
			type: "post",
			url:  "platform6/msystem/sysdatapermissions/sysdatapermissionsauth/sysDataPermissionsAuthController/menu/search",
			dataType:"json",
			data:{search_text:value,falg:falg},
			async: false,
