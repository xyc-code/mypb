...
<div data-options="region:'center',split:true,border:false">
	<form id='form'>
		<input type="hidden" id="id" name="id" value="<c:out value='${sysLookupHiberarchyDTO.id}'/>" />
		<input type="hidden" name="version" value="<c:out  value='${sysLookupHiberarchyDTO.version}'/>"/>
		<input type="hidden" id="treeLevel" name="treeLevel" value="<c:out  value='${sysLookupHiberarchyDTO.treeLevel}'/>"/>
		<table class="form_commonTable">
			<tr>
				...
				<td>
					<div class="input-group  input-group-sm">
						<input type="hidden" id="parentId" name="parentId" value="<c:out  value='${sysLookupHiberarchyDTO.parentId}'/>" />
						<input class="form-control input-sm" placeholder="请选择父级节点" type="text" id="parentTypeValue" name="parentTypeValue" value="<c:out  value='${sysLookupHiberarchyDTO.parentTypeValue}'/>" readonly="readonly" />
				...
				<td>
					<input class="form-control input-sm" type="text" name="typeValue"  id="typeValue" value="<c:out value='${sysLookupHiberarchyDTO.typeValue}'/>">
				</td>
			</tr>
			<tr>
				<th>
					<label for="uniqueKey">代码标识:</label></th>
				<td>
					<input class="form-control input-sm" type="text" name="uniqueKey"  id="uniqueKey" value="<c:out value='${sysLookupHiberarchyDTO.uniqueKey}'/>" readonly>
				</td>
				<th>
					<label for="systemFlag">使用级别:</label></th>
				<td>
					<pt6:h5select css_class="form-control input-sm" name="systemFlag" id="systemFlag" title="" isNull="true" lookupCode="PLATFORM_USAGE_MODIFIER" defaultValue="${sysLookupHiberarchyDTO.systemFlag}" />
				</td>
				<%--<th>
					<label for="typeKey">系统代码值:</label></th>
				<td>
					<input class="form-control input-sm" type="text" name="typeKey"  id="typeKey" value="<c:out value='${sysLookupHiberarchyDTO.typeKey}'/>">
				</td>--%>
			</tr>
			<tr>
				<th>
				...
				<td>
					<pt6:h5select css_class="form-control input-sm" name="validFlag" id="validFlag" title="" isNull="true" lookupCode="PLATFORM_VALID_FLAG" defaultValue="${sysLookupHiberarchyDTO.validFlag}" />
				</td>
				<th>
					<label for="treeSort">节点当前排序:</label></th>
				<td>
					<div class="input-group input-group-sm spinner" data-trigger="spinner">
						<input  class="form-control"  type="text" name="treeSort" id="treeSort" data-min="-9999999999" data-max="9999999999" data-step="1" data-precision="0" value="<c:out value='${sysLookupHiberarchyDTO.treeSort}'/>">
			...
			<tr>
				...
				<td width="39%">
					...
					<select width="39%" class="form-control input-sm" name="sysLanguageCode"  id="sysLanguageCode">
					</select>
				</td>
			</tr>
			<tr>
				<%--<c:if test="${showType eq 'show'}">
					...
					<td width="37%">
						<input class="form-control input-sm" type="text" name="lookupType"  id="lookupType" value="<c:out value='${sysLookupHiberarchyDTO.lookupType}'/>">
					</td>
				</c:if>--%>
			</tr>
			<tr>
				...
				<td width="39%" colspan="3">
					<textarea class="form-control" rows="3" name="remark"  id="remark"><c:out value='${sysLookupHiberarchyDTO.remark}'/></textarea>
				</td>
			</tr>
		</table>
	</form>
...
<script type="text/javascript">
	...
	function closeForm(){
		parent.newSysLookupHiberarchy.closeDialog("edit");
	}
	...
	function updateParent() {
		...
		var selectIndex = layer.open({
			...
			content: parent.newSysLookupHiberarchy.getUrl() + 'toSysLookupHiberarchyParentSelect',
			btn: ['确定', '取消'],
			yes: function (index, layero) {
				...
				if (parentNode == '' || parentNode == null) {
					layer.alert('请选择父节点！', {
						icon: 7, area: ['400', ''], closeBtn: 0, btn: ['关闭'], title: '提示'
					});
					return;
				}
				//校验所选择父节点是否是当前的节点的子节点
				var curNodeId = $('#id').val();
				...
				if (parentNode != '' && parentNode != null) {
					...
					$('#parentTypeValue').val(parentNode.typeValue);
				}
				//刷新页面
	...
	function saveForm(){
		...
		parent.newSysLookupHiberarchy.save($('#form'),"edit");
	}
	//清空日期值
	...
	$(document).ready(function () {
		...
		// 父节点选择绑定事件
		$('#parentTypeValue').closest('div').bind('click', function () {
			updateParent();
		});
		//返回按钮绑定事件
        ...
        <%--$("#sysLanguageCode").select2("val", ["+'${sysLookupHiberarchyDTO.sysLanguageCode}'+"]);--%>
        $("#sysLanguageCode").val('${sysLookupHiberarchyDTO.sysLanguageCode}').trigger("change");
	});
</script>
</body>
</html>
