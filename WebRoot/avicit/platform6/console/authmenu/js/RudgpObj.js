...
RudgpObj.prototype.init=function(){
	...
	$(_self._datagridId).jqGrid({
        ...
        gridComplete: function () {
    		var wvalue = document.documentElement.clientWidth - 250;
            $(_self._datagridId).jqGrid('setGridHeight',parent.document.documentElement.clientHeight-100);
            $(_self._datagridId).jqGrid('setGridWidth',wvalue);
        }
    });
	...
	$('#add'+this._type).bind('click',function(){
		...
		var _layer=layer.open({
			...
			content:"avicit/platform6/console/authmenu/SelectRole.jsp?orgId="+_self.orgId+"&appId="+_self.applictionId,
  			success: function(layero, index){
  				...
  				if('R'===_self._type){
  					...
  					var jqGridRowid=$('#jqGridRole').jqGrid("getGridParam","selarrrow");

					for(var i = 0; i < jqGridRowid.length; i++) {
						$('#jqGridRole').setSelection(jqGridRowid[i], false)
						i--
					}*/
					/*------------------------------新添加页面初始化----------------------------------------------*/
  			...
  			yes: function(index, layero){
				...
				if('R'===_self._type){
					var iframeWin = layero.find('iframe')[0].contentWindow;
					var rows = iframeWin.getRoleList();
					var ids = [];
					var l =rows.length;
					if(l > 0){
