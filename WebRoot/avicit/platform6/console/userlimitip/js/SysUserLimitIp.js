 ...
 */
function SysUserLimitIp(datagrid, url, searchDialogId, form, keyWordId,
 ...
 * 初始化操作
 */
SysUserLimitIp.prototype.init = function() {
	var _self = this;
	$(_self._datagridId).jqGrid({
		url : this.getUrl() + 'getSysUserLimitIpsByPage.json',
		mtype : 'POST',
		datatype : "json",
		...
		onCellSelect:function(rowId, status) {
			...
			if(rowData != '' && rowData.operFlag == '0'){
				$(this).setColProp("userLimitUserIdAlias",{editable:false});
			}else {
				$(this).setColProp("userLimitUserIdAlias",{editable:true});
			}
		},
	...
	//初始化校验字段
	_self.notnullFiled.push("userLimitUserId");
	_self.notnullFiledComment.push("受限人");
	_self.lengthValidFiled.push("userLimitUserId");
	_self.lengthValidFiledSize.push(32);
	_self.notnullFiled.push("limitTypeIpType");
	_self.notnullFiledComment.push("IP类型");
	_self.lengthValidFiled.push("limitTypeIpType");
	_self.lengthValidFiledSize.push(32);
	_self.notnullFiled.push("limitUserType");
	_self.notnullFiledComment.push("用户访问类型");
	_self.lengthValidFiled.push("limitUserType");
	_self.lengthValidFiledSize.push(32);
	_self.notnullFiled.push("userLimitIpFrom");
	_self.notnullFiledComment.push("开始IP");
	_self.lengthValidFiled.push("userLimitIpFrom");
	_self.lengthValidFiledSize.push(50);
	_self.lengthValidFiled.push("userLimitIpEnd");
	_self.lengthValidFiledSize.push(50);
};
/**
 * 添加页面
...
SysUserLimitIp.prototype.insert = function() {
 ...
 * @param 
 */
SysUserLimitIp.prototype.nullvalid = function(data, item, nullfiled,
		notnullFiledComment) {
	var msg = "";
 ...
 * @param 
 */
SysUserLimitIp.prototype.lengthvalid = function(data, item, lengthValidFiled,
		lengthValidFiledComment, lengthValidFiledSize) {
	var msg = "";
	$.each(data,function(i, dataitem) {
		if (dataitem[item] != ""
				&& dataitem[item].replace(/[^\x00-\xff]/g, "**").length > lengthValidFiledSize[$.inArray(item, lengthValidFiled)]) {
 ...
 * @param callback
 */
SysUserLimitIp.prototype.save = function() {
	var _self = this;
	$(this._datagridId).jqGrid('endEditCell');
	...
	//IP格式验证
	if(hasvalidate && data != null && data.length > 0){

		for ( var i = 0; i < data.length; i++) {
			if (data[i].userLimitIpFrom && !/^\d+\.\d+\.\d+\.\d+$/.test(data[i].userLimitIpFrom)) {
				layer.alert('开始IP格式不正确', {
					icon : 7,
					area : [ '400px', '' ], //宽高
					closeBtn : 0
				});
				hasvalidate = false;
				return false;
			}else if(data[i].userLimitIpEnd && !/^\d+\.\d+\.\d+\.\d+$/.test(data[i].userLimitIpEnd)){
				layer.alert('结束IP格式不正确', {
					icon : 7,
					area : [ '400px', '' ], //宽高
					closeBtn : 0
				});
			...
			if(data[i].limitTypeIpType == "1"){
				...
				if(data[i].userLimitIpEnd == null || data[i].userLimitIpEnd == ""){
					layer.alert('配置为IP段时，请输入结束IP！', {
						icon : 7,
						area : [ '400px', '' ], //宽高
						closeBtn : 0
					});
				...
				if(data[i].userLimitIpFrom == data[i].userLimitIpEnd){
					layer.alert('配置为IP段时，开始IP和结束IP不能相同！', {
						icon : 7,
						area : [ '400px', '' ], //宽高
						closeBtn : 0
					});
				...
				if(!reg.test(data[i].userLimitIpFrom)){
					layer.alert('开始ip必须以1开头！', {
					icon : 7,
					area : [ '400px', '' ], //宽高
					closeBtn : 0
				});
			...
			if(!reg.test(data[i].userLimitIpEnd)){
					layer.alert('结束ip必须以1开头！', {
					icon : 7,
					area : [ '400px', '' ], //宽高
					closeBtn : 0
				});
 ...
 * @param id
 */
SysUserLimitIp.prototype.detail = function(id) {
 ...
 * 删除
 */
SysUserLimitIp.prototype.del = function() {
	...
	if (l > 0) {
		layer.confirm('确定要删除该数据吗?', {
			...
			for (; l--;) {
				...
				if(rowData != '' && rowData.operFlag == '0' && delFlag != '0'){
					delFlag = '0';
				}
			}
			//是否可以被删除标识:20210303
			if(delFlag == '0'){
				layer.alert('默认用户不能删除！' , {
					icon : 7,
					area : [ '400px', '' ],
					closeBtn : 0
				},function (index) {
 ...
 * 重载数据
 */
SysUserLimitIp.prototype.reLoad = function() {
	var searchdata = {
		keyWord : null,
 ...
 */
SysUserLimitIp.prototype.openSearchForm = function(searchDiv) {
	var _self = this;
 ...
 * 高级查询
 */
SysUserLimitIp.prototype.searchData = function() {
	var searchdata = {
		keyWord : null,
		param : JSON.stringify(serializeObject($(this._formId)))
	}
 ...
 * 关键字查询
 */
SysUserLimitIp.prototype.searchByKeyWord = function() {
