function ConsoleRole(datausergrid,datarolegrid,url,searchD,form,keyWordId,keyUserWordId,searchNames,searchUserNames,datauserGridColModel,dataroleGridColModel){
...
ConsoleRole.prototype.insert = function(){
	...
	for (; l--;) {
		...
		/*
		 * if(cache['n'+row.roleCode]){ layer.alert('输入的角色编码已经存在，请重新输入！' , {
		 * icon: 0, title :'提示', area: ['400px', ''], //宽高 closeBtn: 0 } );
		 * return false; }
		 */
	...
	if(maxorderBy==""){
		$.ajax({
			url :  "platform/console/role/getMaxOrderby",
			type : 'post',
			dataType : 'json',
			async:false,
			success : function(r) {
...
ConsoleRole.prototype.loaduserByid = function(roleId){
	...
	$(this._datausergridId).jqGrid('setGridParam',{url: 'console/role/operation/togetSysUserByPostitionId/'+roleId,postData: searchdata}).trigger("reloadGrid");
};
...
ConsoleRole.prototype.importrole =function(){
	...
	importorgIndex = layer.open({
	    ...
	    skin: 'bs-modal', // bootstrap 风格皮肤 需加载skin
        maxmin: false, //开启最大化最小化按钮
	    content: 'platform/excelImportController/excelimportnew/importRole/xls'
	});   
};
...
ConsoleRole.prototype.insertUser =function(){
	var _self = this;
	var rows = $(this._datarolegridId).jqGrid('getGridParam','selarrrow');
	//解决类似 _self._roleId=new_row0 问题
	var rowIdExg = /new_row[0-9]+/;
	if(_self._roleId=="" || rowIdExg.test(_self._roleId)){
...
function exportClientData(){
	layer.confirm('确认要导出当前页角色吗?',
        ...
        if (index) {
            ...
            var url = "platform/console/role/exportClient";
...
function exportServerData(){
	layer.confirm('确认要导出所有角色吗?',
        ...
        if (index) {
            ...
            var url = "platform/console/role/exportServer";
