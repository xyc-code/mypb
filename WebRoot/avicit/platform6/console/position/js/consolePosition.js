...
ConsolePosition.prototype.searchUserByKeyWord = function() {
	...
	if(keyWord=="输入用户姓名或登录名"){
		keyWord='';
	}
	var searchdata = {};
	if(keyWord==''){
		 searchdata = {
			keyWord : JSON.stringify(statusParam),
			param : null
			}
	}else{
...
ConsolePosition.prototype.insert = function(){
	...
	for(;l--;){
		...
		/*
		 * if(cache[row.positionCode]){ layer.alert('岗位已经存在' , { title : '提示',
		 * icon: 0, area: ['400px', ''], //宽高 closeBtn: 0 } ); return false; }
		 */
	...
	if(maxorderBy==""){
		$.ajax({
			url :  "platform/console/position/getMaxOrderby",
			type : 'post',
			dataType : 'json',
			async:false,
			success : function(r) {
				if(r.flag=="success"){
					maxorderBy=r.orderBy;
				}
			}
		});
	}else{
		maxorderBy=maxorderBy+10;
	}
	var newRowId = newRowStart + newRowIndex;
	var parameters = {
		rowID : newRowId,
		initdata : {orderBy:maxorderBy},
		position :"first",
		useDefValues : true,
		useFormatter : true,
	...
	if(l > 0){
		layer.confirm('确认要删除选择的岗位吗?', 
			...
			function(index){
				for(;l--;){
                    ids.push(rows[l]);
                    var rowsdata =$(_self._datapositiongridId).jqGrid('getRowData',rows[l]);
...
ConsolePosition.prototype.loaduserByid = function(positionId){
	...
	$(this._datausergridId).jqGrid('setGridParam',{url: 'console/position/operation/togetSysUserByPostitionId/'+positionId,postData: searchdata}).trigger("reloadGrid");
};
...
ConsolePosition.prototype.insertUser =function(){
	var _self = this;
	var rows = $(this._datapositiongridId).jqGrid('getGridParam','selarrrow');
	var rowIdExg = /new_row[0-9]+/;
	if(_self._positionId=="" || rowIdExg.test(_self._positionId)){
		layer.alert('请选择添加用户的岗位数据！',{
			icon: 0,
			area: ['400px', ''],
			closeBtn: 0,
			title:'提示'
...
function selectUserBack(user){
	...
	$.ajax({
		url :  "platform/console/position/getUserInsertParam",
		type : 'post',
		data:	{ids:user.userids,deptIds:user.userdeptids},
		dataType : 'json',
		async:false,
		success : function(r) {
			if(r.flag=="success"){
	...
	if (map!="") {
		var useridsArray=user.userids.split(";");
		for (var i = 0; i < useridsArray.length; i++) {
			var userid = useridsArray[i];
			var userParam = map[userid];
			var parameters = {
				rowID : newRowStart_User+"_"+newRowIndex_User,
				initdata : {
					status: userParam["status"],
					name: userParam["name"],
					loginName: userParam["loginName"],
					secretLevel: userParam["secretLevel"],
					consoleType: userParam["consoleType"],
					deptId: userParam["deptId"],
					deptName: userParam["deptName"]
				},
			...
			$("#jqGrid").jqGrid('setSelection',newRowStart_User+"_"+newRowIndex_User);
			newRowIndex_User++;
		}
	}
}
...
ConsolePosition.prototype.saveUser = function () {
	var _self = this;
	var rows = $(this._datapositiongridId).jqGrid('getGridParam','selarrrow');
	if(_self._positionId==""){
		layer.alert('请选择保存用户的岗位数据！',{
			icon: 0,
			area: ['400px', ''],
			closeBtn: 0,
			title:'提示'
	...
	var changedData = $(this._datausergridId).jqGrid('getChangedCells');//修改数据
	var ids = $(this._datausergridId).jqGrid('getGridParam','selarrrow');//新增的选中数据行id
	var insertedData = new Map();
	var data = [];
	for(var i = 0; i < ids.length; i++){
		var rowData = $(this._datausergridId).jqGrid('getRowData',ids[i]);
		insertedData.set(ids[i],rowData);
	}
	if(changedData && changedData.length > 0){
		for(var i = 0; i < changedData.length; i++){
			if(!insertedData.has(changedData[i].id)){
				data.push(changedData[i]);
			}
		}
	}
	insertedData.forEach(function(value, key) {
		data.push(value);
	});
	if (data && data.length > 0) {
		for (var i = 0; i < data.length; i++) {
		...
		$.ajax({
			url:_self.getUrl()+'/operation/savePositionUser',
			data:	{positionId:_self._positionId, data:JSON.stringify(data)},
			type : 'post',
			dataType : 'json',
			success : function(r){
				if (r.flag == "success") {
					...
					_self.loaduserByid(_self._positionId);
				}else if(r.flag == "emptyDept"){
					/*_self.loaduserByid(_self._positionId);*/
					layer.alert('添加失败！部门数据不能为空！',{
						icon: 2,
						area: ['400px', ''],
						closeBtn: 0
					});
				} else if(r.flag == "chiefData"){
					/*_self.loaduserByid(_self._positionId);*/
					layer.alert('添加失败！已存在主部门主岗位数据！！',{
						icon: 2,
						area: ['400px', ''],
						closeBtn: 0
					});
				} else if(r.flag == "existData"){
					/*_self.loaduserByid(_self._positionId);*/
					layer.alert('添加失败！['+r.userName+']在部门['+r.userDept + ']下已存在该岗位！！',{
						icon: 2,
						area: ['400px', ''],
						closeBtn: 0
					});
		...
		/*var rowIds = $(_self._datausergridId).jqGrid('getDataIDs');
		var flag = true;
		if (rowIds && rowIds.length > 0) {
			for (var i = 0; i < rowIds.length; i++) {
				if(rowIds[i].indexOf(newRowStart_User) > -1){
					var rowdata = $	(_self._datausergridId).jqGrid('getRowData',rowIds[i]);
					if(rowdata.deptName == null || rowdata.deptName == undefined || rowdata.deptName == ''){
						layer.alert('添加失败！部门数据不能为空！',{
							icon: 2,
							area: ['400px', ''],
							closeBtn: 0
...
ConsolePosition.prototype.deleteUser =function(){
		...
		layer.confirm('确认要删除选择的用户吗?',
			...
			function(index){
				for(var i =rows.length-1;i>=0;i--){
					if (rows[i].indexOf(newRowStart_User) > -1) {
						$(_self._datausergridId).jqGrid('delRowData',rows[i]);
					}else{
						var rowdata = $(_self._datausergridId).jqGrid('getRowData',rows[i]);
						data.push(rowdata);
					}
				}
				$.ajax({
					url:_self.getUrl()+'/operation/deletePositionUser',
					data:	{data:JSON.stringify(data),positionId:_self._positionId},
					type : 'post',
					dataType : 'json',
					success : function(r){
						if (r.flag == "success") {
							...
							_self.loaduserByid(_self._positionId);
						}else if(r.flag == "chiefDeptData"){
							layer.alert('删除失败！不能删除主部门数据！',{
								icon: 2,
								area: ['400px', ''],
								closeBtn: 0
							});
