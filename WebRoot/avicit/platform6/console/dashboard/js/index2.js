...
var drowFirstMenu = function(array, $parent) {
    ...
    $.each(array, function(i, menu) {
        ...
        $('<span class="taburl" id="' + menu.id + '" openType="' + menu.openType + '" rel="' + menu.url + '" title="' +  _tempDiv.html(menu.name).text() + '">' + _newFirstMenuName + caret + '</span>').appendTo($li);
...
var drowSecendMenu = function(array, $parent) {
    ...
    $.each(array, function(i, menu) {
        ...
        var $menu = $('<div class="subtitle taburl" id="' + menu.id + '" openType="' + menu.openType + '" rel="' + menu.url + '" title="' +_tempDiv.html(menu.name).text() + '"><i class="'+menuIcon+'"></i>' + _newSecMenuName + '</div>');
        $menu.appendTo($div);
        if (menu.children.length > 0) {
...
var drowThirdMenu = function(array, $parent) {
    ...
    $.each(array, function(i, menu) {
        ...
        //if(i%2==0){
        $('<li class="afont taburl" id="' + menu.id + '" openType="' + menu.openType + '" rel="' + menu.url + '" title="' +  _tempDiv.html(menu.name).text() + '">' + _tempDiv.html(_newThirdMenuName).text() + '</li>').appendTo($ul);
        //}else{
...
//            	perfectScroll($(this).parent());
    ...
    if (!update && update !== 'update') {
        //打开标签
        //此处加undelegate 为了防止事件重复被触发
        $('.dropdown').undelegate(".taburl" , 'click').delegate(".taburl", 'click', function(e) {
            e.preventDefault();
             e.stopPropagation();
            ...
            if(openType == '_blank' && url && url != "null"){
               ...
               for(var i=0;i<allTabs.length;i++){
                ...
                if (!exist && !flag && url != undefined && url != "" && url && url != "null") {
                    $('#tabs-panel').tabs('add', {
                        title: title,
                        content: '<iframe scrolling="auto" frameborder="0"  src="' + url + '" style="width:100%;height:100%;"></iframe>',
                        closable: true
                    });
                } else if(exist&& !flag && url != undefined && url != "" && url && url != "null"){
                     $('#tabs-panel').tabs('add', {
                        title: title,
                        content: '<iframe scrolling="auto" frameborder="0"  src="' + url + '" style="width:100%;height:100%;"></iframe>',
                        closable: true
