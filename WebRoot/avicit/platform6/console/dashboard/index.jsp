...
<%
	...
	Integer deadLine = (Integer) session.getAttribute("deadLine");
	...
	boolean flag = false;
	SysUserRoleAPI sysUserRoleAPI = CacheSpringFactory.getInstance().getBean(SysUserRoleAPI.class);
	List<SysRole> sysRoleListBySysUserIdNoAppId = sysUserRoleAPI.getSysRoleListBySysUserIdNoAppId(userId);
	for (SysRole sysRole: sysRoleListBySysUserIdNoAppId) {
		if (PlatformConstant.FixedRole.system_manager.getCode().equals(sysRole.getRoleCode()) ||
				PlatformConstant.FixedRole.platform_manager.getCode().equals(sysRole.getRoleCode())) {
			flag = true;
			break;
		}
	}
...
<head>
	...
	<![endif]-->
	<script>
		function checkAuth(deadLine) {
			<%session.removeAttribute("deadLine");%>
			if (deadLine != null && deadLine <= 15 && <%=flag%>) {
				layer.alert("授权还有" + deadLine + "天过期，请及时更换授权文件！", {
					icon: 7,
					area: ['400px', ''],
					title:'提示',
					closeBtn: 0
...
<body onload="checkAuth(<%=deadLine%>)">
	<div class="mainBody">
		<!-- 头部工具栏Start -->
		<div class="header">
					    ...
					    <a href="javascript:void(0);">
						   <li class="icon icon-bell-fill" title="通知"><span id="unreadMessage" style="display: none"></span>
							<ul class="subMenu info auto" id="personMessage">
								...
								<ul class="avic-smenu xl" id="personalMessageUl" style="overflow-y:auto;height:240px;padding:0;"></ul>
								<div class="btns">
	...
	<script type="text/javascript" >
		 ...
		 $(document).ready(function () {
			 ...
			 //切换头像后不能及时展示问题处理
			 $('.userNameLi').on('mouseenter',function(e){
				 var imagSrc = baseUrl+"platform/console/userupload/headerphoto?sysUserId=<%=userId%>" + "&o=" + Math.random();
				 $("#sysUserHeadPhotoImg").prop('src',imagSrc);
			 });
