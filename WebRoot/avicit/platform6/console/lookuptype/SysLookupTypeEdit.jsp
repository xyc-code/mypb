		...
		<form id='form'>
			...
			<table class="form_commonTable">
				<tr>
					...
					<td width="39%"><input class="form-control input-sm" type="text" name="lookupType" id="lookupType" value='<c:out  value='${sysLookupTypeDTO.lookupType}'/>'/></td>
					<th width="10%"><label for="lookupTypeName">代码名称:</label></th>
					<td width="39%"><input class="form-control input-sm" type="text" name="lookupTypeName" id="lookupTypeName" value='<c:out  value='${sysLookupTypeDTO.lookupTypeName}'/>'/></td>
				</tr>
				<tr>
					...
					<td width="39%"><pt6:h5radio css_class="radio-inline" name="validFlag" 
						title="是否有效" canUse="0" lookupCode="PLATFORM_VALID_FLAG" defaultValue='${sysLookupTypeDTO.validFlag}' /></td>
					<th width="10%"><label for="usageModifier">使用级别</label></th>
					<td width="39%"><pt6:h5select css_class="form-control input-sm" name="usageModifier" id="usageModifier" title="部门类型" isNull="true" defaultValue='${sysLookupTypeDTO.usageModifier}' lookupCode="PLATFORM_USAGE_MODIFIER" /></td>
				</tr>
				<tr>
					...
					<th width="10%"><label for="lookupDesc">代码描述:</label></th>
					<td width="39%"><input class="form-control input-sm" type="text" name="lookupTypeDesc" id="lookupTypeDesc" value='<c:out  value='${sysLookupTypeDTO.lookupTypeDesc}'/>'/></td>
				</tr>
			</table>
		</form>
	</div>
	...
	<script type="text/javascript">
		...
		function saveForm(){
	        ...
	        if (!isValidate.checkForm()) {
			  ...
			  parent.sysLookupType.save($('#form'),"eidt");
		}
		...
		document.ready = function () {
            ...
            //编码唯一性校验
            jQuery.validator.addMethod("validateLookupType", function(value, element, param) {
                var mark = false;
                var data = {
                    'lookupCode' : value,
                    'id' : $('input[name=id]').val()
                };
                var validateUrl = parent.sysLookupType.getUrl() + "validateLookupType";
                $.ajax({
                    url : validateUrl,
