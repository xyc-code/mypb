function SysLookup(datagrid, url, formSub, dataGridColModel, searchDialogSub, pid, searchSubNames, demoSubUser_KeyWord) {
...
SysLookup.prototype.insert = function() {
	...
	if (rowData.length > 0) {
		...
		for (var i = 0; i < rowData.length; i++) {
			orderArr[i] = rowData[i].displayOrder;
		}
		max = getMaxMin(orderArr, "max") + 10;
	} else {
		max = 10;
	}
 ...
 * @param 
 */
SysLookup.prototype.nullvalid = function(data, item, nullfiled, notnullFiledComment) {
	var msg = "";
 ...
 * @param 
 */
SysLookup.prototype.lengthvalid = function(data, item, lengthValidFiled, lengthValidFiledComment, lengthValidFiledSize) {
	var msg = "";
	$.each(data, function(i, dataitem) {
...
SysLookup.prototype.del = function() {
	...
	layer.confirm('确认要删除选择的数据吗?', {
		...
		// 删除新行
		if (idsNew.length > 0) {
			for (var i = 0; i < idsNew.length; i++) {
				$(_self._datagridId).jqGrid('delRowData', idsNew[i]);
			}
		}
		// 删除旧行
		if (ids.length > 0) {
