...
<html>
<head>
<!-- ControllerPath = "demo/syslanguage/sysLanguageController/toSysLanguageManage" -->
<title>管理</title>
	...
	<div id="tableToolbar" class="toolbar">
		<div class="toolbar-left">
			<sec:accesscontrollist hasPermission="3"
				domainObject="formdialog_sysLanguage_button_add" permissionDes="添加">
				<a id="sysLanguage_insert" href="javascript:void(0)"
					class="btn btn-primary form-tool-btn btn-sm btn-point" role="button"
					title="添加"><i class="icon icon-add"></i> 添加</a>
			</sec:accesscontrollist>
			<sec:accesscontrollist hasPermission="3"
				domainObject="formdialog_sysLanguage_button_del" permissionDes="删除">
				<a id="sysLanguage_del" href="javascript:void(0)"
					class="btn btn-primary form-tool-btn btn-sm" role="button"
					title="删除"><i class="icon icon-delete"></i> 删除</a>
			</sec:accesscontrollist>
			<sec:accesscontrollist hasPermission="3"
				domainObject="formdialog_sysLanguage_button_save" permissionDes="保存">
				<a id="sysLanguage_save" href="javascript:void(0)"
					class="btn btn-primary form-tool-btn btn-sm" role="button"
					title="保存"><i class="icon icon-save"></i> 保存</a>
			</sec:accesscontrollist>
		</div>
	</div>
	<table id="sysLanguagejqGrid"></table>
...
<script src="avicit/platform6/console/language/js/language.js"
...
<script type="text/javascript">
	...
	$(document).ready(
			function() {
				...
				sysLanguage = new SysLanguage('sysLanguagejqGrid', '${url}',
						'searchDialog', 'form', '',
						'', dataGridColModel);
				//添加按钮绑定事件
				$('#sysLanguage_insert').bind('click', function() {
					sysLanguage.insert();
				});
				//删除按钮绑定事件
				$('#sysLanguage_del').bind('click', function() {
					sysLanguage.del();
				});
				//保存按钮绑定事件
				$('#sysLanguage_save').bind('click', function() {
					sysLanguage.save();
				});
				//查询按钮绑定事件
				$('#sysLanguage_searchPart').bind('click', function() {
					sysLanguage.searchByKeyWord();
				});
				//打开高级查询框
				$('#sysLanguage_searchAll').bind('click', function() {
					sysLanguage.openSearchForm(this);
				});
				//回车键查询
				$('#sysLanguage_keyWord').on('keydown', function(e) {
					if (e.keyCode == 13) {
						sysLanguage.searchByKeyWord();
					}
				});
