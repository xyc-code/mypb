...
function formatOrg(cellvalue, options, rowObject) {
	...
	$(document).ready(
					function() {
						...
						deptlist = new DeptList('deptlistjqGrid', 'platform/console/user/getMapDept?id='+'<c:out value="${id}"/>',dataGridColModel,'<c:out value="${id}"/>');
						//添加按钮绑定事件
...
function selectDeptBack(dept){
	var id = $("#deptlistjqGrid").jqGrid('getGridParam','selrow');
	var orgId = (dept.orgIdentitys == undefined) ? '' : dept.orgIdentitys.split(";")[0];
	$('#deptlistjqGrid').jqGrid('setCell', id,'orgIdentity',orgId);
	$('#deptlistjqGrid').jqGrid('setCell', id,'orgIdentityId',orgId);
}
</script>
</html>