    ...
    $(document)
        .ready(
            function () {
                ...
                var newPath = null;
                portalFlag = '<c:out value="${param.portalFlag}"/>';
                if(portalFlag == "1"){//前台用户
                    newPath = 'platform/ShowPermissController/portalfindUserResources.json?userId=' + '<c:out value="${param.userId}"/>';
                }else{//后台用户
                    newPath = 'platform/ShowPermissController/newfindUserResources.json?userId=' +  '<c:out value="${param.userId}"/>';
                }

                $('#jqGrid')
                    .jqGrid(
                        {
                            //url: 'platform/ShowPermissController/newfindUserResources.json?userId=' + '<c:out value="${param.userId}"/>',
                            url: newPath,
                            mtype: 'POST',
                            datatype: "json",
