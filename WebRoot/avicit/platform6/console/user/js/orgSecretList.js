...
function OrgSecretList(datagrid, url, dataGridColModel, userId) {
    ...
    this._doc = document;
    this.notnullFiled = ["secretLevelName","orgName"];//非空字段
    this.notnullFiledComment = ["密级","多组织应用"]; //非空字段注释
    //除时间,数字等类型长度校验字段
    ...
    _self._$grid = $(_self._datagridId).jqGrid({
        ...
        loadComplete:function(){
			...
			for(var i=0;i<rows.length;i++){
			    var row=rows[i];
			     if(row.id.indexOf("new_row") == -1){
                    $(_self._datagridId).jqGrid('setCell', row.id, "orgName", '', 'not-editable-cell');
                }
            }
		},
    });
        ...
        layer.confirm('确定要删除该数据吗?', {icon: 2, title: "请确认：", area: ['400px', '']}, function (index) {
            for (; l--;) {
                if(rows[l].indexOf("new_row")!=-1){
                    $(this._datagridId).jqGrid('delRowData', rows[l]);
                }else{
                    ids.push(rows[l]);
                }
            }
