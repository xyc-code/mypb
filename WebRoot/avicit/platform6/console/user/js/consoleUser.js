...
ConsoleUser.prototype.init=function(){
	...
	$(_self._datagridId).jqGrid({
		...
		autowidth: true,
        shrinkToFit: true,
		hasColSet:true,  //列显隐按钮是否显示
		hasTabExport:false,  //是否显示导出按钮
...
ConsoleUser.prototype.del=function(){
	...
	if(l > 0){
       ...
       for(var i = 0 ; i < rows.length ; i++){
           ...
           if(rowData.id == "1" || rowData.id == "8a58a6b44c275dce014c275dced90000" || rowData.id == "8a58ab4d4c25bd67014c25bdc1980001" || rowData.id == "8a58ab4d4c25bd67014c25beb8780002"){
			   parent.layer.alert('选择的用户【' + rowData.name + '】数据是默认用户，不允许删除，请重新选择！', {
					   icon: 7,
					   title: '提示',
					   area: ['400px', ''], //宽高
					   closeBtn: 0
...
ConsoleUser.prototype.searchByKeyWord =function(){
	...
	if(keyWord==''){
		 searchdata = {
			keyWord : null,
			param : JSON.stringify(treeNode),
		    'searchdatapara' : JSON.stringify(statusParam)
		}
	}else{
...
//批量调整
/*ConsoleUser.prototype.adjustment =function(){
    H5adjustmentSelect({
        selectModel : 'multi',
        idFiled : 'user',
        textFiled : 'userfile',
        hidTab:["user"],
        callBack:adjustmentRole,
        viewScope : 'currentOrg',
        getAll:'true',
		userSelectUrl : "avicit/platform6/console/user/consoleUserAdjustment.jsp"
    });
};*/
var adjustTab;
ConsoleUser.prototype.roleAdjustment =function(){
	adjustTab="role";
    H5adjustmentSelect({
        selectModel : 'multi',
		title:"批量调整角色",
        ...
        viewScope : 'currentOrg',
        getAll:'true',
		userSelectUrl : "avicit/platform6/console/user/consoleUserAdjustment.jsp?adjustTab="+adjustTab
    });
};
ConsoleUser.prototype.deptAdjustment =function(){
	adjustTab="dept";
    H5adjustmentSelect({
        selectModel : 'multi',
		title:"批量调整部门",
...
function H5adjustmentSelect(option) {
    // 用户参数
    ...
    /*this.userSelectUrl = "avicit/platform6/console/user/consoleUserAdjustment.jsp";*/
	this.userSelectUrl = option.userSelectUrl;
...
function init(){
    ...
    var selectDialog = openDialog({
        ...
        submit : function(index, layer) {
            ...
            //当前tabID
            /*var tab = iframeWin.document.getElementsByClassName("tab-pane active");
            var tabId = tab[0].id;*/
            var copyUserId;
            var delMethodRole;
            var copyUserDeptId;
			if(adjustTab=="role"){//调整角色
				//需要复制角色的用户id
				copyUserId = iframeWin.document.getElementById("userId").value;
				//是否删除角色标识
				delMethodRole = iframeWin.document.getElementById("defMethodRole").checked;
			}else{//调整部门
				 //需要复制部门id
            	copyUserDeptId = iframeWin.document.getElementById("userDeptId").value;
			}
            ...
            //判断是否选择复制的用户
			if(adjustTab=="role") {//调整角色tab
				if (copyUserId == '' || typeof (copyUserId) == 'undefined' || copyUserId == null) {
					if (copyUserDeptId == '' || typeof (copyUserDeptId) == 'undefined' || copyUserDeptId == null) {
						top.layer.alert('请选择模版用户！', {
								title: '提示',
								icon: 0,
								area: ['400px', ''], //宽高
								closeBtn: 0
            ...
            //判断是否选择复制的用户
            if(user.userids == '' || typeof(user.userids) == 'undefined' || user.userids == null){
                top.layer.alert('请选择目标用户！', {
                        title : '提示',
                        icon : 0,
                        area: ['400px', ''], //宽高
                        closeBtn: 0
            ...
            if(_self.callBack!=null && _self.callBack!='undefined'){
                if(typeof(_self.callBack) === 'function'){
                    _self.callBack(user,copyUserId,delMethodRole,copyUserDeptId);
                }
            }
        },
            ...
            top.layer.open({
                ...
                yes: function(index, layero){
                    ...
                    if(option.submit!=null){
                        flag = option.submit(index, layero);
                    }
...
//user:目标用户   copyUserId：模板用户  delMethodRole：删除原角色标识
function adjustmentRole(user,copyUserId,delMethodRole,copyUserDeptId){
    user['copyUserId'] = copyUserId;
	user['copyUserDeptId'] = copyUserDeptId;
    user['delMethodRole'] = delMethodRole;
    console.log(JSON.stringify(user));
    var _self = this;
    var url = 'console/user/operation/AddAdjustment';
	//判断是否选择需要调整的用户
	if(user.userids == '' || typeof(user.userids) == 'undefined' || user.userids == null){
		layer.alert('请选择目标用户！', {
				title : '提示',
    ...
    if(copyUserId == '' || typeof(copyUserId) == 'undefined' || copyUserId == null){
		...
		if(copyUserDeptId == '' || typeof(copyUserDeptId) == 'undefined' || copyUserDeptId == null){
			...
			return false;
			 ...
			 url = "console/user/operation/AddAdjustmentDept";
		}
    }
    user['copyUserId'] = copyUserId;
	user['copyUserDeptId'] = copyUserDeptId;
		...
		function(index){
			$.ajax({
				...
				success : function(r){
					...
					}else if(r.flag == "uninit"){
                        parent.layer.alert('没有要初始化的用户！',{
                                icon: 1,
                                area: ['400px', ''], //宽高
                                closeBtn: 0
                            }
