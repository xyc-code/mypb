                ...
                <tr>
                    ...
                    <td width="23%">
                        ...
                        <select class="form-control input-sm" title="" name="filter-like-SYS_LOOKUP_NAME"
                            ...
                            <option value="编码">编码管理</option>
                            <option value="系统参数">系统参数</option>
                            <option value="通用代码">通用代码</option>
                            <option value="多维通用代码">多维通用代码</option>
                            <option value="表单事件">表单事件</option>
                            <option value="流程事件">流程事件</option>
                    ...
                    <td width="33%" style="text-align: right">
                        ...
                        <a href="javascript:void(0)" class="btn btn-grey form-tool-btn btn-sm" role="button"
                           onclick="clearData();" title="重置" id="resetFrom">重置</a>
                    </td>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        var importType = "${fileName}";
        if(importType == "importNewOrgInfo"){
            sysLookupName="组织";
        }else if(importType == "importDeptInfo"){
            sysLookupName="部门";
        }else if(importType == "importEmployeeInfo"){
            sysLookupName="用户";
        }else if(importType == "importRole"){
            sysLookupName="角色";
        }else if(importType == "importPosition"){
            sysLookupName="岗位";
        }else if(importType == "importGroup"){//群组导入功能暂未添加
            sysLookupName="群组";
        }else if(importType == "importMenu"){
            sysLookupName="菜单";
        }else if(importType == "importAutoCode"){
            sysLookupName="编码";
        }else if(importType == "importProfile"){
            sysLookupName="系统参数";
        }else if(importType == "importLookupType"){
            sysLookupName="通用代码";
        }else if(importType == "importLookupHiberarchy"){
            sysLookupName="多维通用代码";
        }else if(importType == "importEformClass"){
            sysLookupName="表单事件";
        }else if(importType == "importEvent"){
            sysLookupName="流程事件";
        }else {
            sysLookupName = "";
        }
        ...
        var dataGridColModel = [
            ...
            , {label: '导入类型', name: 'sysLookupName', width: 100, align: 'center',sortable:false}
            , {label: '成功数', name: 'successCount', width: 200, align: 'center',sortable:false}
            , {label: '失败数', name: 'failCount', width: 200, align: 'center',sortable:false}
            , {label: '导入人', name: 'opetatorPerson', width: 200, align: 'center',sortable:false}
            , {
                label: '导入时间',
                name: 'creationDate',
                width: 200,
            ...
            , {label: '下载', name: 'fileUrl', width: 200, align: 'center', formatter: formatURL,sortable:false}
        ...
        $('#jqGrid').jqGrid({
            ...
            mtype: 'POST',
            datatype: "json",
            postData:{'param':JSON.stringify({'filter-like-SYS_LOOKUP_NAME': sysLookupName})},
            ...
            pager: "#jqGridPager",
            loadComplete: initSelectElement()
        });
    ...
    function searchData() {
        ...
        if ($('#syslookUpName').val() == "0") {
            searchFrom = JSON.stringify({
                /*'filter-like-SYS_LOOKUP_NAME': sysLookupName,*/
    ...
    function initSelectElement() {
        var selectElement = $("#syslookUpName")[0];
        var selectChildCount = selectElement.length;
        for (var i = 0; i < selectChildCount; i++) {
            var selectValue = selectElement[i].value;
            if (selectValue == sysLookupName) {
