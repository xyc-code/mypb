 ...
 *            表格列属性Array
 */
function ResteasyOrg(datagrid, url, searchDialogId, form, keyWordId,
	...
	this.getUrl = function() {
		return 'platform/platfrom6/newrestmanage/controller/resteasyOrgController/operation/';
	}
 ...
 * 初始化操作
 */
ResteasyOrg.prototype.init = function() {
	var _self = this;
	$(_self._datagridId)
			.jqGrid(
					{
						url : 'platform/platfrom6/newrestmanage/controller/resteasyOrgController/operation/getResteasyOrgsByPage',
						mtype : 'POST',
						datatype : "json",
	...
	// 初始化校验字段
	_self.lengthValidFiled.push("orgName");
	_self.lengthValidFiledComment.push("单位名称");
...
ResteasyOrg.prototype.insert = function() {
 ...
 * @param
 */
ResteasyOrg.prototype.specialvalid = function(data, item, lengthValidFiled,
		lengthValidFiledComment) {
	var msg = "";
	$.each(data, function(i, dataitem) {
		if (!/^[\u4e00-\u9fa5a-zA-Z0-9_^s*|s*$]+$/.test(dataitem[item])) {
			temp = false;
			msg = lengthValidFiledComment[$.inArray(item, lengthValidFiled)] + " 只允许输入中英文 数字 下划线！";
		}
	})
	return msg;
}
 ...
 * @param
 */
ResteasyOrg.prototype.nullvalid = function(data, item, nullfiled,
		notnullFiledComment) {
	var msg = "";
	$.each(data, function(i, dataitem) {
		if (dataitem[item] == "") {
 ...
 * @param
 */
ResteasyOrg.prototype.lengthvalid = function(data, item, lengthValidFiled,
 ...
 * @param callback
 */
ResteasyOrg.prototype.save = function() {
	var _self = this;
	$(this._datagridId).jqGrid('endEditCell');
	var data = $(this._datagridId).jqGrid('getChangedCells');
	var hasvalidate = true;
	$.each(_self.lengthValidFiled, function(i, item) {
		var msg = _self.specialvalid(data, item, _self.lengthValidFiled,
				_self.lengthValidFiledComment);
		if (msg && msg.length > 0) {
			layer.alert(msg, {
				icon : 7,
				area : [ '400px', '' ], // 宽高
 ...
 * @param id
 */
ResteasyOrg.prototype.detail = function(id) {
 ...
 * 删除
 */
ResteasyOrg.prototype.del = function() {
			...
			$.ajax({
				...
				success : function(r) {
					if (r.success) {
						...
						_self.reLoad();
						layer.alert('删除失败！' + r.msg + " 已关联系统注册管理", {
							icon : 7,
							area : [ '400px', '' ],
							closeBtn : 0,
							title : "提示",
 ...
 * 重载数据
 */
ResteasyOrg.prototype.reLoad = function() {
	var searchdata = {
		keyWord : null,
 ...
 *            高级查询框高度
 */
ResteasyOrg.prototype.openSearchForm = function(searchDiv) {
	var _self = this;
 ...
 * 高级查询
 */
ResteasyOrg.prototype.searchData = function() {
	var searchdata = {
		keyWord : null,
		param : JSON.stringify(serializeObject($(this._formId)))
	}
 ...
 * 关键字查询
 */
ResteasyOrg.prototype.searchByKeyWord = function() {
