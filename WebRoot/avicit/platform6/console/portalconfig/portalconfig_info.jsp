...
<div class="easyui-layout" fit="true">
	<div data-options="region:'north'" style="height:280px;overflow: hidden;">
			<div class="toolbar-right" style="padding:4px 0 4px 4px;">
					...
					<a id="delete" href="javascript:void(0)" class="btn btn-primary form-tool-btn btn-sm btn-point" role="button" title="重置"><i class="icon icon-delete"></i> 重置</a>
			</div>
...
jQuery("#roleList").jqGrid({
    ...
    loadComplete: function () {
        ...
        if (navigator.userAgent.indexOf("MSIE 8.0") > -1
            ...
            $('.ui-jqgrid-bdiv').css("height",$(window).height()-310);
        } else if(navigator.userAgent.indexOf("Firefox") > 0){//Firefox
            $('.ui-jqgrid-bdiv').css("height",$(window).height()-308);
        }else {//谷歌
            $('.ui-jqgrid-bdiv').css("height",$(window).height()-316);
        }
...
$('#delete').bind('click', function() {
	...
	$.ajax({
		url:'platform/console/portal/toDeletePortalConfig',
		data : {
			typeKey : typeKey,
			skinsType : $("input[name='skinRadio']:checked").val(),
			roleListIds : JSON.stringify(ids),
