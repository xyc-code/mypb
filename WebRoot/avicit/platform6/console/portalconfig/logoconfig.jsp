...
<div class="easyui-layout" fit="true">
	...
	<form id='form' method="post" action="platform/console/logo/operation/save" enctype="multipart/form-data">
		...
		<input type="hidden" name="isInit" id="isInit" value="2"></input>
		<input type="hidden" name="isReset" id="isReset" value="1"></input>
		<div class="form_box">
			<div class="form_left">
				<div class="from_box">
							...
							<tr>
								...
								<td>
									<div class="img_box" id="imgBox">
										<c:if test="${logoConfigDTO.corLogo != null && fn:length(logoConfigDTO.corLogo) > 0 }">
											<img id="preview_img" src="platform/console/logo/operation/getLogo" style="width: 80px;height: 80px;" />
										</c:if>
										<%--<img id="preview_img" src="platform/console/logo/operation/getLogo" style="width: 80px;height: 80px;" />--%>
										<input id="logo" name="logo" type="file" accept="image/x-png,image/gif,image/jpeg,image/bmp" style="width:104px;height:104px;filter:alpha(opacity=0);opacity:0;z-index:10;cursor: pointer;" onchange='fileInputOnChange(this);' />
										<span>为了保证显示效果，请将图片背景设定为透明</span>
									</div>
								</td>
								<td align="left" valign="bottom">
									<a id="delete" href="javascript:void(0)" class="btn btn-primary form-tool-btn btn-sm" role="button" title="清除"><i class="icon icon-delete"></i> 清除</a>
								</td>
							</tr>
							<tr>
								<th>
...
// 			$("#preview_img").attr("src","data:image/png;base64," + logoConfigDTO.corLogo);
	...
	$(document).ready(function(){
		...
		$("#reset").bind('click', function() {
			...
			init();
			//解决重置后标题大小不重置问题
			if(initData.oneFontSize != "" && initData.oneFontSize != undefined){
				oneFontSizeJR.setValue(initData.oneFontSize);
			}
			if(initData.secondFontSize != "" && initData.secondFontSize != undefined){
				secondFontSizeJR.setValue(initData.secondFontSize);
			}
			if(initData.extFontSize != "" && initData.extFontSize != undefined){
				extFontSizeJR.setValue(initData.extFontSize);
			}
			if (initData.oneFontBold == "Y") {
				$('#oneFontBold').prop('checked', true)
			} else {
				$('#oneFontBold').removeProp('checked')
			}
			if (initData.oneFontItalic == "Y") {
				$('#oneFontItalic').prop('checked', true)
			} else {
				$('#oneFontItalic').removeProp('checked')
			}
			if (initData.secondFontBold == "Y") {
				$('#secondFontBold').prop('checked', true)
			} else {
				$('#secondFontBold').removeProp('checked')
			}
			if (initData.secondFontItalic == "Y") {
				$('#secondFontItalic').prop('checked', true)
			} else {
				$('#secondFontItalic').removeProp('checked')
			}
			if (initData.extFontBold == "Y") {
				$('#extFontBold').prop('checked', true)
			} else {
				$('#extFontBold').removeProp('checked')
			}
			if (initData.extFontItalic == "Y") {
				$('#extFontItalic').prop('checked', true)
			} else {
				$('#extFontItalic').removeProp('checked')
			}
			if(initData.oneFont != "" && initData.oneFont != undefined){
				$("#oneFont").val(initData.oneFont).select2();
			}
			if(initData.secondFont != "" && initData.secondFont != undefined){
				$("#secondFont").val(initData.secondFont).select2();
			}
			if(initData.extFont != "" && initData.extFont != undefined){
				$("#extFont").val(initData.extFont).select2();
			}
			if (initData.oneFontColor != "" && initData.oneFontColor != undefined) {
				$.iColor($('#oneFontColorSelect'), {
					start: function ($elem) {
						$elem.css('background', initData.oneFontColor);
						$('#oneFontColor').val(initData.oneFontColor);
					}
			...
			if (initData.secondFontColor != "" && initData.secondFontColor != undefined) {
				$.iColor($('#secondFontColorSelect'), {
					start: function ($elem) {
						$elem.css('background', initData.secondFontColor);
						$('#secondFontColor').val(initData.secondFontColor);
					}
...
// 			}
            //直接base64赋值在IE8下不生效
			$.ajax({
				url :  "platform/console/logo/operation/getLogo",
				type : 'get',
				async : false,
				success : function(r) {
					if(r && r.length > 0){
		...
		$.each($("ul.dropdown-menu>li>a"),function(index,a){
			$(a).bind('click', function() {
				var _self = this;
				$("#isTemp").val(2);
				//此处是为了判断清除图标时 IE 8下无法清除的情况,0是已经清除，1为未清除
				//var arrs=$("#logo").val().split('\\');
				var isClear = "1";
				if(!$("#preview_img").length>0){
					isClear = "0";
				}
				var imgSrc = $("#preview_img").attr("src");
				/*if(arrs == null || arrs ==""){
					isClear = "0";
				}*/
				var options = {
					...
					data:{"isClear":isClear,"imgSrc":imgSrc},
					//dataType: 'json',
					//contentType: "application/json; charset=utf-8",
					success: function (result) {
						var data = JSON.stringify(serializeObject($('#form')));
						window.open('platform/console/logo/operation/toPortalTemp?themeTypeKey='+_self.id+'&bgColor=blue&data='+encodeURIComponent(data)+'&isClear='+isClear,'_blank');
					}/* ,
				        error: function(e){
				        	debugger;
				        	alert(2);
		...
		$('#delete').bind('click', function() {
			layer.confirm('确定要清除logo吗？', {
						...
						var logoclone = logo.cloneNode();
						logoclone.value=null;
						imgBox.removeChild(logo);
						imgBox.appendChild(logoclone);
	...
	//加载图片显示
	function fileInputOnChange(obj){
		$("#preview_img").remove();
		$("#logo").before($("<img id='preview_img' style='width: 80px;height: 80px;'/>"));
		/*var acceptFileType = obj.accept;*/
		var upLoadFilePath = obj.value;
		if(upLoadFilePath != null){
			var upLoadFileName = upLoadFilePath.substring(upLoadFilePath.lastIndexOf("\\")+1);
			var upLoadFileType = upLoadFileName.substring(upLoadFileName.lastIndexOf(".")+1);
			/*if (acceptFileType.indexOf(upLoadFileType) == -1) {
                layer.alert('请选择图片格式！');
                return;
            }*/
			if ((upLoadFileType!='jpg')&&(upLoadFileType!='gif')&&(upLoadFileType!='jpeg')&&(upLoadFileType!='png')&&(upLoadFileType!='bmp')){
				layer.alert('仅支持上传图片格式文件!', {
		            icon: 7,
		            area: ['400px', ''],
		            title:'提示',
		            closeBtn: 0
		...
		/*//IE8无法使用.files
		var upLoadFile = obj.files;
        if(upLoadFile != null){
            if (! (/image/.test(upLoadFile[0].type))) {
                layer.alert('请选择图片格式！');
                return;
            }
        }*/
