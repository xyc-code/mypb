...
<body class="easyui-layout" fit="true">
	...
	<div data-options="region:'north'" split="false" border="false" style="height:179px;">
		<form id="form" fit="true">
		    <input type="hidden" name="id" value='<c:out  value='${sysJobDTO.id}'/>' />
			<input type="hidden" name="status" value="<c:out  value='${sysJobDTO.status}'/>">
			<table class="form_commonTable">
				<tr>
					<th width="10%"><label for="group">任务分组</label></th>
                	<td width="40%">
						<pt6:h5select css_class="form-control input-sm" name="group" id="group" title="任务分组" isNull="false" lookupCode="PLATFORM_QUARTZ_JOB_GROUP" defaultValue="${sysJobDTO.group}"/>
                	</td>
					<th width="10%"><label for="name">任务名称</label></th>
                	<td>
                    	<input id="name" name="name" title="任务名称" class="form-control input-sm" type="text" value="<c:out  value='${sysJobDTO.name}'/>"/>
                	</td>
                </tr>
                <tr>
                	<th><label for="type">任务类型</label></th>
                	<td><select id="type" name="type" title="任务类型" class="form-control input-sm"  onChange="onChangeType();">
                	...
                	<td>
                    	<input id="cron" name="cron" title="表达式" class="form-control input-sm" type="text"  readonly="readonly" value="${sysJobDTO.cron}" />
                	</td>
                </tr>
                <tr>
                	...
                	<td>
                    	<input id="program" name="program" title="执行程序" class="form-control input-sm"  type="text"  value='<c:out  value='${sysJobDTO.program}'/>' />
                	</td>
                </tr>
                <tr>
                	<th><label for="description">描述</label></th>
                	<td colspan="3">
                    	<input id="description" name="description" title="描述" class="form-control input-sm" type="text"  value='<c:out  value='${sysJobDTO.description}'/>' />
                	</td>
                </tr>
            </table>
		</form>
	...
	<div data-options="region:'center'" split="false" collapsible="false" border="false">
	    ...
	    <!-- 参数列表 -->
	   <div id="myTabContent" class="tab-content" style="height: calc(100% - 33px);border-bottom: 1px solid #e8e8e8;">
		 <div class="tab-pane fade in active" id="home" style="height: 100%">
			<iframe frameborder="0" id="paramifr" src="avicit/platform6/console/quartz/SysJobVariablesUpdate.jsp?masterId=${sysJobDTO.id}" style="height:100%;width:100%"  ></iframe>
		 </div>
		 <!-- 排除日期 -->
	    <div class="tab-pane fade" id="excludedDate" style="height: 100%">
	        <iframe frameborder="0" id="excludedDateifr" src="avicit/platform6/console/quartz/ExcludedDate.jsp?jobCalendarIds=${jobCalendarIds}"  style="height:100%;width:100%" ></iframe>
	    </div>
	  </div>
	</div>
...
function closeForm(){
	parent.sysJob.closeDialog("edit");
}
$(document).ready(function () {
	$('#type').val('${sysJobDTO.type}');
	var type = $('#type').val();
	if(type != null && type != ''){
		setTypeRemark(type);
	}
	...
	$.ajax({
		...
		success : function(result) {
			if (result.flag == 'success') {
				...
				parent.sysJob.tip('修改定时任务成功！');
				closeForm();
			} else {
				if(result.error){
					layer.alert('修改定时任务失败！ '+ result.error, {
						icon : 7,
						area : [ '400px', '' ], //宽高
						closeBtn : 0,
						btn: ['关闭'],
