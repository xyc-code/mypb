 ...
 * 
 */
function SysJobCalendar(datagrid, url, searchForm, dataGridColModel,
		searchDialog, afterInit, clickRowLoad) {
...
//初始化操作
SysJobCalendar.prototype.init = function() {
	var _self = this;
	var issubinit = false;
	$(_self._datagridId).jqGrid({
		url : this.getUrl()+'getSysJobCalendarsByPage.json',
		mtype : 'POST',
		multiselect : true,
		datatype : "json",
		toolbar : [ true, 'top' ],
		colModel : this.dataGridColModel,
		height : 286 -39,//120:顶部工具栏高+表头高+底部翻页模块高，17：顶部toolbar的内边距高度
		width:300,
		scrollOffset : 10,
		rowNum : 20,
		...
		hasColSet:false,
		hasTabExport:false,
		pager: "#sysJobCalendarJqGridPager",
...
//添加页面
SysJobCalendar.prototype.insert = function() {
	this.insertIndex = layer.open({
		...
		content : 'avicit/platform6/console/quartz/SysJobCalendarAdd.jsp'
	});
};
//编辑页面
SysJobCalendar.prototype.modify = function() {
...
//详细页
SysJobCalendar.prototype.detail = function(id) {
...
SysJobCalendar.prototype.formValidate = function(form) {
	form.validate({
		rules : {
			name:{
				required : true,
...
//保存功能
SysJobCalendar.prototype.save = function(form, id) {
	var _self = this;
	avicAjax.ajax({
		...
		success : function(r) {
			if (r.flag == "success") {
				...
				var msg = "";
				if(r.error.indexOf('重复键值') != -1){
                    msg = "名称【" + serializeObject(form).name + "】已存在！";
                }
                layer.msg('保存失败！' + msg, {icon : 2 ,title: "提示",area: ['400px', '']});
			}
		}
	});
};
//删除
SysJobCalendar.prototype.del = function() {
...
//重载数据
SysJobCalendar.prototype.reLoad = function() {
	var searchdata = {
...
//关闭对话框
SysJobCalendar.prototype.closeDialog = function(id) {
	if (id == "insert") {
...
//打开高级查询框
SysJobCalendar.prototype.openSearchForm = function(searchDiv, par) {
	var _self = this;
	par = null;
	//if(!par) par = $(window);
...
//隐藏查询框
SysJobCalendar.prototype.hideSearchForm = function() {
	layer.close(searchDialogindex);
};
/*清空查询条件*/
SysJobCalendar.prototype.clearData = function() {
