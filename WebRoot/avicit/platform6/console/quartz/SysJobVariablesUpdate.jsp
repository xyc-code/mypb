...
var dateTypeData = {"1":"字符串","2":"整型","3":"长整型","4":"浮点型","5":"日期类型"};
/**
 * 格式化状态
 */
function formatterDataType(cellvalue, options, rowObject){  
	var dataType = "";
	if(cellvalue == 'String'){
		dataType = "字符串";
	}else if(cellvalue == 'Integer'){
		dataType = "整型";
	}else if(cellvalue == 'Long'){
		dataType = "长整型";
	}else if(cellvalue == 'Float'){
		dataType = "浮点型";
...
$(document).ready(function() {
			...
			$("#sysJobVariablesjqGrid").jqGrid({
				url: 'jobquar/sysjobvariables/JobVariablesController/operation/getSysJobVariablessByPage.json',
		        mtype: 'POST',
		        postData : {pid : '<%=masterId %>'},
		        datatype: "json",
		        colModel: dataGridVariablesColModel,//表格列的属性
		        height:document.documentElement.clientHeight-78,//初始化表格高度
