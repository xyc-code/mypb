...
<%@ page import="avicit.platform6.commons.utils.JsonHelper" %>
...
String importlibs = "common,table,form";
	...
	String json=JsonHelper.getInstance().writeValueAsString(platform_quartz_job_group);
%>
<!DOCTYPE html>
<html>
<head>
	...
	<form id='form' style="">
		<table class="form_commonTable">
			...
			<tr>
			    <th>任务分组:</th>
				<td>
					<pt6:h5select css_class="form-control input-sm" name="group" id="group" title="任务分组" isNull="true" lookupCode="PLATFORM_QUARTZ_JOB_GROUP" />
...
function formatterGroup(cellvalue, options, rowObject){
	var groupName = '';
	var job_groups = '<%=json%>';
	var groupList = JSON.parse(job_groups);
	for (var i in groupList) {
		if (groupList[i].lookupCode == cellvalue) {
			groupName = groupList[i].lookupName;
			break;
		}
	}
	return groupName;
...
$(document).ready(function () {
	var dataGridColModel =  [
      { label: 'id', name: 'id', key: true, width: 75, hidden:true,	sortable : false }
      <sec:accesscontrollist hasPermission="3" domainObject="sysJob_table_group" permissionDes="任务分组">
 	  ,{ label: '任务分组', name: 'group', width: 150,formatter:formatterGroup, sortable : false}
 	  </sec:accesscontrollist>
