...
//		var enabled = $("#useWeek").attr('checked') == 'checked' ;
		var enabled =$("#useWeek").is(':checked');
	...
	_setInputControlValues = function(controlName, timeValues) {
		...
		for ( var i = 0; i < timeValueArray.length; i++) {
			...
			for ( var j = 0; j < controlsArray.length; j++) {
				var control = controlsArray[j];
				if (control.defaultValue == timeValue) {
	...
	this.parseSecondToUI = function(secondString) {
		...
		if ("*" == secondString) {
			...
			if (2 != secondStringArray.length) {
				//alert("解释[秒]字段错误");
                layer.alert('解释[秒]字段错误！', {icon : 2 ,title: "提示",area: ['400px', '']});
			}
		 
			if(isNaN(secondStringArray[0]) || isNaN(secondStringArray[1]))
			{
				//alert("[秒]中含有非数字字符");
                layer.alert('[秒]中含有非数字字符！', {icon : 2 ,title: "提示",area: ['400px', '']});
			}
	...
	this.parseMinuteToUI = function(minuteString) {
		...
		if ("*" == minuteString) {
			...
			if (2 != minuteStringArray.length) {
				//alert("解释[分]字段错误");
                layer.alert('解释[分]字段错误！', {icon : 2 ,title: "提示",area: ['400px', '']});
			}
			if(isNaN(minuteStringArray[0]) || isNaN(minuteStringArray[1])){
				//alert("[分]中含有非数字字符");
                layer.alert('[分]中含有非数字字符！', {icon : 2 ,title: "提示",area: ['400px', '']});
			}
	...
	this.parseHourToUI = function(hourString) {
		...
		if ("*" == hourString) {
			...
			if (2 != hourStringArray.length) {
				//alert("解释[小时]字段错误");
                layer.alert('解释[小时]字段错误！', {icon : 2 ,title: "提示",area: ['400px', '']});
			}
			if(isNaN(hourStringArray[0]) || isNaN(hourStringArray[1])){
				//alert("[小时]中含有非数字字符");
                layer.alert('[小时]中含有非数字字符！', {icon : 2 ,title: "提示",area: ['400px', '']});
			}
	...
	this.parseDayToUI = function(dayString) {
		...
		if ("*" == dayString) {
			...
			if (2 != dayStringArray.length) {
				//alert("解释[天]字段错误");
                layer.alert('解释[天]字段错误！', {icon : 2 ,title: "提示",area: ['400px', '']});
			}
	...
	this.parseMonthToUI = function(monthString) {
		...
		if ("*" == monthString) {
			...
			if (2 != monthStringArray.length) {
				//alert("解释[月]字段错误");
                layer.alert('解释[月]字段错误！', {icon : 2 ,title: "提示",area: ['400px', '']});
			}
