...
ConsoleGroup.prototype.insert = function(){
	...
	if(maxorderBy==""){
		$.ajax({
			url :  "platform/console/group/getMaxOrderby",
			type : 'post',
			dataType : 'json',
			async:false,
			success : function(r) {
	...
	if(l > 0){
		layer.confirm('确认要删除选择的群组吗?', 
			...
			function(index){
				for(;l--;){
                    ids.push(rows[l]);
                    var rowsdata =$(_self._datagroupgridId).jqGrid('getRowData',rows[l]);
...
ConsoleGroup.prototype.loaduserByid = function(groupId){
	...
	$(this._datausergridId).jqGrid('setGridParam',{url: 'console/group/operation/togetSysUserByPostitionId/'+groupId,postData: searchdata}).trigger("reloadGrid");
};
...
ConsoleGroup.prototype.insertUser =function(){
	...
	if(rows.length==1){
		_self._groupId = rows[0];
	}
	if(_self._groupId=="new_row0"){
		layer.alert('请先保存群组数据！',{
			icon: 0,
			area: ['400px', ''],
			closeBtn: 0,
			title:'提示'
	...
	function selectUserBack(user){
		if(_self._groupId==""){
			...
			return;
		}else if(user.userids==""){
            layer.alert('请选择添加的用户！', {
                    title : '提示',
                    icon : 0,
                    area: ['400px', ''], //宽高
                    closeBtn: 0
...
function exportServerData(){
	layer.confirm('是否确认导出Excel文件?',function(index) {
        if (index) {
            ...
            expSearchParams.fileName='群组导出数据';
