...
<body>
	...
	<div id="pagerViewLogJqGrid"></div>
	<table id="viewLogJqGrid"></table>
</body>
...
<script tpye="text/javascript">
	...
	function previewLog(id){
		var data = $("#viewLogJqGrid").jqGrid("getRowData",id);
		parent.viewDesignPage.previewLog(id);
	};

	function clickBack(id){
		var data = $("#viewLogJqGrid").jqGrid("getRowData",id);
		parent.viewDesignPage.setViewContent(data.treeNode,data.viewLayoutStr.replace(/\\/g, ""),data.tableJs,data.tableUrlCss);
		parent.layer.close(parent.viewDesignPage.logDialog);
	};


	$(document).ready(function(){
		$("#viewLogJqGrid").jqGrid({
	    	url: "platform/eform/eformViewInfoController/getViewLogByPage",
			postData : {
    ...
    */
	function del(){
		$("#viewLogJqGrid").jqGrid('endEditCell');
		var rows = $("#viewLogJqGrid").jqGrid('getGridParam','selarrrow');
		var ids = [];
		var l =rows.length;
		if(l>0){
			layer.confirm('确定要删除该数据吗？',{icon:2,title:"请确认：",area:['400px','']},function(index){
				...
				avicAjax.ajax({
					url: 'platform/eform/eformViewInfoController/deleteEformViewLog',
					...
					success: function (r) {
						if (r.flag == "success") {
							...
							$("#viewLogJqGrid").trigger("reloadGrid");
						} else {
	...
	function save(){
		//取消行编辑
		$("#viewLogJqGrid").jqGrid('endEditCell');
		var datas = $("#viewLogJqGrid").jqGrid('getChangedCells');
		if(datas.length == 0){
			layer.alert('请先修改数据！', {
				  icon: 7,
				  area: ['400px', ''], //宽高
		...
		avicAjax.ajax({
			url: "platform/eform/eformViewInfoController/saveEformViewLogList",
			data: {
