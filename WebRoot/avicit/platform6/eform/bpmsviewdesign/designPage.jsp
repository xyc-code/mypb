		...
		<div id="navbar" class="navbar navbar-default ace-save-state" style="margin-bottom:0px;">
			<div class="navbar-container ace-save-state" id="navbar-container">
				<div class="navbar-header pull-left">
					<h4 id="formName" style="color: white; padding-left: 10px;">${formPath}</h4>
				</div>
                ...
                <div class="modal-body container" style="text-align: center;">
                    ...
                    <div id="styleList">
                        <div class="closeButton">
                            <button id="closeButton" type="button" class="btn btn-sm btn-danger" data-dismiss="modal"><i class="ace-icon fa fa-arrow-up bigger-110"></i>收起</button>
						</div>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    loadExtraJs(function(){
		...
		engine.initLayoutArea();
		engine.initLayoutMouseEvent();
...
function copyLayout(id){
    layer.open({
        ...
        area: ['800px', '500px'],
        maxmin: false,
		content: "avicit/platform6/eform/bpmsviewdesign/viewLayout.jsp?id="+id+"&type=copy",
        btn : [ '保存', '保存并使用', '取消' ],
        ...
        yes : function(index, layero) {
			...
			var viewLayout = frm.getEditorValue();
			$.ajax({
				url: 'platform/eform/bpmsManageController/insertEformViewLayout',
				...
				success: function (r) {
					if (r.flag == "success") {
						var id = r.id;
						var button = "<div class=\"operationButton\" style=\"display:none;\" >\n" +
								"\t\t\t\t\t\t\t\t<span class=\"label operationLabel\" onclick=\"copyLayout('"+id+"')\"><i class=\"icon iconfont icon-niantie\"></i></span>\n" +
								"\t\t\t\t\t\t\t\t<span class=\"label operationLabel\" onclick=\"editLayout('"+id+"')\"><i class=\"icon iconfont icon-edit-square\"></i></span>\n" +
								"\t\t\t\t\t\t\t\t<span class=\"label operationLabel\" onclick=\"delLayout('"+id+"')\"><i class=\"icon iconfont icon-delete\"></i></span>\n" +
								"\t\t\t\t\t\t\t</div>";
						var layoutStr = "<div class=\"layoutDiv editableLayout\" id=\""+id+"\" style=\"display: inline-block;position: relative;\">" + viewLayout + button + "</div>";
						$("#styleList").append(layoutStr);
						$(".eform-item").bind("click", function () {
							viewEditor.setLayout(this);
						});
						layer.msg('复用成功！');
						engine.initLayoutMouseEvent();
					}initLayoutMouseEvent
				}
			});
			...
			$.ajax({
				...
				success: function (r) {
					if (r.flag == "success") {
						...
						layer.msg('复用成功！');
						engine.initLayoutMouseEvent();
						$('#'+id).find(".eform-item").trigger("click")
...
function editLayout(id){
    layer.open({
        ...
        area: ['800px', '500px'],
        maxmin: false,
		content: "avicit/platform6/eform/bpmsviewdesign/viewLayout.jsp?id="+id+"&type=edit",
        btn : [ '保存', '保存并使用', '取消' ],
        success : function(layero, index) {
        },
		yes : function(index, layero) {
			...
			var viewLayout = frm.getEditorValue();
			$.ajax({
				url: 'platform/eform/bpmsManageController/updateEformViewLayout',
				data: {viewLayout: viewLayout, id: id},
				type: 'post',
				dataType: 'json',
				success: function (r) {
					if (r.flag == "success") {
						$('#'+id).find(".eform-item").replaceWith(viewLayout)
						$(".eform-item").bind("click", function () {
							viewEditor.setLayout(this);
						});
						layer.msg('编辑成功！');
...
function delLayout(id){
	layer.confirm('确认要删除此视图布局吗?', {
		...
		$.ajax({
			url: 'platform/eform/bpmsManageController/delEformViewLayout',
			data: {id: id},
			type: 'post',
			dataType: 'json',
			success: function (r) {
