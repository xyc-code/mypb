...
function verifyIsSpecial(value) {
	var regEn = /[`~!@#$%^&*()+<>?:\-{},.\/;'[\]]/im,
		regblank = /^\S*$/;
	if(regEn.test(value) ||!regblank.test(value)) {
		return true;
	}
	else {
		return false;
...
function ViewEngine(){
	...
	var menuItems = {
		...
		"treeGridToolbarButtonGroup": [{"type":"dataListToolbarButton", "name":"添加按钮","only":false,"icon":"fa fa-caret-square-o-right"}],
		"tableToolbarButtonGroup": [{"type":"treeGridToolbarButton", "name":"添加按钮","only":false,"icon":"fa fa-caret-square-o-right"}],
		...
		if (isbatch){
			...
			if (typeof (treeNode.name) != "undefined" && treeNode.name == "查询区") {
				...
				if (isBpm == "N") {//表格的关联流程选择“否”，右键添加工具栏查询区时，流程查询不可选择；
					...
					menuItems.dataListSearchbarArea = [{
						"type": "dataListKeyWordSearch",
						"name": "添加基本查询",
						"only": true,
						"icon": "fa fa-search"
					}, {
						"type": "dataListFormSearch",
						"name": "添加高级查询",
						"only": true,
						"icon": "fa fa-search-plus"
					}, {"type": "dataListSortSearch", "name": "添加排序查询", "only": true, "icon": "fa fa-search-minus"}];
					menuItems.treeGridSearchbarArea = [{
						...
						"only": true,
						"icon": "fa fa-search-plus"
					}, {"type": "treeLevelSearch", "name": "添加层级查询", "only": true, "icon": "fa fa-search-minus"}];
	...
	this.changSave = function(){
		...
		if(form.hasOwnProperty("name")&&form.name.replace(/[\u0391-\uFFE5]/g,"aa").length > 200){
            layer.msg('名称最多可以输入 200 个字符！', {icon: 2});
			return;
		}
		if(form.hasOwnProperty("type") && form.hasOwnProperty("name") && verifyIsSpecial(form.name)){
			layer.msg('名称不能包含特殊字符！', {icon: 2});
			return;
		}
		if(form.hasOwnProperty("layername")&&form.layername.replace(/[\u0391-\uFFE5]/g,"aa").length > 200){
			layer.msg('页面名称最多可以输入 200 个字符！', {icon: 2});
			return;
		}
		if(form.hasOwnProperty("layername") && verifyIsSpecial(form.layername)){
			layer.msg('页面名称不能包含特殊字符！', {icon: 2});
			return;
		}
...
ViewEngine.prototype.initLayoutArea = function () {
	$.ajax({
		url: 'platform/eform/bpmsManageController/selectEformViewLayoutList',
		async: false,
		cache: false,
		success: function (r) {
...
ViewEngine.prototype.initLayoutMouseEvent = function () {
	$(".layoutDiv").hover(function(){
		$(this).find(".operationButton").css("display","block")
	},function(){
		$(this).find(".operationButton").css("display","none")
	});
}
ViewEngine.prototype.initButtonArea = function () {
	...
	if(viewPublishStatus==1){
		...
		"\t\t                <span style=\"border-left: 1px solid #B5B5B5;padding-bottom: 0px;padding-top: 7px;\">\n" +
			...
			"                    <i class=\"icon iconfont icon-Release\" title=\"日志\" onclick=\"viewDesignPage.toLog()\"></i>\n" +
			"                </span>\n" +
			"\t\t                </span>\n" +
		"\t\t				 <span style=\"color: #B5B5B5;\">\n" +
		"\t\t                    <i class=\"icon iconfont icon-simulation\" title=\"JS全局查看\" onclick=\"viewDesignPage.showFunc()\"></i>\n" +
		"\t\t                </span>\n" +
		"\t\t                <span style=\"color: #B5B5B5;\">\n" +
