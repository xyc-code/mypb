...
<div id="treeGridConfigDiv" style="overflow-y: auto; height:100%" data-options="region:'center',split:true,border:false,width:fixwidth(1.0,e),height:fixheight(1.0,e),onResize:function(a){$('#dbCols').setGridHeight(getJgridTableHeight($('#treeGridConfigDiv')));$('#dbCols').jqGrid('resizeGrid');} ">
    <form>
        ...
        <table class="form_commonTable">
            ...
            <tr>
                ...
                <th width="15%"><label><i class="required">*</i>树列表名称</label></th>
                <td width="34%"><input type="text" id="name" name="name"
                                       class="form-control input-sm" placeholder="树列表名称"></td>
            </tr>
            <tr>
            ...
            <tr>
                <th width="15%"><label><i class="required">*</i>展开字段</label></th>
                <td width="34%"><select id="expandname" name="expandname"
                                        class="form-control input-sm col-select"></select></td>
            ...
            <tr class="Pagingbar">
                ...
                <th width="15%"><label><i class="required">*</i>默认页数量</label></th>
                <td width="34%">
...
<script type="text/javascript">
    ...
    function jqgridInit() {
        $(grid_selector).jqGrid({
            ...
            colModel: [{
                ...
                editoptions: {value: ':;userid:用户ID转名称;orgid:组织ID转名称;deptid:部门ID转名称;positionid:岗位ID转名称;roleid:角色ID转名称;groupid:群组ID转名称;lookup:通用代码',
                    dataEvents:[{type:'change',fn:function(e){
                            ...
                            if(value != "lookup"){
                                $("#"+rowid+"LookupType").val(null);
                                $(grid_selector).jqGrid('setCell', rowid, "LookupTypeName", null);
                                $(grid_selector).jqGrid('setCell', rowid, "LookupTypeId", null);
                            }
            ...
            width:$(window).width(),//宽度必须指定，使用autoweidth导致主子表是后面的表格横向滚动条有问题
            rownumbers: true,
            rownumWidth: 30,
            rowNum:2000,
            autowidth: true,
            ...
            loadComplete:function(){
                ...
                $(".ui-jqgrid ").css("position","absolute");
                if (ids.length > 0) {
                    $(".ui-jqgrid ").css("position","static");
                ...
                $(grid_selector).setGridHeight($(document).height()-310);
            },
    ...
    function getNodes() {
        ...
        var treeGridId = "treeGrid" + parent.uuid();
        returnNode.push({
            "id": treeGridId,
            "name": $("#name").val(),
            "type": "treeGrid",
        ...
        var treeGridModelId = "treeGridModel" + treeGridId;
        ...
        var treeGridToolbarId = treeGridId + "Toolbar";
        returnNode.push({
            "id": treeGridToolbarId,
            "name": "工具栏",
            "type": "treeGridToolbar",
            "parentId": treeGridId,
            "attribute": {
                "id": treeGridToolbarId,
                "name": "工具栏",
        ...
        if (searchArray.length > 0 || searchAllArray.length > 0) {
            var treeGridSearchbarAreaId = treeGridToolbarId + "treeGridSearchbarArea";
            returnNode.push({
                "id": treeGridSearchbarAreaId,
                "name": "查询区",
                "type": "treeGridSearchbarArea",
                "parentId": treeGridToolbarId,
                "attribute": {
                    "id": treeGridSearchbarAreaId,
                    "name": "查询区",
                },
            ...
            if (searchArray.length > 0) {
                var treeGridKeyWordSearchId = treeGridSearchbarAreaId + "treeGridKeyWordSearch";
                returnNode.push({
                    "id": treeGridKeyWordSearchId,
                    "name": "基本查询",
                    "type": "treeGridKeyWordSearch",
                    "parentId": treeGridSearchbarAreaId,
                    "attribute": {
                        "id": treeGridKeyWordSearchId,
                        "name": "基本查询",
                    },
                ...
                for (var j = 0; j < searchArray.length; j++) {
                    returnNode.push({
                        ...
                        "type": "keyWordFiled",
                        "parentId": treeGridKeyWordSearchId,
                        "attribute": {
                            "id": searchArray[j].id,
            ...
            if (searchAllArray.length > 0) {
                var formSearchId = treeGridSearchbarAreaId + "formSearch";
                returnNode.push({
                    "id": formSearchId,
                    "name": "高级查询",
                    "type": "treeGridFormSearch",
                    "parentId": treeGridSearchbarAreaId,
                    "attribute": {
                        "id": formSearchId,
                        "name": "高级查询",
        ...
        if ($("#addBtn").is(':checked') || $("#editBtn").is(':checked') || $("#delBtn").is(':checked')) {
            var buttonAreaId = treeGridToolbarId + "ButtonArea";
            returnNode.push({
                "id": buttonAreaId,
                "name": "按钮区",
                "type": "treeGridToolbarButtonArea",
                "parentId": treeGridToolbarId,
                "attribute": {
                    "id": buttonAreaId,
                    "name": "按钮区",
            ...
            if ($("#addBtn").is(':checked')) {
                var addBtnId = "treeGridToolbarButton" + parent.uuid();
                returnNode.push({
                    "id": addBtnId,
                    "name": "添加",
                    "type": "treeGridToolbarButton",
            ...
            if ($("#editBtn").is(':checked')) {
                var eidtBtnId = "treeGridToolbarButton" + parent.uuid();
                returnNode.push({
                    "id": eidtBtnId,
                    "name": "编辑",
                    "type": "treeGridToolbarButton",
            ...
            if ($("#delBtn").is(':checked')) {
                var delBtnId = "treeGridToolbarButton" + parent.uuid();
                returnNode.push({
                    "id": delBtnId,
                    "name": "删除",
                    "type": "treeGridToolbarButton",
    ...
    function isValidate() {
        ...
        if($.trim($("#name").val())==""){
            ...
            return false;
        }else if($.trim($("#expandname").val())==""){
            layer.msg('展开字段不可为空，请检查！', {icon: 7});
            return false;
