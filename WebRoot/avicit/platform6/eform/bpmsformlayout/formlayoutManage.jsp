...
<script src="avicit/platform6/eform/formdefine/js/eformCustomDialog.js"></script>
	...
	<script type="text/javascript">
		...
		function test() {
				var tableId=$('#tableId').val();
				url = 'platform/eform/bpmsCRUDClient/bpmtest?version='+encodeURI("${version}")+'&tableId='+tableId;
		...
		function initWorkFlow(status, owner){
			...
			var tableId=$('#tableId').val();
			url = 'platform/eform/bpmsCRUDClient/list?tableId='+tableId+"&bpmState="+status+"&bpmType="+owner;
			$('#dg').datagrid({
				url:url
			});
		...
		function newObj() {
			var tableId=$('#tableId').val();
			url = 'platform/eform/bpmsCRUDClient/add?version='+encodeURI("${version}")+'&tableId='+tableId;
			var  dlg = new CommonDialog("insert","680","400",url,"添加",false,true,false);
			dlg.show();
		}
		...
		function editObj() {
			...
			if(l>1){
				...
				if (row) {
					url = 'platform/eform/bpmsCRUDClient/get?version='+encodeURI("${version}")+'&id=' + row.ID+'&tableId='+tableId;
		...
		function deleteObj() {
			beforeEvent();
			url='platform/eform/bpmsCRUDClient/delete.json';
			//var row = $('#dg').datagrid('getSelected');
			var tableId=$('#tableId').val();
		...
		function bpmObj(rowId) {
			var tableId=$('#tableId').val();
			url = 'platform/eform/bpmsCRUDClient/bpm?version='+encodeURI("${version}")+'&id=' + rowId+'&tableId='+tableId;
		...
		function detailObj(rowId) {
			var tableId=$('#tableId').val();
			url = 'platform/eform/bpmsCRUDClient/detail?version='+encodeURI("${version}")+'&id=' + rowId+'&tableId='+tableId;
			var  dlg = new CommonDialog("update","680","400",url,"详细",false,true,false);
			dlg.show();
		}
...
<div data-options="region:'center',split:true,border:false" style="padding:0px;height:0;overflow:hidden;">
	<!-- LIST表单 start-->
	<table id="dg"  class="easyui-datagrid"  data-options="onLoadSuccess:cellTip,fit:true,toolbar:'#toolbar',pagination:true,rownumbers:true,fitColumns:true,singleSelect:true,checkOnSelect:true,selectOnCheck:false,autoRowHeight:false,striped:true,url:'platform/eform/bpmsCRUDClient/list?tableId=${table.id}'">
		<thead>
			<tr>
	...
	<div id="toolbar">
		...
		<sec:accesscontrollist hasPermission="3" domainObject="eform_toolbar_${table.tableName}_search" permissionDes="查询">
				     ...
				     <td><a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-search" plain="true" onclick="test()">测试</a></td>
