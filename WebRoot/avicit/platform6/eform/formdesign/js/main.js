...
function FEformEditor(id) {
    ...
    this.templateTypeId = "";//模板分类ID
    this.eformJs = "";//电子表单应用的JS
    this.eformUrlCss = "";//电子表单应用的CSSS
    this.configColData = null;
    this.configColCount = 4;
    ...
    if(colData!=null && colData!=undefined){
        ...
        if(colData.length>0) {
            ...
            if(!verifyIsEmpty(formEditor.formName)){
                tinymce.activeEditor.setContent("<div style=\"text-align: center;font-size: 24px; margin: 15px;\">"+formEditor.formName+"</div>"+table.prop('outerHTML'));
            }else{
        ...
        if(parent.EformEditor.configColData != null && parent.EformEditor.configColData.length != 0){
            var table = drawTableHtml(parent.EformEditor.configColData, parent.EformEditor.configColCount);
            if(!verifyIsEmpty(formEditor.formName)){
                tinymce.activeEditor.setContent("<div style=\"text-align: center;font-size: 24px; margin: 15px;\">"+formEditor.formName+"</div>"+table.prop('outerHTML'));
            }else{
                tinymce.activeEditor.setContent(table.prop('outerHTML'));
            ...
            $.ajax({
                url: "platform/eform/bpmsManageController/getConfigColDataByEformId/" + parent.EformEditor.eformFormInfoId,
                type: "post",
                async: false,
                dataType: "json",
                success: function (backData) {
                    if (backData.result == "1") {
                        if(backData.configColData.length>0) {
                            var table = drawTableHtml(backData.configColData, backData.configColCount);
                            if(!verifyIsEmpty(formEditor.formName)){
                                tinymce.activeEditor.setContent("<div style=\"text-align: center;font-size: 24px; margin: 15px;\">"+formEditor.formName+"</div>"+table.prop('outerHTML'));
                            }else{
                                tinymce.activeEditor.setContent(table.prop('outerHTML'));
...
FEformEditor.prototype.setButtonArea = function(){
    ...
    html = html +
        ...
        "                    <i class=\"icon iconfont icon-Release\" title=\"日志\" onclick=\"EformEditor.toLog()\"></i>\n" +
        "                </span>\n" +
        "                </span>\n" +
        "<span style=\"color: #B5B5B5;\">\n" +
        "                    <i class=\"icon iconfont icon-simulation\" title=\"JS全局查看\" onclick=\"EformEditor.showFunc()\"></i>\n" +
        "                </span>\n" +
/*        "                <span style=\"color: #B5B5B5;\">\n" +
        "                    <i class=\"icon iconfont icon-open\" title=\"打开模板\" onclick=\"EformEditor.openTemplate()\"></i>\n" +
        "                </span>\n" +*/
        "                <span style=\"color: #B5B5B5;\">\n" +
        "                    <i class=\"icon iconfont icon-open\" title=\"打开模板\" onclick=\"EformEditor.openTemplateNew()\"></i>\n" +
        "                </span>\n" +
        "                <span style=\"color: #B5B5B5;\">\n" +
...
FEformEditor.prototype.initTemplateInfo = function (editor) {
    var _this = this;
    $("#eformTypeSelect").bind("click", function() {
        var eformTypeId = $("#id").val();
        var url = "avicit/platform6/eform/bpmsformmanage/eformFormInfo/changeEformTypeDirectory.jsp";
...
FEformEditor.prototype.toLog = function () {
    var _this = this;
    _this.logDialog = layer.open({
        ...
        area: ['80%', '70%'],
        maxmin: false,
        content: "avicit/platform6/eform/formdesign/eformLog.jsp"
    });
}

FEformEditor.prototype.showFunc = function () {
    var _this = this;
    _this.funcDialog = layer.open({
        ...
        area: ['80%', '70%'],
        maxmin: false,
        content: "avicit/platform6/eform/formdesign/eformFunc.jsp"
    });
}

/*FEformEditor.prototype.saveLog = function () {
    $("#saveLog").modal("show");

    $("#saveLogoButton").unbind("click");
    $("#saveLogoButton").click(function () {
        var logContent = $("#logContent").val();
        if (verifyIsEmpty(logContent)) {
            layer.msg('日志内容不能为空！', {icon: 7});
        }else{
            $.ajax({
                ...
                success: function (backData, status) {
                    if (backData.success == true) {
                        ...
                        _this.templateTypeId = templateTypeId;
...
FEformEditor.prototype.saveAsTemplate = function () {
    ...
    if (verifyIsEmpty(_this.templateName) && verifyIsEmpty(_this.templateTypeId)) {
        ...
        layer.confirm('确定要更新当前模板吗？', {
            ...
            $("#templateTypeId").val(null);
            $("#typeName").val(null);
            $("#templateName").val(null);
            _this.templateName = "";
            _this.templateTypeId = "";
            saveTemplate(tinymceContentStyle, sourceContent);
        });
    }
    ...
    function saveTemplate(tinymceContentStyle, sourceContent) {
        ...
        $("#saveTemplateButton").click(function () {
            ...
            }else if(verifyIsEmpty(templateTypeId)){
                layer.msg('模板分类不能为空！', {icon: 7});
            }
            else {
...
FEformEditor.prototype.temppreview = function (colList,tableLength) {
    ...
    if(colList.length>0) {
        ...
        var content;
        if(!verifyIsEmpty(formEditor.formName)){
            content = "<div style=\"text-align: center;font-size: 24px; margin: 15px;\">"+formEditor.formName+"</div>"+table.prop('outerHTML');
        }else{
...
FEformEditor.prototype.redesign = function (type,colList,tableLength,dbName) {
    ...
    if(colList != null && colList != undefined){
        EformEditor.configColData = colList;
        EformEditor.configColCount = tableLength;
    }
 ...
 * 打开模板
 */
FEformEditor.prototype.openTemplateNew = function () {
    var _this = this;

    _this.templateDialog = layer.open({
        type: 2,
        title: '打开模板',
        skin: 'bs-modal',
        area: ['90%', '80%'],
        maxmin: false,
        content: "avicit/platform6/eform/bpmsformmanage/efromDesignTemplate/eformtemplatemanager.jsp"
    });
};
...
FEformEditor.prototype.previewtemplate = function (style,sourceContent,tinymceContentStyle) {
    var _this = this;
    //源代码外包一层
    var showContent = $('<div class="mce-content-body"></div>');
    showContent.append(sourceContent);
...
FEformEditor.prototype.save = function (publish) {
    ...
    if(isDesign == false || isDesign == "false") {
        ...
        if(hasDefaultCol){
            hasDefaultColMsg = "表单字段名称存在默认[DATA_]字符，";
        }
    }
...
FEformEditor.prototype.initEformEditor = function (id) {
    ...
    this.initTemplateInfo(this.editor);
};
...
function bindDragEleEvent(eleItemObj) {
    eleItemObj.click(function () {
        ...
        if (eleCode == "datatable" || eleCode == "print-datatable") {
            ...
            var td = $('#tinymceArea_ifr').contents().find('#' + eleViewId);
            if (td) {
                var html = td.prop("outerHTML");
                html = html.replace(/&nbsp;/ig, '');
                td.replaceWith(html);
            }
        }
...
function drawTableHtml(colData,trLength,tableLength){
    ...
    for (var j = 0; j < colData.length; j++) {
        ...
        var td = '<td style="width: '+ Math.floor(tableLength*0.25/trLength)+'px; text-align: right;" ><span class="element label-box" contenteditable="false" id="' + labelId + '"><label for="' + colData[j].colName + '" id="' + GenNonDuplicateID() + '">' +
            ...
            '<td style="width: '+ Math.floor(tableLength*0.75/trLength*(colData[j].realColspan-1))+'px;"';
        ...
        if (colData[j].plugins == 'text') {
            ...
            td += '<span class="element radio-box onchoose" contenteditable="false" id="' + GenNonDuplicateID() + '">';
...
function setParentNode(currentTreeNode){
    $("#templateTypeId").val(currentTreeNode.id);
    $("#typeName").val(currentTreeNode.name);
}