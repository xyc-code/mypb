	...
	if(isMain == "1"){
	    ...
	    for (var dom in doms){
	        if (dom != "clone" && (doms[dom].colType!="NOT_DB_COL_ELE"||doms[dom].domType == "virtual-box")){
                ...
                || (doms[dom].domType == "dictionary-box" && colIsKeyValue == "Y")){
...
function changeTargetNameFunc(doms,title){
    ...
    if(rootNode == undefined){
        rootNode = zTreeObj.getNodeByTId("1");
    }
    ...
    for (var dom in doms){
        if (dom != "clone"){
        	if(doms[dom].data){
                ...
                || doms[dom].data.colIsKeyValue == "Y"
                || doms[dom].data.transform == "userid"
                || doms[dom].data.transform == "orgid"
                || doms[dom].data.transform == "deptid"
                || doms[dom].data.transform == "positionid"
                || doms[dom].data.transform == "roleid"
                || doms[dom].data.transform == "groupid"
                || doms[dom].data.transform == "lookup"
                ){
                ...
                || (doms[dom].elementType == "dictionary" && doms[dom].colIsKeyValue == "Y")
                ){
		        	nameJson[id] = doms[dom].colLabel+"(值)";
		        	nameJson[id + "Name"] = doms[dom].colLabel;
	        	}else{
...
var dictionaryProperty = {
    ...
    dataGridColModel:[
        ...
        { label:'列转换', name:'transform', width:75, hidden:true},
        {
            label:'列转换',
            name:'transformName',
            width : 50,
            ...
            edittype:"custom",
            formatter:formatDictionaryTableData,
            editoptions: {custom_element:selectElem, custom_value:selectValue, forId:'transform',
                value: {"1":"用户转换","2":"组织转换","3":"部门转换","4":"岗位转换","5":"角色转换","6":"群组转换","7":"通用代码转换"},
                callBack: transformCallBack
                }
        },
        { label:'通用代码', name:'lookupType', width:75, hidden:true},
        {
            label: '通用代码',
            name: 'lookupTypeName',
            width: 100,
...
function transformCallBack(e){
    var rowid=$(e.target).attr("data-rowid");//通过事件获得行ID
    var value = $(e.target).val();
    if(value != "7" ){
        $("#propertytable").jqGrid('setCell', rowid, "lookupType", null);
        $("#propertytable").jqGrid('setCell', rowid, "lookupTypeName", null);
    }
}
//自定义选择通用代码
function dictValue(elem, operation, value) {
    if (operation === 'get') {
        var selectText = $(elem).find("input[type='text']").val();
        if( selectText != "请选择"){
            return selectText;
        }else{
            return "";
...
function dictElem(value, options){
    ...
    $elem.find('#' + rowId + options.name+', .input-group-addon').on('click',function() {
        ...
        if (rowData.transform != "7") {
            layer.msg("列转换类型为“通用代码”才可编辑此项");
            return false;
        }
        layer.open({
            type:  2,
            ...
            yes: function(index, layero){
                ...
                $("#" + rowId+options.name).attr("value", objData.lookupType);
                $("#" + rowId+options.name).parents("td").attr("title",objData.lookupType);
                $("#propertytable").jqGrid('setCell', rowId, "lookupType", objData.lookupType);
                layer.close(index);
            },
...
function lookupCallBack(rowObjData) {
    if(row_id != null && row_id != undefined){
        $("#propertytable").jqGrid('setCell', row_id, "lookupType", rowObjData.lookupType);
        row_id = null;
    }
...
function commitForm(){
    ...
    for(var i=0;i<obj.datagridData.length;i++){
        var rowData = obj.datagridData[i];
        var transform = rowData.transform;
        var lookupType = rowData.lookupType;
        if(transform == "7" && (lookupType == "" || lookupType == undefined)){
            layer.msg('[源字段名称]是'+rowData.name+'的[通用代码]列不能为空！');
            return false;
        }
    }
