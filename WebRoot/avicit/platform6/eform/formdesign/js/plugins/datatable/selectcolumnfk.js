...
$(document).ready(function(){
	...
	// 	addSqlItem();
	// });
	initTargetCol(parent.$("#subTableName").val());
	...
	$("#targetcol").change(function (e) {
		var colType = $("#targetcol").find("option:selected").data('type');
		if(colType == "VARCHAR2"){
			$("#paracoltype").val("VARCHAR");
			$("#paracol option[data-type='VARCHAR2']").show();
			$("#paracol option[data-type='NUMBER']").hide();
			var firstVal = $("#paracol option[data-type='VARCHAR2']:first").val();
			if(firstVal == null || firstVal == undefined){
				$("#paracol").val(null);
			}else{
				$("#paracol").val(firstVal);
			}

		}else{
			$("#paracoltype").val("DECIMAL");
			$("#paracol option[data-type='VARCHAR2']").hide();
			$("#paracol option[data-type='NUMBER']").show();
			var firstVal = $("#paracol option[data-type='NUMBER']:first").val();
			if(firstVal == null || firstVal == undefined){
				$("#paracol").val(null);
			}else{
				$("#paracol").val(firstVal);
 ...
 * @param  {[string]} subTableName [目标存储模型id]
 * @return {[void]}
 */
function initTargetCol(subTableName) {

	var list = $.parseJSON(parent.$("form").serializeObject().colList);;
	var $colDom = $("#targetcol");
	$colDom.each(function() {
		$(this).empty();
		for (var i = 0; i < list.length; i++) {
			if(list[i].colType == "VARCHAR2" || list[i].colType == "NUMBER"){
				var $option = $('<option value="' + list[i].colName + '" data-type="' + list[i].colType + '" >' + list[i].colName + '</option>');
				$(this).append($option);
			}
		}
	});
	/*if (!isEmptyObject(subTableName)){
		$.ajax({
...
function validateDataJsonList(){
	if(dataJsonList != undefined && dataJsonList.length > 0){
		for(var i = 0; i< dataJsonList.length; i++){
			var jsonObj = dataJsonList[i];
			var data = jsonObj.data;
			if(data.paracol == null || data.paracol == "" || data.paracol == undefined){
				layer.msg("数据源参数[" + data.targetcol + "]对应的主表字段不能为空！", {icon: 7});
				return false;
			}
			if(data.targetcol == null || data.targetcol == "" || data.targetcol == undefined){
				layer.msg("数据源参数对应的子表字段不能为空！", {icon: 7});
				return false;
			}
		}
		return true;
	}else{
		layer.msg('请配置数据源参数信息！', {icon: 7});
		return false;
	}
