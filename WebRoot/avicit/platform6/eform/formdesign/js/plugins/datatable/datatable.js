var MyElement = {
    ...
    update: function (form, ui) {
        ...
        var fkColName = "";
        var fkChk = attrJson.fkChk;
        var paraLength = 0;
        if(fkChk == "Y" && $("#paralist").val() != "" && $("#paralist").val() != undefined ){
            var FkList = $.parseJSON($("#paralist").val());
            paraLength = FkList.length;
            for(var i = 0; i < FkList.length; i ++){
                fkColName += "," + FkList[i].targetcol;
            }
            fkColName += ",";
        }else{
        ...
        if (attrJson.colList == "") {
            ...
            for (var i = 0; i < colList.length; i++) {
                ...
                if(fkChk == "Y"){
                    if (fkColName.indexOf(colName) != -1) {
                        continue;
                    }
                }else{
            ...
            if (fkColName != null && fkColName != "") {
                parentTable.find("tbody").find("tr:first").find("td").attr("colspan", colList.length - paraLength);
            } else {
...
//        }
    ...
    validateForm: function (eleattr) {
        ...
        if (eleattr.subTableType == "1") {
            if(!eleattr.fkChk){
                ...
                if ((!eleattr.paralist)) {
                    layer.msg('子表控件子表外键配置不能为空', {icon: 7});
                    return false;
                }
            }
    ...
    addDefaultButton: function(){
        ...
        var importNewButton = new Object({
            data: {
                colIsVisible: "N",
                iconId: tableName + "_importNewBtn",
                iconName: "导入(新)",
                buttonType: "importNew",
                buttonclick: "",
                templetCode: "",
                buttonIcon: "<span class = 'glyphicon glyphicon-import'></span>"
            },
        ...
        //this.defaultButtonMap["importNew"] = importNewButton;
    ...
    afterInitAttrForm: function(form, attrJson){
        if (attrJson) {
            ...
            for (var i = 0; i < list.length; i++) {
                ...
                var coltypediv = $('<div class="col-xs-3">' + list[i].data.buttonIcon.replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&amp;/g, "&") + '</div>');
            ...
            var buttondatastr = JSON.stringify(buttondatas).replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&amp;/g, "&");
            ...
            if(json.paralist != undefined && json.paralist != ""){
                ...
                if (paralist.length > 0) {
                    for (var i = 0,length=paralist.length; i < length; i++) {
                        ...
                        var colLabeldiv = $('<div class="col-xs-9"></div>');
                        colLabeldiv.append('<span class="lbl">' + data.targetcol + '</span>');
                        li.append(coltypediv).append(colLabeldiv);
                        $("#paraarea").append(li);
                        rs.push(data);
                    }
    ...
    initAttrForm: function (form, attrJson) {
        ...
        $("#addpara").click(function () {
            ...
            publishDialog = top.layer.open({
                ...
                area: ['45%', '85%'],
                maxmin: false,
                content: "avicit/platform6/eform/formdesign/js/plugins/datatable/selectcolumnfk.jsp",
                btn: ['确定', '取消'],
                yes: function(index, layero){
                    var frm = layero.find('iframe')[0].contentWindow;
                    var jsonData = [], rs = [];
...
function singleFk(fkColName){
    $("#colArea").html("");
    var jsonData = [];
    var attrJson = $("#attrform").serializeObject();
    if ($("#colList").val() && $("#colList").val() != "") {
        jsonData = $.parseJSON($("#colList").val());
    } else {
        jsonData = $.parseJSON($.parseJSON(attrJson).colList);
    ...
    if(fkColName == null || fkColName == "" || fkColName == undefined){
        var fkColName = attrJson.fkColName;
    }
...
function multiFk(){
    ...
    for (var i = 0; i < jsonData.length; i++) {
        ...
        colLabeldiv.append('<span class="lbl">' + data.colLabel + '</span>');
        li.append(coltypediv).append(colLabeldiv);
        if (!(fkColName && data.colName && fkColName.indexOf(","+data.colName+",") != -1)) {
            $("#colArea").append(li);
            data.colIsVisible = "Y";
        } else {
            data.colIsVisible = "N";
