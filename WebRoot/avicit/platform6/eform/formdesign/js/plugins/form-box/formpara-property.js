...
var config = {
	dbid:$("#dbid", window.parent.document).val(),
	dbArray:[],
	paralist:$('#paralist', window.parent.document).val(),
	EformEditor:parent.EformEditor,
	editorUtil:parent.editorUtil

}

//初始化入口，使用此页面需要调用此方法
function initPage(inconfig){

	if (inconfig!=null && inconfig!=undefined){
	...
	initTargetCol(config.dbid);
	bindDbselect(config.dbArray,config.EformEditor.nowElement);
...
function initTargetCol(dbid) {
	if (!isEmptyObject(dbid)){
		...
		var mainCol1 = config.editorUtil.getMainTableDomAttr();
		$("#targetcol").empty();
...
function bindParaCol(did){
	...
	if (!isEmptyObject(did)){
		...
		var mainCol = config.editorUtil.getMainTableDomAttr();
		$("#paracol").empty();
        addBaseCol();
...
function bindDbselect(dbtableArray,dom) {
	...
	var tablesArray = config.editorUtil.getAllTableAttr(dom.attr("id"));
