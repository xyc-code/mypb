var MyElement = {
    ...
    dragElement: function () {
    	var dragelement = {};
    	dragelement.name = "单选";
    	dragelement.icon = "fa fa-dot-circle-o";
    	return dragelement;
    },
    dropElement: function () {
        return '<input type="radio" value=""><span class="option">单选按钮</span>';
    },

    validateForm:function(eleattr){
        if(!eleattr.selectedoption || !eleattr.selectedvalues)
        {
            layer.msg('单选控件选项属性不能为空', {icon: 7});
            return false;
        }
        return true;
    },
    ...
    initAttrForm:function(form,attrJson){
    		...
    		var aa = new selectarea("selectarea",{
    			lookup_url:"avicit/platform6/eform/formdesign/js/plugins/radio-box/addLookup.jsp",
    			onChange: function(obj){
    			
    				var eleattr = ele.find(".eleattr-span");
    				ele.empty();
    				if (!obj.length>0){
    					ele.append('<input type="radio"><span class="option">单选按钮</span>');
    				}
    		        for (var i = 0; i < obj.length; i++) {
    		            var value = obj[i].value;
    		            var option = obj[i].option;
    		            ele.append('<input type="radio"  value="' + value + '"><span class="option">' + option + '</span>');
    		        }
    		        ele.append(eleattr);
    			}
    		});
