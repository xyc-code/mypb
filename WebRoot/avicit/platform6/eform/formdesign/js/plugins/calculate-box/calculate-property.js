    ...
    for (var dom in doms) {
        if (dom != "clone") {
            ...
            if (doms[dom].hasOwnProperty("data")){
                data = doms[dom].data;
                data.calculatecoltype = "sub";
            }else{
                data.calculatecoltype = "main";
            }
            if (data.colType != "NUMBER") {
                continue;
            }

            var domId = data.domId || data.colName,label;
            if (data.calculatecoltype === "sub"){
                label =  data.colLabel + "[" + domId + '](子表)';
            }else{
                label =  data.colLabel + "[" + domId + ']';
                domId = domId + "_"+ data.calculatecoltype;
            }
            if (domId == targetDomId) {
                continue;
            }
            var item = $('<li id="' + domId + '" title="' + data.colLabel + '" class="item-list"></li>');
            item.append('<span class="item-title">' + label+'</span>');
