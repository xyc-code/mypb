var MyElement = {
    ...
    initAttrForm: function (form, attrJson) {
        ...
        if (attrJson) {
            ...
            currentDomId = json.domId;
            formSecretValue = json.formSecret;
        }
        for (ele in allEle) {
            if (ele != "clone" && allEle[ele].domType != "label-box"
                && ("domId" in allEle[ele] || "colName" in allEle[ele])) {
                if (allEle[ele]["domId"] != undefined && allEle[ele]["domId"] != ""
                    && currentDomId != allEle[ele]["domId"]) {
                    var op = new Option(allEle[ele]["domId"], allEle[ele]["domId"]);
                    //formSecret.options[i] = op;
                    formSecretJson.push({id:allEle[ele]["domId"],text:allEle[ele]["domId"]});
                    i = i + 1;
                } else if (allEle[ele]["colName"] != undefined && allEle[ele]["colName"] != "") {
                    var op = new Option(allEle[ele]["colName"], allEle[ele]["colName"]);
                    //formSecret.options[i] = op;
                    formSecretJson.push({id:allEle[ele]["colName"],text:allEle[ele]["colName"]});
                    i = i + 1;
                }

            }
        }
        $("#formSecret").select2({data: formSecretJson});
        $('#formSecret').select2('val', formSecretValue);
