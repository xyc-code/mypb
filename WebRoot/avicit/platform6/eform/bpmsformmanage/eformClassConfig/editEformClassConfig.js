...
//打开表单事件demo页
EditEformClassConfig.prototype.openExplain =function(){
    layer.open({
        type: 2,
        title: '事件说明',
        closeBtn : 1,
        ...
        area: ['100%', '100%'],
        maxmin: false,
        content: "avicit/platform6/eform/bpmsformmanage/eformClassConfig/eventexplain.jsp"
    });
};
 ...
 * 导出
 */
EditEformClassConfig.prototype.doExport = function () {
    ...
    if (l > 0) {
        ...
        var url = _self.getUrl() + 'exportEformClass';
        var ep = new exportData('export', 'export', params, url);
        ep.excuteExport();
    } else {
        layer.confirm('未勾选数据，是否导出全部数据吗?', {
            icon: 3,
            title: "提示",
            area: ['400px', '']
        }, function (index) {
    ...
    /*var _self = this;
    var rows = $(_self._datagridId).jqGrid('getRowData');
    if(rows.length > 0 ){
        var downloadUrl = "platform/eform/bpmsManageController/exportEformClass";
        new DownLoad4URL('downloadIFrame_eform', 'downloadForm_eform', null, downloadUrl).downLoad();
    }else{
        layer.alert('没有需要导出的表单事件！', {
            icon: 3,
            area: ['400px', ''],
            closeBtn: 0
        });
 ...
 * 导入
 */
EditEformClassConfig.prototype.openImport = function () {
    var _self = this;
...
//关闭弹出框
EditEformClassConfig.prototype.closeDialog = function (type) {
    if (type == "upload") {
