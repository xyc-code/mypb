...
<body class="easyui-layout" fit="true" style="overflow-y: auto;">
    ...
    $(document).ready(function() {
    	var module = 
    		...
    		$("#formViewModel").jqGrid({
    			url : "platform/eform/eformViewInfoController/checkTabVersion",
    		        datatype: "json",
    		        postData: {eformFormViewId:"${param.eformFormViewId}"},
    ...
    function getformviewButtons(cellvalue, options, rowObject) {
    	if(rowObject.ynCurrent == "是"){
    		...
    		+'   title="重新生成页面" onClick="doCreate(\''+rowObject.viewVersion+'\')"></a>&nbsp;&nbsp;'
    		...
    		+'  title="置为当前版本" onClick="setCurrentVersion(\''+rowObject.viewVersion+'\')"></a>&nbsp;&nbsp;'
    ...
    function getformviewEdit(cellvalue, options, rowObject){
		...
		+'  title="名称" onClick="openDesigner(\''+rowObject.viewVersion+'\')">'+cellvalue+'</a>';
    }
    
    function openDesigner(version){
        var id = "${param.eformFormViewId}";
        parent.eformFormView.openDesigner(id,version);
    }
    //置当前版本
    function setCurrentVersion(version){
    	//手动请求根节点数据
        $.ajax({
            url: "platform/eform/eformViewInfoController/setCurrentVersion",
            data: {eformFormViewId:"${param.eformFormViewId}",version:version},
            type: "post",
            async: false,
            dataType: "json",
            success: function (backData) {
            	if(backData.result == '1'){
            		if(backData.data != null){
                        var formView = $('#formViewDiv', window.parent.document).find("#${param.eformFormViewId}");
                        formView.remove();
                        parent.reloadEformFormView(backData.data);//刷新父页面信息
            		}
    ...
    function gridReload(){
    	var searchdata = {
            eformFormViewId:"${param.eformFormViewId}"
    	};
    ...
    function doCreate(version){
		layer.confirm('确定要重新生成页面吗？',{
			...
			avicAjax.ajax({
	            ...
	            data: "id=" + "${param.eformFormViewId}"+"&version="+version,
	            type: "post",
	            async: true,
	            dataType: "json",
	            success: function (backData) {
    ...
    function deleteVersion(row){
    	if(row.ynCurrent === "是"){
    		...
    		layer.confirm('确定要删除此版本吗？',{
    			...
    			layer.close(index);
    			avicAjax.ajax({
    	            url: "platform/eform/eformViewInfoController/deleteEformViewContentVersion",
    	            data: {id:row.id},
    	            type: "post",
    	            async: true,
    	            dataType: "json",
