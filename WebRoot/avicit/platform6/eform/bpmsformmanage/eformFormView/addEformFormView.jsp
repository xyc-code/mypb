...
<body class="easyui-layout" fit="true">
	<div data-options="region:'center',split:true,border:false">
		<form id="addFormView" role="form">
			<ul class="nav nav-tabs" style="margin:20px 20px 0px 20px">
				<li class="active"><a href="#viewInfo" data-toggle="tab" id="viewInfoTab"><span id="viewInfoTabSpan">基本信息</span></a></li>
			...
			<div id="myTabContent" class="tab-content" style="margin:20px 40px">
				...
				<div class="tab-pane fade in active" id="viewInfo">
						<br>
						<input type="hidden" name="eformComponentId" id="eformComponentId"/>
						<table class="form_commonTable">
							...
							<tr>
				                ...
				                <td width="80%">
									<label class="radio-inline">
										<input type="radio" name="viewStyle" value="1" id="displayRuleBootstrap" title="Bootstrap" checked>Bootstrap
									</label>
									<label class="radio-inline">
										<input type="radio" name="viewStyle" value="0" id="formStyleEasyUI" title="EasyUI">EasyUI
									</label>
				                </td>
				             </tr>
							 <tr>
							...
							<tr>
								...
								<td width="80%">
									...
									<input class="form-control input-sm" type="text" name="tableName" id="tableName" title="存储模型"/>
								</td>
							</tr>
							<tr>
								<th width="6%" style="word-break: break-all; word-warp: break-word;">
								...
								<td width="80%">
									<textarea class="form-control input-sm" rows="3" name="viewWhere" id="viewWhere" title="Where过滤"></textarea>
					    ...
					    <div class="panel-body">
					        <table class="form_commonTable">
					        	<tr>
					        		...
					        		<td width="5%">
					        			<input type="checkbox" value="Y" name="pagination" id="pagination" title="允许分页"/>
					        		</td>
					        		<th width="10%" style="word-break: break-all; word-warp: break-word;">
					        			<label for="selection">显示复选框</label>
					        		</th>
					        		<td width="5%">
					        			<input class="input-sm" type="checkbox" value="Y" name="selection" id="selection" title="显示复选框"/>
					        		</td>
					        	</tr>
					        	<tr>
					        		<th width="6%" style="word-break: break-all; word-warp: break-word;">
					        		...
					        		<td width="5%">
					        			<input class="input-sm" type="checkbox" value="Y" name="gridHeader" id="gridHeader" title="显示表头"/>
					        		</td>
					        		<th width="10%" style="word-break: break-all; word-warp: break-word;">
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
				...
				/*****************************表单基本信息*************************************/
		        eformFormViewDetail.formValidate($('#addFormView'));
		        //绑定确认按钮单机事件
		        $('#eformViewInfo_saveForm').bind('click', function () {
		            eformFormViewDetail.subAdd($("#addFormView"), $("#jqGridColumnInfo"), $("#jqGridCustomButton"), $("#jqGridBasicSearch"), $("#jqGridAdvenceSearch"));
		        });

				//绑定返回按钮单机事件
		        $('#eformViewInfo_closeForm').bind('click', function () {
		            eformFormViewDetail.closeDialog("add");
		        });
				...
				//实例基本查询jqgrid
				//eformFormViewDetail.instanceJqGridBasicSearch($("#jqGridBasicSearch"));

				//实例高级查询jqgrid
				//eformFormViewDetail.instanceJqGridAdvenceSearch($("#jqGridAdvenceSearch"));

		        //绑定事件
		        $("#tableId").on("change",function(){
					eformFormViewDetail.initTableColumeSelect("tableId", "viewInfoId", "orderCol","orderColHidden","add");
					//eformFormViewDetail.loadColumnData("tableId","viewInfoId",$("#jqGridColumnInfo"),"add");
					
		        });

				/*****************************按钮*************************************/
				//实例按钮jqgrid
				eformFormViewDetail.instanceJqGridCustomButton("viewInfoId", $("#jqGridCustomButton"), "add", "custumButtonToolbar");
				//绑定按钮事件
				...
				$("#jqGridCustomButtonDelete").on("click", function(){
					eformFormViewDetail.deleteOperation($("#jqGridCustomButton"),"","add","customButton");
		        });
				...
				// 	eformFormViewDetail.deleteOperation($("#jqGridBasicSearch"),"","add","");
		        // });
				...
				$("#jqGridAdvenceSearchDelete").on("click", function(){
					eformFormViewDetail.deleteOperation($("#jqGridAdvenceSearch"),"","add","");
		        });
