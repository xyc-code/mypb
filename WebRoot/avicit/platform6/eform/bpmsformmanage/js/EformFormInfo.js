...
EformFormInfo.prototype.init = function () {
    ...
    var template = $(
        ...
        + '<div class="copyFormInfo"><i class="glyphicon glyphicon-copy"></i>复制表单</div>'
...
//页面新建一个EformFormInfo
EformFormInfo.prototype.setFormInfo = function (data, searchFormInfoDiv, initFlag) {
    var _this = this;
...
/*			+'<tr><td width="40%" height="25"><lable>创建时间:</lable></td><td>'+data.creationDate+'</td></tr>'*/
			+'<tr><td width="40%" height="25"><lable>修改时间:</lable></td><td>'+new Date(data.lastUpdateDate).format("yyyy-MM-dd hh:mm:ss")+'</td></tr>'
			+'</table>';
    ...
    formInfo.find(".copyFormInfo").click(function () {
        _this.copyData(data.id);
        return false;
    });
    if (data.publishStatus == "1") {
...
//复制
EformFormInfo.prototype.copyData = function (eformFormInfoId) {
    var _this = this;

    copyDialog = layer.open({
        type: 2,
        title: '复制电子表单',
        skin: 'bs-modal',
        area: ['100%', '100%'],
        maxmin: false,
        content: "platform/eform/bpmsManageController/copyEformFormInfo?id=" + eformFormInfoId
    });
};
...
EformFormInfo.prototype.reImportEformXml = function (data) {
    ...
    uploadDialog = layer.open({
        ...
        area: ['400px', '200px'],
        maxmin: false,
        content: "avicit/platform6/eform/bpmsformmanage/eformFormInfo/reImportEform.jsp?formId=" + data.id+"&currentVersion=" + data.currentVersion+"&eformComponentId=" + data.eformComponentId +"&publishStatus=" + data.publishStatus
    });
};
//切换目录
EformFormInfo.prototype.changeDirectory = function (eformFormInfoId) {
    ...
    changeDirectoryDialog = layer.open({
        ...
        area: ['30%', '60%'],
        maxmin: false,
        content: "avicit/platform6/eform/bpmsformmanage/eformFormInfo/changeFormDirectory.jsp?changeId=" + eformFormInfoId + "&isForm=1"
    });
};
...
EformFormInfo.prototype.subAdd = function (form,colData,labelCount) {
    ...
    var configColData = JSON.stringify(colData);
...
//提交复制
EformFormInfo.prototype.subCopy = function (form, oldFormCode) {
    var _this = this;
    ...
    var parm = "formDataJson=" + formDataJson + "&oldFormCode=" + oldFormCode;
    $.ajax({
        url: "platform/eform/bpmsManageController/subCopyEformFormInfo",
        data: parm,
        type: "post",
        async: false,
        dataType: "json",
    ...
    $.ajax({
        ...
        success: function (backData) {
            ...
            for (var i = 0; i < formInfoList.length; i++) {
                _this.setFormInfo(formInfoList[i], undefined, true);
            }
        }
    });
};
...
EformFormInfo.prototype.toEformLog = function (resourceId) {
    var _this = this;

    logoDialog = layer.open({
        type: 2,
        title: '日志',
        skin: 'bs-modal',
        area: ['60%', '60%'],
...
EformFormInfo.prototype.reloadGrid = function () {
    if (eformFormViewModel!=null && typeof(eformFormViewModel)!="undefined"){
        eformFormViewModel.reLoad(eformComponent.selectedEformComponentId);
    }
}
