...
EformFormSearch.prototype.searchEform = function (searchParm) {
    ...
    if (showType == "1") {
        $.ajax({
            ...
            success: function (backData) {
                ...
                for (var i = 0; i < formInfoList.length; i++) {
                    eformFormInfo.setFormInfo(formInfoList[i], _this.searchFormInfoDiv,true);
                }
            }
        });
        ...
        $.ajax({
            ...
            success: function (backData) {
                ...
                for (var i = 0; i < formViewList.length; i++) {
                    eformFormView.setViewInfo(formViewList[i], _this.searchFormViewDiv,true);
                }
            }
        });

        var hasNonFormInfo = bpmsFormInfo.getFormInfoList('null',searchParm);
        if(!hasFormInfo && hasNonFormInfo){
            searchTabIndex = 1;
        }
        $('.nav-tabs>li').eq(searchTabIndex).trigger("click");
    }else{
        ...
        if (bpmsformInfoModel==null || bpmsformInfoModel==undefined){
            ...
            $('#bpmFormModel_keyWord').attr('placeholder', '请输入' + searchSubTips[0] + '或' + searchSubTips[1]);
        ...
        setTimeout(function () {
            var formDatas = $("#formViewModel").jqGrid("getRowData");

            var hasNonFormInfo = bpmsFormInfo.getFormInfoList('null',searchParm);
            if(formDatas.length == 0 && hasNonFormInfo){
                searchTabIndex = 1;
            }
            $('.nav-tabs>li').eq(searchTabIndex).trigger("click");
        },400);
