...
<div id="eformDiv" data-options="region:'center',split:true,border:false,width:fixwidth(1.0,e),height:fixheight(1.0,e),onResize:function(a){$('#dbCols').setGridHeight(getJgridTableHeight($('#eformDiv')-168));$('#dbCols').jqGrid('resizeGrid');} ">
    <form id='addForm'>
        ...
        <div class="form_commonTable" fit="true">
            <div class="toolbar">
                <div class="toolbar-left">
                    ...
                    <div class="dropdown">
                        <label for="configColCount" style="display: inline-block;vertical-align: middle;line-height: 32px;">每行列数：</label>
                        <input type="number" id="configColCount" name="configColCount" title="每行列数" class="form-control input-sm" style="display: inline-block;width: 70px;" value="4" min="2" max="10">
                    </div>
...
<script type="text/javascript">
    ...
    function jqgridInit(){
        ...
        $(grid_selector).jqGrid({
            ...
            colModel: [{
                ...
                editable: true,
                edittype: "custom",
                editoptions: {custom_element : pluginElem,
                    custom_value : pluginValue,
                    dataEvents:[{type:'change',fn:function(e){
                            ...
                            if(colTypeMap[value] == undefined){                             //新建字段控件改变字段类型
                                ...
                                if(value == "date"){
                                    ...
                                    $(grid_selector).jqGrid('setCell', rowid, "colLength", '255');
                                }else{
                                    $(grid_selector).jqGrid('setCell', rowid, "colLength", '255');
                                }
                                if(value != "select" && value != "radio" && value != "check"){
                                    $("#"+rowid+"LookupTypeName").val(null);
                                    $(grid_selector).jqGrid('setCell', rowid, "LookupType", null);
                                }
                                $(grid_selector).jqGrid('setCell', rowid, "colDecimal", '');
        ...
        avicAjax.ajax({
            ...
            success: function (r) {
                if (r != null) {
                    ...
                    for (var i = 0; i < colList.length; i++) {
                        ...
                        if (colList[i].colIsSys != "Y") {
                            dataTable.push({
                                ...
                                "colDecimal":colList[i].attribute02
                            })
                        }
                    }
                    ...
                    $("input[name=colName]").on("keyup change", function () {
    ...
    function drowColspan(maxColSpan) {
        if(maxColSpan < 2){
            $("#configColCount").val(2)
            maxColSpan = 2;
        }else if(maxColSpan > 10){
            $("#configColCount").val(10)
            maxColSpan = 10;
        }
        var spanValue = '';
    ...
    function verification(row) {
        ...
        /*var colLabelContent = row.colLabel;
        if(verifyIsSpecial(colLabelContent)){
            layer.msg("字段描述不能包含特殊字符！");
            return false;
        }*/
    ...
    document.ready = function () {
        ...
        if(parent.FEformEditor){       //判断入口为重新设计
            ...
            if(parent.EformEditor){
                //获取上次配置信息
                if(parent.EformEditor.configColData != null && parent.EformEditor.configColData != undefined){
                    $("#configColCount").val(parent.EformEditor.configColCount);
                    $("#dbCols").setGridParam({data: parent.EformEditor.configColData}).trigger('reloadGrid');
                }else{
                    $.ajax({
                        ...
                        success: function (backData) {
                            if (backData.result == "1" && backData.configColCount != null && backData.configColData != null) {
                                $("#attrArea").html("");
                                $("#configColCount").val(backData.configColCount);
                                $("#dbCols").setGridParam({data: backData.configColData}).trigger('reloadGrid');
                            }else{
        ...
        $('#saveForm').bind('click', function () {
            ...
            $.each(rows,function(i,v){
                v.LookupTypeName = $("#"+$(v.LookupTypeName).find("input[type='text']").attr("id")).val();
                v.colLabel = $("#"+$(v.colLabel).attr("id")).val();
                //v.colLabel = "";
                v.colName = $("#"+$(v.colName).attr("id")).val();
                v.colspan = $("#"+$(v.colspan).attr("id")).val();
                v.plugins = $("#"+$(v.plugins).find("select").attr("id")).val();
            ...
            if(!saveVerification){
                if(parent.FEformEditor){
                    $.ajax({
                        url: "platform/eform/bpmsManageController/updateEformConfigColData",
                        data: {
                            eformId: parent.EformEditor.eformFormInfoId,
                            configColData: JSON.stringify(rows),
                            configColCount: $("#configColCount").val()
                        },
                        type: "post",
                        async: false,
                        dataType: "json",
                        success: function (backData) {
                            if(backData == 1){
                                parent.EformEditor.redesign("add",rows,$("#configColCount").val(),$("#dataSourceName").val());
                            }
                        }
                    })
                }else{
                    parent.eformFormInfo.subAdd($("#addForm"),rows,$("#configColCount").val());
                }
            }
        });
        ...
        $('#toAdd').bind('click', function () {
            ...
            $(grid_selector).jqGrid('addRowData', rows.length+1, {colLength:255,colDecimal:0,colIsVisible:"YES",plugins:"text",colName:colName+(newNumber-1),colLabel:colName+(newNumber-1),colType:"VARCHAR2",colspan:"1",isNew:true}, 'last');
            ...
            var selectDialog = openDialog({
                ...
                submit : function(index, layer) {
                    ...
                    for(var i = 0; i< r.length; i++) {
                        ...
                        rowData.colName = ele.colName.toUpperCase();  //字段名称
                        rowData.plugins = ele.colDomType;
                        rowData.colType = ele.colType;
                        rowData.colIsVisible = 'YES';
                        rowData.colspan = '1';
                        rowData.isNew = true;
                        rowData.LookupType = ele.lookupType;
                        rowData.LookupTypeName = ele.lookupTypeName;
                        rowData.colDecimal = ele.colDecimal; //字段长度

                        $(grid_selector).jqGrid('addRowData', rows.length+1+i, rowData, 'last');
                        $(grid_selector).jqGrid("editRow", rows.length+1+i, {
                            keys: false
                        })
        ...
        $('#toPreview').bind('click', function () {
            ...
            if(parent.FEformEditor){
                parent.EformEditor.temppreview(rows,$("#configColCount").val());
            }else{
                temppreview(rows,$("#configColCount").val());
            }
        ...
        $("#configColCount").on('change',function(e){
            drowColspan($("#configColCount").val());
        });
    ...
    function dictElem(value, options){
        ...
        $elem.find('#' + rowId + options.name+', .input-group-addon').on('click',function() {
            ...
            layer.open({
                ...
                yes: function(index, layero){
                    ...
                    $("#"+rowId+options.name).val(objData.lookupTypeName);//对其方式
                    $(grid_selector).jqGrid('setCell', rowId, "LookupType", objData.lookupType);
                    layer.close(index);
                },
    ...
    function lookupFeedback(rowObjData){
        if(row_id != null && row_id != undefined){
            $("#"+row_id+"LookupTypeName").val(rowObjData.lookupTypeName);
            $(grid_selector).jqGrid('setCell', row_id, "LookupType", rowObjData.lookupType);
            row_id = null;
        }
    }
    ...
    */
    ...
    function pluginElem(value, options) {
        var el = document.createElement("select");
        el.style = 'width:80px;';
        el.className = 'editable inline-edit-cell form-control';
        var isNew = $(grid_selector).jqGrid("getRowData",options.rowId).isNew;
        var colTypeName = "all";
        if(isNew != "true"){
            if(value == "text"){
        ...
        var colDomStr = colType[colTypeName];
        var colDomList = colDomStr.split(";");
        for(var i = 0; i< colDomList.length; i++){
            var colDom = colDomList[i].split(":");
            var optel = document.createElement("option");
            optel.innerHTML = colDom[1];
            optel.value = colDom[0];
            optel.title = colDom[1];
            if(value != null && value != undefined && value == colDom[0]){
                optel.selected = true;
            }
    ...
    function pluginValue(elem, operation, value) {
        return elem.val();
    }
