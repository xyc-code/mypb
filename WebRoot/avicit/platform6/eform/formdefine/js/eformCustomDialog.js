...
function openCustomDialog(colName,url,callBack,ynMax,width,height,paralist){
	...
	var buttons = [{
		...
		handler:function(){
			 ...
			 if (typeof(frm.commitData) === 'function'){
				 ...
				 if (jsonData){
				 	 if(paralist){
						var paraObjList = $.parseJSON(paralist);
						for(var i = 0; i< paraObjList.length; i++){
							var paraObj = paraObjList[i];
							$("#"+paraObj.mainCol).val(eval("jsonData."+paraObj.returnCol));
						}
					 }else{
						 if (jsonData.hasOwnProperty("name") && jsonData.hasOwnProperty("value")){
							 ...
							 $.messager.alert('提示',"弹出页返回参数错误1！",'error');
							 return;
						 }
					 }
				 }else{
...
function openCustomDialogLayer(colName,url,callBack,ynMax,width,height,paraList){
    var backValue = $("#"+colName).val();
    if (backValue){
        url = url + "?backValue=" + backValue;
    }
    ...
    selectDialog = top.layer.open({
        ...
        yes: function(index, layero){
            ...
            if (typeof(frm.commitData) === 'function'){
                ...
                if (jsonData){
                	if(paraList){
						var paraListObj = $.parseJSON(paraList);
						for(var i = 0; i< paraListObj.length; i++){
							paraObj = paraListObj[i];
							$("#"+paraObj.mainCol).val(eval("jsonData." + paraObj.returnCol));
						}
					}else{
						if (jsonData.hasOwnProperty("name") && jsonData.hasOwnProperty("value")){
