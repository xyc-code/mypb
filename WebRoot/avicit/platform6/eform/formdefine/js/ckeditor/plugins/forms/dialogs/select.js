...
CKEDITOR.dialog.add( 'select', function( editor ) {
	...
	return {
		...
		onOk: function() {
			...
			element.setAttribute("data-cke-editable","1");
			if ( isInsertMode ) {
		...
		contents: [ {
			...
			},
			{
				...
				children: [ {
					...
					setup: function( name, element ) {
						if ( name == 'select' )
							if (element.getStyle( 'width' ))
								this.setValue( element.getStyle( 'width' ).substring(0,element.getStyle( 'width' ).length-2) || '' );
							else 
								this.setValue( '173' );
					...
					commit: function( element ) {
						if ( this.getValue() )
							element.setStyle('width',this.getValue()+'px');//.setAttribute( 'size', this.getValue() );
						else
