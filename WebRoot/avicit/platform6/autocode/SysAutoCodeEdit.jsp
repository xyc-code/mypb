...
<body class="easyui-layout" fit="true">
				...
				<div class="panel-body">
					<table class="form_commonTable" style="margin-bottom:10px">
						<tr>
							...
							<%--是否使用跳码--%>
							<th width="5%" style="white-space:nowrap;position: relative;">
								是否使用跳码：<input type="checkbox" id="isUsedJump" />
								<i class="tips-iconhelp icon iconfont icon-question-circle" id="isUsedJumpTips"></i>
							</th>
							<td width="12%"></td>
						</tr>
					</table>
				...
				<table class="table-striped form_commonTable" style="position:relative;width: 97%;margin:0 20px;" id="segmentTable" >
					<c:forEach items="${segmentList}" var="segment" varStatus="status">
						<tr id="segmentTr_${status.index + 1}" style="position:relative;height: 50px;">
                            ...
                            <%--<c:if test="${segment.segmentType eq '2'}">
                                <th width="5%" style="white-space:nowrap;">
                                    <label for="isJump">是否跳码</label>
                                </th>
                                <td width="6%" style="white-space:nowrap;">
                                    <select class="form-control input-sm isJump" id="isJump_${status.index + 1}" name="isJump" title="是否跳码">
                                        <option value="Y" <c:if test="${segment.isJump eq 'Y'}">selected</c:if>>是</option>
                                        <option value="N" <c:if test="${segment.isJump eq 'N'}">selected</c:if>>否</option>
                                    </select>
                                </td>
                            </c:if>
                            <c:if test="${segment.segmentType ne '2'}">
                                <th width="5%" style="white-space:nowrap;">
                                </th>
                                <td width="6%" style="white-space:nowrap;">
                                </td>
							...
							<c:if test="${segment.segmentType eq '2'}">
								<td id="isJumpTd">
									<input type="hidden" id="isJump" name="isJump" value="${segment.isJump}"/>
								</td>
							</c:if>
						</tr>
					</c:forEach>
	...
	function initTips() {
		var tipsIndex;
		$("#isUsedJumpTips").mouseover(function () {
			var message = "<span style='color:#333333;'>跳码开关：跳码只和流水码有关<br/>开启跳码开关后，新生成的流水码会自动从小到大补充删除的流水码；<br/>关闭后，新生成流水码会在已存在的最大码值上+1。</span>";
		...
		$(document).ready(function () {
			...
			$(".segmentType").on("change", function (event, initFlag) {
				parent.sysAutoCode.layoutSetting(event,initFlag,$("#codeSegmentForm"),_isJumpV);
			});
	...
	$('#isUsedJump').bootstrapSwitch({
		...
		onSwitchChange:function(event,state){//控件状态改变时触发
			if (state.value) {
				$('#isJump').val('Y');
				parent.sysAutoCode.currentJump = 'Y';
			} else {
				$('#isJump').val('N');
				parent.sysAutoCode.currentJump = 'N';
			}
		}
	});
	$('#isUsedJump').on('switchChange.bootstrapSwitch', function(event, state) {
		if (state.value) {
			$('#isJump').val('Y');
			parent.sysAutoCode.currentJump = 'Y';
		} else {
	...
	if($('#isJump').val() == 'Y'){
		$('#isUsedJump').attr("checked","checked");
