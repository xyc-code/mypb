...
AutoCode.prototype.init = function () {
    ...
    var editHtml = $('<div id="" style="position: relative">'+
                        '<div class="selector-div" style="float: none">'+
                            '<input type="hidden" name="autoCodeValue" id="autoCodeValue"/>'+
                            '<input class="inputbox easyui-validatebox validatebox-text selector-div-input" type="text" name="autoCode" id="autoCode" readonly/>'+
                            '<span class="input-right-icon" id="autoCode_edit">'+
                                /*'<a href="javascript:;" class="spin-down" data-spin="down" ><i class="glyphicon glyphicon-triangle-bottom"></i></a>'+*/
                            '</span>'+
                        '</div>'+
                        '<div style="height:100px;border:1px solid #ccc;display:none;position:absolute;background-color:White;z-index:9" id="displayContentDiv">'+
                            '<table class="form_commonTable autocodetable" style="border: 0; height: 50px; margin:5px;" id="displayContentTable">' +
                                '<tr id="displayContentTr">'+
                               //'<td><input class="form-control input-sm" type="text" name="" id="editAutoCode_"/></td>'+
                               '</tr>'+
                            '</table>'+
                            '<div id="toolbar" class="datagrid-toolbar datagrid-toolbar-extend foot-formopera">' +
                                '<table class="tableForm" style="border: 0; cellspacing: 1;height: 40px;width:100%">'+
                                    '<tr>'+
                                        '<td></td><td width="160px" style="padding-right: 2%;">'+
                                            '<a href="javascript:void(0)" class="easyui-linkbutton primary-btn" title="确认" id="autoCode_save">确认</a>'+
                                            '<a href="javascript:void(0)" class="easyui-linkbutton"  title="返回" id="autoCode_close">返回</a>'+
                                        '</td>'+
                                    '</tr>'+
                                '</table>'+
                            '</div>'+
    ...
    var noEditHtml = $('<div>'+
                        '<input class="inputbox easyui-validatebox" type="text" name="autoCode" id="autoCode" readonly/>'+
                        '<input type="hidden" name="autoCodeValue" id="autoCodeValue"/>'+
                        '</div>');
...
AutoCode.prototype.drawEditHtml = function(){
	...
	$.each(_this.segmentData, function (idx, obj) {
		if(obj.segmentType == "1"){//分类码
			elemHtml = $('<select class="easyui-combobox autocodeclass" data-options="{panelHeight:\'auto\'}" id="" style="width:100px;" name="classifCode"></select>');
			$(JSON.parse(obj.codeValue)).each(function(i, elem){
			...
			if(obj.autoCodeEditFlag == "1"){
				elemHtml = $('<input class="easyui-validatebox autocodeclass" style="width:100px" type="text" id="" name="serialCode" title="流水码"/>');
			...
			if(obj.autoCodeEditFlag == "1"){
                elemHtml = $('<input class="easyui-validatebox autocodeclass" style="width:100px" type="text" id="" name="dateCode"  title="日期码"/>');
                elemHtml.val(obj.codeValue);
			...
			if(obj.segmentValue == "1"){
				elemHtml = $('<input class="easyui-validatebox autocodeclass" style="width:100px" type="text" id="" name="inputCode" title="输入码-字符串"/>');
			}else if(obj.segmentValue == "2"){
                elemHtml = $('<input class="easyui-validatebox autocodeclass" style="width:100px" type="text" id="" name="inputCode" title="输入码-数值"/>');
                
			}else{
				elemHtml = $('<input class="easyui-datebox autocodeclass" editable="false" style="width:110px" type="text" name="inputCode" id="" title="输入码-日期"/>');
        ...
        if(obj.autoCodeEditFlag == "1"){//可编辑
            ...
            if(obj.segmentType == "2"){
                ...
                if(obj.autoIncreaseFlag == "Y") {
                    ...
                    if (obj.segmentValue < data[idx].length) {
                        validateResult = false;
                        layer.alert("流水码的最大长度是" + obj.segmentValue + "个数字！", {icon: 7,title: "提示",closeBtn: 0,area: ['400px', '']}, function (index) {
                            _this.divId.find("#editAutoCode_" + (idx + 1)).focus();
                            layer.close(index);
                        });
                        return false;
...
AutoCode.prototype.setCss = function () {
    ...
    _this.divId.find('#toolbar').css("width", _this.divId.find('#displayContentDiv').css("width"));
...
AutoCode.prototype.appendToDiv = function () {
    var _this = this;
    //解析easyui
    $.parser.parse(_this.template);
    _this.template.find('.form_commonTable input[class*="validatebox-text"]').each(function(index, elem){
        $(elem).css({"cssText": "width:" +$(elem).width()+"px!important"});
    });
...
AutoCode.prototype.getAutoCodeDataFromLocal = function(){
    ...
    $.each(_this.template.find(".autocodeclass"), function (idx, elem) { 
        ...
        }else if($(elem).next(".combo").find(".combo-value").length > 0){
            data.push($.trim($(elem).next(".combo").find(".combo-value").val()));
        }else{
