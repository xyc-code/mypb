...
SysAutoCode.prototype.save=function(codeForm,codeSegmentForm,windowName){
	...
	for(var i = 0; i < segmentArray.length ; i++){
		...
		if(segment.isJump != '' && segment.isJump !=undefined){
			isJumpValue = segment.isJump;
		}
		if(segment.segmentType === '2'){
			//设置是否跳码
			segment.isJump = isJumpValue;
...
SysAutoCode.prototype.upTr = function(event){
	...
	if(prevTr.length > 0){
		...
		currentTr.find("input,select,a,td>div").each(function(idx,e){
			...
			//设置是否使用跳码值
			if(($(this).attr("id") == 'isJump')){
				$(this).val((_self.currentJump == undefined) ? 'N' : _self.currentJump);
			}
...
SysAutoCode.prototype.layoutSetting = function(event,initFlag,codeSegmentForm){
	...
	if(segmentTypeValue == "1"){//分类码
		...
		siblings.find("#autoIncreaseFlag_" + idArray[1]).prop("disabled", false);
		//设置是否使用跳码
		var currentJump = _self.currentJump;
		currentJump = (currentJump == undefined) ? 'N' : currentJump;
		$(siblings[len]).append('<input type="hidden" id="isJump" name="isJump" value="'+ currentJump +'"/>');
...
SysAutoCode.prototype.previewResult = function(data, previewContent){
	...
	$.each(data, function (idx, obj) {
		if(obj.segmentType == "1"){//分类码
			...
			if(obj.autoCodeEditFlag == "1"){
				elemHtml = $('<input class="form-control input-xs autocodeclass" style="width:100px" type="text" id="" name="dateCode" title="日期码" value="'+obj.codeValue+'"/>');
 ...
 * 导入
 */
SysAutoCode.prototype.importData = function () {
	var _self = this;
	layer.open({
		type : 2,
		area : [ '70%', '70%' ],
		...
		skin : 'bs-modal', // bootstrap 风格皮肤 需加载skin
		maxmin : false, //开启最大化最小化按钮
		content : _self._url + 'importManage',
 ...
 * 导出
 */
SysAutoCode.prototype.exportData = function () {
    var rows = $(this._datagridId).jqGrid('getGridParam', 'selarrrow');
    var _self = this;
    var url = _self._url + 'export';
    var l = rows.length;
    if (l > 0) {
    	var ids = [];
        for (; l--;) {
