...
<script type="text/javascript">
                ...
                if ($("#viewType").attr("checked") == "checked") {
                    ...
                    for (var i = 0; i < useridlist.length; i++) {
                        ...
                        if(rowData.name.indexOf("<") != -1){
                        	 var   name = rowData.name.replaceAll(">", "&gt;");
                        ...
                        h5KadsView.add(rowData.id, [rowData.id, name, rowData.deptid, rowData.deptname, rowData.orgIdentity,rowData.loginName]);
                        }else{
                        h5KadsView.add(rowData.id, [rowData.id, rowData.name, rowData.deptid, rowData.deptname, rowData.orgIdentity,rowData.loginName]);
                        }
    ...
    function addToSelectedUser(rowId, rowData) {
        ...
        if (viewType == "H5KadsView") {
            ...
            //修改存在同部门同名无法识别的问题，增加编号标识 这里为单选
            rowData.name = rowData.name + "("+rowData.loginName+")"
            if (!exist) {
    ...
    function addToSelectedUserBatch(addingData) {
        ...
        //修改存在同部门同名无法识别的问题，增加编号标识 这里为多选
        for (var i = 0; i <addingData.length ; i++) {
            addingData[i].name = addingData[i].name + "("+addingData[i].loginName+")"
        }
        if (viewType == "H5KadsView") {
            var temp = h5KadsView.template();
