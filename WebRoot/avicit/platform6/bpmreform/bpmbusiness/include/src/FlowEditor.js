...
BpmRetreat.prototype.getHtml = function() {
	...
	if (this.isEnable()) {
		...
		if(this.doretreattodraft != null){
			var li_draft = $('<li></li>');
			var a_draft = $('<a href="javascript:void(0);">' + (this.doretreattodraft.alias || this.doretreattodraft.lable) + '</a>');
		...
		if(this.doretreattoprev != null){
			var li_prev = $('<li></li>');
			var a_prev = $('<a href="javascript:void(0);">' + (this.doretreattoprev.alias || this.doretreattoprev.lable) + '</a>');
	...
	avicAjax.ajax({
		...
		success : function(result) {
			if (flowUtils.notNull(result.error)) {
				...
				_self.validJsonData(result.taskUserSelect);
			}
		}
	});
};
 ...
 * @param taskUserSelect
 */
BpmSupersede.prototype.validJsonData = function(taskUserSelect) {
	if (flowUtils.notNull(taskUserSelect)) {
		// 初始化路由对象
		this.conditions = this.getConditions(taskUserSelect);
		if (this.validIdeaMust()) {
			new BpmActor(this.data, taskUserSelect, this).open();
		}
	}
};
/**
 ...
 * @returns {Boolean}
 */
BpmSupersede.prototype.validIdeaMust = function() {
	if(this.isEvent){
		return true;
	}
	if (this.conditions.isNeedZbIdea()) {// 意见必须填写
		var ideaText = this.getIdeaText();
		if (!flowUtils.notNull(ideaText)) {
			flowUtils.warning('请填写流程意见');
			this.focusIdeaText();
 ...
 * @param users
 */
BpmWithdrawpart.prototype.submit = function (data, users) {
    if (!flowUtils.notNull(users) || users.length == 0) {
        flowUtils.warning("选人错误");
        return;
    }
	...
	top.layer.open({
		...
		yes : function(index, layero) {
			...
			if((selectedUsers == null || selectedUsers.length == 0)
			  && _self.data.type != 'stepUserDefined'){
