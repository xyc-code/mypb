	...
	<div class="toolbar-right">
		...
		<div id="commonSearch" class="input-group form-tool-search" >
			<input type="text" name="bpmTask_keyWord"
				id="bpmTask_keyWord" style="width:200px;"
...
<script type="text/javascript">
	...
	function selectSearchType(obj){
		...
		if('title'==selectVal){
			$("#commonSearch").html('<input type="text" name="bpmTask_keyWord"  id="bpmTask_keyWord" class="form-control input-sm" style="width:200px;" placeholder="请输入标题" onkeydown="if(event.keyCode==13){queryByKeyWord();}"> <label id="bpmTask_searchPart" class="icon icon-search form-tool-searchicon" onClick="queryByKeyWord()"></label>');
		}else if('priority'==selectVal){
			$("#commonSearch").html('<select name="priority" id="priority" class="easyui-combobox form-control input-sm" style="width:200px;" onChange="queryByPriority(this)"><option value="0">一般</option><option value="1">紧急</option><option value="2">特急</option></select>');
		}else if('acceptDate'==selectVal){
			var htmlAcceptDate = '<div class="dropdown">'
		        +'<a id="queryAcceptDateName" class="btn btn-primary form-tool-btn btn-sm form-control" role="button" style="text-align:center;align:center;width:200px;"  href="javascript:void(0);" data-toggle="dropdown" id="dropdownMenu">时间不限 <span class="caret"></span></a>'
	...
	function getTraceButtons(cellvalue, options, rowObject){
		...
		var _dataContent = "<table width='240px' border='0' style='font-size:12px;' align='center'>"
			...
			+"</table>";
		var html = rowObject.originalTaskTitle;
		if(cellvalue=='' || cellvalue=='null'){
			html = rowObject.activityalias_;
		}
		if(rowObject.sendType && rowObject.sendType.startsWith("doretreatto")){
			html = "退回：" + html;
		}
		if(rowObject.sendType && rowObject.sendType.startsWith("dowithdraw")){
			html = "拿回：" + html;
		}
		/**
		...
		var title = "'"+encodeURIComponent(encodeURIComponent(html))+"'";
	...
	function formatTaskOpt(cellvalue, options, rowObject){
		...
		return '<a href="javascript:flowUtils.quickDoTask('+processInstance+','+executionId+','+dbid+','+businessId+','+url+','+taskTitle+','+taskType+','+taskName+')">'+'已阅'+'</a>';
	}
...
<% } %>
		...
		var url = "bpm/todo/portal/querywaittodo?taskType=1&taskFinished=0";
		bpmTask = new BpmTask('bpmTask', url, "formSub", bpmTaskGridColModel, 'searchDialogSub', '',
				'', searchSubNames, "bpmTask_keyWord",'','tab2','我的待阅',true);
