...
CommonActor.prototype.submit = function(iframeWin,compelManner){
	...
	//流程转办
	if(_self.data.event=='dosupersede'){
		if (!_self.submitObj.isEvent && _self.actorModel.currentActivityAttr.isNeedZbIdea == 'yes') {// 意见必须填写
			if (!flowUtils.notNull(ideaText)) {
				flowUtils.warning('请填写流程意见');
				iframeWin.focusIdeaText();
				return false;
	...
	if(_self.actorModel.nextTask){
		if(!_self.noNeedSelectUser(iframeWin)){
			var selectedUsers = iframeWin.getSelectedUsers();
			if(selectedUsers.length<1){
				if(_self.data.type!='stepUserDefined'){
			 ...
			 *
			 */
			if(_self.data.event=='doretreattodraft'
		     ...
		     if(workHandUsersList && workHandUsersList.length > 0){
		    	 ...
		    	 _self.workHandWindow = top.layer.open({
		    			...
		    			yes:function(index, layero){
	    					...
	    					if(_self.workHandWindow || _self.selectUserWindow){
 ...
 * 不需要选人
 */
CommonActor.prototype.noNeedSelectUser = function(iframeWin){
	var iaAll = false;
	var count = 0;
	for(var i=0;i<this.actorModel.nextTask.length;i++) {
		...
		if(_nextActorModel.activityType=='end'){
		...
		}else if(_nextActorModel.activityType=='state'){
			//增加自动节点判断，如果有自动节点并且其他分支节点没有配置必须选人并且都没有选人，则不判断选人直接提交
			var selectedUsers = iframeWin.getSelectedUsers();
			var mustSelectUser = this.isMustUser();
			if(mustSelectUser){
				return false;
			}
			if(selectedUsers.length<1){
			...
			var hasSelectUsers = false;
			for(var j=0;j<selectedUsers.length;j++){
				if(selectedUsers[j].selectedUsers.length>0){
					hasSelectUsers = true;
					break;
				}
			}
...
CommonActor.prototype.isMustUser = function(index){
	if(index>-1){
		for(var i=0;i<this.actorModel.nextTask.length;i++){
			var _nextActorModel = this.actorModel.nextTask[i];
			if(i==index){
				if(_nextActorModel.currentActivityAttr.isMustUser=="yes" && _nextActorModel.activityType != 'foreach'){
					return true;
				}else{
					return false;
				}
