 ...
 * 用户维度全选
 */
function checkAllUserList(obj){
    var checkAllObj = $(obj);
    var isCheckAll = checkAllObj.prop("checked");
    var userList = checkAllObj.parents(".userListDiv").find(".user-lsit input[type='checkbox']");
    if(userList && userList.length){
        for(var i=0;i<userList.length;i++){
            var userCheck = $(userList[i]).prop("checked");
            if(isCheckAll){
                if(!userCheck){
                    userList[i].click();
                }
 ...
 * 岗位、角色、群组维度全选
 */
function checkAllTree(obj,treeId){
    var checkAllObj = $(obj);
    var isCheckAll = checkAllObj.prop("checked");
...
$(function () {
    var height = $(".tree-nav").height();
    if(height){
        $(".multi-node").css("paddingTop", height + "px");
        $(".tree-nav").css("top", 0 - height - 5 + "px");
        $(".chose-panel").css("height", 325 - height + "px");
        $(".tree-list").css("height", 305 - height + "px");
        $(".checkedbox-list").css("height", 305 - height + "px");
        $(".tree-nav-body").css("height", 295 - height + "px");
        $(".userListDiv").css("height", 275 - height -17 + "px");
        $(".sub-tree-body").css("height", 275 - height + "px");
        $(".sub-tree-nav-panel").css("height", 275 - height + "px");
        $(".chose-panel .sub-tree-nav-panel .ztreeother").css("height", 235 - height -35 + "px");
        $(".chose-panel .sub-tree-nav-panel .ztreedept").css("height", 222 - height + "px");
    }
})
</script>
</html>
