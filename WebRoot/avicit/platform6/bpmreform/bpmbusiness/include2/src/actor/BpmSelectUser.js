...
BpmSelectUser.prototype.getNodeTabHeader = function(){
	...
	for(var i=0;i<_self.actorModel.nextTask.length;i++){
		var _nextActorModel = _self.actorModel.nextTask[i];
		if(_nextActorModel.activityType == 'state'){
			continue;
		}
...
BpmSelectUser.prototype.getNodeTabContent = function(){
	...
	for(var i=0;i<_self.actorModel.nextTask.length;i++){
		...
		html += ">\n";
		html +="		"+_self.getActorTabHeader(_nextActorModel);
...
BpmSelectUser.prototype.getActorTabHeader = function(_nextActorModel){
	var _self = this;
...
BpmSelectUser.prototype.getTree = function(type,treeId,url,_nextActorModel){
	var html = "";
	if(type=='group' || type=='role' || type=='position'){
		html+="<div class='ztree-checkall'>";
		html+="<label ><input type='checkbox' onClick='checkAllTree(this,\""+treeId+"\")'/>&nbsp;&nbsp;全选</label>";
		html+="</div>";
	}
	if(type=='group' || type=='role' || type=='position'){
		html+= "<ul id='"+treeId+"' class='ztree ztreeother'";
		html+= " style='height: 170px !important;'";
	}else{
		html+= "<ul id='"+treeId+"' class='ztree ztreedept'";
	}
...
BpmSelectUser.prototype.getUserList = function(_nextActorModel){
	var html = "";
		html+="<div class='user-lsit-checkall'>";
		html+="<label ><input type='checkbox' onclick='checkAllUserList(this)'/>&nbsp;&nbsp;全选</label>";
		html+="</div>";
