 ...
 * 退回前是否自动保存
 */
DefaultForm.prototype.isAutoSaveByRetreat = true;
 ...
 * @param userJson
 */
DefaultForm.prototype.afterSubmit = function (data, userJson) {
};
 ...
 * @returns {Boolean}
 */
DefaultForm.prototype.beforeAdduserandsubmit = function (data) {
    return true;
};
/**
 * 加签并提交后事件
 ...
 * @param data
 */
DefaultForm.prototype.afterAdduserandsubmit = function (data) {
};
/**
 * 转办前事件
 *
 ...
 * @returns {Boolean}
 */
DefaultForm.prototype.beforeWithdrawpart = function (data) {
    return true;
};
/**
 * 拿回部分待办后事件
 ...
 * @param data
 */
DefaultForm.prototype.afterWithdrawpart = function (data) {
};
/**
 * 引导按钮code
 */
...
/**
 * 上下结构审批页面，有多个提交按钮时，设置是否平铺显示
 * 返回true，表示平铺显示
 */
DefaultForm.prototype.tileSettingSubmit = function () {
};
/**
 * 上下结构审批页面，有多个退回按钮时，设置是否平铺显示
 * 返回true，表示平铺显示
 */
DefaultForm.prototype.tileSettingRetreat = function () {
};
;
...
;
///<jscompress sourcefile="BpmAdduserandsubmit.js" />
/**
 * 流程加签并提交
 */
		...
		success : function(layero, index) {
			$("#ideaTable").jqGrid({
				...
				colModel : [ {
					...
					editoptions:{custom_element:timeElemWithSecond,custom_value:timeValue}
				}, {
					label : '操作类型',
					name : 'opType',
					align : 'center'
...
BpmRetreat.prototype.executeTodraft = function() {
	...
	var _self = this;
	// 自动保存
	if (this.flowEditor.defaultForm.isAutoSaveByRetreat && this.flowEditor.bpmSave.isEnable()) {
...
BpmRetreat.prototype.getHtml = function() {
	...
	if (this.isEnable()) {
		...
		//有多个退回按钮时，设置是否平铺显示
		if(this.defaultForm.tileSettingRetreat()){
			temp_int = 1;
		}
		if(temp_int > 1){
			if(this.doretreattodraft != null){
				var label = $("#" + this.retreattodraftDomeId).find("span").text();
				if(this.doretreattodraft && this.doretreattodraft.alias){
					label = this.doretreattodraft.alias;
				}
				var li = $('<li class="sub-list-li"><a href="javascript:void(0);" title="'+label+'">' + label + '</a></li>');
				li.children("a").on("click", function() {
			...
			if(this.doretreattoprev != null){
				...
				if(this.doretreattoprev && this.doretreattoprev.alias){
					label = this.doretreattoprev.alias;
				}
				var li = $('<li class="sub-list-li"><a href="javascript:void(0);" title="'+label+'">' + label + '</a></li>');
				li.children("a").on("click", function() {
					_self.executeToprev();
			...
			if(this.doretreattodraft != null){
				if(this.doretreattodraft && this.doretreattodraft.alias){
					var label = this.doretreattodraft.alias;
					$("#" + this.retreattodraftDomeId).find("span").text(label);
					$("#" + this.retreattodraftDomeId).find("a").attr("title", label);
				}
			...
			if(this.doretreattoprev != null){
				if(this.doretreattoprev && this.doretreattoprev.alias){
					var label = this.doretreattoprev.alias;
					$("#" + this.retreattoprevDomeId).find("span").text(label);
					$("#" + this.retreattoprevDomeId).find("a").attr("title", label);
				}
...
;
///<jscompress sourcefile="BpmWithdrawpart.js" />
/**
 * 拿回部分待办
 */
...
FlowEditor.prototype.drawButtons = function (json) {
    ...
    this.bpmWithdrawpart = new BpmWithdrawpart(this, this.defaultForm, buttonData);
    this.bpmAdduser = new BpmAdduser(this, this.defaultForm, buttonData);
    this.bpmAdduserandsubmit = new BpmAdduserandsubmit(this, this.defaultForm, buttonData);
 ...
 * @type {boolean}
 */
FlowEditor.prototype.needRefreshTrack = false;
/**
 * 刷新文字跟踪
 */
FlowEditor.prototype.refreshTrack = function () {
    this.needRefreshTrack = true;
    if($("#bpm_track_tab").is(':hidden')){
        return;
    }
    this.needRefreshTrack = false;
    if(flowUtils.notNull(this.flowModel.entryId)){
