		...
		<div id="toolbar_bpmProcInst" class="toolbar">
			<div class="toolbar-left">
		        ...
		        <sec:accesscontrollist hasPermission="3"
					domainObject="bpmProcinst_button_update" permissionDes="修改">
					<a id="bpmProcinst_button_update" href="javascript:void(0)" class="btn btn-default form-tool-btn typeb btn-sm"
		                   role="button" title="修改"><i class="icon icon-edit"></i> 修改</a>
	...
	<form id="formSub">
		...
		<table class="form_commonTable" >
			...
			<tr>
				...
				<td width="30%">
					<div class="input-group  input-group-sm">
						...
						<input class="form-control" placeholder="请选择用户" type="text"
							id="receptUserName" name="receptUserName" readonly> <span
				...
				<td width="30%">
					<div class="input-group  input-group-sm">
							      ...
							      <input class="form-control" placeholder="请选择部门" type="text" id="applyDeptidAlias" name="applyDeptidAlias" readonly>
			...
			<tr>
				<th width="15%">是否挂起：</th>
				<td width="30%">
					<select name="suspended" id="suspended" class="easyui-combobox form-control input-sm">
						<option value="">请选择</option>
						<!-- <option value="start">拟稿中</option> -->
...
<script src="avicit/platform6/bpmreform/bpmprocess/js/BpmCatalogTree.js"
	type="text/javascript"></script>
...
<script type="text/javascript">
	...
	function selectSearchType(obj){
		...
		if('title'==selectVal){
		...
		}else if('state'==selectVal){
			$('#commonSearch').attr("style","border:0;");
		    var html = '<select name="businessState" id="businessState" onChange="queryByState(this)" class="easyui-combobox form-control input-sm" style="width:150px;">'
			 			  +'<option value="">请选择</option>'
						  +'<option value="start">拟稿中</option>'
						  +'<option value="active">流转中</option>'
						  +'<option value="ended">已完成</option>'
					  +'</select>';
		    $("#commonSearch").html(html);
			...
			var cumtomHtml = '<div class="dropdown">'
		        ...
		        +'	&nbsp;从&nbsp;<input class="date-picker inputborder" style="width:100px;font-size:12px;" type="text" '
				+'	name="customDateBegin" id="customDateBegin" onClick="onSelectCustomDate()"/>'
		       // +'  <span	class="input-group-addon"><i class="glyphicon glyphicon-calendar"></i></span>'
		        +'  </li>'
		        +'	<li role="presentation" style="align:center;font-size:12px;" data-stopPropagation="true">'
		        +'	&nbsp;至&nbsp;<input class="date-picker inputborder" style="width:100px;font-size:12px;" type="text"'
	...
	function queryByState(obj){
		...
		bpmProcInst.queryByState(selectVal);
	...
	function getTraceButtons(cellvalue, options, rowObject){
		...
		var title = "'"+encodeURIComponent(encodeURIComponent(cellvalue))+"'";
		var dbid = "'"+rowObject.dbId+"'";
		var extType = "'2'";
		return '<a href="javascript:flowUtils.detail('+dbid+','+extType+','+ title +')">'+cellvalue+'</a>';
		//return cellvalue;
	}
	...
	$(document).ready(function() {
		...
		bpmCatalog = new BpmCatalogTree('treeDemo', 'bpm/monitor/getFlowModelTree?needPermission=1', '', '', '',
				function(nodeId,nodeType,pdId) {
					var url = "bpm/process/searchManageHistProcessByPage?type=3";

					bpmProcInst = new BpmProcInst('bpmProcInst', url, "formSub", bpmProcInstGridColModel, 'searchDialogSub', nodeId, nodeType, searchSubNames, "bpmProcInst_keyWord",pdId,'','');
				},
				function(nodeId,nodeType,pdId) {
					bpmProcInst.reLoad(nodeId,nodeType,pdId);
				}
