...
BpmProcInst.prototype.afterQuery = function(data){
	if(data.records!=undefined && data.records!=null){
		if(this.tabId == "tab1"){
			...
			if(data.startCount!=undefined && data.startCount!=null){
				var tabName1 = '我的申请'+'(<span style="color:red">'+data.startCount+'</span>)';
				$('#tab2', window.parent.document).html(tabName1);
			}
			if(data.doneCount!=undefined && data.doneCount!=null){
				var tabName2 = '我的经办'+'(<span style="color:red">'+data.doneCount+'</span>)';
				$('#tab3', window.parent.document).html(tabName2);
			}
...
//重载数据
BpmProcInst.prototype.reLoad = function(nodeId,nodeType,pdId,refrash) {
	if (nodeId != undefined) {
		this.nodeId = nodeId;
	}
