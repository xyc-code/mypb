	...
	if (histArray != null && histArray.length > 0) {
		$.each(histArray, function(i, xywh){
			...
			if(_iconType == "1"){
				...
				var newStyle = _self.editor.graph.model.getStyle(cell).replace('/48/norun/','/48/');
		...
		for (var i = 0; i < lineArray.length; i++) {
			...
			if (flowUtils.notNull(start, end)) {
				...
				if (edgeArr.length > 0 && !edgeArr[0].isAdd) {
					...
					var forkJoinFlg = this.forkJoinLine(startNode, startCell, endCell);
					if(forkJoinFlg){
						jumpFlg = false;
					}
				}
...
MyDesignerEditor.prototype.forkJoinLine = function(startNode, startCell, lastCell){
	...
	for(var j = 0; j < startNode.forkJoinArr.length; j ++){
		...
		var tempEdgeArr = this.editor.graph.getEdgesBetween(forkJoinCell, lastCell, true);
		if(tempEdgeArr.length > 0){
			break;
		} else {
			var nextForkJoinFlg = null;
			if(forkJoinNode.hasForkJoin()){
				nextForkJoinFlg = this.forkJoinLine(forkJoinNode, forkJoinCell, lastCell);
			}
			if(nextForkJoinFlg){
				endCell = nextForkJoinFlg;
				break;
			}
		}
		forkJoinCell = null;
	...
	if(_iconType == "1"){
		...
		var newStyle = this.editor.graph.model.getStyle(forkJoinCell).replace('/48/norun/','/48/');
		this.editor.graph.model.setStyle(forkJoinCell, newStyle);
	}
	var edgeArr = this.editor.graph.getEdgesBetween(forkJoinCell, endCell, true);
	this.allEdges = this.allEdges.concat(edgeArr);
		...
		if (flowUtils.notNull(node)) {
			...
			node.actNodeClick = function() {
				if (isCurrent == "true") {
					...
					if (type == 'dowithdrawpart') {
						_self.dowithdrawpart(executionId, activityName);
					}
...
MyDesignerEditor.prototype.dowithdrawpart = function(executionId, activityName) {
	parent.bpmWithdrawpart.callback(executionId, activityName);
};
