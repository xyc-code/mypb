...
MyProcess.prototype.init = function() {
	...
	this.selectPublishBpmEform1.init( function(data) {
		var formType = data.treeNode.attribute.formsType;
		$('#' + _self.id).find('#guan_lian_biao_dan').attr("data-formType", formType);
		var values = _self.designerEditor.myCellMap.values();
 		$.each(values, function(i, n){
 			if(n.tagName == "task"){
...
MyState.prototype.initLoad = function(xmlValue, rootXml) {
	...
	this.initJBXX();// 初始化基本信息
	/*****以上是公共的*******/
	this.setDomCheckByMeta("is_end", xmlValue, "isEnd", "yes");
	/*****以下是公共的*******/
	rootXml.appendChild(this.createMxCell(xmlValue));// 创建mxCell
	this.designerEditor.myLoadMap.put(this.name, this);//加载时的赋值，用于连线加载时按照name来寻找target节点
...
MyState.prototype.getOtherAttr = function(node) {
	this.setXmlMetaByCheck("is_end", node, "isEnd");
// 获取事件xml
	this.nodeEvent.setEventXml(node, "");
};
;
...
//       console.log(data);
    ...
    $("#"+_self.id+" button[name='formFieldParameterConfig']").on("click", function(){
        ...
        subProcessKey:subProcessKey,subDeploymentId:subProcessId});
    });
...
MySubprocess.prototype.doubleClickCell = function(){
	...
	if(flowUtils.notNull(subFlowId)){
		layer.open({
			...
			title: subFlowName,
			area : [ "800px", "500px" ],
			content:"avicit/platform6/bpmreform/bpmdesigner/picture/pic.jsp?deploymentId="+ subFlowId
...
MyTask.prototype.initLoad = function(xmlValue, rootXml) {
	...
	// 待阅任务
	this.setDomCheckByMeta("dai_yue_jie_dian", xmlValue, "isReadTodo", "no");
	// 短信通知
	...
	//附件扩展配置
	//循环节点只能查看本分支附件
	var branch_attachment_auth = this.getMeta(xmlValue, "branch_attachment_auth");
	if(branch_attachment_auth == "yes"){
		$("#" + this.id).find("#branch_attachment_auth").prop("checked", true);
	}
	...
	// 必须转办意见
	this.setDomCheckByMeta("tast_zhuan_ban_yi_jian", xmlValue, "isNeedZbIdea", "no");
	// 允许提交不弹框
	this.setDomCheckByMeta("tast_ti_jiao_bu_tan_kuang", xmlValue, "canHideDialog", "no");
	...
	//成飞自定义选人 流程转办获取部门接口地址
	this.setDomValByMeta("zhuan_zu_zhi_jie_kou", xmlValue, "zhuanDeptImpl");
	// 自动选人
	// 兼容旧数据处理
	...
	if(dealtype == "4"){
		...
		$(xmlValue).children("meta[name='custom_class']").each(function(){
			$("#" + _self.id).find("input[type=radio][name=input-radio-human-task-select-task]").eq(7).trigger('click');
			$("#" + _self.id).find("input[name=input-num]").eq(7).val($.trim($(this).text()));
		});
		...
		$(xmlValue).children("meta[name='custom_parameter']").each(function(){
			$("#" + _self.id).find("#custom_parameter").val($.trim($(this).text()));
		});
	}
	...
	$(xmlValue).children("docRights").children("docRight[type='formSave']").each(function() {
		...
		if($(xmlValue).children("attrsAuth").length>0){
			...
			$(xmlValue).children("fieldsAuth").children().each(function() {
				...
				if(type=='eform_subtable_bpm_button_auth' || type=='eform_subtable_all_bpm_auth'){
	...
	this.createQXDom(xmlValue, "input_liu_cheng_bu_fen_na_hui", "withdrawPart");
...
MyTask.prototype.getOtherAttr = function(node) {
	...
	//附件扩展配置
	//循环节点只能查看本分支附件
	this.setXmlMetaByCheck("branch_attachment_auth", node, "branch_attachment_auth");
	...
	// 必须转办意见
	this.setXmlMetaByCheck("tast_zhuan_ban_yi_jian", node, "isNeedZbIdea");
	// 允许提交不弹框
	this.setXmlMetaByCheck("tast_ti_jiao_bu_tan_kuang", node, "canHideDialog");
	...
	//成飞自定义选人 流程转办后台获取部门接口
	this.setXmlMetaByVal("zhuan_zu_zhi_jie_kou", node, "zhuanDeptImpl");
	// 自动获取用户
	...
	if(dealtype == "4"){
		...
		if (t != undefined && t.val() != 'none' && t.val() != 'dept_one') {
            ...
            }else if(t.val() == "custom_class"){
				this.addMeta("custom_parameter" , $base.find("#custom_parameter").val(), node);
			}
	...
	this.createQXXml(magicsNode, "input_liu_cheng_bu_fen_na_hui", "withdrawPart", "拿回部分待办", node);
	...
	this.createQXXml(magicsNode, "liu_cheng_zeng_fa_submit", "addUserAndSubmit", "加签并提交", node);
...
//	 域值同步
	        		...
	        		for(var i=0;i<data.length;i++){
        			   ...
        			   $td2.find("a[name='modifyData']").off("click").on("click",function() {
        				   var process = _self.designerEditor.myCellMap.get(_self.designerEditor.processKey);
        			        ...
        			        subProcessKey:subProcessKey,subDeploymentId:subProcessId,hiddenVarParameter:'1'});
        			   });
	...
	$("#"+_self.id+" button[name='btn-fieldRelationControlConfig']").on("click", function(){
		...
		$.ajax({
			url: "platform/bpm/designer/getFormType",
			data: {
				formId:globalformid
			},
			type: "POST",
...
MyTask.prototype.syncFormAttrs = function(refresh) {
	 var _self = this;
	$("#"+_self.id +" #refreshForm").attr("disabled", "disabled");
/*	 var html = " <tr>" +
	' <td>aaaaaa(字段名称)</td>' +
	'  <td>' +
			...
			success : function(result) {
				...
				if(datas) {
					$.each(datas, function(i,v){
						...
						}else if(v.elementType=='eform_subtable_bpm_button_auth' || v.elementType=='eform_subtable_all_bpm_auth'){
				...
				$("#"+_self.id +" #refreshForm").removeAttr("disabled");
			},
			 error : function(msg){
				 $("#"+_self.id +" #refreshForm").removeAttr("disabled");
			 }
...
DesignerEditor.prototype.showProcess = function() {
	...
	if(start == 0 || task == 0){
		this.allowSave = false;
		flowUtils.warning("流程文件至少应包含开始节点和一个人工节点");
		return;
	}
