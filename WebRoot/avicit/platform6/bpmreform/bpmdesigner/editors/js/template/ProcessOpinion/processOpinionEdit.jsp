...
<body class="easyui-layout" fit="true">
		...
		<form id='form' enctype="multipart/form-data">
			...
			<table class="form_commonTable">
				...
				<tr class="type0">
					...
					<td width="39%">
					 <div id="nodeSelectBox" class="input-group input-group-sm avicselect">
						...
						<!-- <select class="form-control input-sm"  name="eSign" id="opinionNode" title="节点">
				...
				<tr class="type0">
					...
					<td width="35%"  >
						<input title="显示样式" class="form-control input-sm" type="text" name="ideaStyle" id="ideaStyle" />
					</td>
				</tr>
				<tr class="type0">
					...
					<td width="35%"  >
						<select class="form-control input-sm" id="ideaStyle_select" title="">
							<option value="idea,day,user">意见，日期，用户</option>
	...
	var idx = flowUtils.getUrlQuery("idx");

		function initForm() {
			var jsonData = parent.MyProcess.prototype.getInitOpinion(pid, idx);
			$("#opinionName").val(jsonData.opinionName);
			$("#opinionCode").val(jsonData.opinionCode);
			$("#opinionNode").val(jsonData.opinionNode);
			$("#opinionNodeText").val(jsonData.opinionNodeText);
			$("#opinionOrder").val(jsonData.opinionOrder);
			$("#positionId").val(jsonData.positionId.replaceAll(",",";"));
			$("#positionText").val(jsonData.positionText.replaceAll(",",";"));
			$("#showPositionId").val(jsonData.showPositionId.replaceAll(",",";"));
			$("#showPositionText").val(jsonData.showPositionText.replaceAll(",",";"));
			$("#eSign").val(jsonData.eSign);
			$("#ideaStyle").val(jsonData.ideaStyle);
            $("#ideaStyle_select").val(jsonData.ideaStyle);
			$("#opinionType").val(jsonData.opinionType);
			$("#subprocessDefId").val(jsonData.subprocessDefId);
			$("#subopinionCode").val(jsonData.subopinionCode);
			$("#subopinionName").val(jsonData.subopinionName);
		...
		$(function() {
            ...
            var codes = parent.MyProcess.prototype.getInitOpinionCodes(pid, idx);
