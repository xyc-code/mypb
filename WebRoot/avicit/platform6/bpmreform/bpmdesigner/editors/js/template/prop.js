...
$(function() {
	// 显示名称监听 流程名称监听
	$("input[id = 'liu_cheng_ming_cheng']").ipt(function() {
		var value = $(this).val();
		designerEditor.myCellMap.get(designerEditor.processKey).labelChanged(value);
	});
	$("input[id = 'xian_shi_ming_cheng']").ipt(function() {
		var value = $(this).val();
		designerEditor.myCellMap.get(designerEditor.myCurCellId).labelChanged(value);
	});
	$("textarea[id = 'xian_shi_ming_cheng']").ipt(function() {
		var value = $(this).val();
		designerEditor.myCellMap.get(designerEditor.myCurCellId).labelChanged(value);
	});
	$("input").on("keydown", function(event){
		if(mxClient.IS_IE8){
			...
			if(flowUtils.notNull(maxlength)){
				if(event.keyCode != 8){
	...
	$('.js_add_e').on('click',function (){
        ...
        myForm.load(_designerPath + "config/form_event.xml", function(){
			...
			myForm.attachEvent("onChange", function(name, value) {
				if (name == "eventList") {
					myForm.setItemValue("fnName", "");
					myForm.setItemValue("fnEvt", "");
					...
					if (value != "") {
						$.ajax({
							...
							success : function(result) {
								...
								if (bpmClass) {
									myForm.setItemValue("fnName", bpmClass.name);
									myForm.setItemValue("fnEvt", bpmClass.path);
									toolbar.enableItem("ok");
								}
								if (properties) {
									for(var i = 0; i < properties.length; i++){
										myGrid.cellById(i, 0).setValue(properties[i].name);
										myGrid.cellById(i, 1).setValue(properties[i].initExpr);
									}
								}
							}
						});
    ...
    $('.js_cfgz').on('click',function (){
		...
		toolbar.attachEvent("onClick", function(id) {
			if (id == "ok") {
                ...
                var result = "";
                if(flowUtils.notNull(sjgz)){
                	result += sjgz;
                	if(flowUtils.notNull(sjlx)){
                		result += " " + sjlx;
                	}
                	if(flowUtils.notNull(sjdw)){
                		result += " " + sjdw;
                	}
                }
	...
	$('.js_add_c').on('click',function (){
        ...
        myForm.load(_designerPath + "config/form_condition.xml", function(){
			var eventType = "流程路由条件";
			if(designerEditor.myCurCellId == designerEditor.processKey){
				eventType = "流程启动条件";
			}
			$.ajax({
				type : "POST",
					...
					if (value != "") {
						$.ajax({
							...
							success : function(result) {
								var bpmClass = result.bpmClass;
								if (bpmClass) {
									myForm.setItemValue("class", bpmClass.path);
									toolbar.enableItem("ok");
								}
							}
						});
    ...
    $('.js_edit_c').on('click',function (){
        ...
        myForm.load(_designerPath + "config/form_condition.xml", function(){
        	...
        	var eventType = "流程路由条件";
			if(_myCurCellId == designerEditor.processKey){
				eventType = "流程启动条件";
			}
			$.ajax({
				type : "POST",
    ...
    $('.js_cl').on('click',function (){
					...
					if (value != "") {
						$.ajax({
							...
							success : function(result) {
								var bpmClass = result.bpmClass;
								if (bpmClass) {
									myForm.setItemValue("alias", bpmClass.name);
		...
		toolbar.attachEvent("onClick", function(id) {
			if (id == "ok") {
				...
				var alias = myForm.getItemValue("alias");
				var name = myForm.getItemValue("name");
				if(flowUtils.notNull($.trim(alias)) && flowUtils.notNull($.trim(name))){
					value = alias + "【" + name + "】";
				}
				$(self).prev(".prop_txt").val(value);
				addWin.close();
