...
<script>
	...
	//记录主键，已发布的取得事lob表主键
	var _id = '<c:out value="${id}"/>';
	//类型，1是未发布，2是已发布，3是新建
	var _type = '<c:out value="${type}"/>';
	//初始化泳道
	var _swimlane = '<c:out value="${swimlane}"/>';
	//目录
	var _catalog = '<c:out value="${catalog}"/>';
	//pdid
	var _pdId = '<c:out value="${pdId}"/>';
	//deployId
	var _deployId = '<c:out value="${deployId}"/>';
	//流程名称，新建时使用
	var _name = '<c:out value="${name}"/>';
	//
	var _bmpsName = '<c:out value="${name}"/>';
	//
	var _iconType = '<c:out value="${iconType}"/>';//1是新图元，2是旧图元
	//是否自由流程，1标识固定流程，2标识自由流程
	var _isUflow = '<c:out value="${isUflow}"/>';
	//拟稿节点能不能配置参与者
	var _needfirstuser = '<c:out value="${needfirstuser}"/>';
	//网安项目定制，流程数据同步自定义js路径
	var _processDataSynJs = '<c:out value="${processDataSynJs}"/>';
...
<body onload="init();" style="overflow-x:hidden;">
	<div id="navbar" class="navbar ace-save-state" style="background-color:#FFF">
		<div class="navbar-container ace-save-state" id="navbar-container" >
			<div id="header-title" class="navbar-header pull-left" style="padding-top:2px;padding-left:10px">
					<h4 id="formName"><c:out value="${bpmNav}"/>(${isUflow == 2 ? "自由流程" : "固定流程" })</h4>
			</div>
	...
	<script type="text/javascript" src="avicit/platform6/bpmreform/bpmdesigner/editors/js/template/EventSelect/EventSelect.js"></script>
	<script type="text/javascript">
