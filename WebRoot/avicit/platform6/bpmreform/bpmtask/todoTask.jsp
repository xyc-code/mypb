...
<script type="text/javascript">
	...
	function selectSearchType(obj){
		...
		if('title'==selectVal){
			...
			$("#commonSearch").html('<input type="text" name="bpmTask_keyWord"  id="bpmTask_keyWord" class="form-control input-sm" placeholder="请输入查询条件" onkeydown="if(event.keyCode==13){queryByKeyWord();}"> <label id="bpmTask_searchPart" class="icon icon-search form-tool-searchicon" onClick="queryByKeyWord()"></label>');
		}else if('priority'==selectVal){
			$('#commonSearch').attr("style","border:0;");
			$("#commonSearch").html('<select name="priority" id="priority" class="easyui-combobox form-control input-sm" style="width:150px;" onChange="queryByPriority(this)"><option value="0">一般</option><option value="1">急</option><option value="2">紧急</option></select>');
			...
			var htmlAcceptDate = '<div class="dropdown">'
		        ...
		        +'	&nbsp;从&nbsp;<input data-stopPropagation="true" class="date-picker" style="width:100px;font-size:12px;" border="1" type="text" '
...
<% } %>
		...
		var url = "bpm/task/searchHistTaskByPage?taskType=0&taskFinished=0";
		bpmTask = new BpmTask('bpmTask', url, "formSub", bpmTaskGridColModel, 'searchDialogSub', nodeId,
				nodeType, searchSubNames, "bpmTask_keyWord",pdId,'tab1','我的待办',true);
