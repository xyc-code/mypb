		...
		<table class="form_commonTable">
			...
			<tr>
				...
				<td>
					<select id="type" name="type" class="form-control input-sm" title=""
							data-options="" style="" value="<c:out  value='${bpmEvent.type}'/>">
						<option value="流程事件监听" <c:if test="${bpmEvent.type eq '流程事件监听'}">selected</c:if>>流程事件监听</option>
						<option value="流程路由条件" <c:if test="${bpmEvent.type eq '流程路由条件'}">selected</c:if>>流程路由条件</option>
						<option value="流程动作监听" <c:if test="${bpmEvent.type eq '流程动作监听'}">selected</c:if>>流程动作监听</option>
						<option value="流程自定义选人" <c:if test="${bpmEvent.type eq '流程自定义选人'}">selected</c:if>>流程自定义选人</option>
						<option value="流程启动条件" <c:if test="${bpmEvent.type eq '流程启动条件'}">selected</c:if>>流程启动条件</option>
					</select>
				</td>
			</tr>
			<tr>
				...
				<td>
					<div class="input-group input-group-sm spinner"
						...
						<input class="form-control" type="text" name="orderBy"
							   id="orderBy" value="<c:out  value='${bpmEvent.orderBy}'/>"
			...
			<tr>
				...
				<td><input class="form-control input-sm"
						   ...
						   value="<c:out  value='${bpmEvent.remark}'/>"/>
				</td>
			</tr>
			<tr>
				<th>说明：</th>
				<td>
					...
					<p>流程事件监听：包括全局和人工节点的前置事件、后置事件，流转线上的流转事件，需要实现接口"EventListener" <a href="avicit/platform6/bpmconsole/eventManage/EventListener.jsp" target="_blank">demo</a></p>
...
<script type="text/javascript">
	function closeForm() {
		parent.bpmEvent.closeDialog("edit");
	}
	function saveForm() {
		...
		parent.bpmEvent.save($('#form'), "eidt");
	}
