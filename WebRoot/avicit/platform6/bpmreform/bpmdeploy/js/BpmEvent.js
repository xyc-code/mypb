...
BpmEvent.prototype.init = function() {
    ...
    $(_self._datagridId).jqGrid({
        url : this.getUrl() + 'getEventListByPage.json',
        mtype : 'POST',
        multiselect : true,
        datatype : "json",
        toolbar : [ true, 'top' ],
        colModel : this.dataGridColModel,
        height : $('#north1').height() - $("#toolbar_bpmEvent").innerHeight() -36  -48-16,//120:顶部工具栏高+表头高+底部翻页模块高，17：顶部toolbar的内边距高度
        scrollOffset : 10,
        rowNum : 20,
...
//添加页面
BpmEvent.prototype.insert = function() {
...
//编辑页面
BpmEvent.prototype.modify = function() {
    ...
    this.eidtIndex = layer.open({
        ...
        skin : 'bs-modal', // bootstrap 风格皮肤 需加载skin
        maxmin : false, //开启最大化最小化按钮
        content : this.getUrl() + 'Edit/' + rowData.dbid
    });
};
//详细页
BpmEvent.prototype.detail = function(id) {
...
//保存功能
BpmEvent.prototype.save = function(form, id) {
    var _self = this;
    avicAjax.ajax({
...
//删除
BpmEvent.prototype.del = function() {
...
//打开高级查询框
BpmEvent.prototype.openSearchForm = function(searchDiv, par) {
    var _self = this;
    par = null;
    //if(!par) par = $(window);
...
//高级查询
BpmEvent.prototype.searchData = function() {
    var searchdata = {
        keyWord : null,
        param : JSON.stringify(serializeObject($(this.searchForm)))
    };
...
//隐藏查询框
BpmEvent.prototype.hideSearchForm = function() {
    layer.close(searchDialogindex);
};
/*清空查询条件*/
BpmEvent.prototype.clearData = function() {
    clearFormData(this.searchForm);
    this.searchData();
};
// 导出菜单xml
BpmEvent.prototype.doexport = function () {
    var rows = $(this._datagridId).jqGrid('getGridParam', 'selarrrow');
    var _self = this;
    var ids = [];
    var l = rows.length;
...
// 导入菜单xml
BpmEvent.prototype.doimport = function() {
    var _self = this;
    layer.open({
        type : 2,
        area : [ '70%', '70%' ],
