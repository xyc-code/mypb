...
<%
	...
	String showtype = sysProfileAPI.getProfileValueByCode("PLATFORM_V6_MODEL_SHOWTYPE");
	if(showtype == null){
		showtype = "card";
	}
%>
		...
		<div class="toolbar">
		   <div class="toolbar-left">
			    ...
			    <a id="buttonManage" href="javascript:void(0)" class="btn btn-default form-tool-btn btn-sm"
			                role="button" title="流程按钮管理"><i class="icon icon-gongju-fill"></i> 流程按钮管理</a>
		...
		<div class="toolbar">
			<div class="toolbar-left">
				<div class="input-group  input-group-sm">
					<input class="form-control" placeholder="输入名称查询" style="width:210px;" type="text" id="txt" name="txt">
    ...
    <div data-options="region:'center'" style="overflow:auto;">
    	...
    	<div class="flow-operation">
    		...
    		<div style="float:right;">
    			<div id="cardShowDiv" style="margin: 0;padding: 0;display: flex;align-items: center;font-size:14px; height: 28px!important;">
				  <span class="flow-operation-box"> 显示停用模板:</span><input type="checkbox" id="isShowStopModel" style="height: 12px!important;float:left;margin:4px 10px 0 0;"/>
				   <span class="flow-operation-box">显示条数:</span><select id="limitCount" class="form-control input-sm" style="width:60px;float:left;margin-right:10px;">
                            <option value="20">20</option>
							<option value="50" selected>50</option>
							<option value="100">100</option>
							<option value="200">200</option>
                        </select>
                <div class="input-group form-tool-search" style="float:left;">
					<input type="text" class="form-control input-sm" placeholder="请输入流程名称" id="searchProcessName"> <label id="searchProcessNameBtn" class="icon icon-search form-tool-searchicon"></label>
				</div>
...
<script type="text/javascript">
	...
	function setTreeHeight(){
		$('#mdiv').height(document.documentElement.clientHeight- 90);
		$('#lb').width(document.documentElement.clientWidth- 250);
	}
	setTreeHeight();
    ...
    $(document).ready(function () {
        ...
        flowTypeTree = new FlowTypeTree("flowTypeTreeUL", "txt", "searchbtn");
		...
		$('#limitCount').bind('change',function(){
			flowTypeTree.clickNode();
		});

		$('#searchProcessNameBtn').bind('click',function(){
			flowTypeTree.clickNode();
		});

		$("#searchProcessName").on('keydown', function(e) {
			if (e.keyCode == '13') {
				flowTypeTree.clickNode();
			}
		});
	...
	$(function () {
		<%=showtype.equals("list") ? "changeShowType(2);" : ""%>
	});
</script>
</body>
<!-- 子表高级查询 -->
