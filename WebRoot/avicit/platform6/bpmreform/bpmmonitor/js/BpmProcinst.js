...
BpmProcinst.prototype.delPart = function () {
	var idDatas = $(this._datagridId).jqGrid('getGridParam', 'selarrrow');
	...
	for (var i = 0; i < idDatas.length; i++) {
		var rowData = $(this._datagridId).jqGrid('getRowData', idDatas[i]);
		entryIds.push(rowData.entryid);
	}
	flowUtils.confirm('此操作将仅仅删除流程实例数据，不会删除关联的业务实例数据，确定操作吗？', function (index) {
		avicAjax.ajax({
			url: "platform/bpm/bpmConsoleAction/deleteProcessEntryPart",
			data: "entryIds=" + entryIds.join(","),
			type: "post",
			dataType: "json",
			success: function (backData) {
				if (backData != null && backData.success == true) {
...
BpmProcinst.prototype.delException = function(){
	...
	flowUtils.confirm('您确认要删除异常信息吗?', function (index) {
        avicAjax.ajax({
