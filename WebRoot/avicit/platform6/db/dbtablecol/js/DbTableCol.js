...
DbTableCol.prototype.save = function(id) {
	...
	for(var i=0;i<data.length;i++)
	{
			...
			if(iscreated == "Y"){
				...
				if(data[i].colType == "VARCHAR2"){
					if(data[i].colLength > 4000){
						layer.alert("保存失败！" + data[i].colName+"字段长度超过4000，请考虑使用CLOB类型", {
							icon : 7,
							area : [ '400px', '' ], //宽高
							closeBtn : 0
						});
				...
				}else if(data[i].colType == "NUMBER"){
					if(data[i].colLength > 38 || data[i].colLength < 1 || data[i].attribute02 > 38){
						layer.alert("保存失败！" + data[i].colName+"字段数字精度说明符超出范围 (1 到 38)", {
							icon : 7,
							area : [ '400px', '' ], //宽高
							closeBtn : 0
						});
	...
	if (data && data.length > 0) {
		for ( var i = 0; i < data.length; i++) {
			...
			var regEn = /[`~!@#$%^&*()+<>?:\-{},.\/;'[\]]/im,
				regCn = /[·！#￥（——）：；“”‘、，|《。》？、【】[\]]/im,regblank = /^\S*$/;;
			if(regEn.test(data[i].colName) || regCn.test(data[i].colName) ||!regblank.test(data[i].colName)) {
				layer.alert(data[i].colName+'列名英文名称不能包含特殊字符！', {
					icon : 7,
					area : [ '300px', '' ], //宽高
					closeBtn : 0
				});
...
//				}
				if (rows[l].indexOf("new_row")!=-1){
					...
					var colIsSys = $('#'+$(rowData.colIsSys).attr("id")).val();
					var colName = $('#'+$(rowData.colName).attr("id")).val();
					if (colIsSys == "Y"){
						sysColName += colName + ",";
					}
					ids.push(rows[l]);
			...
			if(sysColName.length > 0){
				layer.alert('['+sysColName.substr(0,sysColName.length-1)+']为系统字段，不能删除！', {
					icon : 7,
					area : [ '400px', '' ],
					closeBtn : 0
				});
				...
				avicAjax.ajax({
					...
					success : function(r) {
						if (r.flag == "success") {
							...
							var k = rows.length;
							for (; k--;) {
								$(_self._datagridId).jqGrid("delRowData", ids[k]);
							}
							dbTableIndex.reLoad();
						} else {
