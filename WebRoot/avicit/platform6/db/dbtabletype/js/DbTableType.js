...
//提交编辑
EformType.prototype.subEdit = function (form, nodeName,refreshTreeFlag) {
    var _this = this;
    ...
    $.ajax({
        ...
        success: function (backData) {
            if (backData.result == "1") {
                ...
                var updateNode =dbTableTypeTree.tree.getNodeByParam("id",form.find("#id").val());
                var oldParentId = updateNode.pId;
                if(updateNode){
                    updateNode.text=form.find("#name").val();
                    updateNode.name=form.find("#name").val();
                    updateNode.pId=form.find("#parentId").val();
                }
                if(refreshTreeFlag){
                    //移除修改前节点下的被修改目录
                    dbTableTypeTree.removeNode(form.find("#id").val());
                    //修改后的父级目录
                    var afterNode = dbTableTypeTree.tree.getNodeByParam("id",form.find("#parentId").val());
                    //防止修改后子节点重复显示,当节点是打开状态时手动添加，否则刷新该节点
                    if(afterNode != null && afterNode != "" && afterNode != undefined){
                        if(afterNode.open == true){
                            dbTableTypeTree.tree.addNodes(afterNode,updateNode);
                            var spanId = dbTableTypeTree.tree.getNodeByParam("id", form.find("#id").val(), null).tId + "_span";
                            $("#" + spanId + "").click();
                        }else{
                            ...
                            dbTableTypeTree.tree.reAsyncChildNodes(afterNode, "refresh",false);
                            // 用于捕获异步加载正常结束的事件回调函数
                            dbTableTypeTree.tree.setting.callback.onAsyncSuccess = function(event, treeId, treeNode, msg) {
