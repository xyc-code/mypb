 ...
 * 保存方法
 */
DbCommonCol.prototype.save=function(form,id,saveElement,saveForm){
	var jsonData = serializeObject(form);
	if ((jsonData.lookupTypeId == undefined || jsonData.lookupTypeId == "") &&
		(jsonData.colDomType == "select" || jsonData.colDomType ==  "radio" || jsonData.colDomType ==  "check")) {
		layer.msg("请选择通用代码!");
...
DbCommonCol.prototype.clear=function(){
	var _self = this;
	avicAjax.ajax({
...
DbCommonCol.prototype.reLoad=function(colClass){
	...
	if(colClass != undefined && colClass != '523534EDA8874E4AAE91AD19DF02BF37'){
...
DbCommonCol.prototype.searchData =function(){
	...
	if(colClass != null && colClass != "" && colClass != undefined && colClass != "523534EDA8874E4AAE91AD19DF02BF37"){
...
DbCommonCol.prototype.initDomSelect = function(colType,dom,colDomType){
	dom.html('');
	...
	for(var i = 0; i< colDomList.length; i++){
		...
		if(colDomType != undefined && colDomType == colDom[0]){
			dom.append("<option selected value='"+colDom[0]+"'>"+colDom[1]+"</option>");
		}else{
			dom.append("<option value='"+colDom[0]+"'>"+colDom[1]+"</option>");
		}
