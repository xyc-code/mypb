...
<div class="easyui-layout" fit="true" id="dbCommonCol">
	<div data-options="region:'west',split:true,disabled:false ,width:fixwidth(0.15,e),height:fixheight(1.0,e)"
		 style="background: #ffffff; height: 0px; padding: 0px;overflow-y: hidden;">
		<div style="height: 100%;">
			<ul id="dbColClassUL" class="ztree" style="height: 100%; overflow-y: auto;">
			</ul>
		</div>
	</div>
	<div data-options="region:'center',disabled:false ,width:fixwidth(0.85,e),height:fixheight(1.0,e),onResize:function(a){$('#dbCommonColjqGrid').setGridHeight(getJgridTableHeight($('#tableDbCommonCol_div')));$('#dbCommonColjqGrid').jqGrid('resizeGrid');} "
		...
		<div id="tableToolbar" class="toolbar">
			<div class="toolbar-left">
				...
				<sec:accesscontrollist hasPermission="3" domainObject="formdialog_dbCommonCol_button_clear" permissionDes="清除">
					<a id="dbCommonCol_clear" href="javascript:void(0)" class="btn btn-primary form-tool-btn btn-sm" role="button" title="清除无效数据"><i class="fa fa-trash-o"></i> 清除无效数据</a>
				</sec:accesscontrollist>
                <span class="remind-text">◆ 提示：左侧类型分类树在通用代码类型【PLATFORM_DB_COL_CLASS】下维护</span>
			</div>
...
$(document).ready(function () {
	...
	dbCommonCol.initDomSelect("all",$("#colDomType"));
	//添加按钮绑定事件
	...
	//删除按钮绑定事件
	$('#dbCommonCol_clear').bind('click', function(){
		dbCommonCol.clear();
	});
	//查询按钮绑定事件
