function PortalProgramVersion(datagrid, url, formSub, dataGridColModel,
...
PortalProgramVersion.prototype.init = function(pid) {
	...
	$(_self._datagridId).jqGrid({
		url : this.getUrl() + 'getPortalProgramVersion',
		postData : {
			pid : _self.pid
		},
		mtype : 'POST',
...
PortalProgramVersion.prototype.formValidate = function(form) {
	form.validate({
		rules : {
			programVersionName : {
				required : true,
			...
			programVersionEntrance : {
				required : true,
				maxlength : 200
			},
			programVersionOpenMode : {
				required : true,
				maxlength : 200
			},
			programVersionManifest : {
				maxlength : 2000
			},
			programVersionModuleName : {
				maxlength : 200
			},
			programVersionDependance : {
				maxlength : 2000
			},
			programVersionUrl : {
				maxlength : 200
			...
			programVersionMsgHandling : {
				maxlength : 200
			},
			programVersionGateWay : {
				required : true,
				maxlength : 200
			},
			programVersionVpn : {
...
// 删除
PortalProgramVersion.prototype.del = function() {
	...
	if (l > 0) {
		layer.confirm('确认要删除选中的数据吗?', {
			...
			for (; l--;) {
				if("启用"==rowDatas[l].programVersionState){
...
// 重载数据
PortalProgramVersion.prototype.reLoad = function(pid) {
	if (pid != undefined) {
		this.pid = pid;
	}
	var searchdata = {
...
// 打开高级查询框
PortalProgramVersion.prototype.openSearchForm = function(searchDiv, par) {
	var _self = this;
	par = null;
	// if(!par) par = $(window);
...
// 高级查询
PortalProgramVersion.prototype.searchData = function() {
	var searchdata = {
		keyWord : null,
		pid : this.pid,
...
// 关键字段查询
PortalProgramVersion.prototype.searchByKeyWord = function() {
...
// 隐藏查询框
PortalProgramVersion.prototype.hideSearchForm = function() {
	layer.close(searchDialogindex);
};
/* 清空查询条件 */
PortalProgramVersion.prototype.clearData = function() {
	clearFormData(this.searchForm);
	this.searchData();
};

/* 启用按钮 */
PortalProgramVersion.prototype.start = function() {
	var ids = $(this._datagridId).jqGrid('getGridParam','selarrrow');
	var _self = this;
	var l =ids.length;
	if (l> 0) {
...
/* 禁用按钮 */
PortalProgramVersion.prototype.stop = function() {
	var ids = $(this._datagridId).jqGrid('getGridParam','selarrrow');
	var _self = this;
	var l =ids.length;
	if (l> 0) {
