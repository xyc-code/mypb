...
%>
<link rel="stylesheet" href="static/css/platform/themes/<%=skinColor %>/portlet/jquery-ui-1.10.4.custom.css" />
<link rel="stylesheet" href="static/css/platform/themes/<%=skinColor %>/portlet/jquery.portlet.css?v=1.3.1" />
<!-- add by xingc-->
<link href="static/css/platform/themes/<%=skinColor %>/portlet/portlet.css" rel="stylesheet" type="text/css">
...
function saveNewPortlet(portletList,layout,newConX){
	...
	$.ajax({ 
		url: '<%=ViewUtil.getRequestPath(request)%>platform/cc/sysportal/saveIndexPortlet.json',
		async : false,
		type: "POST",
...
function addPortlet(ids){
	...
	for(var i=0; i<idArray.length; i++){
		...
		$.ajax({
			url : '<%=ViewUtil.getRequestPath(request)%>platform/cc/sysportal/getPortletInfo.json?portletId='+idArray[i]+'&baseUrl='+baseUrl+'&xy='+xy,
			data : {},
			type : 'post',
			dataType : 'json',
			async:false,
...
function loadPortlet(){
	$.ajax({
		url : baseUrl+'platform/cc/sysportal/getIndexPortlet.json',
