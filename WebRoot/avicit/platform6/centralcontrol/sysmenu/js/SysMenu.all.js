function SysMenu(tree,url,searchId,form){
    ...
    _onSelect=function(){},//选中node事件
    _onClick=function(){};//单击node事件
    this.level={};//级别
...
//初始化操作
SysMenu.prototype.constructor=SysMenu;
SysMenu.prototype.init=function(appId){
	var _self = this;
	$(this._searchId).searchbox({
        ...
        searcher: function (value) {
        	...
        	$.ajax({
                cache: true,
                type: "post",
                url:_self.getUrl()+_self._appId+"/search",
                dataType:"json",
	...
	this._tree=$(this._treeId).tree({    
		url:this.getUrl()+appId+"/sysmenutree/2",
		method:'GET',
		...
		onLoadSuccess:function(node, data){
			!node && _self.getOnLoadSuccess()(_self,node,data);
	}
	});
};
//选择根节点
...
SysMenu.prototype.loadMenuById=function(_self,node){
	$.ajax({
        cache: true,
        type: "get",
        url:_self.getUrl()+_self._appId+"/sysmenu/"+node.id,
        dataType:"json",
        async: false,
        error: function(request) {
        	throw new Error('操作失败，服务请求状态：'+request.status+' '+request.statusText+' 请检查服务是否可用！');
...
SysMenu.prototype.insert=function(){
	...
	this._iDg = new CommonDialog("insert","700","400",this.getUrl()+this._appId+'/operation/add/'+this._selectNode._parentId,"添加菜单",false,true,false,null,false,false);
	this._iDg.show();
};
//添加子菜单
SysMenu.prototype.insertsub=function(){
	...
	this._iDg=new CommonDialog("insert","700","400",this.getUrl()+this._appId+'/operation/add/'+this._selectNode.id,"添加菜单",false,true,false,null,false,false);
	this._iDg.show();
};
//添加菜单资源
SysMenu.prototype.insertRes=function(){
	...
	this._iDg=new CommonDialog("insertRes","700","400",this.getUrl()+this._appId+'/res/add/'+this._selectNode.id,"添加资源",false,true,false);
	this._iDg.show();
};
//添加子菜单
SysMenu.prototype.modify=function(){
	...
	this._eDg=new CommonDialog("modify","700","400",this.getUrl()+this._appId+'/operation/modify/'+this._selectNode.id,"编辑菜单",false,true,false);
	this._eDg.show();
};
//编辑资源
SysMenu.prototype.modifyRes=function(){
	...
	this._eDg=new CommonDialog("modifyRes","700","400",this.getUrl()+this._appId+'/res/modify/'+this._selectNode.id,"编辑资源",false,true,false);
	this._eDg.show();
};
...
SysMenu.prototype.del=function(){
	...
	ids.push(this._selectNode.id);
	  $.messager.confirm('请确认','您确定要删除当前节点？',function(b){
		 if(b){
			 $.ajax({
				 url:'platform/cc/sysmenu/operation/sysmenu/delete',
 ...
 * 根据父配置文件id加载配置文件属性值
 */
SysMenu.prototype.loadByAppId=function(pid){
	this._appId=pid;
	this.init(pid);
	this.loadByAppId=function(p){
		this._appId=p;
		this._tree.tree({url:this.getUrl()+p+"/sysmenutree/2.json"});
	};
};
SysMenu.prototype.reload=function(){
	 $.ajax({
		 url:'platform/cc/sysmenu/reload',
		 type : 'get',
		 dataType : 'json',
		 success : function(r){
			 if (r.flag == "success") {
...
SysMenu.prototype.openLanguageForm=function(){
	...
	this._cData = new CommonDialog("chooseL","700","400",this.getUrl()+'/'+this._appId+'/chooseLanguage/'+this._selectNode.id,"多语言设置",false,true,false);
	this._cData.show();
};
...
SysMenu.prototype.toMove=function(){
	if(this._selectNode.attributes.isMenu=='0'){alert('只能移动菜单!');return true;}
	this._cData = new CommonDialog("moveMenu","300","400",this.getUrl()+this._appId+'/move',"移动菜单到",false,true,false);
	this._cData.show();
};
...
SysMenu.prototype.saveToNode=function(node){
	...
	if(node.attributes.isMenu=='0'){alert('菜单不能移动到资源下面!');return true;}
	...
	$.ajax({
		 url:_self.getUrl()+_self._appId+"/save/move",
