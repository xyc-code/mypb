...
function refreshMenuSecurityCache() {
	$.messager.confirm('系统', '真的要刷新菜单资源的权限信息吗？', function(r){
		if (r)
			$.ajax({
				url: "platform/cc/componentmanager/refreshComponentAuthority/"+_APPID+".json",
				type: "POST",
				dataType: "json",
				success: function(r) {
					if (r.flag == "success"){
...
function remarkformat(value, rowData, rowIndex) {
	...
	if("1" == value) 
		return "<img src='avicit/platform6/centralcontrol/sysmenuaccesscontrol/images/ok.png' title='允许访问' onclick='window.changeSingleMenuAuth(this);' alt=" + rowData.ID +">";
	if("0" == value) 
		return "<img src='avicit/platform6/centralcontrol/sysmenuaccesscontrol/images/no.gif' title='禁止访问' onclick='window.changeSingleMenuAuth(this);' alt=" + rowData.ID +">";

	return "<img src='avicit/platform6/centralcontrol/sysmenuaccesscontrol/images/untitled.png' title='未设置权限' onclick='window.changeSingleMenuAuth(this);' alt=" + rowData.ID +">";	
}
...
/*function initForSearch(){
	 $('#formSearch').searchbox({
	        ...
	        searcher: function (value) {
	        	if(null == value||"" == value) {
					$("#tg").treegrid("options").url = "platform/cc/componentmanager/getSysMenusByParentId.json";
					$('#tg').treegrid('load',{
					...
					$("#tg").treegrid("options").url = "platform/cc/componentmanager/searchMenu.json";
					$('#tg').treegrid('load', {
						 search_name: value,
						 targetType: window.TARGET_TYPE,
						 targetId: window.TARGET_ID,
