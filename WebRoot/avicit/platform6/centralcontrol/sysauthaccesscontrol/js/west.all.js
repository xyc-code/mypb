			...
			.tabs(
					{
						onSelect : function(title, index) {
							...
							if (index == 1) {
								...
								$("#dgUser").datagrid("options").url ="platform/cc/sysdept/getUserDataByPage.json?_status=1";
									...
									searcher : function(value) {
										...
										var searchData = {
										 ...
										 var param="multipleOrg=n&selectType=dept&deptKeyWord="+value;
								           ajaxRequest("POST",searchData,"platform/cc/sysdept/searchDept?_status=1","json","loadDeptTreeByData");
										//停止datagrid的编辑.
									},
									prompt : "请输入查询内容"
								});
		 ...
		 $('#orgTree_user').tree({
	 		...
	 		onCheck :function(node, checked){
	 		 ...
	 		 onBeforeExpand:function(node,param){
	 			var beforeExpandUrl_ = 'platform/cc/centralizedauthorization/getDepts.json';
...
//	    if(orgData.length==1){
	    	orgTree_dept.tree({
		 		 ...
		 		 method : 'post',
//		 		data: orgData,
		 		url:'platform/cc/sysdept/getChildOrgDeptByIdAndOrgId.json?_status=1',
		 		//url:'platform/sysdept/sysDeptController/getChildOrgDeptById.json?_status=1',
		 		onCheck :function(node, checked){
		 			
		 		},onBeforeExpand:function(node,param){
		 			 if(node){
		 				var beforeExpandUrl = 'platform/cc/sysdept/getChildOrgDeptByIdAndOrgId.json';
...
//	var dataGridHeight = $(".easyui-layout").height()-73;
	$('#roleList').datagrid({
		fit: true,
	    url:'platform/cc/centralizedauthorization/getSysRole.json?type=role&appId='+_APPID,
		idField:'id',
		//width: '100%',
		singleSelect: true,
	    nowrap: false,
...
function loadGroupList(){
	...
	$('#groupList').datagrid({
		fit: true,
		url:'platform/cc/centralizedauthorization/getSysGroup.json?type=group&appId='+_APPID,
		idField:'id',
		//width: '100%',
		singleSelect: true,
	    nowrap: false,
...
function loadPositionList(){
	...
	$('#positionList').datagrid({
		fit: true,
		url:'platform/cc/centralizedauthorization/getSysPosition.json?type=position&appId='+_APPID,
		idField:'id',
		//width: '100%',
	    nowrap: false,
	    striped: true,
...
function reloadTabData(){
	switch(window.TARGET_TYPE){
		case "R" : loadRoleList(); break;
		case "D" : loadDeptTree(); break;
		case "G" : loadGroupList(); break;
		case "P" : loadPositionList(); break;
	}
}
...
function authSelectDept(){
	...
	deptSelector.init(false,"selectDeptDialogCallBack",1);
}
//部门选择回调
