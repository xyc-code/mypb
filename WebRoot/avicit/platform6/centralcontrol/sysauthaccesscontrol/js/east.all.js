...
function setComponentAuth(authType,readStatus,writeStatus,rowIndex){
	...
	if(_APPID != null && _APPID.length > 0){
		requestParam += "&appId="+_APPID ;
	}
	$.ajax({
		url: "platform/cc/componentmanager/updateComponentResourceAuth.json",
		type: "POST",
		dataType: "json",
		data : requestParam ,
		success: function() {
...
function refreshComponent(){
	...
	$.ajax({
		url: "platform/cc/componentmanager/refreshComponent4Page.json?resourceId="+resourceId+"&appId="+_APPID,
		type: "POST",
		dataType: "json",
		data : resourceId,
...
function deleteComponentResources(){
	...
	if( deleteIds!= null && deleteIds.length > 0 ){
		$.messager.confirm('删除组件', '确认要删除选择的组件及相关的权限吗？', function(retValue){
		 if(retValue){
			$.ajax({
				url: "platform/cc/componentmanager/deleteComponentResources.json",
				type: "POST",
				dataType: "json",
