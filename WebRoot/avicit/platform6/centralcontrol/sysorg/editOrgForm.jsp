...
function save(){
	...
	if($('#ORG_NAME').val().length >100){
	 ...
	 if (orgCode != $("#ORG_CODE").val()){
		 ...
		 $.messager.confirm('请确认','更改组织编码可能导致数据同步及相关数据操作出现问题，是否继续？',
		function(b){
		if(b){
	 $.ajax({
	        cache: false,
			...
			<form id="orgEditForm" name="orgEditForm" method="post">
				...
				<table class="form_commonTable">
					<tr>
	                    ...
	                    <td width="40%" >
	                        <input title="组织代码" class="easyui-validatebox disabled pt6-disabled" data-options="required:true"  type="text" name="ORG_CODE" id="ORG_CODE"/>
	                    </td>
	                ...
	                <tr>
	                    ...
	                    <td>
	                    	<c:forEach items="<%=lookUpYesNo%>" var="data">
								<input type="radio" name="IS_MUL_ORG"  id="IS_MUL_ORG_${data.lookupCode}" value="${data.lookupCode}" style="width: 20px"/>${data.lookupName}
							</c:forEach>
	                    </td>
	                </tr>
	                <tr>
