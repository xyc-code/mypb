...
<script type="text/javascript">
    ...
    var setting = {
        async: {
            enable: true,
            url: "bpm/monitor/getAllFlowModelTree",//异步请求树子节点url
            autoParam: ["id"]//父节点id
        },
        data: {
            simpleData: {
        ...
        callback: {
            ...
            onClick: function (event, treeId, treeNode) {
            	$.ajax({
      			  ...
      			  success: function(r){
      				  if(r=="yes"){
      					parent.$("#${param.bdformId}").val(treeNode.id);
                        parent.$("#${param.bpmModel}").val(treeNode.name);
   				     
                        parent.layer.close(parent.selectFolwModel.selectDialog);
      				  }
      			  },
      			  error: function(){
    ...
    //手动请求根节点数据
   $.ajax({
        url: "bpm/monitor/getAllFlowModelTree",
        data: "id=root",
        type: "post",
        async: false,
        dataType: "json",
        success: function (backData) {
            ...
            $("#dbTableTypeTreeUL_1_switch").click();
        }
    });
    //绑定搜索事件
