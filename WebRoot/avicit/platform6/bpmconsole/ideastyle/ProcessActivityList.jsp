	...
	function loadList(){
		...
		$('#processActivityTable').datagrid({
			...
			onLoadSuccess : function(){
				if(parent && parent.getActNames){
				 	var actNamesForLoad = parent.getActNames();
			 		if(actNamesForLoad){
				 		var arr = actNamesForLoad.split(",");
				 		for(var i = 0; i < arr.length; i++){
				 			var index = $('#processActivityTable').datagrid('getRowIndex', arr[i]);
				 			if(index > -1){
					 			$('#processActivityTable').datagrid('checkRow', index);
				 			}
				 		}
			 		}
			 	}
