...
function loadTaskActDuration(){
	...
	$('#processentrylistTable').datagrid({
		url: 'platform/bpm/processAnalysisAction/getTaskActDurationListByPage.json?pdId=<%=pdId%>&activityName=<%=activityName%>&activityType=auto&startDateBegin=<%=startDateBegin%>&startDateEnd=<%=startDateEnd%>',
		width: '100%',
	    nowrap: false,
	    striped: true,
	    autoRowHeight:false,
		...
		columns:[[
		    ...
		    {field:'duration',title:'耗时(毫秒)',width:40,align:'left',sortable:true}
		]]
	});
	//设置分页控件
