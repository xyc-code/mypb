...
<script>
	...
	function init(o) {
		...
		var dataGridColModel = [
			...
			, {label : '应用编码', name : 'applicationCode', width : 150 }
			, { label : '应用名称', name : 'applicationName', width : 150 }
		];
		var mult;
		...
		$('#jqGrid').jqGrid({
			url : 'console/profile/operation/sub/getApplist',
			mtype : 'POST',
			datatype : "json",
	...
	function ondblClickRow() {
		if (selectModel == "single") {
			...
			//parent.$("#" + idFiled).val( rowData.id);
			//parent.$("#" + textFiled).val(rowData.applicationName);
			setValue({ids:rowId, Names:rowData});
	...
	function getDataList() {
		...
		for ( var i = 0; i < roleidlist.length; i++) {
			...
			siteNames = siteNames + rowData.applicationName + ",";
		}
