...
<div id="tableToolbarrole" class="toolbar">
	<div class="toolbar-left">
		...
		<sec:accesscontrollist hasPermission="3" domainObject="formdialog_consoleRole_button_edit" permissionDes="编辑">
		<a id="consolerole_save" href="javascript:void(0)" class="btn btn-primary form-tool-btn btn-sm" role="button" title="编辑"><i class="icon icon-save"></i> 保存</a>
		</sec:accesscontrollist>
...
<script src="avicit/platform6/bs3centralcontrol/crole/js/cconsoleRole.js" type="text/javascript"></script> 
...
$(document).ready(function () {
	...
	var sysAppTree = new AppList('jqGridApp','0','searchDialog','form','keyWord');

        sysAppTree.setOnSelect(function(appId){
        appid = appId;
        if(consolerole){
        	consolerole.reLoadByAppid(appId);
             return;
        }
        if(appid=='-11'){
      		$('#exportrole').addClass("hide");
        }else{
        	$('#exportrole').removeClass("hide");
        }
		consolerole = new ConsoleRole('jqGrid','jqGridrole','${url}','searchDialog','form','sysRole_keyWord','sysUser_keyWord',searchMainNames,searchUserMainNames,dataGridUserColModel,dataGridroleColModel,appId);
