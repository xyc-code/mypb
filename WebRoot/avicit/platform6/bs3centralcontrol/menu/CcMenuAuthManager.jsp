...
<script type="text/javascript">
    ...
    var _map={};
    var appid="";
    var ifrm={'console_ifrm':'platform/console/authmenu/unified/admin?appid='+appid,
    'portal_ifrm':'platform/console/authmenu/unified?appid='+appid};

    $(function(){
        sysAppTree = new AppList('jqGridApp','1','searchDialog','form','keyWord');
        sysAppTree.setOnSelect(function(appId){
            appid=appId;
            var portalsrc="platform/console/authmenu/unified?appid="+appid;
            $("#portal_ifrm").attr("src",portalsrc);
        });
        $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
            if(!$('#'+e.target.rel).attr('src')){
                var portal_ifrm = $("#portal_ifrm");
                $('#'+e.target.rel).attr('src',ifrm[e.target.rel]);
                portal_ifrm.load(function(){
                    $("#portal_ifrm").contents().find("#mdiv").height(document.documentElement.clientHeight-88)
                })
            }
        });
