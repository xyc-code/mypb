	...
	avicAjax.ajax({
		...
		success : function(r) {
			if (r.flag == "success") {
				...
				if(r.error.indexOf('ORA-00001') != -1){
					layer.alert('保存失败，通用代码编码不能重复！', {
						icon : 7,
						area : [ '400px', '' ], //宽高
						closeBtn : 0
					});
...
//重载数据
SysLookupType.prototype.reLoad = function(appid) {
...
SysLookupType.prototype.doimport = function() {
    if(this.appid == -11){
        layer.alert('公共数据必须属于一个应用，不能直接导入，请选择应用后导入！', {
            icon : 7,
            area : [ '400px', '' ], //宽高
            title:'提示',
            closeBtn : 0
    ...
    layer.open({
        ...
        skin : 'bs-modal', // bootstrap 风格皮肤 需加载skin
        maxmin : false, //开启最大化最小化按钮
        content : this.getUrl() + 'importManage?appid='+_self.appid,
...
SysLookupType.prototype.doImportExcel = function() {
    ...
    importIndex = layer.open({
        ...
        skin: 'bs-modal', // bootstrap 风格皮肤 需加载skin
        maxmin: false, //开启最大化最小化按钮
        content: 'platform/excelImportController/excelimportnew/importLookupType/xls?extPara='+_self.appid,
        cancel: function(index, layero){
            _self.reLoad();
        }
    });
 ...
 * @param appid
 */
SysLookupType.prototype.switchApp = function(appid) {
	if(appid === this.appid){
		// reload
