...
<div data-options="region:'north',split:false,title:''" style="height: 70px; padding:0px;">
		...
		<form id="searchLookupForm">
			<table style=" MARGIN-RIGHT: auto; MARGIN-LEFT: auto;">
				<tr>
					...
					<td>系统代码类型:</td>
					<td>
						<input class="easyui-validatebox" name="filter-LIKE-LOOKUP_TYPE" id="filter_LIKE_loginName"/>
					</td>
					<td>系统代码类型名称:</td>
					<td>
						<input class="easyui-validatebox" name="filter-LIKE-LOOKUP_TYPE_NAME" id="filter_LIKE_name"/>
					</td>
					<td>有效标识:</td>
					<td>
						<input id="filter-EQ-VALID_FLAG" class="easyui-combobox" name="filter-EQ-VALID_FLAG"
							   data-options="panelHeight:'auto', editable:false,valueField:'lookupCode',textField:'lookupName', data: [{lookupCode:'1', lookupName: '有效'},{lookupCode:'0', lookupName: '无效'}]"/>
					</td>
				</tr>
	...
	<div style="TEXT-ALIGN: center; ">
		...
		<div id="searchBtns" style=" MARGIN-RIGHT: auto; MARGIN-LEFT: auto;">
			<a class="easyui-linkbutton" iconCls="icon-search" plain="true" onclick="searchData();"
			   href="javascript:void(0);">查询</a>
			<a class="easyui-linkbutton" iconCls="icon-cancel" plain="true" onclick="clearData();"
...
<div data-options="region:'center',split:true,border:false" style="overflow:hidden;">
	...
	<table id="tablelookup" class="easyui-datagrid" fit="true" pagination="true" rownumbers="true" fitColumns="true"
		   singleSelect="true" autoRowHeight="false" striped="true"
		   url="platform/eform/tabledefine/<%=timestamp%>/<%=appId%>/<%=language%>/allSysLookupType.json"
		   data-options="onLoadSuccess:cellTip">
		<thead>
		<tr>
			...
			<th data-options="field:'usageModifier',align:'center',align:'center',formatter:formatModifier" width="120">
				使用级别
			</th>
...
<!-- button js event -->
	<script type="text/javascript">
		...
		var url;
		 var ele = parent.EformEditor.getElement("check-box");
