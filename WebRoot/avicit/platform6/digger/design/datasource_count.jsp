...
<body >
        ...
        <table class="form_commonTable" style='width: 80%;'>
            <tr style="height:48px;" >
                ...
                <td width="40%">
                   <select id='lineHeight' name='lineHeight' class='form-control input-sm'>
                      ...
                      <option value='1' select>中</option>
            ...
            <tr style="height:48px;">
              <th width="10%" style="word-break: break-all; word-warp: break-word;"><label for="columnLock">列锁定:</label></th>
                ...
                <td width="40%" style="color:#999999">

                </td>
            </tr>
            <tr style="height:48px;">
                <th width="10%" style="word-break: break-all; word-warp: break-word;"><label for="showAggregateHeader">聚合表头:</label></th>
               <td width="40%" >
                    <input type='radio' name='showAggregateHeader' value='1'>是</input>&nbsp;
                    <input type='radio' name='showAggregateHeader' value='0' checked>否</input>
               </td>
                <th width="10%" style="word-break: break-all; word-warp: break-word;display:none;" id="aggregateHeader_th"><label for="aggregateHeader" >表头内容:</label></th>
                <td width="40%" id="aggregateHeader_td" style='display:none;'>
                    <input title="" class="form-control input-sm" type="text" name="aggregateHeader" id="aggregateHeader"  value='<c:out  value='${diggerTable.aggregateHeader}'/>'/>
                </td>
            </tr>
	...
	<script type="text/javascript">
		...
		var showAggregateHeader = '<c:out  value='${diggerTable.showAggregateHeader}'/>';//聚合表头
		var summaryColumn = '<c:out  value='${diggerTable.summaryColumn}'/>';//汇总列显示方式
		...
		$(function() {
            ...
            $(":radio[name='showAggregateHeader'][value='" + showAggregateHeader + "']").prop("checked", "checked");
            if(showNo == 'on'){
            ...
            if(showAggregateHeader == 1){
                $('#aggregateHeader_th').css('display','');
                $('#aggregateHeader_td').css('display','');
            }
            $(":radio[name='showAggregateHeader'][value='" + showAggregateHeader + "']").prop("checked", "checked");
		...
		var dataGridColumnModel = [
                 ...
                 formatter : formatAction,
                  ...
                  formatter : formatAlignType,
                   ...
                   editoptions:{
                      ...
                      value:{'DEFAULT':'普通列','CROSS_GROUP':'汇总列','CROSS_COUNT':'统计值'}
                   },
          ...
          $(function() {
                 diggerColumnGrid = new DiggerColumnGrid('diggerColumnGrid','${url}',dataGridColumnModel,'<c:out  value='${diggerInfo.id}'/>');
                 $('#datasource_insert').bind('click', function() {
                        diggerColumnGrid.insert();
                });
