...
$(function(){
	//$("#bpmImageDiv").width("100%").height("340").css({overflow:"auto",zindex:"300",position:"relative"});
	setTimeout(function(){
		//doglobaltransmit doglobalreaderdialog doglobaljumpdialog dosupplementdialog
		var width = $("#img").width() + 50;
		var height = $("#img").height() + 50;
		var array = ["doglobaltransmit","doglobalreaderdialog","doglobaljumpdialog","dosupplementdialog","dowithdrawdialog"];
		for(var i = 0; i < array.length; i++){
			var $iframe = parent.$("#"+array[i]+" iframe");
			if($iframe.length > 0){
...
function draw(obj){
	...
	for(var key in activityMap){
		...
		}else if(type == 'dowithdraw' && isCurrent == "true"){//拿回不需要展示选择框
			//有影响正常功能，先还原回来吧
			//parent.dowithdrawActivitySelect(processInstanceId,executionId,taskId,outcome,activityName);
		}
		if(curExecutionId == executionId && type == 'dowithdraw' && isCurrent == "true"){
			parent.dowithdrawActivitySelect(processInstanceId,executionId,taskId,outcome,activityName);
		}
	}
}
...
function doreader(isCurrent,activityName,activityAlias){
	if(isCurrent==true||isCurrent=="true"){
		alert("流程未流转节点不能作为流程读者的目标节点，请您重新选择。");
		return;
	}
	var b = confirm("您选择了【"+activityAlias+"】为流程读者的目标节点，确定吗？提示：如果该节点未流转将不能发送成功。");
	if(b){
...
<body style="margin: 0px; overflow: hidden;">
	<div id="tip" style="position: relative; padding-left: 24px; background-color: #fdfcc2; border: solid 2px #ffe4c4; font-family: Microsoft YaHei; FONT-SIZE: 12px">
		<span style="margin: 0px 12px 0px 2px; width: 16px; height: 16px; left: 0px; top: 0px; position: absolute; background-image: url(static/images/platform/bpm/client/images/icon-info-small.gif)">
		</span>
		<span><%=mes%></span>
	</div>
	<div id="bpmImageDiv" style="position: relative;">
		<img id="img" src="platform/bpm/clientbpmdisplayaction/getfiguretrack.gif?processInstanceId=<%=processInstanceId%>"/>
	</div>
</body>
</html>
