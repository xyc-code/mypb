...
function douserdefined(procinstDbid,executionId,taskId,outcome,targetActivityName,activityName,mySelectUser){
	var para = "procinstDbid="+procinstDbid+"&executionId="+executionId+"&taskId="+taskId+"&outcome="+outcome+"&targetActivityName="+activityName+"&mySelectUser="+ mySelectUser +"&type=dostepuserdefined&doSubmitUrl=" + escape("platform/bpm/clientbpmoperateaction/douserdefined") +  "&doSubmitCallEvent=backFinished&random="+Math.random();
	...
	var buttons = [{
		...
		handler:function(){
			...
			if(typeof(seleUserJson) != 'undefined'){
				var data = 'processInstanceId=' + processInstanceId + '&selectUserIds=' + seleUserJson +  '&activityName=' + activityName +  '&targetActivityName=' + activityName;
