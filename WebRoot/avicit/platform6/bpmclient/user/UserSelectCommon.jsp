...
var baseurl = '<%=ViewUtil.getRequestPath(request)%>';
	...
	var datas = parent.selectUserDatas;
	//面门的调用方式
// 	function eventClick(){
// 		var usd = new UserSelectDialog('userSelectCommonDialog',700,400,getPath2() + '/platform/user/bpmSelectUserAction/userSelectCommon?isMultiple=true','地址簿');
// 		var buttons = [{
// 			text:'确定',
// 			id : 'processSubimt',
// 			iconCls : 'icon-submit',
...
function loadOrgTree(){
	 $('#orgTree').tree({
 		 ...
 		 onBeforeExpand:function(node,param){
			...
			$('#orgTree').tree('options').url = getPath2() + beforeExpandUrl + 'nodeType=' + node.attributes.nodeType +'&para=' + para;
	    }
 	 });
}
/**
...
function loadSelectGridDatas(datas){
	if(typeof(datas) != 'undefined' && datas != null){
		$('#selectTargetDataGrid').datagrid('loadData',datas);
	}
}
//展开树
function expand() {
...
function eventSelectButtonClick(){
	...
	for(var i = 0; i < nodes.length; i++){
		if(nodes[i].attributes.nodeType == 'user' && getRecordIndexInTargetDataGrid(nodes[i].id) == -1){
			var record = {
		    		userId : nodes[i].id,
		    		userName : nodes[i].text,
		    		deptId : nodes[i].attributes.deptId,
		    		deptName : nodes[i].attributes.deptName
			};
			if(!isMultiple){
				var data = {
						total : 0,
...
function eventRemoveButtonClick(){
	...
	for(var i = 0 ; i < datas.length; i++){
		var data = datas[i];
		var index = getRecordIndexInTargetDataGrid(data.userId);
		$('#selectTargetDataGrid').datagrid('deleteRow', index);

		for(var i = 0; i < nodes.length; i++){
			if(getRecordIndexInTargetDataGrid(nodes[i].id) == -1 && nodes[i].id == data.userId){
