...
function doPlay(formId){
    ...
    function doPlay2(formId){
        layer.open({
            ...
            skin: 'bs-modal', // bootstrap 风格皮肤 需加载skin
            maxmin: false, //开启最大化最小化按钮
            content: 'platform/eform/bpmsCRUDClient/toViewJsp/xxbszb?id='+formId
        });
    }
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        avicAjax.ajax({
            url : 'avicit/video/videoPlayerController/operation/getVideoPlaysByPage',
            data : {
                videoType: '${videoType}'
            },
            type : 'POST',
            ...
            success : function(result) {
                if(result.dataListFirst.length>0){
                    drawFirstHtml(result.dataListFirst);
                }
                ...
                if(result.dataListFirst.length==0&&result.videoList.length==0){
                    var li = $('<li>暂无数据</li>');
                    $(".clearfix").append(li);
                }
            }
    ...
    function drawFirstHtml(videoList){
        for(var i=0;i<videoList.length;i++){
            var video=videoList[i];
            var li = $('<li></li>');
            var project_box = $('<div class="project_box">');
            ...
            project_box.append(project_box_span);
            var project_box_div = $('<div class="project_right"> </div>');
            project_box_div.append("<h4 class='project_title'>"+video.SCRIPT_NAME+"</h4>");
            //var play_div = $("<div class=\"project_content\" style=\"height: 250px;width:300px\"> </div>");
            var play_div = $("<div onclick=\"doPlay2('"+video.ID+"');\" class=\"project_content\" style=\"height: 250px;width:300px\"> </div>");
            var play_img = $("<img src=\"data:image/jpg;base64,"+video.LOGO_IMG+"\" style=\"max-width:300px;min-height:200px\"/>");
            play_div.append(play_img);

            var play_dept = $("<span class=\"project_time\">"+video.DRAF_UNIT+"<i style=\"margin-left: 10px;\">"+video.DRAF_USER+"</i><i style=\"margin-left: 10px;\">数量："+video.V_NUM+"</i></span>");
            var play_time = $("<span class=\"project_time\"><i>"+video.DRAF_DATE+"</i></span>");
            project_box_div.append(play_div);
            project_box_div.append(play_dept);
            project_box_div.append(play_time);
