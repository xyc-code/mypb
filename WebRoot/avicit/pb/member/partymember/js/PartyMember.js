 ...
 */
function PartyMember(datagrid, url, searchD, form, keyWordId, searchNames,
		dataGridColModel,status) {
	...
	if (status == 'partyMember') {
		this.init.call(this);
	} else {
		this.initSyn.call(this);
	}
};


	PartyMember.prototype.initSyn = function() {
		var _self = this;
		$(_self._datagridId)
			.jqGrid(
				{
					url : _self.getUrl() + 'getPartyMembersSnyByPage.json',
					mtype : 'POST',
					datatype : "json",
					toolbar : [ true, 'top' ],
 ...
 * 初始化操作
 */
PartyMember.prototype.init = function() {
	var _self = this;
	$(_self._datagridId)
			.jqGrid(
					{
						url : _self.getUrl() + 'getPartyMembersByPage.json',
						mtype : 'POST',
						datatype : "json",
						toolbar : [ true, 'top' ],
						colModel : _self.dataGridColModel,
 ...
 * 添加页面
 */
PartyMember.prototype.insert = function() {
 ...
 * 编辑页面
 */
PartyMember.prototype.modify = function() {
 ...
 * 详情页面
 */
PartyMember.prototype.detail = function(id) {
 ...
 * 打开高级查询框
 */
PartyMember.prototype.openSearchForm = function(searchDiv) {
	var _self = this;
 ...
 */
PartyMember.prototype.formValidate = function(form, regularDateRequireYN) {
	...
	form.validate({
		rules : {
			...
			/*
			 * graduationTime:{ required:true, dateISO:true },
			 */
			workTime : {
				required : true,
				dateISO : true
 ...
 * 保存方法
 */
PartyMember.prototype.save = function(form, id) {
	// debugger;
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
PartyMember.prototype.del = function() {
	var _self = this;
 ...
 * 重载数据
 */
PartyMember.prototype.reLoad = function() {
	var searchdata = {
 ...
 * 关闭对话框
 */
PartyMember.prototype.closeDialog = function(id) {
	if (id == "insert") {
 ...
 * 后台查询
 */
PartyMember.prototype.searchData = function() {
 ...
 * 快速查询
 */
PartyMember.prototype.searchByKeyWord = function() {
 ...
 * 隐藏查询框
 */
PartyMember.prototype.hideSearchForm = function() {
	layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
PartyMember.prototype.clearData = function() {
 ...
 * 数据同步到党员信息历史库
*/
PartyMember.prototype.dataSny = function () {
	var _self = this;
	avicAjax.ajax({
		url : _self.getUrl() + "dataSync",
		type : 'post',
		dataType : 'json',
		success : function(r) {
			if (r.flag == "success") {
