 ...
 */
function DynWorkCompany(datagrid,url,searchD,form,keyWordId,searchNames,dataGridColModel,pid){
 ...
 * 初始化操作
 */
DynWorkCompany.prototype.init=function(){
	var _self = this;
	$(_self._datagridId).jqGrid({
    	url: _self.getUrl()+'getDynWorkCompanysByPage.json',
    	postData : {
			pid : _self._pid
        },
        mtype: 'POST',
        ...
        pager: "#dynWorkCompanyJqGridPager"
    });
    $(_self._jqgridToolbar).append($("#dynWorkCompanyTableToolbar"));
 ...
 * 根据外键ID加载数据
 */
DynWorkCompany.prototype.loadByPid=function(pid){
	var _self = this;
	//将外键值放入高级查询form中
	$("#workId").val(pid);
	_self.reLoad(pid);
 ...
 * 添加页面
 */
DynWorkCompany.prototype.insert=function(pid){
 ...
 * 编辑页面
 */
DynWorkCompany.prototype.modify=function(){
	...
	this.editIndex = layer.open({
	    ...
	    content: this.getUrl()+'Edit/'+rowData.id + '/' + this._pid
	});
};
/**
 * 详情页面
 */
DynWorkCompany.prototype.detail=function(id){
 ...
 * 打开高级查询框
 */
DynWorkCompany.prototype.openSearchForm = function(searchDiv){
	var _self = this;
	var contentWidth = 800;
	var contentHeight = getClientHeight() - $(searchDiv).height() - event.clientY;
 ...
 */
DynWorkCompany.prototype.formValidate=function(form){
	form.validate({
		rules: {
			planDate:{
				dateISO:true
 ...
 * 保存方法
 */
DynWorkCompany.prototype.save=function(form,id){
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
DynWorkCompany.prototype.del=function(){
	var _self = this;
 ...
 * 重载数据
 */
DynWorkCompany.prototype.reLoad=function(pid){
	if(pid != undefined){
		this._pid = pid;
	}
 ...
 * 关闭对话框
 */
DynWorkCompany.prototype.closeDialog=function(id){
	if(id == "insert"){
 ...
 * 后台查询	
 */
DynWorkCompany.prototype.searchData =function(){
 ...
 * 快速查询
 */
DynWorkCompany.prototype.searchByKeyWord =function(){
 ...
 * 隐藏查询框
 */
DynWorkCompany.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
DynWorkCompany.prototype.clearData =function(){
