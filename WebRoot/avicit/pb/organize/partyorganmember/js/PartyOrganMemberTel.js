 ...
 * @param pid
 * @param searchNames
 * @param PartyOrganMember_KeyWord
 */
function PartyOrganMember(datagrid, url, form, dataGridColModel,searchD, pid, searchSubNames, partyOrganMember_KeyWord){
	...
	this._keyWordId="#" + partyOrganMember_KeyWord;
 ...
 * 回车查询
 */
PartyOrganMember.prototype.init=function(pid){
	var _self = this;
	$(_self._datagridId).jqGrid({
    	url: _self.getUrl()+'getPartyOrganMember',
 ...
 * 添加页面
 */
PartyOrganMember.prototype.insert=function(pid){
 ...
 * 编辑页面
 */
PartyOrganMember.prototype.modify=function(id){
	if(this.pid == null || this.pid == ""){
		 layer.alert('请选择一个主表节点！', {
				  icon: 7,
				  area: ['400px', ''], //宽高
	...
	var rowData = $(this._datagridId).jqGrid('getRowData', id);
	this.editIndex = layer.open({
		...
		area: ['960px', '500px'],
        maxmin: false,
	    content: this.getUrl()+'TelEdit/'+rowData.id 
	});
};
/**
 * 详情页面
 */
PartyOrganMember.prototype.detail=function(id){
 ...
 */
PartyOrganMember.prototype.formValidate=function(form){
	form.validate({
		rules: {
			       userIdAlias:{
						required:true,
 ...
 * 保存方法
 */
PartyOrganMember.prototype.save=function(form,id){
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
PartyOrganMember.prototype.del=function(){
	var _self = this;
 ...
 * 重载数据
 */
PartyOrganMember.prototype.reLoad=function(pid){
	if(pid != undefined){
		this.pid = pid;
	}
 ...
 * 关闭对话框
 */
PartyOrganMember.prototype.closeDialog=function(id){
	if(id == "insert"){
 ...
 * 打开高级查询框	
 */
PartyOrganMember.prototype.openSearchForm = function(searchDiv,par){
	var _self = this;
	par = null;
 ...
 * 去后台查询
 */
PartyOrganMember.prototype.searchData =function(){
 ...
 * 关键字段查询
 */
PartyOrganMember.prototype.searchByKeyWord =function(){
	//debugger;
	var keyWord = $(this._keyWordId).val()==$(this._keyWordId).attr("placeholder") ? "" :  $(this._keyWordId).val().replace(/(^\s*)|(\s*$)/g, "");
 ...
 * 隐藏查询框
 */
PartyOrganMember.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
PartyOrganMember.prototype.clearData =function(){
