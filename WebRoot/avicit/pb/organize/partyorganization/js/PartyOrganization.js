 ...
 * @returns
 */
function PartyOrganization(ztreeId, url, form, searchD, searchbtn,afterInit, clickRowLoad){
			...
			callback: {
				...
				onClick: function(event, treeId, treeNode){ //绑定左右联动事件
					if(issubinit == false){
			        	...
			        	if(treeNode != null && treeNode != ""){
			        		if($("#partyName")){
			        			$("#partyName").val(treeNode.text);
			        		}
			        		if($("#partyCode")){
			        			$("#partyCode").val(treeNode.attributes.partyCode);
			        		}
			        		if($("#attribute03")){
			        			$("#attribute03").val(treeNode.attributes.attribute03);
			        		}
					 ...
					 if (_self.firstAsyncSuccessFlag == 0) {
		                 ...
		                 if(nodes.length > 0){
		                	...
		                	layer.confirm('未找到根节点,确定要初始化根节点吗?',  {icon: 3, title:"提示", area: ['400px', ''],btn: ['确定', '取消']}, function(index){
			                		...
			                		$('#toolbar_partyOrganMember').hide();// 没有初始化根节点时,隐藏操作按钮
			                });
		                }
					 } 
 ...
 * 查询
 * */
PartyOrganization.prototype.searche = function (param) {
	var _self = this;
	console.log(JSON.stringify(param))
	$.ajax({
		type: "post",
		url:  _self.getUrl()+"/searchALL",
		dataType:"JSON",
		data:param,
		error: function(request) {
			throw new Error('操作失败，服务请求状态：'+request.status+' '+request.statusText+' 请检查服务是否可用！');
 ...
 * 添加平级节点
 */
PartyOrganization.prototype.insert = function(){
	hideRMenu();
	var _self = this;
 ...
 * 添加子节点
 */
PartyOrganization.prototype.insertSub = function(){
	hideRMenu();
	var _self = this;
 ...
 * 编辑节点
 */
PartyOrganization.prototype.modify = function(){
	hideRMenu();
	var _self = this;
	var selectedNodes = _self.tree.getSelectedNodes();
	if(null == selectedNodes[0]){
 ...
 * 更新父节点
 */
PartyOrganization.prototype.updateParent = function(){
	var _self = this;
	var selectedNodes = _self.tree.getSelectedNodes();
	if(selectedNodes[0]._parentId=== '-1'){
	...
	var selectIndex = layer.open({
        ...
        skin: 'bs-modal', // bootstrap 风格皮肤 需加载skin
        maxmin: false, //开启最大化最小化按钮
        content: this.getUrl() + '/toPartyOrganizationParentSelect',
        btn: ['确定', '取消'],
 ...
 * 保存方法
 */
PartyOrganization.prototype.save = function(form,dialog){
	debugger;
	    var _self = this;
		var selectedNodes = _self.tree.getSelectedNodes();
		if(null == selectedNodes[0] || undefined == selectedNodes[0]){
		...
		avicAjax.ajax({
			 url:_self.getUrl()+'/savePartyOrganization',
				 ...
				 if (r.flag == "success"){
					 if(r.type =='1'){//新建成功
						 ...
						 if (selectedNodes.length > 0) {
							 ...
							 selectedNodes[0].attributes.partyCode = r.data.attributes.partyCode;
							 _self.tree.updateNode(selectedNodes[0]);
							// _self.tree.reAsyncChildNodes(selectedNodes[0], "refresh");
 ...
 * 关闭对话框
 */
PartyOrganization.prototype.closeDialog=function(id){
	hideRMenu();
	if(id == "insert"){
 ...
 * 删除节点
 */
PartyOrganization.prototype.del=function(){
	  hideRMenu();
	  var _self = this;
	  var selectedNodes = _self.tree.getSelectedNodes();
	   if(selectedNodes[0]._parentId=== '-1'){
 ...
 */
PartyOrganization.prototype.insertRoot = function() {
	var _self = this;
	avicAjax.ajax({
		...
		success : function(r) {
			if (r.flag == "success") {
				$('#toolbar_partyOrganMember').show();// 初始化根节点成功,显示操作按钮
				_self.init();
				layer.msg('初始化根节点成功！');
			} else {
 ...
 */
PartyOrganization.prototype.formValidate = function(form){
	form.validate({
		rules: {
				   partyCode:{
						required:true,
