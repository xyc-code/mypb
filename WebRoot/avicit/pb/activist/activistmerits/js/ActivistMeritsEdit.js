 ...
 */
function ActivistMeritsEdit(datagrid,url,dataGridColModel,pid){
 ...
 * 初始化操作
 */
ActivistMeritsEdit.prototype.init=function(){
	var _self = this;
	...
	_self.lengthValidFiled.push('merits');
	_self.lengthValidFiledSize.push(50);
	_self.lengthValidFiledComment.push('绩效');
	_self.lengthValidFiled.push('type');
	_self.lengthValidFiledSize.push(50);
	_self.lengthValidFiledComment.push('类别');
};
/**
 * 添加页面
 */
...
ActivistMeritsEdit.prototype.insert=function(){
	//隐藏暂无数据的图片
	$("#subEmptyRecords").hide();
	var _self = this;
 ...
 * @param 
 */
ActivistMeritsEdit.prototype.nullvalid = function(data,item,nullfiled,notnullFiledComment){
	var msg = "";
 ...
 * @param 
 */
ActivistMeritsEdit.prototype.lengthvalid = function(data,item,lengthValidFiled,lengthValidFiledComment,lengthValidFiledSize){
	var msg = "";
	$.each(data,function(i,dataitem){
 ...
 * 删除
 */
ActivistMeritsEdit.prototype.del = function(){
	var _self = this;
 ...
 * 重载数据
 */
ActivistMeritsEdit.prototype.reLoad = function(pid){
	if(pid != undefined){
		this._pid = pid;
	}
 ...
 *  获取编辑表格数据
 */
ActivistMeritsEdit.prototype.getEditData = function(){
	var _self = this;
	var $info = $(_self._datagridId).jqGrid('endEditCell');
	if($info[0].flag){
	   layer.alert("非法含有JS脚本完整标签");
