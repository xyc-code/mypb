 ...
 */
function ActivistAchievementEdit(datagrid,url,dataGridColModel,pid){
 ...
 * 初始化操作
 */
ActivistAchievementEdit.prototype.init=function(){
	var _self = this;
	...
	_self.lengthValidFiled.push('awards');
	_self.lengthValidFiledSize.push(4000);
	_self.lengthValidFiledComment.push('获得奖项');
};
/**
 * 添加页面
 */
...
ActivistAchievementEdit.prototype.insert=function(){
	//隐藏暂无数据的图片
	$("#subEmptyRecords").hide();
	var _self = this;
 ...
 * @param 
 */
ActivistAchievementEdit.prototype.nullvalid = function(data,item,nullfiled,notnullFiledComment){
	var msg = "";
 ...
 * @param 
 */
ActivistAchievementEdit.prototype.lengthvalid = function(data,item,lengthValidFiled,lengthValidFiledComment,lengthValidFiledSize){
	var msg = "";
	$.each(data,function(i,dataitem){
 ...
 * 删除
 */
ActivistAchievementEdit.prototype.del = function(){
	var _self = this;
 ...
 * 重载数据
 */
ActivistAchievementEdit.prototype.reLoad = function(pid){
	if(pid != undefined){
		this._pid = pid;
	}
 ...
 *  获取编辑表格数据
 */
ActivistAchievementEdit.prototype.getEditData = function(){
	var _self = this;
	var $info = $(_self._datagridId).jqGrid('endEditCell');
	if($info[0].flag){
	   layer.alert("非法含有JS脚本完整标签");
