 ...
 */
function DynWorkPlan(datagrid,url,searchD,form,keyWordId,searchNames,dataGridColModel){
 ...
 * 初始化操作
 */
DynWorkPlan.prototype.init=function(){
	var _self = this;
	$(_self._datagridId).jqGrid({
    	url: _self.getUrl()+'getDynWorkPlansByPage.json',
        mtype: 'POST',
        datatype: "json",
        toolbar: [true,'top'],
        ...
        pager: "#dynWorkPlanJqGridPager"
    });
    $(_self._jqgridToolbar).append($("#dynWorkPlanTableToolbar"));
 ...
 * 添加页面
 */
DynWorkPlan.prototype.insert=function(){
 ...
 * 编辑页面
 */
DynWorkPlan.prototype.modify=function(){
 ...
 * 详情页面
 */
DynWorkPlan.prototype.detail=function(id){
 ...
 * 打开高级查询框
 */
DynWorkPlan.prototype.openSearchForm = function(searchDiv){
	var _self = this;
 ...
 */
DynWorkPlan.prototype.formValidate=function(form){
	form.validate({
		rules: {
			workName:{
                maxlength: 255
 ...
 * 保存方法
 */
DynWorkPlan.prototype.save=function(form,id){
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
DynWorkPlan.prototype.del=function(){
	var _self = this;
	var rows = $(_self._datagridId).jqGrid('getGridParam','selarrrow');
	var ids = [];
	var l =rows.length;
	if(l > 0){
		layer.confirm('删除当前数据同时会删除子表关联数据，您确定要删除当前数据?',  {icon: 3, title:"提示", area: ['400px', '']}, function(index){
				for(;l--;){
					 ids.push(rows[l]);
				 }
				 avicAjax.ajax({
					 ...
					 success : function(r){
						 if (r.flag == "success") {
							 _self.reLoad();
							var rows = $(_self._datagridId).jqGrid('getGridParam','getRowData');
							if(!rows){
								dynWorkCompany.loadByPid("-1");
							}
							layer.msg('删除成功！');
						}else{
 ...
 * 重载数据
 */
DynWorkPlan.prototype.reLoad=function(){
 ...
 * 关闭对话框
 */
DynWorkPlan.prototype.closeDialog=function(id){
	if(id == "insert"){
 ...
 * 后台查询	
 */
DynWorkPlan.prototype.searchData =function(){
 ...
 * 快速查询
 */
DynWorkPlan.prototype.searchByKeyWord =function(){
 ...
 * 隐藏查询框
 */
DynWorkPlan.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
DynWorkPlan.prototype.clearData =function(){
 ...
 * 返回主表id
 */
DynWorkPlan.prototype.getSelectID=function(){
