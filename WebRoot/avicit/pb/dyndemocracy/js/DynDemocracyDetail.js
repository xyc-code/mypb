 ...
 */
function DynDemocracyDetail(form){
	this._formId="#"+form;
	DefaultForm.call(this);
};
DynDemocracyDetail.prototype = new DefaultForm();
DynDemocracyDetail.prototype.formcode = "dynDemocracy";
/**
 * 初始化操作
 */
DynDemocracyDetail.prototype.initFormData = function(){
	var _self = this;
	avicAjax.ajax({
		url : 'platform/avicit/lc/youth/dynDemocracy/dynDemocracyController/initDetailFormData.json',
		data : {
			id : _self.id
		},
		type : 'POST',
		...
		success : function(result) {
			if (result.flag == "success") {
				_self.setForm($(_self._formId),result.dynDemocracyDTO);
			} else {
				layer.msg('数据加载失败！');
			}
		}
 ...
 * 启动流程
 */
DynDemocracyDetail.prototype.start = function(defineId, callback){
	var _self = this;
	//表单校验
	...
	avicAjax.ajax({
		url : 'platform/avicit/lc/youth/dynDemocracy/dynDemocracyController/operation/saveAndStartProcess',
		data : {
 ...
 * 更新数据
 */
DynDemocracyDetail.prototype.save=function(callback){
	var _self = this;
	//表单校验
	var isValidate = $("#form").validate();
	if (!isValidate.checkForm()) {
	...
	var dataVo = JSON.stringify(serializeObject($(_self._formId)));
	avicAjax.ajax({
		url : 'platform/avicit/lc/youth/dynDemocracy/dynDemocracyController/operation/save',
		data : {
			data : dataVo
		},
		type : 'POST',
 ...
 * 表单重载json对象数据
 */
DynDemocracyDetail.prototype.setForm = function(formObj, jsonValue){
    var obj = formObj;
...
//控制附件
DynDemocracyDetail.prototype.setAttachMagic = function(attachMagic) {
...
//流程控制-多附件必填
DynDemocracyDetail.prototype.setAttachRequired = function(tagId,required,obj){
     eval("this.attachRequiredMap." + tagId + " = " + required);
};

//流程控制-多附件密级等级是否可编辑
DynDemocracyDetail.prototype.setAttachSecretLevelModify = function(tagId,modify,obj){
...
DynDemocracyDetail.prototype.attachRequiredMap = {};
DynDemocracyDetail.prototype.validAttachRequired = function(){
...
//多附件增删控制
DynDemocracyDetail.prototype.setAttachCanAddOrDel = function(tagId,operability,obj){
    if(operability) {
 ...
 */
DynDemocracyDetail.prototype.formValidate=function(form){
	form.validate({
		rules: {
			democracyDate:{
				dateISO:true
