 ...
 */
function PartyMilepost(datagrid,url,searchD,form,keyWordId,searchNames,dataGridColModel,pid){
 ...
 * 初始化操作
 */
PartyMilepost.prototype.init=function(){
	var _self = this;
	this.flowListEditor = new FlowListEditor(this._formCode);
	$(_self._datagridId).jqGrid({
    	url: _self.getUrl()+'getPartyMilepostsByPage.json',
    	postData : {
        	pid :_self._pid
        },
        mtype: 'POST',
        ...
        pager: "#partyMilepostJqGridPager"
    });
    $(_self._jqgridToolbar).append($("#partyMilepostTableToolbar"));
 ...
 * 根据外键ID加载数据
 */
PartyMilepost.prototype.loadByPid=function(pid){
	var _self = this;
	//将外键值放入高级查询form中
	$("#commandosId").val(pid);
	_self.reLoad(pid);
 ...
 * 添加页面
 */
PartyMilepost.prototype.insert=function(pid){
 ...
 * 添加页面
 */
PartyMilepost.prototype.insertFlow=function(){
	//debugger;
	var _self = this;
	if(_self.getSelectID() == "null" || _self.getSelectID() == "undefined"){
		 layer.alert('请选择一条数据！', {
			  icon: 7,
			  area: ['400px', ''],
			  closeBtn: 0,
	 ...
	 return false;
		...
		var selectedRow = _self.getSelectRow();
		if(selectedRow != null){
			if(selectedRow.businessstate_ != ''){
				 layer.alert('流程已经发起，无法重复发起！', {
					  icon: 7,
					  area: ['400px', ''],
					  closeBtn: 0,
					  btn: ['关闭'],
				...
				var selectId = _self.getSelectID();
				this.flowListEditor.addFlow(selectId);
			}
		}
 ...
 * 编辑页面
 */
PartyMilepost.prototype.modify=function(){
 ...
 * 详情页面
 */
PartyMilepost.prototype.detail=function(id){
 ...
 * 打开高级查询框
 */
PartyMilepost.prototype.openSearchForm = function(searchDiv){
	var _self = this;
 ...
 */
PartyMilepost.prototype.formValidate=function(form){
	form.validate({
		rules: {
			userIdAlias:{
			},
 ...
 * 保存方法
 */
PartyMilepost.prototype.save=function(form,id){
	var _self = this;
	avicAjax.ajax({
 ...
 * 删除方法
 */
PartyMilepost.prototype.del=function(){
	var _self = this;
 ...
 * 重载数据
 */
PartyMilepost.prototype.reLoad=function(pid){
	if(pid != undefined){
		this._pid = pid;
	}
 ...
 * 关闭对话框
 */
PartyMilepost.prototype.closeDialog=function(id){
	if(id == "insert"){
 ...
 * 后台查询	
 */
PartyMilepost.prototype.searchData =function(){
 ...
 * 快速查询
 */
PartyMilepost.prototype.searchByKeyWord =function(){
 ...
 * 隐藏查询框
 */
PartyMilepost.prototype.hideSearchForm =function(){
	layer.close(searchDialogindex);
};
/**
 * 清空查询条件
 */
PartyMilepost.prototype.clearData =function(){
 ...
 * 返回主表id
 */
PartyMilepost.prototype.getSelectID=function(){
 ...
 * 返回所选数据
 */
PartyMilepost.prototype.getSelectRow=function(){
	var rows = $(this._datagridId).jqGrid('getGridParam', 'selarrrow');
	var rowData = $(this._datagridId).jqGrid('getRowData', rows[0]);
	return rowData;
};
PartyMilepost.prototype.flowDetail=function(id,value){
