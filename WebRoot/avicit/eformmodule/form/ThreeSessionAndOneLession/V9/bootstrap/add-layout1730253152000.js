...
function saveForm(aftercallback) {
    ...
    if(isInsert) {
    	if($("#LEAVE_PEOPLE_NUM").val()!='0'&&$("#LEAVEL_PEOPLE").val()==''){
  layer.msg("当有缺席人员时请填写请假人员!");
  $("#page_saveButton").removeClass('disabled');
     return false;
   }
if($("#LEAVEL_PEOPLE").val().split(",").length!=$("#LEAVE_PEOPLE_NUM").val()&&$("#LEAVE_PEOPLE_NUM").val()!='0'){
  layer.msg("缺席人数与请假人员人数不符!");
  $("#page_saveButton").removeClass('disabled');
     return false;
