...
for(var i = 0;i < dyn_names.length;i++){
 ...
 for(var j = 0 ;j < rows.length ;j++){
  ...
  jsonData.TOTAL_PRICE += total_price;
    ...
    var processDef = new FlowListEditorBySec("PARTY_ACTIVITY_MONEY",JSON.stringify(jsonData));
    ...
    if(saveFlag){
        FlowListEditorBySec.prototype.doStart = function(pdId){
            ...
            var url = 'platform/eform/bpmsCRUDClient/addbpm?formInfoId=402811817f19a849017f1f1222c41c07&tableId='+tableId+'&defineId='+pdId+'&formcode=PARTY_ACTIVITY_MONEY';
            avicAjax.ajax({
                url: url,
                data : jsonData,
                type : 'post',
