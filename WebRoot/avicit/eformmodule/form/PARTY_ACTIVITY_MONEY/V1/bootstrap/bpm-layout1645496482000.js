 ...
 * formcode
 */
EformFlow.prototype.formcode = "PARTY_ACTIVITY_MONEY";
...
for(var i = 0;i < dyn_names.length;i++){
 ...
 for(var j = 0 ;j < rows.length ;j++){
  ...
  jsonData.TOTAL_PRICE += total_price;
    ...
    var url = 'platform/eform/bpmsCRUDClient/addbpm?formInfoId=402811817f19a849017f1f1222c41c07&tableId='+tableId+'&defineId='+defineId+'&formcode='+ _self.formcode;
...
for(var i = 0;i < dyn_names.length;i++){
 ...
 for(var j = 0 ;j < rows.length ;j++){
  ...
  jsonData.TOTAL_PRICE += total_price;
    ...
    jsonData["idmap"] = idmap;
    var tableId = $('#tableId').val();
    var url = 'platform/eform/bpmsCRUDClient/bpm?formInfoId=402811817f19a849017f1f1222c41c07&tableId='+tableId+'&id='+pageParams.id;
...
var DYN_PA_XXZLTabInitFlag = false;

					
var dataGridColModel_DYN_PA_XXZL =  [
...
var extraParamDYN_PA_XXZL = '{}';


DYN_PA_XXZLUpdateColData = function(value,colname) {
    colname = colname || "";
...
if (window.DYN_PA_XXZL_comid == null || window.DYN_PA_XXZL_comid == undefined || window.DYN_PA_XXZL_comid == '') {
	window.DYN_PA_XXZL_comid = id;
}

$('#DYN_PA_XXZL').jqGrid({
	url: 'eform/bpmsCRUDClient/getSubList.json?subTableName=DYN_PA_XXZL',
    mtype: 'POST',
    datatype: "local",
    postData: {comId:window.DYN_PA_XXZL_comid,fkColName:"FK_SUB_COL_ID",formInfoId:"402811817f19a849017f1f1222c41c07",version:pageParams.version,taskId:typeof(_taskName) != "undefined"?_taskName:"",pageParams:JSON.stringify(pageParams)},
    toolbar: [true,'top'],
    colModel:dataGridColModel_DYN_PA_XXZL,
    scrollOffset: 20,
    altRows:true,
	jsonReader:{
	},
...
beforeRequest: function () {
    $("#DYN_PA_XXZLnorecords").hide();//隐藏提示信息
    $("#DYN_PA_XXZLPager_left").css("display", "block");//隐藏分页信息
},

loadComplete:function(xhr){

    var rowdata = $('#DYN_PA_XXZL').jqGrid('getRowData');
    if (rowdata != null && rowdata.length > 0) {

    } else {
        if ($("#DYN_PA_XXZLnorecords").html() == null) {
            $('#DYN_PA_XXZL').parent().append("<div class='no_data' style='display: none' id='DYN_PA_XXZLnorecords'> <img style='width:120px;' src='static/images/platform/eform/no-data.png' /> <p>暂无记录</p> </div>");
        }
        var height = $('#DYN_PA_XXZL').closest('.ui-jqgrid-bdiv').height();
        $("#DYN_PA_XXZLnorecords").css('margin-top',(height/5)+'px');
        if ((height/1.9)>120){
            $("#DYN_PA_XXZLnorecords img").css("width","120px");
        }else{
            $("#DYN_PA_XXZLnorecords img").css("width",(height/1.9)+"px");
        }
        $("#DYN_PA_XXZLnorecords").show();//显示提示信息
        $("#DYN_PA_XXZLPager_left").css("display", "none");//显示分页信息
    }
},
...
beforeEditCell:function(){
	$(".datatable").not("#DYN_PA_XXZL").each(function(){
...
DYN_PA_XXZLReload = function(){
	var _isInvalid = true;
	$("#DYN_PA_XXZL").parents('.tab-pane').each(function(){
	...
	DYN_PA_XXZLTabInitFlag = true;
	$('#DYN_PA_XXZL').jqGrid('setGridParam',{datatype:'json'}).trigger("reloadGrid");
}

DYN_PA_XXZLTabReload = function(forceFlag){
	if(!DYN_PA_XXZLTabInitFlag  || forceFlag){
		DYN_PA_XXZLReload();
	}
...
//放入表格toolbar中
$('#t_DYN_PA_XXZL').append($('#DYN_PA_XXZLToolbar'));
		...
		var rowid = $('#DYN_PA_XXZL').jqGrid("getGridParam", "selrow");
		var rowdata = $('#DYN_PA_XXZL').jqGrid("getRowData", rowid);
		...
		$('#DYN_PA_XXZL').jqGrid("setCell",rowid,"XXZL_TOTAL_PRICE",calculateValue);
		DYN_PA_XXZLUpdateColData(0,'XXZL_TOTAL_PRICE');
	}
 ...
 * 表格编辑参数
 */
var newRowIndex_DYN_PA_XXZL = 0;
var newRowStart_DYN_PA_XXZL = "new_row";


/**
 * 添加页面
 */
insertTableDYN_PA_XXZL = function(){
	$('#DYN_PA_XXZL').jqGrid('endEditCell');
	$("#DYN_PA_XXZLnorecords").hide();//隐藏提示信息
	$("#DYN_PA_XXZLPager_left").css("display", "block");//隐藏分页信息
	var newRowId = newRowStart_DYN_PA_XXZL + newRowIndex_DYN_PA_XXZL;
	var parameters = {
		rowID : newRowId,
		initdata : {
																												},
	...
	$('#DYN_PA_XXZL').jqGrid('addRow', parameters);
	newRowIndex_DYN_PA_XXZL++;  
};
 ...
 * 删除
 */
deleteTableDYN_PA_XXZL = function(){
	var rows = [];
	rows = $('#DYN_PA_XXZL').jqGrid('getGridParam','selarrrow');
		...
		layer.confirm('确定要删除该数据吗?', function(index){
			for(;l--;){
				if (rows[l].indexOf("new_row")!=-1){
					$('#DYN_PA_XXZL').jqGrid("delRowData", rows[l]);  
				}else{
					ids.push(rows[l]);
				}
			}
			if (ids.length>0){
				avicAjax.ajax({
					url: 'eform/bpmsCRUDClient/deleteByIds.json?subTableName=DYN_PA_XXZL',
...
$('#DYN_PA_XXZL').setGridWidth(700);
$('#DYN_PA_XXZL').jqGrid('resizeGrid');



//按钮绑定事件
$('#DYN_PA_XXZL_insertBtn').bind('click',function(){
	insertTableDYN_PA_XXZL();
});

//按钮绑定事件
$('#DYN_PA_XXZL_deleteBtn').bind('click',function(){
	deleteTableDYN_PA_XXZL();
});
...
//流程控制
workflowControl.addcontrolFunc(function(tagId, operability){
	if (tagId != "DYN_PA_XXZL_control"){
		return;
	}
	$("#"+tagId).find("input").removeAttr("disabled");
	var id = tagId.split("_control")[0];
	if(operability){
		jQuery("#t_"+id).find("#DYN_PA_XXZL_insertBtn").css("display","inline-block");
		jQuery("#t_"+id).find("#DYN_PA_XXZL_deleteBtn").css("display","inline-block");
		jQuery("#"+id).jqGrid('setGridParam',{'cellEdit':true});
	}else{
		jQuery("#t_"+id).find("#DYN_PA_XXZL_insertBtn").css("display","none");
		jQuery("#t_"+id).find("#DYN_PA_XXZL_deleteBtn").css("display","none");
...
workflowControlSubTableButtonForAccess.addcontrolFunc(function(tagId, operability){
	if (tagId != "DYN_PA_XXZL_insertBtn"){
		return;
	}
	if(operability){
		$("#DYN_PA_XXZL_insertBtn").css("display","inline-block");
	}else{
		$("#DYN_PA_XXZL_insertBtn").css("display","none");
	}
});
//子表删除按钮流程控制
workflowControlSubTableButtonForAccess.addcontrolFunc(function(tagId, operability){
	if (tagId != "DYN_PA_XXZL_deleteBtn"){
		return;
	}
	if(operability){
		$("#DYN_PA_XXZL_deleteBtn").css("display","inline-block");
		var subTableName = "DYN_PA_XXZL_deleteBtn".replace("deleteBtn","control");
		$("#"+subTableName).find(".checkbox").removeAttr("disabled");
	}else{
		$("#DYN_PA_XXZL_deleteBtn").css("display","none");
		var flag = true;
...
beforeSaveEvent.addBeforeSaveEvent(function(formData){
	return $('#DYN_PA_XXZL').validateJqGrid("validate");
});

$('#DYN_PA_XXZL').validateJqGrid("addValidate","XXZL_NAME","required");
		$('#DYN_PA_XXZL').validateJqGrid("addValidate","XXZL_NAME","maxlength",{maxlength:255});
		$('#DYN_PA_XXZL').validateJqGrid("addValidate","XXZL_NUM","required");
		$('#DYN_PA_XXZL').validateJqGrid("addValidate","XXZL_NUM","maxlength",{maxlength:20});
		$('#DYN_PA_XXZL').validateJqGrid("addValidate","XXZL_UNION_PRICE","required");
		$('#DYN_PA_XXZL').validateJqGrid("addValidate","XXZL_UNION_PRICE","maxlength",{maxlength:20});
		$('#DYN_PA_XXZL').validateJqGrid("addValidate","XXZL_TOTAL_PRICE","required");
		$('#DYN_PA_XXZL').validateJqGrid("addValidate","XXZL_TOTAL_PRICE","maxlength",{maxlength:20});
				$('#DYN_PA_XXZL').validateJqGrid("addValidate","FK_SUB_COL_ID","maxlength",{maxlength:50});
...
$('#DYN_PA_KNDY').parents('div.ui-jqgrid-bdiv').css("max-height","150px");
