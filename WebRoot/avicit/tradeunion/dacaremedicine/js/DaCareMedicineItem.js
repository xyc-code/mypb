 ...
 */
function DaCareMedicineItem(datagrid,url,dataGridColModel,pid){
 ...
 * 初始化操作
 */
DaCareMedicineItem.prototype.init=function(){
    var _self = this;
    $(_self._datagridId).jqGrid({
        url: _self.getUrl()+'getDaCareMedicineItemsByPage.json',
        mtype: 'POST',
        datatype: "json",
        postData : {
            pid : _self._pid
    ...
    _self.lengthValidFiled.push('hospital');
    _self.lengthValidFiledSize.push(50);
    _self.lengthValidFiledComment.push('治疗医院');
    _self.lengthValidFiled.push('inhospDate');
    _self.lengthValidFiledSize.push(50);
    _self.lengthValidFiledComment.push('住院日期开始');
    _self.lengthValidFiled.push('outhospDate');
    _self.lengthValidFiledSize.push(50);
    _self.lengthValidFiledComment.push('住院日期结束');
    _self.lengthValidFiled.push('healthExpense');
    _self.lengthValidFiledSize.push(4000);
    _self.lengthValidFiledComment.push('医疗费金额');
    _self.lengthValidFiled.push('overallExpense');
    _self.lengthValidFiledSize.push(50);
    _self.lengthValidFiledComment.push('统筹资金支付');
...
DaCareMedicineItem.prototype.insert=function(){
    //去除暂无数据的图片
    $("#subEmptyRecords").hide();
    var _self = this;
 ...
 * @param
 */
DaCareMedicineItem.prototype.nullvalid = function(data,item,nullfiled,notnullFiledComment){
    var msg = "";
 ...
 * @param
 */
DaCareMedicineItem.prototype.lengthvalid = function(data,item,lengthValidFiled,lengthValidFiledComment,lengthValidFiledSize){
    var msg = "";
    $.each(data,function(i,dataitem){
 ...
 * 删除
 */
DaCareMedicineItem.prototype.del = function(){
 ...
 * 重载数据
 */
DaCareMedicineItem.prototype.reLoad = function(pid){
    if(pid != undefined){
        this._pid = pid;
    }
 ...
 *  子表校验
 */
DaCareMedicineItem.prototype.dataValidte = function(){
    var _self = this;
    var $info = $(_self._datagridId).jqGrid('endEditCell');
    if($info[0].flag){
        layer.alert("非法含有JS脚本完整标签");
 ...
 *  获取编辑表格数据
 */
DaCareMedicineItem.prototype.getEditData = function(){
    //debugger;
    var _self = this;
    var data = $(_self._datagridId).jqGrid('getChangedCells');
 ...
 *  表格是否有数据
 */
DaCareMedicineItem.prototype.getDataExist = function(){

    var _self = this;
    var recordNum = $(_self._datagridId).jqGrid('getGridParam','records');
    if(recordNum > 0){
        return true;
