...
//$("#tiphs").html("");//此次申请核算报销金额
// $("#tipsy").html("");//此患者本年额度仅剩
//$("#tipyr").html("");//本次申请报销余额

} else {


}//非流程表单页面，也就是点击发起流程按钮弹出的页面
var msg = "";
beforeSaveEvent.addBeforeSaveEvent(function (jsonData) {
    ...
    var retVal = true;
    if (status != "yes") {
        /*按照最新要求，余额不足的情况下，给出提示，只能按照年度剩余金额 进行报销，确认同意后流程提交，不同意则不发起流程*/
        //layer.msg(msg);
        retVal = window.confirm(msg);

    }

    if (retVal) {//confirm 对话框 返回值
        ...
        if (diseaseType == "0") {//一般住院治疗
            ...
            for (var i = 0; i < allRowData.length; i++) {
                ...
                if (firstDate && lastDate) {
                    ...
                    //判断两笔住院时间是否满足10个工作日
                    avicAjax.ajax({
                        url: 'platform/avicit/tradeunion/daCareMedicineitem/daCareMedicineItemController/operation/getCallLeaveDays.json',
                        data: {
                            firstDate: firstDate,
                            lastDate: lastDate
                        },
                        ...
                        success: function (result) {
                            ...
                            if (result.leaveDays > 10) {//上次住院日期结束到下次住院日期开始间不超过10个工作日
                                flag = true;
                                layer.msg("转院间隔不能超过10个工作日");
                            }
    ...
    avicAjax.ajax({
        ...
        success: function (result) {
            ...
            if (result.isSubmit == "yes") {
                ...
                /*年度余额不够的情况下，按照最新要求，直接将余额设置为0*/
                // layer.msg(result.retMessage);
                //$("#BALANCE").val(0);//余额
                //msg=result.retMessage;
                $("#BALANCE").val(0.00);//余额
                var sbjr = Number(Number(submitExpensess).toFixed(2) - Number(Math.abs(result.retValue)).toFixed(2)).toFixed(2);
                $("#SUBMIT_EXPENSES").val(sbjr);//申请报销金额 为仅剩余的金额，//申请报销金额-余额
                msg = "本年度个人报销已超过5万元，仅能按年度剩余额度报销，是否确定提交报销？";
                $("#STATUS").val("no");
            }
...
function submitExpenses(type) {
    ...
    var overallExpnses = $("#OVERALL_EXPENSES").val().replace(reg, "");//统筹基金支付总金额

    if (personExpenses && personExpenses >= 0 && overallExpnses >= 0) {
        ...
        if ("0" == diseaseType) {//一般住院医疗
            ...
            if (personExpenses >= 2000 && personExpenses <= 3000) {
                ...
                $("#BXBL").val(0.10).trigger("change");//申请报销金额
            } else if (personExpenses > 3000 && personExpenses <= 4000) {
                //console.log("个人支付总金额："+personExpenses);
                var personExpense = Number(personExpenses * 15 / 100).toFixed(2);
                //console.log("申请报销金额："+personExpense);
                $("#SUBMIT_EXPENSES").val(personExpense).trigger("change");//申请报销金额
                $("#BXBL").val(0.15).trigger("change");

            } else if (personExpenses > 4000 && personExpenses <= 5000) {
                // console.log("个人支付总金额："+personExpenses);
                var personExpense = Number(personExpenses * 20 / 100).toFixed(2);
                //console.log("申请报销金额："+personExpense);
                $("#SUBMIT_EXPENSES").val(personExpense).trigger("change");//申请报销金额
                $("#BXBL").val(0.20).trigger("change");

            } else if (personExpenses > 5000 && personExpenses <= 6000) {
                //console.log("个人支付总金额："+personExpenses);
                var personExpense = Number(personExpenses * 25 / 100).toFixed(2);
                //console.log("申请报销金额："+personExpense);
                $("#SUBMIT_EXPENSES").val(personExpense).trigger("change");//申请报销金额
                $("#BXBL").val(0.25).trigger("change");
            } else if (personExpenses > 6000 && personExpenses <= 7000) {
                // console.log("个人支付总金额："+personExpenses);
                var personExpense = Number(personExpenses * 30 / 100).toFixed(2);
                //console.log("申请报销金额："+personExpense);
                $("#SUBMIT_EXPENSES").val(personExpense).trigger("change");//申请报销金额
                $("#BXBL").val(0.30).trigger("change");
            } else if (personExpenses > 7000 && personExpenses <= 8000) {
                //console.log("个人支付总金额："+personExpenses);
                var personExpense = Number(personExpenses * 35 / 100).toFixed(2);
                //console.log("申请报销金额："+personExpense);
                $("#SUBMIT_EXPENSES").val(personExpense).trigger("change");//申请报销金额
                $("#BXBL").val(0.35).trigger("change");
            } else if (personExpenses > 8000 && personExpenses <= 9000) {
                //console.log("个人支付总金额："+personExpenses);
                var personExpense = Number(personExpenses * 40 / 100).toFixed(2);
                //console.log("申请报销金额："+personExpense);
                $("#SUBMIT_EXPENSES").val(personExpense).trigger("change");//申请报销金额
                $("#BXBL").val(0.40).trigger("change");
            } else if (personExpenses > 9000 && personExpenses <= 10000) {
                //console.log("个人支付总金额："+personExpenses);
                var personExpense = Number(personExpenses * 45 / 100).toFixed(2);
                //console.log("申请报销金额："+personExpense);
                $("#SUBMIT_EXPENSES").val(personExpense).trigger("change");//申请报销金额
                $("#BXBL").val(0.45).trigger("change");
            } else if (personExpenses > 10000) {
                //console.log("个人支付总金额："+personExpenses);
                var personExpense = Number(personExpenses * 50 / 100).toFixed(2);
                //console.log("申请报销金额："+personExpense);
                $("#SUBMIT_EXPENSES").val(personExpense).trigger("change");//申请报销金额
                $("#BXBL").val(0.50).trigger("change");
            } else {//小于2000以下的

                $("#SUBMIT_EXPENSES").val(0.00).trigger("change");//申请报销金额
                $("#BXBL").val(0.00).trigger("change");
            }
...
var submitExpenses2 = 0;

function getSubmitExpenses() {
    return submitExpenses2;
}

function submitExpensesFunc() {
    console.log('申请金额改变事件执行了.......')
    var submitExpenses = $("#SUBMIT_EXPENSES").val();//申请报销金额
    console.log("申请金额初始金额", submitExpenses);
    submitExpenses2 = submitExpenses;

    var SYBXPFJE = Number(document.getElementById('SYBXPFJE').value);//商业保险赔付金额


    var SUBMIT_EXPENSES = Number(document.getElementById('SUBMIT_EXPENSES').value);//申请报销金额
    var PERSON_EXPENSES = Number(document.getElementById('PERSON_EXPENSES').value);//个人支付金额

    console.log((SUBMIT_EXPENSES + SYBXPFJE) > PERSON_EXPENSES);

    console.log('商业保险赔付金额', SYBXPFJE);
    console.log('申请报销金额', SUBMIT_EXPENSES);

    console.log('个人支付金额', PERSON_EXPENSES);

    if(SYBXPFJE>0){
        if ((SUBMIT_EXPENSES + SYBXPFJE) > PERSON_EXPENSES) {
            SUBMIT_EXPENSES = parseFloat((PERSON_EXPENSES - SYBXPFJE).toFixed(2));
            document.getElementById('SUBMIT_EXPENSES').value = SUBMIT_EXPENSES;
            console.log(SUBMIT_EXPENSES);
        }else{
