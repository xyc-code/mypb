 ...
 * @param form 表单id
 */
function PmProjectYsHpj(datagrid,url,searchD,form){
 ...
 * 回车查询
 */
PmProjectYsHpj.prototype.init=function(){
	var _self = this;
	...
	_self._datagrid=$(_self._datagridId).datagrid({
		url:_self.getUrl()+"getPmProjectYsHpjsByPage.json"
	});
};
 /**
 * 打开添加页面
 */
PmProjectYsHpj.prototype.insert=function(){
 ...
 * 打开编辑页面
 */
PmProjectYsHpj.prototype.modify=function(){
 ...
 * @param id 选中数据id
 */
PmProjectYsHpj.prototype.detail=function(id){
 ...
 * @param callback 回调函数
 */
PmProjectYsHpj.prototype.save=function(form,callback){
	var _self = this;
 ...
 * 删除方法
 */
PmProjectYsHpj.prototype.del=function(){
	var rows = this._datagrid.datagrid('getChecked');
	var _self = this;
	var ids = [];
	var l =rows.length;
 ...
 * 重载数据
 */
PmProjectYsHpj.prototype.reLoad=function(){
 ...
 * @param id 关闭的对话框id
 */
PmProjectYsHpj.prototype.closeDialog=function(id){
	$(id).dialog('close');
};
 ...
 * 打开查询框
 */
PmProjectYsHpj.prototype.openSearchForm =function(){
	this.searchDialog.dialog('open',true);
};
/**
 * 去后台查询(查询日期时,结束日期不能小于起始日期)
 */
PmProjectYsHpj.prototype.searchData =function(){
 ...
 * 隐藏查询框
 */
PmProjectYsHpj.prototype.hideSearchForm =function(){
	this.searchDialog.dialog('close',true);
};
/**
 * 清空查询条件
 */
PmProjectYsHpj.prototype.clearData =function(){
 ...
 * @param value 格式化的值
 */
PmProjectYsHpj.prototype.format=function(value){
	if(value){
		if($.browser.msie){
 ...
 * @param value 格式化的值
 */
PmProjectYsHpj.prototype.formatDateTime=function(value){
	if(value){
		if($.browser.msie){
			if(typeof(value) == "number"){
 ...
 */
PmProjectYsHpj.prototype.searchDataBySfn =function(conditions){
    this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
    this._datagrid.datagrid('load',conditions);
};
        PmProjectYsHpj.prototype.secretLevel=[];
Platform6.fn.lookup.getLookupType('PLATFORM_FILE_SECRET_LEVEL',function(r){r&&( PmProjectYsHpj.prototype.secretLevel=r);});  
