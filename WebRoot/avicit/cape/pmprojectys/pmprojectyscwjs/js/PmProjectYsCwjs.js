 ...
 * @param form 表单id
 */
function PmProjectYsCwjs(datagrid,url,searchD,form){
	...
	this._formCode = "pmProjectYsCwjs";
	};
/**
 * 初始化操作
 * 回车查询
 */
PmProjectYsCwjs.prototype.init=function(){
	var _self = this;
	...
	_self._datagrid=$(this._datagridId).datagrid({
		url:_self.getUrl()+"getPmProjectYsCwjssByPage.json",
 ...
 * 打开添加页面
 */
PmProjectYsCwjs.prototype.insert=function(){
 ...
 * 打开编辑页面
 */
PmProjectYsCwjs.prototype.modify=function(){
 ...
 */
PmProjectYsCwjs.prototype.detail=function(id, value){
					var _self = this;
 ...
 * @param callback 回调函数
 */
PmProjectYsCwjs.prototype.save=function(form,callback){
	var _self = this;
	$.ajax({
 ...
 * 删除方法
 */
PmProjectYsCwjs.prototype.del=function(){
	var rows = this._datagrid.datagrid('getChecked');
	var _self = this;
	var ids = [];
	var l =rows.length;
 ...
 * 重载数据
 */
PmProjectYsCwjs.prototype.reLoad=function(){
 ...
 * @param id 对话框id
 */
PmProjectYsCwjs.prototype.closeDialog=function(id){
	$(id).dialog('close');
};
 ...
 * 打开查询框
 */
PmProjectYsCwjs.prototype.openSearchForm =function(){
	this.searchDialog.dialog('open',true);
};
/**
 * 去后台查询(日期查询时,结束日期不能小于起始日期)
 */
PmProjectYsCwjs.prototype.searchData =function(){
 ...
 * 隐藏查询框
 */
PmProjectYsCwjs.prototype.hideSearchForm =function(){
	this.searchDialog.dialog('close',true);
};
/**
 * 清空查询条件
 */
PmProjectYsCwjs.prototype.clearData =function(){
 ...
 * @param value 格式化的值
 */
PmProjectYsCwjs.prototype.format=function(value){
	if(value){
		if($.browser.msie){
 ...
 * @param value 格式化的值
 */
PmProjectYsCwjs.prototype.formatDateTime=function(value){
	if(value){
		if($.browser.msie){
			if(typeof(value) == "number"){
 ...
 */
PmProjectYsCwjs.prototype.searchDataBySfn =function(conditions){
	this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
 ...
 * @param msg 显示信息
 */
PmProjectYsCwjs.prototype.ShowMask = function(msg){
	if(msg == null || msg == "")
		msg = "正在提交数据，请等待...";
	$('#mask').dialog({
	    title: msg
 ...
 * @param time 时间
 */
PmProjectYsCwjs.prototype.HidMask = function(time){
	var _self = this;
	if(time == null){
		if(_self.maskisopen){
			$("#mask").dialog("close");
 ...
 * @param type 归属类型
 */
PmProjectYsCwjs.prototype.initWorkFlow = function(state, type) {
	setBpmMenuState(state,type);
	$('#bpmState').val(state);
	$('#bpmType').val(type);
	
	pmProjectYsCwjs.reLoad();
};
 ...
 * @param page 对话框id
 */
PmProjectYsCwjs.prototype.saveFormAndStartFlow = function(form, page) {
	var _self = this;
	...
	processDef.start();
        ...
        PmProjectYsCwjs.prototype.secretLevel=[];
Platform6.fn.lookup.getLookupType('PLATFORM_FILE_SECRET_LEVEL',function(r){r&&( PmProjectYsCwjs.prototype.secretLevel=r);});  
