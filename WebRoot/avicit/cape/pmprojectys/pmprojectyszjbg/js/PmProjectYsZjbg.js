 ...
 * @param form 表单id
 */
function PmProjectYsZjbg(datagrid,url,searchD,form){
	...
	this._formCode = "pmProjectYsZjbg";
	};
/**
 * 初始化操作
 * 回车查询
 */
PmProjectYsZjbg.prototype.init=function(){
	var _self = this;
	...
	_self._datagrid=$(this._datagridId).datagrid({
		url:_self.getUrl()+"getPmProjectYsZjbgsByPage.json",
 ...
 * 打开添加页面
 */
PmProjectYsZjbg.prototype.insert=function(){
 ...
 * 打开编辑页面
 */
PmProjectYsZjbg.prototype.modify=function(){
 ...
 */
PmProjectYsZjbg.prototype.detail=function(id, value){
					var _self = this;
 ...
 * @param callback 回调函数
 */
PmProjectYsZjbg.prototype.save=function(form,callback){
	var _self = this;
	$.ajax({
 ...
 * 删除方法
 */
PmProjectYsZjbg.prototype.del=function(){
	var rows = this._datagrid.datagrid('getChecked');
	var _self = this;
	var ids = [];
	var l =rows.length;
 ...
 * 重载数据
 */
PmProjectYsZjbg.prototype.reLoad=function(){
 ...
 * @param id 对话框id
 */
PmProjectYsZjbg.prototype.closeDialog=function(id){
	$(id).dialog('close');
};
 ...
 * 打开查询框
 */
PmProjectYsZjbg.prototype.openSearchForm =function(){
	this.searchDialog.dialog('open',true);
};
/**
 * 去后台查询(日期查询时,结束日期不能小于起始日期)
 */
PmProjectYsZjbg.prototype.searchData =function(){
 ...
 * 隐藏查询框
 */
PmProjectYsZjbg.prototype.hideSearchForm =function(){
	this.searchDialog.dialog('close',true);
};
/**
 * 清空查询条件
 */
PmProjectYsZjbg.prototype.clearData =function(){
 ...
 * @param value 格式化的值
 */
PmProjectYsZjbg.prototype.format=function(value){
	if(value){
		if($.browser.msie){
 ...
 * @param value 格式化的值
 */
PmProjectYsZjbg.prototype.formatDateTime=function(value){
	if(value){
		if($.browser.msie){
			if(typeof(value) == "number"){
 ...
 */
PmProjectYsZjbg.prototype.searchDataBySfn =function(conditions){
	this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
 ...
 * @param msg 显示信息
 */
PmProjectYsZjbg.prototype.ShowMask = function(msg){
	if(msg == null || msg == "")
		msg = "正在提交数据，请等待...";
	$('#mask').dialog({
	    title: msg
 ...
 * @param time 时间
 */
PmProjectYsZjbg.prototype.HidMask = function(time){
	var _self = this;
	if(time == null){
		if(_self.maskisopen){
			$("#mask").dialog("close");
 ...
 * @param type 归属类型
 */
PmProjectYsZjbg.prototype.initWorkFlow = function(state, type) {
	setBpmMenuState(state,type);
	$('#bpmState').val(state);
	$('#bpmType').val(type);
	
	pmProjectYsZjbg.reLoad();
};
 ...
 * @param page 对话框id
 */
PmProjectYsZjbg.prototype.saveFormAndStartFlow = function(form, page) {
	var _self = this;
	...
	processDef.start();
        ...
        PmProjectYsZjbg.prototype.secretLevel=[];
Platform6.fn.lookup.getLookupType('PLATFORM_FILE_SECRET_LEVEL',function(r){r&&( PmProjectYsZjbg.prototype.secretLevel=r);});  
                           PmProjectYsZjbg.prototype.zjType=[];
Platform6.fn.lookup.getLookupType('PM_YS_ZJBG_TYPE',function(r){r&&( PmProjectYsZjbg.prototype.zjType=r);});  
