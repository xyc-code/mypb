package avicit.pb.activist.activistmerits.controller;
...
@Controller
@Scope("prototype")
@RequestMapping("avicit/pb/activist/activistMerits/activistMeritsController")
public class ActivistMeritsController implements LoaderConstant {
	private static final Logger LOGGER = LoggerFactory.getLogger(ActivistMeritsController.class);
	 ...
	 */
	@RequestMapping(value = "/operation/getActivistMeritssByPage")
	@ResponseBody
	@RequiresPermissions("activist:activistMerits:view")
	public Map<String, Object> getActivistMeritsByPage(PageParameter pageParameter, HttpServletRequest request) {
		QueryReqBean<ActivistMeritsDTO> queryReqBean = new QueryReqBean<ActivistMeritsDTO>();
		...
		if (sidx != null && sord != null && !"".equals(sord) && !"".equals(sidx)) {
			cloumnName = ComUtil.getColumn(ActivistMeritsDTO.class, sidx);
			if (cloumnName != null) {
				//这里先进行判断是否有对应的数据库字段
				...
				if (sidx.indexOf("Alias") != -1) {
					cloumnName = ComUtil.getColumn(ActivistMeritsDTO.class,
		...
		ActivistMeritsDTO param = null;
		DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
		QueryRespBean<ActivistMeritsDTO> result = null;
		if(pid == null || "".equals(pid)){
			pid = "-1";
		}
		if (json != null && !"".equals(json)) {
			param = JsonHelper.getInstance().readValue(json, dateFormat, new TypeReference<ActivistMeritsDTO>() {
			});
		}else{
            param = new ActivistMeritsDTO();
        }
		param.setPaId(pid);
		param.setOrgIdentity(SessionHelper.getCurrentOrgIdentity(request));
		queryReqBean.setSearchParams(param);
		try {
			result = activistMeritsService.searchActivistMeritsByPage(queryReqBean,orderBy,keyWord);
		} catch (Exception ex) {
			return map;
		}
		for (ActivistMeritsDTO dto : result.getResult()) {
			convertDto(dto);
		}
		...
		LOGGER.info("成功获取ActivistMeritsDTO分页数据");
		return map;
	}
	...
	public ModelAndView toOpertionPage(@PathVariable String type, @PathVariable String id, HttpServletRequest request)
		...
		if (!"Add".equals(type)) {
			...
			convertDto(dto);
			mav.addObject("activistMeritsDTO", dto);
		}
		
		mav.setViewName("avicit/pb/activist/activistmerits/ActivistMerits" + type);
		return mav;
	}
    ...
    public void exportExcel(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (selectConditions != null && !"".equals(selectConditions)) {
            param = JsonHelper.getInstance().readValue(selectConditions, dateFormat,
                    new TypeReference<ActivistMeritsDTO>() {
                    });
        } else {
            param = new ActivistMeritsDTO();
        }
        param.setOrgIdentity(SessionHelper.getCurrentOrgIdentity(request));
        try {
            List<ActivistMeritsDTO> dtoList = activistMeritsService.searchActivistMeritsForExport(param, idsList);
			for (int i = 0; dtoList != null && i < dtoList.size(); i++) {
				convertDto(dtoList.get(i));
			}
            List<Map<String,Object>> dataList = objectsToMaps(dtoList);
            byte[] bytes = activistMeritsService.exportExcel(templateCode, colsList, dataList);
     ...
     */
    private List<Map<String, Object>> objectsToMaps(List<ActivistMeritsDTO> objList) {
        List<Map<String, Object>> list = new ArrayList<>();
		for (int i = 0; objList != null && i < objList.size(); i++) {
			ActivistMeritsDTO bean = objList.get(i);
			if (bean != null) {
	 ...
	 * 转换dto中通用选择属性的名称
	 */
	private void convertDto(ActivistMeritsDTO dto){
		if (dto == null){
			return;
		}
		dto.setMeritsName(sysLookupLoader.getNameByLooupTypeCodeAndLooupCodeByAppId("PA_MERITS", dto.getMerits(),sysApplicationAPI.getCurrentAppId()));
		dto.setTypeName(sysLookupLoader.getNameByLooupTypeCodeAndLooupCodeByAppId("PA_PARTY_TYPE", dto.getType(),sysApplicationAPI.getCurrentAppId()));
