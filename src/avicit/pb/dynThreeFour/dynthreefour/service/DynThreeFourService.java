...
public class DynThreeFourService implements Serializable {
	...
	public List<Map<String, Object>> queryPartyOrg(String partyName){
		...
		//首先查出所有党组织
		try {
			partyOrganMemberDTOS = jdbcTemplate.queryForList("select * from party_organization t where t.tree_level != '1' and t.attribute_01 not in ('0') and " + partyName + " and t.party_name != '中国航发东安纪委'   order by t.TREE_SORTS");
		} catch (Exception e) {
			e.printStackTrace();
