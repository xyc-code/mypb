...
public class UfmInfoDZBFlowRule {
...
"WITH PSUA AS (SELECT USER_ID,PARTY_ID FROM PARTY_ORGAN_MEMBER),PSUB AS (SELECT  USER_ID,PARTY_ID,PO.ID,PO.PARTY_CODE,PO.PARTY_NAME,PO.PARENT_ID  FROM PSUA LEFT JOIN PARTY_ORGANIZATION PO ON PO.ID=PSUA.PARTY_ID)SELECT * FROM PSUB WHERE PSUB.USER_ID='"
		+loginUserId+"'");
		...
		List<Map<String, Object>> list2 = jdbcTemplate.queryForList("select t.party_id from PARTY_MEMBER  t where t.user_id = '"+loginUserId+"'");//avicit_bpm_start_id 替换 loginUserId   psu
				...
				try {
					...
					for (PartyOrganMemberDTO partyOrganMemberDTO : partyOrganMemberDTOList) {
						...
						if (StringUtils.isNotEmpty(userId) && StringUtils.isNotEmpty(post)) {
							if(StringUtils.equals(post, "0")){//user_post    1 2  为总支、党委书记    0为支部书记
