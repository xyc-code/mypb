...
import avicit.platform6.api.syspermissionresource.permissionanalysis.core.support.LoaderConstant;
//选择拟稿人党组织的党总支部书记和党支部书记
public class PartyCommonZSJRtrueSelectUsers extends UserSelect implements LoaderConstant {
	...
	public Actors getActors(String processInstanceId, String executionId, String activityName, String startFormId, String loginUserId) {
		...
		if(StringUtils.isNotEmpty(avicit_bpm_start_id)){
			List<Map<String, Object>> list = jdbcTemplate.queryForList("select USER_ID from party_organ_member where PARTY_ID = (select PARTY_ID from party_organ_member  where USER_ID = '"+avicit_bpm_start_id+"') and (USER_POST = '1'or USER_POST = '3' )");			
