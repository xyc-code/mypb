...
public class PartyZZZBUserSelect extends UserSelect implements LoaderConstant {
    ...
    public Actors getActors(String processInstanceId, String executionId, String activityName, String startFormId,
        ...
        if (StringUtils.isNotEmpty(avicit_bpm_start_id)) {
            List<Map<String, Object>> list = jdbcTemplate.queryForList("select\n" +
                    "\tuser_id\n" +
                    "\tfrom PARTY_ORGAN_MEMBER t1\n" +
                    "\twhere t1.party_id = (  select REGEXP_SUBSTR(tree_path,'^[^/]+/[^/]+/([^/]+)',1,1,null,1)   as value\n" +
