...
// 经费审批事件 记录账本
public class PartyAccountingJFCondition implements EventListener, LoaderConstant {
    ...
    public void notify(EventListenerExecution execution) throws Exception {
        ...
        tableData.setTableName("DYN_PARTY_ACTIVITY"); // 党费审批表
        ...
        Double xxzlTotalPrice; // 消费总额
        try {
            xxzlTotalPrice = (Double) data.get("TOTAL_PRICE"); // 消费总额
        } catch (Exception e) {
            Integer xxzlTotalPrice1 = (Integer) data.get("TOTAL_PRICE");
            xxzlTotalPrice = Double.valueOf(xxzlTotalPrice1);
        }
        ...
        if (dynPartExpenseLedgerByPartId != null) {
            ...
            if (StringUtils.isBlank(dynPartExpenseLedgerByPartId.getPartyFinanceNotUsed())) {
                partyMoney = dynPartExpenseLedgerByPartId.getPartyFinance();
            } else {
                partyMoney = new BigDecimal(dynPartExpenseLedgerByPartId.getPartyFinanceNotUsed());
            }
//		BigDecimal xxzlTotalPriceBigDecimal = xxzlTotalPrice;
            BigDecimal xxzlTotalPriceBigDecimal = new BigDecimal(xxzlTotalPrice);
            // 活动经费未使用额度
            dynPartExpenseLedgerByPartId.setPartyFinanceNotUsed(String.valueOf(partyMoney.subtract(xxzlTotalPriceBigDecimal)));
            // 已使用额度
            dynPartExpenseLedgerByPartId.setPartyFinanceUsed(String.valueOf(partyMoney.subtract(new BigDecimal(dynPartExpenseLedgerByPartId.getPartyFinanceNotUsed()))));
            // 修改
