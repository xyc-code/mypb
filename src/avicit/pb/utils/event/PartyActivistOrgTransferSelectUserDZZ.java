...
public class PartyActivistOrgTransferSelectUserDZZ extends UserSelect implements LoaderConstant {
	...
	public Actors getActors(String processInstanceId, String executionId, String activityName, String startFormId,
		...
		List<Map<String, Object>> list = jdbcTemplate.queryForList("select t.out_party_org_id from DYN_AOT_MEMBER t where t.fk_sub_col_id = '"+startFormId+"'");
		...
		String pOutPartyOrgId = "";
		if(list != null && list.size() > 0){
			Map<String, Object> mapList = list.get(0);
			pOutPartyOrgId = (String)mapList.get("out_party_org_id");
			if (StringUtils.isNotEmpty(pOutPartyOrgId)) {
				String parentPartyOrgId = "";
				List<Map<String, Object>> list_tree_parent_id = jdbcTemplate.queryForList(
						"select t.parent_id from PARTY_ORGANIZATION t where t.id = '"+pOutPartyOrgId+"'");
				if(list_tree_parent_id != null && list_tree_parent_id.size() > 0){
					Map<String, Object> mapList_parent_id = list_tree_parent_id.get(0);
					parentPartyOrgId = (String)mapList_parent_id.get("parent_id");
					try {
						List<PartyOrganMemberDTO> partyOrganMemberDTOList = partyOrganMemberService
								.queryPartyOrganMemberByPartyId(parentPartyOrgId);
