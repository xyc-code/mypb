...
public class PartyJWserSelect extends UserSelect implements LoaderConstant {
    ...
    public Actors getActors(String processInstanceId, String executionId, String activityName, String startFormId,
        ...
        if (StringUtils.isNotEmpty(avicit_bpm_start_id)) {
            ...
            if(!StringUtils.isEmpty(firstLevel)){
                String userId = jdbcTemplate.queryForObject("select user_id from PARTY_ORGAN_MEMBER where party_id=(select id from (select id,party_name from party_organization where tree_path like '%" + firstLevel + "%')a\n" +
                        "where a.party_name like '%纪委%' and rownum =1) and user_post='7' and rownum=1", String.class);
