...
@Controller
@Scope("prototype")
@RequestMapping("avicit/pb/organize/partyOrganMember/partyOrganMemberController")
public class PartyOrganMemberController implements LoaderConstant {
	private static final Logger LOGGER = LoggerFactory.getLogger(PartyOrganMemberController.class);
	...
	public Map<String, Object> togetPartyOrganMemberByPid(PageParameter pageParameter, HttpServletRequest request) {
		...
		try {
			result = partyOrganMemberService.searchPartyOrganMemberByPage(queryReqBean,orderBy,keyWord);
		} catch (Exception ex) {
			return map;
		}
	...
	*/
	 ...
	 * 转换dto中通用选择属性的名称
	 */
	private void convertDto(PartyOrganMemberDTO dto){
		if (dto == null){
			return;
		}
