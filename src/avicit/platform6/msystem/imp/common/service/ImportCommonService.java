...
public class ImportCommonService implements Serializable {
	...
	private void setSysDyncProperties(Map dyncMap, PlatformConstant.OpType type, boolean hasOrgIdentity) {
		...
		if (((Map) session).get("userId") == null) {
			...
			deptId = ((Map) session).get("deptId").toString();
		}
		...
		//默认字段CREATED_DEPT维护规则：表中存在字段时，插入语句才拼接CREATED_DEPT
		if ("insert".equals(type.name())) {
			if(!ObjectUtils.isEmpty(dyncMap.get("isExistCol")) && Boolean.parseBoolean((dyncMap.get("isExistCol")).toString())){
				dyncMap.put("CREATED_DEPT", deptId);
			}
		}
		//删除与业务无关的标识信息
		dyncMap.remove("isExistCol");
		switch (type) {
		case insert:
								...
								switch (cell.getCellType()) {
									case XSSFCell.CELL_TYPE_NUMERIC :
										...
										if (HSSFDateUtil.isCellDateFormatted(cell)) {
											...
											try {
												val = dtf.format(cellValue);
											} catch (Exception e) {
												val = dtf1.format(cellValue);
											}
										} else {
											cell.setCellType(1);
	...
	public int doImportData3(String code, ImportHandler importHandler, String param, String swfl,
		...
		for (Map.Entry<String, List<Map<String, String>>> table : data.entrySet()) {
			...
			boolean isExistCol = isExistCol(tableName,"CREATED_DEPT");
			if (validateSheetResult) {
				...
				for (Map<String, String> row : table.getValue()) {
					...
					if (validateRowResult) {
						...
						for (Map.Entry<String, String> col : row.entrySet()) {
							...
							if (dto != null) {
								if ("DATE".equals(dto.getFieldtype().toUpperCase())) {
									rowData.put(colName, formatDate(colValue) == null ? "" : formatDate(colValue));
						...
						try{
							//设置判断表是否存在created_dept
							rowData.put("isExistCol",isExistCol);
	...
	//判断表中是否存在指定列colName
	private boolean isExistCol(String tableName,String colName) {
		List<Map<String, Object>> queryRespBean = this.sysImpTemplateService.getSheetNameTableInfo(tableName);
		//1表示字段存在 0表示字段不存在
		boolean isExistCol = true;
		if(!ObjectUtils.isEmpty(queryRespBean)){
			for (int i = 0; i < queryRespBean.size(); i++) {
				String columnName = (String)queryRespBean.get(i).get("COLUMN_NAME");
				if(!StringUtils.isEmpty(columnName) && colName.equals(columnName)){//存在
					isExistCol = true;
					break;
				}else {//不存在
					isExistCol = false;
	...
	private boolean validateDate(String columnName, String date, StringBuffer message) {
		 ...
		 */
		String[] parsePatterns = { "yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd", "yyyy/MM/dd", "yyyy/MM/dd HH:mm:ss" };
