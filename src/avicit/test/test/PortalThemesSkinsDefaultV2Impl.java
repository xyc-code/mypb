...
public class PortalThemesSkinsDefaultV2Impl extends ThemesSkinsAbst
{
  private List<PortalMenuModel> firstMenus;
  private static final int _COL = 3;
  ...
  public PortalThemesSkinsDefaultV2Impl(String _typeKey)
  {
    super(_typeKey);
  }
  ...
  public String getMenus(String userId, String orgIndentity)
  {
    ...
    for (int i = 0; i < this.firstMenus.size(); i++) {
      ...
      if (firstModel != null) {
        ...
        buffer.append("           <span id='" + firstModel.getId() + "' class=\"hlist taburl firstclass\" openType='").append(firstModel.getMenuOpenType()).append("' ").append(getUrl(firstModel)).append(" title=\"").append(firstModel.getMenuName()).append("\" ").append(">").append(firstModel.getMenuNameExt()).append(arrow).append("</span>\n")
          ...
          for (PortalMenuModel secondMenu : secondMenus) {
            ...
            if ((thirdMenus != null) && (thirdMenus.size() > 0))
            {
              ...
              for (PortalMenuModel thirdMenu : thirdMenus)
              {
                ...
                List fourMenus = super.getThirdSubMenuList(thirdMenu.getId());
                allMenus.addAll(fourMenus);
                buffer.append("     <li class='subChild'>\n");
                if (fourMenus.size() == 0) {
                  buffer.append("<div id='" + thirdMenu.getId() + "' class='afont taburl' openType='").append(thirdMenu.getMenuOpenType()).append("' title='").append(HtmlUtils.htmlEscape(thirdMenu.getMenuName())).append("' rel='").append(thirdMenu.getMenuUrl()).append("'>").append(thirdMenu.getMenuName()).append("</div>\n")
                  ...
                  buffer.append("<div id='" + thirdMenu.getId() + "' class='afont taburl hlist' openType='").append(thirdMenu.getMenuOpenType()).append("' title='").append(HtmlUtils.htmlEscape(thirdMenu.getMenuName())).append("' rel='").append(thirdMenu.getMenuUrl()).append("'>").append(thirdMenu.getMenuName()).append("</div>\n")
                    .append(StringUtils.isBlank(thirdMenu
                    .getMenuMark()) ? "" : "<i class=\"menu-label menu-bg-red jump\">".concat(getMenuMarkName(thirdMenu.getMenuMark())).concat("</i>\n"));
  ...
  private void addSubMenu(List<PortalMenuModel> menuList, StringBuffer buffer, List<PortalMenuModel> allMenus)
  {
    ...
    for (PortalMenuModel portalMenuModel : menuList)
    {
      ...
      if (subMenuList.size() == 0) {
        buffer.append("<div id='" + portalMenuModel.getId() + "' class='afont taburl' openType='").append(portalMenuModel.getMenuOpenType()).append("' title='").append(HtmlUtils.htmlEscape(portalMenuModel.getMenuName())).append("' rel='").append(portalMenuModel.getMenuUrl()).append("'>").append(portalMenuModel.getMenuName()).append("</div>\n")
          .append(StringUtils.isBlank(portalMenuModel
          .getMenuMark()) ? "" : "<i class=\"menu-label menu-bg-red jump\">".concat(getMenuMarkName(portalMenuModel.getMenuMark())).concat("</i>\n"));
      }
      else {
        buffer.append("<div id='" + portalMenuModel.getId() + "' class='afont taburl hlist' openType='").append(portalMenuModel.getMenuOpenType()).append("' title='").append(HtmlUtils.htmlEscape(portalMenuModel.getMenuName())).append("' rel='").append(portalMenuModel.getMenuUrl()).append("'>").append(portalMenuModel.getMenuName()).append("</div>\n")
          .append(StringUtils.isBlank(portalMenuModel
          .getMenuMark()) ? "" : "<i class=\"menu-label menu-bg-red jump\">".concat(getMenuMarkName(portalMenuModel.getMenuMark())).concat("</i>\n"));
