package avicit.lc.organize.leagueorganization.controller;
...
@Controller
@Scope("prototype")
@RequestMapping("avicit/lc/organize/leagueOrganization/leagueOrganizationController")
public class LeagueOrganizationController implements LoaderConstant {
	private static final Logger LOGGER = LoggerFactory.getLogger(LeagueOrganizationController.class);
	...
	public ModelAndView toLeagueOrganizationManage() {
		...
		String url = "platform/avicit/lc/organize/leagueOrganization/leagueOrganizationController";
		mav.setViewName("avicit/lc/organize/leagueorganization/LeagueOrganizationManage");
		mav.addObject("url",url);
		mav.addObject("leagueOrganMemberUrl", "platform/avicit/lc/organize/leagueOrganMember/leagueOrganMemberController/operation/");
		return mav;
	}
	...
	public ModelAndView toLeagueOrganizationParentSelect() {
		ModelAndView mav = new ModelAndView();
		String url = "platform/avicit/lc/organize/leagueOrganization/leagueOrganizationController";
		mav.setViewName("avicit/lc/organize/leagueorganization/LeagueOrganizationSelect");
		mav.addObject("url", url);
		return mav;
	}
	...
	*/
	...
	public ModelAndView toInsertOrModifyLeagueOrganization(@PathVariable(value = "type") String type, @PathVariable(value = "id") String id) throws Exception {
		ModelAndView mav = new ModelAndView();
		try {
			LeagueOrganizationDTO dto = leagueOrganizationService.queryLeagueOrganizationByPrimaryKey(id);
			// 编辑和详情
			if ("Edit".equals(type) || "Detail".equals(type)) {
				LeagueOrganizationDTO parentDTO = leagueOrganizationService.queryLeagueOrganizationByPrimaryKey(dto.getParentId());
			...
			mav.setViewName("avicit/lc/organize/leagueorganization/LeagueOrganization" + type);
	...
	public Map<String, Object> toSaveLeagueOrganization(HttpServletRequest request) {
		...
		try {
			LeagueOrganizationDTO tree = JsonHelper.getInstance().readValue(jsonData, dateFormat,
					new TypeReference<LeagueOrganizationDTO>() {
					});
            //调用校验工具类，校验数据
	...
	public Map<String, Object> toInsertRoot() {
		Map<String, Object> map = new HashMap<>();
		LeagueOrganizationDTO tree = new LeagueOrganizationDTO();
