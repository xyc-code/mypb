package avicit.lc.organize.leagueorganmember.controller;
...
@Controller
@Scope("prototype")
@RequestMapping("avicit/lc/organize/leagueOrganMember/leagueOrganMemberController")
public class LeagueOrganMemberController implements LoaderConstant {
	private static final Logger LOGGER = LoggerFactory.getLogger(LeagueOrganMemberController.class);
	...
	public ModelAndView toLeagueOrganMemberManage() {
		...
		mav.setViewName("avicit/lc/organize/leagueorganmember/LeagueOrganMemberManage");
		mav.addObject("url", "platform/avicit/lc/organize/leagueOrganMember/leagueOrganMemberController/operation/");
		return mav;
	}
	...
	@ResponseBody
	@RequiresPermissions("organize:leagueOrganMember:view")
	public Map<String, Object> togetLeagueOrganMemberByPid(PageParameter pageParameter, HttpServletRequest request) {
		QueryReqBean<LeagueOrganMemberDTO> queryReqBean = new QueryReqBean<LeagueOrganMemberDTO>();
		...
		if (sidx != null && sord != null && !"".equals(sord) && !"".equals(sidx)) {
			cloumnName = ComUtil.getColumn(LeagueOrganMemberDTO.class, sidx);
			//这里先进行判断是否有对应的数据库字段
			if (cloumnName != null) {
				...
				if (sidx.indexOf("Alias") != -1) {
					cloumnName = ComUtil.getColumn(LeagueOrganMemberDTO.class,
		...
		LeagueOrganMemberDTO param = null;
		QueryRespBean<LeagueOrganMemberDTO> result = null;
		if(pid == null || "".equals(pid)){
			pid = "-1";
		}
		if (json != null && !"".equals(json)) {
			DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
			param = JsonHelper.getInstance().readValue(json, dateFormat, new TypeReference<LeagueOrganMemberDTO>() {
			});
		}else{
			param = new LeagueOrganMemberDTO();
		}
		...
		for (LeagueOrganMemberDTO dto : result.getResult()) {
			convertDto(dto);
		}
		...
		LOGGER.info("成功获取LeagueOrganMemberDTO分页数据");
		return map;
	}
	...
	*/
	...
	public ModelAndView toOpertionPage(@PathVariable String type, @PathVariable String id)
		...
		mav.setViewName("avicit/lc/organize/leagueorganmember/LeagueOrganMember" + type);
		return mav;
	}
	...
	public Map<String, Object> toSaveLeagueOrganMember(HttpServletRequest request) {
		...
		DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
		try {
			LeagueOrganMemberDTO leagueOrganMemberDTO = JsonHelper.getInstance().readValue(jsonData, dateFormat,
					new TypeReference<LeagueOrganMemberDTO>() {
					});
            //调用校验工具类，校验数据
	...
	public void exportExcel(HttpServletRequest request, HttpServletResponse response) {
		...
		param.setOrgIdentity(SessionHelper.getCurrentOrgIdentity(request));
		try {
			List<LeagueOrganMemberDTO> dtoList = leagueOrganMemberService.searchLeagueOrganMemberForExport(param, idsList);
     ...
     */
    private List<Map<String, Object>> objectsToMaps(List<LeagueOrganMemberDTO> objList) {
        List<Map<String, Object>> list = new ArrayList<>();
		for (int i = 0; objList != null && i < objList.size(); i++) {
			LeagueOrganMemberDTO bean = objList.get(i);
			if (bean != null) {
	 ...
	 * 转换dto中通用选择属性的名称
	 */
	private void convertDto(LeagueOrganMemberDTO dto){
		if (dto == null){
			return;
		}
		...
		dto.setUserPostName(sysLookupLoader.getNameByLooupTypeCodeAndLooupCodeByAppId("LC_ORGAN_POST", dto.getUserPost(),sysApplicationAPI.getCurrentAppId()));
	}
}
