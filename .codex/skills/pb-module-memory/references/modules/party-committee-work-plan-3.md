...
## 2026-07-21 Work Content, Combined Feedback Review, And Root Statistics

- `DYN_DW_PLAN3_TASK.WORK_CATEGORY` is obsolete and removed from base/rebuild DDL, Java persistence, search, task/detail/statistics UI, and Excel import. Existing databases use repeatable patch `db/dw_work_plan_3_patch_20260721_remove_work_category.sql`; it preserves all task rows and other columns. Keep deployment-script comments ASCII because no-BOM UTF-8 Chinese comments can be misread by GBK `disql` and corrupt anonymous-block line boundaries.
- Import now has exactly six columns: task title, target, content, deadline, receiver, and remark. The receiver column is index 4 and the date style belongs to index 3. Old seven-column templates are rejected and must be downloaded again. The business verifier opens the generated XLSX and checks all six real headers.
- Task and recent-statistics tables show `CONTENT`. `.dw-task-content-cell` uses `white-space: pre-wrap` and `overflow-wrap: anywhere`. The main table minimum width is 1080px with compact columns, so a 1280px viewport keeps the operation column visible without horizontal scrolling.
- Only the direct office director can edit pending staff feedback content. Ordinary confirmation accepts optional reviewed content; return never accepts or saves review edits. `confirmAndForwardFeedback` atomically saves reviewed content, confirms/completes the staff task, creates the office feedback, submits it to a validated department confirmer, and moves the office root to pending confirmation. Invalid targets, party/department users, and other office directors are rejected before mutation; transaction exceptions roll back the whole operation.
- Statistics use only office roots (`PARENT_ID IS NULL AND TASK_LEVEL='OFFICE'`) for summary, department/status nested pie, and recent tasks. Department attribution prefers the office personnel node's direct `DEPT_MINISTER`, then the feedback target, then `未归属部门`. The bar chart contains only DEPT/OFFICE/STAFF task-row totals; the old overdue-level chart is removed.
- The multiple-department-confirmer dialog has its own Layer skin because Layer content is mounted outside `.pb-dwworkplan3-page`; page-scoped input styles do not reach it. Frontend cache version: `20260721_feedback_stats_49`.
